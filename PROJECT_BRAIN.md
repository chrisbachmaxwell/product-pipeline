...
- Mapping and draft workspace candidate: each row opens detail that verifies the current Trading listing and, for Inventory-model records, the Inventory item and Offer. It shows the Shopify variant -> raw SKU -> management model -> offer -> listing chain plus actual listing/content/delivery fields. A verified exact-store Shopify session can preview and append a local draft for the bounded supported fields through `GET`/exact `POST /api/listing-draft`; stale observations or revisions fail closed. The 2026-08-13 census found 112 active Trading listings but only five Inventory items and five Offers, so legacy Trading and Inventory/Offer controls remain separate.
- Scope boundary: this source candidate adds no generic publishing route, provider edit, approval, Apply, Publish, token persistence, order import, inventory or price synchronization, Marketplace Connect mutation, or automatic action. Exact local draft append is the sole non-read API exception; all other non-read API requests remain quarantined. Price and quantity remain visible/read-only under Marketplace Connect, while listing and mapping ownership remain unverified. Any provider action still requires separate RBAC, approval/job authority, single-writer cutover, remote reconciliation, and rollback integration.
- Local store operations: the web runtime requires a pre-existing canonical schema-version-2 exact-scope store, a reviewed single-writer assertion, and safe path/integrity checks; it never initializes, migrates, repairs, or replaces the file. The intended Production path is `/data/product-pipeline/listing-control.sqlite` on the Railway volume. A read-only preflight found that path and parent absent and the new environment variables unset, so this candidate is not yet deployed/configured proof. See `docs/LISTING_CONTROL_ADMIN.md`.
...
- Expanded catalog baseline status: the mounted live reader, status reducer, UI, help, and tests were published as the exact baseline recorded above. The newer local-draft candidate is source work on top of that baseline; its final tests/build, merge/deploy commit, store initialization/configuration, public health identity, and signed-in embedded verification remain required before any production claim.
