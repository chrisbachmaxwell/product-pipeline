...
- Current application release slice: the authenticated Listings catalog reads every current Shopify variant with available inventory above zero and classifies it against one complete, exact-account Shopify/eBay capture. `Active` requires one exact active listing with a compatible managed offer shape or a legacy Trading-only listing; `Not listed` requires zero exact-SKU active listings, Inventory items, and Offers; every incomplete, ambiguous, duplicate, near-collision, non-active-product, or artifact state needs attention. The minimal operator shell has exactly Overview, Listings, Orders, Issues, and Settings and shows the completed capture time.
- Scope boundary: this UI release adds no generic publishing route, durable review/job, token persistence, order import, inventory or price synchronization, Marketplace Connect mutation, or automatic action. View, Review, and Details are read-only navigation. All non-read API requests remain quarantined. A future generic listing workflow requires its own scoped OAuth, durable approval, reconciliation, and rollback integration.
- Prior published application/runtime baseline: authoritative-read capture PR #4 merged as `57001ed777e5a75076cb159e306706eb7efd7d68` after review of feature commit `0244aabdb738d0174032549ea3700243f6f3d18a`. The later minimal Listings release is published at `38facf5166aa5e6a09ab5fb6e3002f9ac9eee69c`. The expanded live catalog described above remains a release candidate until its exact merge/deployment and embedded-app verification are recorded here.
- Inspection boundary: repository source/history plus the signed-in Shopify/embedded-app surfaces described above.
- External access: the prior release used GitHub/Railway deployment evidence and read-only Shopify UI inspection. This catalog slice additionally verified the exact Shopify shop and Production eBay seller through bounded read-only APIs, using existing runtime authority without returning or persisting token material. No Lightspeed, order, listing, price, inventory, Marketplace Connect, policy, mapping, or credential write occurred.
