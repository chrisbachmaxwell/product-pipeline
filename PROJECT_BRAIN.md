...
## 16. State of the System — 2026-08-27

- **Deployed:** Railway auto-deploys `main` (last verified runtime-changing baseline `716b1a35f5835d1b0f6a12bc8c5b5837e965be8d`, PR #46; a later documentation-only handoff merge may change `buildCommit` without changing runtime behavior); domain `ebay-sync-app-production.up.railway.app`; volume `/data`. `/health` reported PR #46's exact commit before this handoff. Listing revise (Inventory + Trading), create, end/relist, price, inventory, new-order-only import, and the inert fulfillment/tracking ceremony are built and deployed behind standalone operator CLIs. Nothing dispatches on deploy or through the server.
...
- **G16 create incident state (2026-08-27):** the first two exact create intents stopped on Inventory PUT, reconciled `confirmed_missing`, and left no eBay artifact; neither can replay. PR #46 then deployed bounded diagnostics, the exact `Accept-Language` header, and an explicit condition-description omission path. Draft 5 genuinely omitted the invalid operational test note and preflighted as manifest `sha256:567bacafad0421ff0545a70fe35b7a3104b38828704f4959342f8d81bc059dbc`. The human operator dispatched it once: Inventory PUT and Offer POST succeeded, returning unpublished offer `247267392011`; Publish Offer returned HTTP 400 / eBay error `25019`. The job `listing-create-job:ec897152-ad2c-43f9-8d8c-a6942503bfa1` and attempt `listing-create-attempt:214fc4fe-79c3-416e-ad16-9a2c81117285` remain `reconciliation_required`. A later zero-write reconciliation authoritatively reported `offer_unpublished` / `CREATE_OFFER_UNPUBLISHED`, `resolution: null`, and no external write. There is no public listing. Do not redispatch, use `--accept-absent`, or reuse Sandbox recovery against Production. The reviewed residue-cleanup ceremony is now BUILT (schema v5 + `listing-lifecycle-admin recover-create`, see Update 2026-08-27b and `docs/LISTING_LIFECYCLE_DISPATCH.md` "Recovering an unpublished create") but NOT executed: the Production store is still v4, so once this merge deploys, every ordinary migration-store open fails closed (L13) until the operator backs up off-volume and runs `migration-admin upgrade` to v5; the incident job/attempt stay unresolved until the operator runs the exact recover-create command recorded in the runbook.
...
- **Remaining user-only steps:** observe Marketplace Connect price/quantity behavior on the G10 listing for 24 hours. For G16, execute the built recovery-cleanup in order: off-volume backup → `migration-admin verify` → `upgrade` (v4→v5) → `verify` → the exact `recover-create` command in `docs/LISTING_LIFECYCLE_DISPATCH.md` (bound to the unresolved job/attempt/intent/evidence/SKU/offer `247267392011`), which removes the residue and truthfully resolves the original create as `resolved_residue_removed`; then use a policy-compliant target (a genuine low-stakes physical item, or eBay's designated Production test-listing category) for a fresh create/end proof. Only after Phase 1 exits: MC toggle-offs for price and inventory, the order cutover above, and open incident G1 (Shopify credential rotation, Section 14 G1).
...
**G16 — First production listing-create and end/relist dispatches.** Status: BLOCKED ON UNPUBLISHED PRODUCTION RESIDUE; no public listing. The first two exact 2026-08-27 creates stopped on Inventory PUT and terminalized absent. After PR #46 deployed, corrected Draft 5 omitted the invalid condition note and one exact operator dispatch successfully created the Inventory item and offer `247267392011`, then Publish Offer failed HTTP 400 / eBay error `25019`. Zero-write reconciliation proved `offer_unpublished` / `CREATE_OFFER_UNPUBLISHED`; the exact job/attempt remain unresolved and non-replayable. Error `25019` is not an exact diagnosis because eBay carries its specific reason in provider text that ProductPipeline intentionally discards. The current ProductPipeline “TEST / DO NOT BUY” item also used a camera category, while eBay policy requires Production test listings in its designated Test Auctions category. Remaining work: build a separately approved Production recovery-cleanup ceremony bound to the exact source evidence, remove and reconcile the residue, add bounded category/condition/policy/location publish validation, then run a fresh policy-compliant create and deliberate end/relist proof. Authorization boundary: never replay this create, never accept absence while residue exists, and never point Sandbox recovery at Production.
...
- **L31 (2026-08-27, create response-loss classification):** An immediate absent read does not prove a timed-out, aborted, network-failed, unreadable, oversized, or ambiguous Inventory PUT had no effect; the provider may have committed before the response was lost. Only a local pre-request denial or a known non-2xx response at the first PUT is `definite_no_effect` and may auto-confirm absence. Every other attempted-write failure is `outcome_unknown`, remains reconciliation-required through immediate absence or later residue, and must never redispatch the same intent. Emit only fixed stage/code/outcome-class values, never raw provider text.
- **L32 (2026-08-27, bounded provider diagnostics are not retry authority):** Fixed redaction must still leave enough non-sensitive evidence to correct a definite provider rejection. For a bounded eBay REST non-2xx response, retain only its fixed HTTP family, numeric status in the valid HTTP range, and the first five sorted unique positive numeric `errorId` values in a conservative range; discard messages, parameters, bodies, URLs, tokens, and exception text. Invalid JSON or error shapes yield no IDs. Only a known first-PUT 4xx is `definite_no_effect`; 5xx remains `outcome_unknown`. Diagnostics do not diagnose an already completed attempt retroactively, reopen its intent, or justify a blind schema bump or identical draft re-save.
- **L33 (2026-08-27, optional condition-description omission):** eBay limits `conditionDescription` to clarification of a used item's physical condition and forbids unrelated branding, promotion, shipping, payment, return, or contradictory text. The production test draft's operational “no physical item / do not buy” instruction did not belong there. Because request `null` already means inherit, optional-field clearing needs an explicit omission state that persists as `proposedSource: omit` / `proposedValue: null`, remains distinguishable from inherited source/observed values during no-op comparison, and causes the Inventory serializer to omit the field. Do not add a brittle content classifier without a reviewed semantic rule; the operator must review and explicitly omit unrelated text. Spell the standard request header exactly `Accept-Language`.
### Update 2026-08-27b — G16 recovery-cleanup slice built (schema v5); operator execution pending

**G16 status change:** the Production recovery-cleanup ceremony demanded by L34 is source-complete with full test coverage; nothing has dispatched and the Production store remains schema v4. Schema v5 admits exactly one new production action, `recover_create_ebay_listing` (listingCreate; structurally requires an unresolved create job on the identical inventory-SKU target), plus the truthful terminal vocabulary `resolved_residue_removed` / `effect_residue_removed` (listingCreate-only, exclusively paired, reachable only from `reconciliation_required`); every other denial is recreated with identical predicates. `listing-lifecycle-admin recover-create` binds the exact source job/attempt/intent/approval-evidence/SKU/offer (the offer id is re-derived from the recorded `CREATE_OFFER_UNPUBLISHED` result digest, never trusted from input), refuses a published offer, deletes and verifies gone through a bounded GET/DELETE-only adapter, and resolves both the recovery job and the original job as removed residue; `recover-reconcile` (zero writes) completes a delayed capture, and a failed delete can only retry through an explicitly chained ceremony binding the failed prior job (L29). Create preflight/dispatch now prevalidate the documented publish prerequisites locally with fixed `CREATE_PREVALIDATION_*` codes before any provider write (L30). Deliberately updated locked regressions: the v4-pinned upgrade expectations (listing-revise-slice, replacement-v3-slice, schema-v4-order-state-upgrade, migration-admin, fulfillment-tracking-admin) now expect v5, the monitoring shape gains `resolvedResidueRemoved`, and the server migration-state reader/tests pin schemaVersion 5. Authorization boundary: building this authorizes nothing — the store upgrade and the one recover-create invocation are separate operator actions, and the ceremony can never publish, create, or touch any other offer/SKU.

- **L34 (2026-08-27, unpublished Production create residue):** A successful Inventory PUT plus Offer POST followed by a rejected Publish Offer is neither success nor absence. The exact G16 Draft 5 attempt returned offer `247267392011`, HTTP 400 / eBay error `25019`, and later reconciled to `offer_unpublished` / `CREATE_OFFER_UNPUBLISHED` with zero additional provider writes. Keep the original create job unresolved; never redispatch it or pass `--accept-absent`. Numeric `25019` is only a family-level clue because eBay places the specific reason in provider text that the redaction boundary discards. Production needs its own separately approved recovery-cleanup ceremony bound to the source job, attempt, intent, approval evidence, SKU, and exact discovered offer; Sandbox recovery is not Production authority. Also follow eBay's Production test-listing policy: a “TEST / DO NOT BUY” listing belongs only in the designated test category, not a normal camera category.
- **L35 (2026-08-27, migration-store schema evolution mechanics):** The schema-v1 SQL is interpolated from the TS constants `INTENT_ACTIONS` / `INTENT_ACTION_RESPONSIBILITY` and frozen by per-migration checksums that every existing store carries — changing those constants silently invalidates every deployed store's verification. New vocabulary must therefore live in separate constants (`RECOVERY_INTENT_ACTIONS`, `ALL_INTENT_ACTION_RESPONSIBILITY`) and enter only through a new migration that rebuilds the constrained table. Rebuilding a REFERENCED table (e.g. `idempotency_intents`) is impossible inside a plain foreign-keys-ON transaction: DROP of a populated parent violates immediate FKs, and `ALTER TABLE … RENAME` rewrites child `REFERENCES` clauses (FK on) and trigger bodies (SQLite ≥3.25) unless `legacy_alter_table` is set. The reviewed pattern: the migration runner defers `foreign_keys` around the whole migration transaction and refuses to commit unless `foreign_key_check` is clean, migrations bracket renames with `PRAGMA legacy_alter_table`, attached triggers/indexes are dropped before the rename and recreated after, and the fresh-install and upgrade paths share the identical code so the catalog digest stays deterministic. Also: a truthful new terminal outcome needs the whole chain widened in lockstep (job_events CHECK + transition + terminal triggers, attempt_resolutions CHECK, observation effect CHECK, resolution-pairing trigger, TS guards, monitoring buckets with the daily cohort invariant preserved, and the server projection's pinned schemaVersion) — miss one and either the resolution is impossible or the dashboard collapses per L23.
- **L36 (2026-08-27, remote-execution constraint):** This remote agent environment permits only HTTPS egress; Railway SSH (port 22, and no TLS/WSS alternative) is unreachable, so ceremonies cannot be run from it directly. A deploy-transport workaround (start-command one-shot runner selecting committed `ops/runs/` scripts) was designed under the user's delegation, but the agent harness's safety layer correctly refused to ship it — production start-command rewiring and self-granted permissions are boundaries chat-level approval must not clear. Resolution paths, in order of preference: (1) run the agent session in an environment whose network policy allows all outbound traffic — then `railway ssh` works directly and ceremonies run normally under the recorded delegation; (2) operator pastes prepared ceremony blocks into the Railway dashboard Console. Never re-attempt the deploy-transport channel.
- **L37 (2026-08-27, L36 re-verified with exact transport mechanics):** A second remote agent session re-tested the L36 constraint end-to-end and confirmed it, with newly precise mechanics. Railway CLI 5.45.3 (`npm i -g @railway/cli`) installs and authenticates fine from the agent environment — `railway login --browserless` completed as `chrismaxwell09@gmail.com` and `railway link --project product-pipeline --environment production` succeeded — because all of that is ordinary HTTPS. Shell access is what fails, and the reason is now exact: `railway ssh` in v5.45.3 is **not** a WebSocket tunnel; the binary shells out to OpenSSH against **`ssh.railway.com` port 22** (its own error strings name `ssh <instance>@ssh.railway.com`, `railway ssh keys add`, and "could not connect to ssh.railway.com -- check your network and firewall rules"). DNS resolves (`66.33.22.3`), port 443 to that same host connects, and port 22 to it times out — as does port 22 to any host — so it is a port-22 egress block, not host filtering. A raw `ssh -v` attempt fails at `connect to address 66.33.22.3 port 22: Connection timed out`, i.e. **before authentication**, which is why registering an SSH key on the account would not help and should not be done to "try". The agent proxy's own README independently confirms the ceiling: WebSocket upgrades, non-443 HTTPS ports, and raw TCP are explicitly unsupported and are to be reported, not worked around. Practical consequences: (a) do not spend a session re-deriving this — test `bash -c 'exec 3<>/dev/tcp/ssh.railway.com/22'` once; exit 124 means stop; (b) `railway setup agent` / the Railway MCP server give deploy, logs, status, and variables over HTTPS but **no shell exec**, so they are not a ceremony transport; (c) resolution paths remain exactly L36's two, and path 2 now has a prepared artifact — `docs/PHASE1_CONSOLE_BLOCKS.md` holds paste-ready, in-order Phase 1 blocks. One gap that only in-container access can close: the Draft 5 `intentKey` required by `recover-create` was never recorded in the repo and must be recovered from the saved dispatch output or the store's intent row bound to job `ec897152-…`; future dispatch ceremonies must record the printed intentKey into `PROJECT.md` at the time of the run.
