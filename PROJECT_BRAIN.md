...
- Shopify credential-rotation source candidate: draft PR #15 at commit `3309dfd` on isolated branch `codex/shopify-credential-rotation` adds exact-store current/previous-secret JWT and webhook verification plus a standalone fixed-purpose compiled credential administrator. The previous-secret overlap is bounded to one hour and continues primary verification at cutoff; a rotation dispatch requires distinct old/new secrets and at least 15 minutes remaining on both the overlap and the dedicated single-writer acknowledgement. Token acquisition uses only the current/new secret.
...
- Candidate proof boundary: focused 8-file / 88-test security checks, the full 54-file / 582-test suite, TypeScript no-emit, Production build, compiled command help and malformed-argument redaction checks, and whitespace checks pass locally. The candidate is committed and pushed on draft PR #15 and independently source-reviewed GO; no GitHub CI checks or reviews are recorded. No Shopify/eBay provider, Railway, Production database, Marketplace Connect, or Lightspeed mutation was performed by the source work. Read-only Railway and public-health preflight confirmed active deployment `259f4262-0943-4c26-a47b-6b722f73fc75` at revision `234e0cb4de8aeafe494492f7039317915969b9aa`, with one replica and the `/data` volume; that is not the candidate, and the candidate is neither deployed nor live-verified. Incident `SCR-2026-08-14-001` remains open until independent incident-state documentation review, exact merge/deployment proof, fixed Production preflight/rotate/verify, variable cleanup, and credential-free closure evidence. See `docs/SHOPIFY_CREDENTIAL_ROTATION.md` and `docs/RELEASE_MAINTENANCE_INCIDENTS.md`.
