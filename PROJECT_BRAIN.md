...
**G4 — First provider listing-revise slice (Apply/Publish from local draft).** Status: authorized to build; user priority. On 2026-08-14 the user directed that ProductPipeline's next deliverable is the ability to push and manage eBay listings, which authorizes implementing this slice: take one exact approved local draft revision to eBay for one SKU, with RBAC/approval and job authority, durable idempotent dispatch through the migration-state store, post-action reconciliation, observation window, and rollback. Prerequisite: G3 proof. This continues the responsibility proven by the successful listing-create canary. Authorization boundary: building the slice is authorized; each actual dispatch still requires a one-action, exact-target operator approval at execution time, and this goal transfers no price, inventory, or order ownership from Marketplace Connect.
