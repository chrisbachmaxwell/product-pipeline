...
- Database diagnostic source candidate: isolated branch `codex/shopify-rotation-diagnostic`, based on merged `origin/main` at `579cc077a6ca4930fbfa88d415b80cc04c12d963`, adds exact option-free `diagnose-shopify-credential-database`. The command requires the exact nonsecret Production Railway/app/database binding and an absent listing-control writer acknowledgement. It keeps the fixed file open with one `O_RDONLY`/`O_NOFOLLOW` descriptor, lets SQLite inspect only a bounded private in-memory snapshot, and proves descriptor content/identity, fixed-path identity, and sidecar absence again before close. SQLite never receives the path; no token value is queried, selected, serialized, or output. The diagnostic and rotation preflight/CAS share exact canonical table SQL, storage, visible/default column, full index collation/order, trigger/FK, and CAS-compilation checks. The npm wrapper was removed; only direct compiled invocation is supported. An initial independent review found five P1s (WAL side effects, path substitution, mutation-blocking schema false positive, npm argv reflection, and conflated descriptor stages); all five have deterministic local regressions and remediations. The revised candidate passes the complete 5-file / 98-test credential-admin matrix, full 55-file / 623-test suite, TypeScript no-emit, Production build, compiled direct-entrypoint help/redaction, absent-wrapper sentinel, and whitespace checks. Fresh independent adversarial review returned GO with no P0/P1 on frozen 29-path manifest `c0a55f38073ca52c138c83635464f168e9245cd7a8c2fc58821ff0a31bf26e28` and confirmed source/dist parity, fixed output, runtime isolation, and zero live action. It remains unmounted, local only, not committed/pushed/merged/deployed, and has not inspected Production; its output cannot authorize repair, rotation, retry, rollback, or a commerce write.
...
### Situation summary as of `main` = `579cc07` (2026-08-14)
...
- PR #15 (Shopify credential rotation, incident `SCR-2026-08-14-001`) has merged to `main` as `579cc07` since the Section 13 handoff text describing it as a draft was written. The merge is source-only: the dual-verifier release is not deployed, no rotation was executed, and the incident remains open under the staged recovery exception in `docs/SHOPIFY_CREDENTIAL_ROTATION.md`.
...
**G1 — Execute and close incident SCR-2026-08-14-001 (Shopify credential rotation).** Status: ready; highest priority. The merged dual-verifier release must be deployed and the fixed rotation executed under the incident-only recovery exception. Sequence and gates are exactly `docs/SHOPIFY_CREDENTIAL_ROTATION.md` and `docs/RELEASE_MAINTENANCE_INCIDENTS.md`: independent incident-state documentation review; deploy with new-primary/old-previous secrets and a fresh canonical cutoff no more than one hour ahead; prove revision, one-replica `/data` topology, shadow health contract, signed-in App Bridge read, and old-secret webhook verification; then set the refresh token and single-writer acknowledgement and run `credential-admin` preflight → rotate → verify; then temporary-variable cleanup, old-secret revocation, new-only live proof, and a credential-free closure note. Time-sensitive: the cutoff variable must be freshly set at execution time. Rollback is allowed only before the provider token-rotation request. Authorization boundary: Railway deployment, one provider credential mutation, and the exact stored-token row only; zero commerce writes; requires operator/user participation for Shopify partner and Railway access.
...
**G3 — Exercise the local draft editor save path.** Status: ready. The deployed workspace shows the Edit control but an actual edit-and-save through `POST /api/listing-draft` has never been exercised (Section 13). Perform one authenticated bounded local draft append on an approved item, verify the stored revision, stale-base rejection, and admin `verify` result, and record the proof. Authorization boundary: the existing sole non-read local-draft exception only; no provider write of any kind.

**G4 — First provider listing-revise slice (Apply/Publish from local draft).** Status: blocked on separate user authorization (Section 13 scope boundary). Design and implement the reviewed slice that takes one exact approved local draft revision to eBay for one SKU: RBAC/approval and job authority, durable idempotent dispatch through the migration-state store, post-action reconciliation, observation window, and rollback. Prerequisite: G3 proof. This is the natural next responsibility after the successful listing-create canary. Authorization boundary: none of it may dispatch until the user explicitly approves the exact target and action.

**G5 — Trading-vs-Inventory listing management-model strategy.** Status: ready (analysis only). The 2026-08-13 census found 112 active Trading-model listings but only 5 Inventory items / 5 Offers. Produce a reviewed strategy for how ProductPipeline will manage and eventually migrate legacy Trading-managed listings (revise-in-place via Trading, migrate to Inventory/Offer, or hybrid), including per-listing risk, category/policy mapping, and rollback. Authorization boundary: read-only evidence and documentation; no listing mutation.
...
Suggested order if the user asks for a default: G1 (open security incident, time-staged state already committed), then G3 → G4 (continues the listing responsibility from its proven canary), with G2 and G5 as parallel low-risk work, G6 as soon as the user can supply the required authority, then G7 → G8, and G9 ongoing in the background.
