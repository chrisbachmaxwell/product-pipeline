...
- Middleware denies every non-read request beneath `/api` with HTTP `423` before a legacy handler can run. The source server mounts authenticated migration status, the live read-only listing catalog, projected local listings, and capability metadata reads; other legacy API reads return `404`.
...
- Shadow server startup does not initialize, migrate, or seed SQLite. Local reads and the listing catalog's authority lookup require an existing database, open it read-only with SQLite `query_only`, and close it; missing or incompatible ledgers fail to unavailable evidence rather than being created or repaired.
...
- Current source hard-codes ProductPipeline to shadow read-only, rejects all non-read `/api` calls, unmounts the scheduler/cloud watcher, prevents webhook dispatch/payload persistence, exposes only legacy CLI `status`, and gates the low-level external writer boundaries described above.
- The mounted server does not initialize/seed the application database; every local ledger/authority lookup uses a file-must-exist, read-only, query-only connection and treats missing local evidence as unavailable.
...
- Mapping workspace: each row opens a GET-only detail that verifies the current Trading listing and, for Inventory-model records, the Inventory item and Offer. It shows the exact Shopify variant -> raw SKU -> management model -> offer -> listing chain plus actual title, category, condition, content, price/quantity, policies, and location. The 2026-08-13 census found 112 active Trading listings but only five Inventory items and five Offers, so legacy Trading and Inventory/Offer controls must remain separate.
- Scope boundary: this UI release adds no generic publishing route, provider edit, token persistence, order import, inventory or price synchronization, Marketplace Connect mutation, or automatic action. View, Review, and Details remain read-only. The new listing-control store can retain immutable local draft revisions for its initial bounded field set only when explicitly initialized; it is not wired to the app, providers, approval, or execution. All non-read API requests remain quarantined. A future listing workflow still requires scoped principal/RBAC, a narrow local-draft route, exact preview, durable approval/job authority, remote reconciliation, and rollback integration.
- Published application/runtime baseline: expanded listing reconciliation and mapping PR #8 merged to `main` as `6a8918677478f919aead632b8d885c23cb6ab738`. Railway reported success and public `/health` served that exact commit at `2026-08-13T23:20:16.302Z` with shadow read-only mode, external writes disabled, historical backfill disabled, and the full quarantine enabled. The prior minimal Listings release was `38facf5166aa5e6a09ab5fb6e3002f9ac9eee69c`.
- Inspection boundary: repository source/history, exact Railway health, and the signed-in Shopify/embedded-app surfaces described above. A post-deploy signed-in reload of the new workspace remains an operator verification item; deployment health alone does not prove embedded data rendering.
- External access: this catalog slice verified the exact Shopify shop and Production eBay seller through bounded read-only APIs, using existing runtime authority without returning or persisting token material, then used GitHub/Railway deployment evidence. No Lightspeed, order, listing, price, inventory, Marketplace Connect, policy, mapping, or credential write occurred.
...
- Expanded catalog candidate status: its mounted live reader, status reducer, UI, help, and tests are newer than the historical evidence-capture figures in the preceding bullet. The frozen predeployment slice passed 39 test files / 393 tests, TypeScript, the production build, and whitespace checks. Exact merge/deploy commit, public health identity, and embedded-app verification remain required; until then, the candidate is not production proof.
- Corrected predeployment catalog evidence: one same-source live audit completed in 7,515 ms with zero external writes and captured Shopify 2,026 total variants / 21 pages / 176 positive-stock rows; classified 111 Active / 44 Not listed / 21 Needs attention; and captured eBay 112 active entries / one Trading page / five Inventory items / one item page / five Offers / five per-SKU pages. No row content, SKU, URL, seller identifier, credential, or token was emitted by the audit. This is aggregate point-in-time read evidence only; the candidate still needs merge, deploy, health-revision, and embedded-app verification.
...
- Current safe implementation gate: publish the reviewed read-capture slice without supplying credentials or running it. A future live evidence run requires an ignored exact-identity configuration bound to the reviewed commit, an ephemeral Shopify token with only the verified read scopes, a short-lived eBay token with exact identity/inventory/fulfillment read-only scopes and expiry metadata, a pinned signing key kept outside the repository, and a fresh independently signed exact-subject Marketplace Connect UI/support attestation or supported export. Historical artifacts also need an archived nonsecret configuration/public-key/build verification context. The signed Shopify/eBay/Marketplace Connect evidence must then be translated into reconciliation v2 and accepted responsibility by responsibility. Any writer reintroduction, persisted production watermark, live canary, Marketplace Connect responsibility disable, or historical-order import remains blocked pending authoritative parity, sandbox proof, responsibility-specific canary evidence, and its own authorization.
