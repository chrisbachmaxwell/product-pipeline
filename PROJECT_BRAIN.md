...
**G2 — Reconcile and reuse the Sandbox Test Lane.** Status: reusable listing lane BUILT and price/inventory source support built; no live Sandbox run. The standalone `sandbox-listing-canary-admin` is target-parameterized, Sandbox-host-pinned, accepts bounded ephemeral authority only on stdin, pseudonymizes seller identity before its separate durable state, and uses distinct approvals for exact create, recovery, and cleanup. The companion `sandbox-price-inventory-admin` is pinned to the exact `Pipeline Test` product/variant/SKU, requires the terminal listing-create manifest digest as provenance, and adds three separate ceremonies: seeded USD 1.00 -> Shopify USD 99.99, quantity 1 -> 2, then 2 -> Shopify 1. Every alignment action requires fresh source/target reads, a ten-minute single-use approval, at most one Sandbox bulk-update request, and exact reconciliation; unknown outcomes have only a zero-write recovery path. Neither slice has a Production host, Production migration-store, Shopify write, order, Marketplace Connect, server, webhook, scheduler, or worker path. Remaining operator work: obtain fresh bounded Sandbox authority and exact Shopify evidence, create/reconcile the exact Pipeline Test listing, separately approve and reconcile the three alignments, then separately approve cleanup and retain/verify both audit stores. Authorization boundary: source/tests only in the alignment slice; every live Sandbox write still needs its own stated approval and never authorizes Production. Runbooks: `docs/SANDBOX_LISTING_CANARY.md` and `docs/SANDBOX_PRICE_INVENTORY.md`.
...
| Pipeline Test Sandbox price / inventory proof | `docs/SANDBOX_PRICE_INVENTORY.md` | `src/sandbox-price-inventory-admin/` |
...
**G16 — First production listing-create and end/relist dispatches.** Status: source support complete; production exercise operator-gated. `listing-lifecycle-admin` now supports opt-in `ucg-branded-v1` across create preflight, dispatch, and recovery reconciliation, with the rendered HTML bound into the manifest digest and verified against fresh raw provider HTML. Remaining proof is one new SKU created end-to-end (item → offer → publish, branded template) and one deliberate end/relist exercise, each with post-dispatch verification. Authorization boundary: one exact SKU per ceremony; the create target must not already exist on eBay.
...
- **L30 (2026-08-26, Sandbox price/inventory isolation):** A Sandbox price or quantity proof must not falsify Production Marketplace Connect genesis or force a Production migration-store schema change. Use a separate Sandbox-only control store, pin both providers and the unmistakable Shopify test identity, and bind the exact terminal listing-create manifest as provenance. Price alignment, deliberate quantity drift, and quantity realignment are three different approved actions. A lost bulk-update response is never permission to retry: reconcile the exact item and offer state with zero writes, leaving before-state or third-state observations unresolved.
