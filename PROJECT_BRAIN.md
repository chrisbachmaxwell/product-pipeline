...
> The provenance-gated shadow application revision `8e5fb4d935d2bbd2c36b75764957d06c4d915345` was source/build/test verified and observed on the public Railway health endpoint on 2026-08-11; remote Shopify/eBay parity remains separate evidence.
...
- Published repository/runtime baseline: `main` at `8e5fb4d935d2bbd2c36b75764957d06c4d915345`. The next inert persistence/read-adapter increment is being developed on `agent/durable-migration-store`.
...
- Implementation status: the local operator CLI foundation, hard writer quarantine, strict authenticated shadow API, read-only five-page control plane, per-source provenance validation, and strict offline snapshot reconciliation are merged. At `2026-08-11T19:14:59.648Z`, the public health endpoint served application revision `8e5fb4d935d2bbd2c36b75764957d06c4d915345` with `shadow-read-only`, external writes and historical backfill false, a null cutover watermark, and remote verification explicitly not performed. Unauthenticated probes of migration status, legacy orders, and test mode returned `401`; the public web asset matched the reviewed build. This is deployment/runtime-policy evidence, not Shopify/eBay connectivity or parity proof.
...
- Current in-progress safe increment: `agent/durable-migration-store` adds a separate, explicitly initialized and currently unwired migration-state store for account-scoped external identities, immutable sandbox order watermark, monotonic cursors, idempotency intents and attempts, ownership versions, approvals, jobs, reconciliation exceptions, and audit evidence. Production scope is hard-limited to the accepted Marketplace Connect order/price/inventory baseline plus non-authoritative zero-write shadow evidence; it rejects a ProductPipeline watermark, ownership transfer, writer intent, approval/job, or canary. The same branch adds fixture-only no-refresh GET/HEAD adapter contracts that have no default network or credential path and can never claim live proof.
- Next implementation gate after this increment is published: add a separate inert migration-state `init`/`verify` administration CLI plus a projection-only request-time read facade for the existing five-page UI. Then choose the trusted collector execution boundary, supply exact-account read authority without exposing secrets, obtain a supported Marketplace Connect export or reviewed attestation, and generate a current version-2 parity packet. Any writer reintroduction, persisted production watermark, live canary, Marketplace Connect responsibility disable, or historical-order import remains blocked pending its own proof and authorization.
