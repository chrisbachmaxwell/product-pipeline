...
- **Remaining user-only steps:** observe Marketplace Connect price/quantity behavior on the G10 listing for 24 hours. For G16, first add and review a Production recovery-cleanup ceremony bound to the exact unresolved job/attempt/intent/SKU/offer, remove the unpublished residue, and reconcile the original create as absent; then use a policy-compliant target (a genuine low-stakes physical item, or eBay's designated Production test-listing category) for a fresh create/end proof. Only after Phase 1 exits: MC toggle-offs for price and inventory, the order cutover above, and open incident G1 (Shopify credential rotation, Section 14 G1).
