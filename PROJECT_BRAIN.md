...
- **Deployed:** Railway auto-deploys `main` (G17 baseline `2b48fcd`); domain `ebay-sync-app-production.up.railway.app`; volume `/data`; health endpoint reports `buildCommit`. Listing revise (Inventory + Trading), create, end/relist, price, inventory, new-order-only import, and the inert fulfillment/tracking ceremony are built and deployed behind standalone operator CLIs. Nothing dispatches on deploy or through the server.
- **Activated so far:** production migration store remains schema v3 on `/data/migration-state/product-pipeline-migration-v1.sqlite`; listingRevise ownership is `product_pipeline` version 2. The deployed schema-v4 code therefore fails every migration-store open closed until the explicit v3→v4 upgrade. Draft 1 for Aputure variant `54881767358755` / eBay listing `147232036779` was saved on 2026-08-26, and exact `ucg-branded-v1` preflight passed with only the description changed, price `$164.95` and quantity `5` preserved, and zero external writes. Marketplace Connect still owns price, inventory, and orders; zero ProductPipeline provider dispatches have executed.
...
- **Remaining user-only steps:** back up the migration store off-volume and run the exact-scope v4 `verify → upgrade → verify` ceremony; then approve and execute the exact G10 manifest dispatch. After G10: MC toggle-offs for price and inventory, the order cutover above, and open incident G1 (Shopify credential rotation, Section 14 G1).
...
**G10 — First production listing-revise dispatch.** Status: PREPARED; blocked on the Production schema-v4 operator upgrade and then the one-action human dispatch approval. Draft 1 is saved for catalog `shopify-variant:gid://shopify/ProductVariant/54881767358755`, SKU `APD0170A3B-OB`, Trading listing `147232036779`. Its 2026-08-26 `ucg-branded-v1` preflight passed with manifest digest `sha256:a5a3ef5cd453e5a7c2e64baa39595fdcf13900037d2650d11c35d5e588155bd7`, exactly one description change, price `$164.95` and quantity `5` preserved, and `externalWritesPerformed: 0`. Production store inspection then proved schema v3 while deployed code requires v4, so dispatch was not attempted. Next: backup → exact-scope v4 upgrade/verify → fresh preflight (digest may change if remote state drifted) → human operator executes the exact dispatch → post-dispatch verification and observation window. Authorization boundary: exactly this listing's description; price/quantity byte-preserved; no ownership change.
