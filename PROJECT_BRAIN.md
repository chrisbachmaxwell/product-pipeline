...
- Open release-maintenance incident `RMI-2026-08-14-001`: maintenance tooling exposed credential material through an unexpected Railway remote-shell environment emission and a separate broad eBay credential-page inspection. No commerce write occurred, but that output is not verification evidence. Maintenance is stopped, containment remains in progress, the active deployment still has its earlier environment snapshot, and local draft/proposal writes plus the version-3 migration remain frozen. Treat every exposed class as unsafe until issuer-side revocation or final invalidation is proven. `railway ssh ... sh -lc`, substitute remote shells, environment introspection, and broad DOM/snapshot/network capture on credential pages are forbidden. Future Production maintenance requires exact narrow value-free browser controls plus a reviewed fixed single-purpose command or audited revision-pinned script whose stdout and stderr emit zero environment output, safe store-baseline verification, a consistent backup, and explicit human approval to lift the freeze. Never record values, masked suffixes, user tokens, personal data, or raw output. See `docs/RELEASE_MAINTENANCE_INCIDENTS.md`.
...
- AI proposal rollout gate: current source passed 54 test files / 574 tests, production build, diff check, compiled parity, and independent review with no remaining source P0/P1. It requires listing-control schema version 3 and explicit `upgrade-v2-v3`; the last safely verified Production store remains version 2 and `AI_PROPOSAL_OPENAI_API_KEY` was absent at that verified baseline. A clean replacement proposal key is now staged in Railway's secret-management surface, but it has not been used, deployed, or accepted as runtime proof. Release maintenance is blocked by `RMI-2026-08-14-001`: no incident output establishes current store/configuration state, and no migration, restart, or local write may resume until all issuer-side rotation/revocation, safe fixed-command proof, store re-verification, backup, and explicit human approval are complete. Deployment evidence and signed-in automatic proposal/local-approval verification remain unperformed. See `docs/AI_LISTING_PROPOSALS.md`, `docs/LISTING_CONTROL_ADMIN.md`, and `docs/RELEASE_MAINTENANCE_INCIDENTS.md`.
- Deployment health-gate source candidate: `railway.json` now uses Railway's current official config schema and declares the existing `/health` route as the deployment activation check with a 300-second startup window. A focused regression requests the configured path and requires HTTP `200` plus the shadow-read-only/no-external-writes/no-historical-backfill projection. This source change is not merged or deployed; Railway release configuration, public health, and continuous runtime monitoring remain separate evidence.
