...
> The inert durable-migration-foundation revision `e5a3fa7b42bdeaeac58701829231c3395dfda9c4` was source/build/test verified and observed on the public Railway health endpoint on 2026-08-11; remote Shopify/eBay parity remains separate evidence.
...
- Middleware denies every non-read request beneath `/api` with HTTP `423` before a legacy handler can run. The source server mounts only authenticated migration status, projected local listings, and capability metadata reads; other legacy API reads return `404`.
...
- Shadow server startup does not initialize, migrate, or seed SQLite. Local status/listing reads require an existing database, open it read-only with SQLite `query_only`, and close it; missing or incompatible ledgers fail to unavailable evidence rather than being created or repaired.
...
- The mounted server does not initialize/seed the application database; its two ledger-backed views use a file-must-exist, read-only, query-only connection and treat missing local evidence as unavailable.
...
- Current Shopify/eBay token validity, granted scopes, webhook registrations, and eBay notification subscriptions. The repository requests read-only Shopify scopes, but no live reauthorization or scope inspection occurred.
...
- Whether a future authoritative read collector can run with exact-account, no-refresh, GET/HEAD-only credentials in a reviewed local or Railway one-off boundary.
...
- Published repository/runtime baseline: `main` at `e5a3fa7b42bdeaeac58701829231c3395dfda9c4`. The next administration/projection increment is being developed on `agent/migration-admin-projection`.
...
- Implementation status: the local operator CLI foundation, hard writer quarantine, strict authenticated shadow API, read-only five-page control plane, per-source provenance validation, strict offline snapshot reconciliation, isolated durable migration-state store, and fixture-only read-contract harness are merged. At `2026-08-11T20:22:13.186Z`, the public health endpoint served application revision `e5a3fa7b42bdeaeac58701829231c3395dfda9c4` with `shadow-read-only`, external writes and historical backfill false, a null cutover watermark, remote verification explicitly not performed, and the canonical ten-responsibility matrix. Unauthenticated probes of migration status, legacy orders, and test mode returned `401`; the public web asset matched the reviewed build. This is deployment/runtime-policy evidence, not Shopify/eBay connectivity or parity proof.
...
- Durable-state foundation status: the separate store is explicitly initialized and still unwired to commerce/runtime writers. It models account-scoped external identities, immutable sandbox order watermark, monotonic cursors, idempotency intents and attempts, ownership versions, approvals, jobs, reconciliation exceptions, and tamper-evident audit evidence. Production scope is hard-limited to the accepted Marketplace Connect order/price/inventory baseline plus non-authoritative zero-write shadow evidence; it rejects a ProductPipeline watermark, ownership transfer, writer intent, approval/job, or canary. Fixture-only no-refresh GET/HEAD adapter contracts have no default network or credential path and can never claim live proof. The full repository suite passed 216 tests before publication, including 22 incident, integrity, and runtime-artifact tests for this foundation.
- Current in-progress safe increment: `agent/migration-admin-projection` adds a separate inert migration-state `init`/`verify` administration CLI plus a projection-only request-time read facade for the existing five-page UI. After that increment, choose the trusted collector execution boundary, supply exact-account read authority without exposing secrets, obtain a supported Marketplace Connect export or reviewed attestation, and generate a current version-2 parity packet. Any writer reintroduction, persisted production watermark, live canary, Marketplace Connect responsibility disable, or historical-order import remains blocked pending its own proof and authorization.
