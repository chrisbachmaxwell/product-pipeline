...
- **G16 create incidents (2026-08-27):** the v1 and corrected-v2 exact production create intents each stopped on their first Inventory PUT, freshly proved `created_state_absent` / `confirmed_missing`, and left no eBay artifact. Neither may be replayed. The v2 response was a definite provider non-2xx, but the prior fixed redaction did not retain a safe status or error identifier. The next build exposes only a fixed HTTP family, bounded status, and at most five validated numeric eBay error IDs; only 4xx is definite-no-effect while 5xx remains unknown. It also corrects `Accept_Language` to `Accept-Language` and adds an explicit optional-field omission path. The next reviewed draft must omit its unrelated operational-test condition description; that genuine desired-state correction creates a new manifest, while diagnostics, an identical re-save, or a blind schema bump do not authorize retry.
...
**G16 — First production listing-create and end/relist dispatches.** Status: v1 and v2 creates safely terminalized absent; bounded diagnostic/header/optional-field correction in review. Both exact 2026-08-27 production attempts stopped on their Inventory-item PUT, freshly proved `created_state_absent` / `confirmed_missing`, and left no eBay artifact. V1 had serialized the 4,470-character branded HTML into eBay's 4,000-character Inventory product-description field. V2 corrected that split plus aspects and `GTC`, but the provider still returned a definite non-2xx. The next build safely exposes only HTTP family/status and at most five numeric eBay error IDs, treats 5xx as unknown, corrects `Accept-Language`, and lets the operator explicitly omit the optional condition description. The next draft must genuinely remove its unrelated operational-test text; both completed intents remain immutable and cannot replay. Remaining proof is that corrected manifest's exact create, then a deliberate end/relist exercise, each with post-dispatch verification. Authorization boundary: one exact SKU per ceremony; the create target must not already exist on eBay.
