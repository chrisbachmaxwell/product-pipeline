...
- The evidence-capture source, network, artifact, and CLI contracts are isolated from the mounted runtime and writer code. Fixture verification covers exact identities, scopes, methods, hosts, paths, recent windows, pagination, record/byte limits, PII/secret rejection, signing, canonical private-file handling, and source-schema verification. No source artifact has been collected from live Shopify or eBay.
...
- Current Shopify/eBay token validity, granted scopes, webhook registrations, and eBay notification subscriptions. The new collector verifies least-privilege Shopify scopes and requires exact eBay read-scope/expiry metadata when it runs, but this task environment had no collector configuration or ephemeral authority and no live scope inspection occurred.
...
- Published application/runtime baseline: reviewed PR #3 merged as `550cf2384a16e6f57f03d315643f3cc1337b4b7d`; later documentation-only evidence commits do not change that runtime behavior. The authoritative-read capture increment is implemented and locally verified on `agent/authoritative-read-collector`, based on `4779bcc`, and is pending reviewed publication.
...
- Runtime actions for this slice: no deployment or external commerce request has occurred yet. No order import/sync, product sync, listing mutation, setting change, credential read, OAuth acquisition/refresh, direct Shopify/eBay/Marketplace Connect access, or Marketplace Connect change occurred.
