...
- Published application/runtime baseline: Railway deployed PR #10 merge `e0d59cd904209c30e815f6cf6a2e4e784208efc5`; public `/health` served that exact build at `2026-08-14T15:55:08.152Z` with shadow read-only mode, external writes disabled, historical backfill disabled, null order watermark, and the full provider-writer quarantine enabled. Health and store verification do not prove the item workspace.
- Open live incident with a green local repair candidate: signed-in verification found the deployed listing workspace returning generic `503` after successful eBay reads because an artificial local 100,000-byte description cap rejected valid 147,595-byte and 144,209-byte Production descriptions. No local draft was appended and no provider write occurred. The independently reviewed local repair aligns parser/store description limits at 500,000 Unicode code points and 2,000,000 UTF-8 bytes; observed-size, exact-boundary, overflow/control, 300 KB inherited-save/reopen/audit/integrity, and non-description-limit regressions pass. Full verification is 48 files / 504 tests, `npm run build`, and `git diff --check`. The incident remains open because commit, merge, deploy, exact repair health revision, signed-in affected-item proof, and post-deploy store verification are pending. See `docs/LISTING_WORKSPACE_INCIDENTS.md`.
- Inspection boundary: repository source/history, exact public Railway health, Railway single-replica/volume/store administration evidence, and signed-in Shopify/embedded-app read verification. The live `503` disproves complete embedded-workspace readiness for the deployed commit even though health and store checks pass.
...
- Local-draft release status: the PR #10 source/merge/deployment identity, single-replica volume topology, explicit schema-version-2 initialization/verification, and baseline backup are evidenced. LWI-2026-08-14-001 now has a locally green, independently reviewed repair candidate, but the deployed commit remains broken for affected workspaces. Do not describe the feature as Production-working until the repair is committed, merged, deployed, verified live, and the store is re-verified.
...
- Current safe implementation gate: freeze the reviewed repair candidate, create and record one exact repair commit, merge/deploy it without unrelated changes, verify `/health` serves that commit, re-open an affected signed-in workspace, and re-verify schema version 2 plus zero external writes. Re-run the 48-file / 504-test suite, build, diff check, and review if the candidate changes. Any Apply/Approve/Publish path, provider writer, ownership transfer, persisted order watermark, Marketplace Connect change, live canary, or historical-order import remains separately blocked and unauthorized.
