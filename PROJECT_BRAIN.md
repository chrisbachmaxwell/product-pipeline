...
- Current `origin/main` merge `579cc077a6ca4930fbfa88d415b80cc04c12d963` contains the fixed-purpose Shopify credential administrator and bounded current/previous-secret inbound verification. It preserves the exact Production Railway/store/app/database/four-scope pins, one no-retry provider credential request, full private backup, exact Shopify-row CAS, fixed redacted output, and zero commerce-write boundary. Merge identity is not deployment, provider execution, database mutation, or live verification.
- The eBay integration candidate preserves those Shopify root commands and routes eBay maintenance only through the literal `credential-admin ebay ...` family. Each provider retains separate configuration, provider transport, validation, mutation, failure output, and recovery semantics; the shared compiled entrypoint performs only fixed-family dispatch and reflects no rejected argument value. Shopify rotation rejects a present eBay rotation Cert variable, and eBay Railway commands reject a present Shopify rotation acknowledgement or temporary refresh-token variable, preventing overlapping credential writers against the shared ledger. Neither family is mounted or imported by the server or a commerce writer.
- The combined integration supports only direct `node dist/credential-admin/index.js ...` invocation. Its npm argument-forwarding wrapper was removed because npm can echo raw appended arguments before either provider parser can redact them; a sentinel regression requires the package surface and every script reference to this compiled entrypoint to remain absent. Verification passed the focused 10-file / 135-test credential and inbound-security suite, the full 55-file / 625-test repository suite, TypeScript no-emit, Production build, direct compiled help, malformed-argument and absent-wrapper redaction, fresh-compile source/dist parity, whitespace, runtime-import/process-launch, and credential-pattern scans. These are local fake-transport and temporary-database proofs only.
- A standalone fixed-purpose eBay Production credential-administrator candidate now exists under `src/credential-admin/`. It uses a local private raw-state consent artifact plus digest-only Railway registration, no-echo callback input, the dedicated new Cert only, exact base-plus-`sell.inventory` scopes, strict Production app/audience and Trading `GetUser` seller `usedcameragear` proof, a read-only Inventory probe, pre-exchange private backup, exact legacy-ledger schema/trigger checks, and one `BEGIN IMMEDIATE` full-baseline CAS upsert of only the eBay auth row without delete-first. Before CAS it file- and parent-directory-syncs private domain-separated access/refresh token digests; verify and cleanup require the exact digest-bound row without minting or persisting, and an exact commit-pending row can be reconciled after a crash immediately following COMMIT. A COMMIT error is classified only after closing SQLite: exact installed, exact backup baseline, and unreadable/mismatched outcomes preserve evidence and truthfully report one, zero, or unknown database-row effect, never automatically revoke, and require exit-`5` reconciliation. Ambiguous/failed evidence can be reset only after exact provider-reconciliation confirmation; a COMMIT-error state additionally requires proof that the fixed ledger exactly matches its named private pre-install backup. Evidence is archived without deletion. Post-commit failures preserve database/provider effect truth; evidence and stale-lock renames sync the target directory before the source, and stale locks require exact owner/time/expiry/dead-PID proof. The entrypoint is not mounted or imported by runtime/writer code. The original eBay candidate is commit `665e9ca`; its combined integration onto current `origin/main` remains uncommitted pending review. Neither source state has been deployed or run against Railway, eBay, or the live ledger, and neither relaxes the writer quarantine.
...
- Whether the prior ProductPipeline eBay seller consent and old Production Cert have been safely revoked/reset, whether a new Cert has been stored in the exact Railway service, and whether one fresh exact-scope grant can pass install/verify and the mounted catalog's signed-in read-only observation. An ambiguous exchange requires provider reconciliation and a completely new consent state; it is never permission to replay.
...
- Shopify credential-rotation source: commits `3309dfd` and `8187878` merged through PR #15 as current `origin/main` `579cc077a6ca4930fbfa88d415b80cc04c12d963`. It adds exact-store current/previous-secret JWT and webhook verification plus a standalone fixed-purpose compiled credential administrator. The previous-secret overlap is bounded to one hour and continues primary verification at cutoff; a rotation dispatch requires distinct old/new secrets and at least 15 minutes remaining on both the overlap and the dedicated single-writer acknowledgement. Token acquisition uses only the current/new secret. The merge has not been treated here as deployment or live proof.
...
- Original Shopify proof boundary: focused 8-file / 88-test security checks, the full 54-file / 582-test suite, TypeScript no-emit, Production build, compiled command help and malformed-argument redaction checks, and whitespace checks passed before merge. No Shopify/eBay provider, Railway, Production database, Marketplace Connect, or Lightspeed mutation was performed by that source work. Historical read-only preflight observed deployment `259f4262-0943-4c26-a47b-6b722f73fc75` at old revision `234e0cb4de8aeafe494492f7039317915969b9aa`; that is not proof the merged release is deployed. Incident `SCR-2026-08-14-001` remains open until exact deployment proof, fixed Production preflight/rotate/verify, variable cleanup, and credential-free closure evidence. See `docs/SHOPIFY_CREDENTIAL_ROTATION.md` and `docs/RELEASE_MAINTENANCE_INCIDENTS.md`.
- Incident-only deployment decision: a clean new Shopify secondary secret and protected Railway new-primary/old-previous values were already committed while the active deployment remained on its old-primary snapshot. The reviewed release may deploy directly only with a fresh canonical cutoff no more than one hour ahead and every gate in the incident runbook. Before the provider token-rotation request, failure requires rollback to deployment `259f4262-0943-4c26-a47b-6b722f73fc75` and a stop; at or after that request, or after old-secret revocation, rollback is prohibited and forward reconciliation is required. This exception does not generalize to later rotations.
- eBay credential-maintenance integration candidate: isolated worktree `/Users/chrismaxwell/Documents/Codex/2026-08-11/project-pipeline/work/product-pipeline-ebay-rotation`, branch `codex/ebay-credential-rotation`. Original PR #16 commit `665e9ca` was based on `234e0cb`; the current rebase resolution targets `origin/main` `579cc077a6ca4930fbfa88d415b80cc04c12d963` and remains uncommitted/unpushed pending independent review. It has made no provider, Railway, deployment, secret, or live-database change. Review `docs/EBAY_CREDENTIAL_ROTATION.md` and `docs/CREDENTIAL_MAINTENANCE_INCIDENTS.md`; do not execute the runbook until a human authorizes the disruptive provider closure/reset window. A terminal failed attempt cannot be retried until exact provider reconciliation is confirmed and its evidence is archived; post-commit and stale-lock conditions follow the durable exit-`5` paths, while commit-pending state requires exact-bound verification or manual reconciliation instead of reinstalling or deleting state.
- Continuation checkout for this integration: use only the isolated eBay worktree and branch above; do not reset, clean, or stage from the sibling `work/product-pipeline` checkout or the Shopify/hotfix worktrees. After independent review, finish the paused rebase and update PR #16 with the reviewed commit using a safe lease-protected workflow.
