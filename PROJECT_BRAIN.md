...
- Shopify credential-rotation source candidate: isolated branch `codex/shopify-credential-rotation` adds exact-store current/previous-secret JWT and webhook verification plus a standalone fixed-purpose compiled credential administrator. The previous-secret overlap is bounded to one hour and continues primary verification at cutoff; a rotation dispatch requires distinct old/new secrets and at least 15 minutes remaining on both the overlap and the dedicated single-writer acknowledgement. Token acquisition uses only the current/new secret.
...
- Candidate proof boundary: focused 8-file / 88-test security checks, the full 54-file / 582-test suite, TypeScript no-emit, Production build, compiled command help and malformed-argument redaction checks, and whitespace checks pass locally. No Shopify/eBay provider, Railway, Production database, Marketplace Connect, or Lightspeed request or mutation was performed; the candidate is not committed, deployed, or live-verified. Incident `SCR-2026-08-14-001` remains open until independent source review, exact deployment proof, fixed Production preflight/rotate/verify, variable cleanup, and credential-free closure evidence. See `docs/SHOPIFY_CREDENTIAL_ROTATION.md` and `docs/RELEASE_MAINTENANCE_INCIDENTS.md`.
