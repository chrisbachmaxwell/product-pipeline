...
7. If cleanup is unresolved, use zero-write `reconcile-cleanup` when the full expected effect is present; never repeat the consumed cleanup dispatch. For exact remaining offer/item residue, run `preflight-recovery-cleanup` bound to the original source responsibility/job/attempt/intent **and its exact approval evidence digest** (`actionDigest`, `cleanupDigest`, or prior `recoveryDigest` as applicable), separately run `approve-recovery-cleanup`, then `dispatch-recovery-cleanup`. That new approval can delete only the exact freshly rediscovered residue. If its response is unknown, use `reconcile-recovery-cleanup`; further residue requires another separately preflighted recovery chained to that outstanding recovery job and its evidence digest. Commands and required exact arguments are shown by `--help`.

Every reconciliation checks the stored job's `approvalEvidenceDigest` before appending provider evidence. A CLI digest that does not equal the digest consumed when that exact job was reserved is denied, even when job, attempt, intent, target, responsibility, and remote state otherwise match.
