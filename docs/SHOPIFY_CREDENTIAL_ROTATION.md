...
The reviewed diagnostic was subsequently merged and deployed as `a50ecfaa0e06cd4d215aba37c0858bf116b8f17c` on deployment `63bce306-5455-4c77-af7e-f441f4684b0b`, one replica. Public `/health` at `2026-08-14T21:40:06.140Z` reported `ok`, `shadow-read-only`, external writes false, and historical backfill false. A direct option-free diagnostic run shortly afterward returned only `file-permissions-denied`: the fixed target existed, was a regular non-symlink single-link nonempty bounded file, and was not mode `0600`. Later parent, sidecar, descriptor, schema, integrity, and cardinality checks were not performed. Database, provider-network, credential, provider-credential, and commerce effect counters were all zero. No exact command timestamp was separately captured.

Maintenance remains stopped at that exact local permission gate. Do not generate another dashboard refresh token, restore or edit the database, rerun preflight, request token rotation, revoke the old secret, or remove the previous-secret verifier. Both fresh independent reviewers returned GO with no P0/P1 on the exact narrow permission-repair manifest `fed26023563a49c1f8b4be10dce5d6ea41bad8a5bbf2c7958150af522d073f4c`; the 6-file/142-test focused suite, 56-file/667-test full suite, TypeScript, Production build, compiled parity, redaction, capability, and diff gates are green. The candidate remains uncommitted, unpushed, without a PR, unmerged, undeployed, and unrun against Production. Repair topology variables, exact deployment identity, one live invocation, and post-repair diagnostic/health/DB-backed-read proof are pending. Review and local gates do not authorize a run.
...
### Fixed database-permission repair

This incident-only reconciliation is eligible for review only because the deployed fixed diagnostic returned exactly `file-permissions-denied` and proved the earlier file facts above. Do not use a shell `chmod`, path-based permission call, generic database tool, SQLite client, source entrypoint, npm wrapper, or command-line option. Do not run this procedure for any other diagnostic stage.

Before a reviewed repair release is deployed, independently preserve all of these gates:

1. The exact reviewed commit and compiled artifact are the deployed one-replica release on the exact Railway project, environment, and service listed above; only one `/data` volume is attached.
2. Public health and the signed-in embedded app are available in shadow-read-only mode with external writes and historical backfill false. No process or operator is performing database maintenance.
3. `LISTING_CONTROL_SINGLE_WRITER_ACK`, `SHOPIFY_CREDENTIAL_ROTATION_SINGLE_WRITER_ACK`, `SHOPIFY_CREDENTIAL_ROTATION_SINGLE_WRITER_ACK_EXPIRES_AT_UTC`, and `SHOPIFY_ROTATION_REFRESH_TOKEN` are absent. Do not create a dashboard refresh token.
4. Through the protected control plane, set the two nonsecret, exact operator assertions `SHOPIFY_DATABASE_PERMISSION_REPAIR_REPLICA_COUNT=1` and `SHOPIFY_DATABASE_PERMISSION_REPAIR_VOLUME_MOUNT_COUNT=1` only after visually confirming those facts. They are assertions, not discovery; a replica ID or mounted path alone is insufficient proof.
5. Run the read-only diagnostic once immediately before repair. It must again return exactly `file-permissions-denied` with the same completed file checks and all effect counters zero. Any different stage stops the procedure.

Only after those gates and fresh independent review, invoke this one option-free direct compiled command once:

```sh
node dist/credential-admin/index.js repair-shopify-credential-database-permissions
```

The command accepts no path, mode, identity, token, SQL, output, repair, or retry option. It revalidates the exact Production/app/path binding, absent listing/rotation authority, and both topology assertions before filesystem access. It checks the fixed target, private parent, and SQLite sidecar absence; opens the target and parent with `O_RDONLY`/`O_NOFOLLOW`; and binds the database by device, inode, type, single-link count, UID, GID, and size. The current Dockerfile has no `USER` override and the Railway start command runs the same image directly, so this release's exact runtime contract is effective UID `0`; the command checks its real effective UID and fails closed on any other value while continuing to prove the file and parent UID/GID never change. It validates the original mode and privately retains the bounded size, content digest, and modification time only for before/after proof. Only the held database descriptor can receive one `fchmod(0600)`. The command then syncs the file and parent descriptors and proves the same identity, UID/GID, size, content digest, modification time, fixed path, and sidecar absence before close. It cannot query database rows, read token values, contact a provider, edit database content, or change commerce state.

Successful output is one fixed redacted report with `status: "permission_repair_verified"`, `stage: "verified"`, `permissionMetadataWritesPerformed: 1`, `databaseContentWritesPerformed: 0`, and zero provider-network, credential, provider-credential, and commerce writes. The permission field is deliberately trinary: `0` means the repair fchmod was not invoked, `1` means establishment of mode `0600` was verified at least once, and `"unknown"` means the sole fchmod invocation's effect could not be proved. The report exposes no path, mode integer, UID, GID, digest, filename, suffix, token, secret, row, provider body, or driver error.

On any failure before `fchmod`, the command returns `permission_repair_failed_closed` with `permissionMetadataWritesPerformed: 0`. The implementation contains exactly one metadata-write call: the initial descriptor-bound `fchmod(0600)`. It never automatically restores the original mode, performs a rollback fchmod, or makes any second metadata write. After that one fchmod is invoked, an exception, close ambiguity, concurrent third mode such as `0666` or `0400`, UID/GID or identity drift, growth, content or mtime change, parent/path substitution, or sidecar change returns `permission_repair_outcome_unknown` with `permissionMetadataWritesPerformed: 1` when establishment of `0600` was already proved and `"unknown"` otherwise. The command leaves any later concurrent mode untouched. A disconnect or unknown result is never a reason to rerun.

After any return or disconnect:

1. Preserve only the fixed redacted result; do not inspect environment values or command history.
2. Run the option-free read-only diagnostic once. `database_diagnostic_verified` classifies the mode and all later database checks as safe to continue to a separately reauthorized rotation preflight. `file-permissions-denied` classifies the original permission gate as still present. Any other stage is a new stopped reconciliation boundary.
3. Recheck public health, shadow-read-only policy, and one expected signed-in DB-backed embedded-app read. An availability failure or ambiguous command outcome requires escalation; never blindly retry the repair or attempt a mode restore.
4. Remove the two permission-repair topology assertion variables through the protected control plane and prove the resulting exact deployment/health. Keep all rotation acknowledgement and refresh-token variables absent.
5. Only a successful post-repair diagnostic plus fresh release, topology, inbound, acknowledgement, and overlap gates may authorize a later preflight. The repair itself never authorizes token rotation.
