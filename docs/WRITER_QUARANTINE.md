...
The fixed-purpose `credential-admin` executable is a release-maintenance boundary, not an application writer. It is never imported or mounted by the server, accepts no path or identity options, and can update only the one exact legacy Shopify token row after Production identity/scope verification, a complete private backup, and a single no-retry provider credential request. It has no listing, order, price, inventory, fulfillment, policy, mapping, Marketplace Connect, or Lightspeed write adapter. See `docs/SHOPIFY_CREDENTIAL_ROTATION.md`.
