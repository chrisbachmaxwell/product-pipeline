...
function registerReviseIdentities(store: MigrationStore): {
  variant: ExternalIdentity;
  listing: ExternalIdentity;
} {
...
function recordReviseOwnershipChain(store: MigrationStore): void {
...
describe('migration store schema upgrade', () => {
  function createVersionOneStoreFile(scope: IntegrationScope, createdAtUtc: string): string {
