...
type SeededCreate = {
  ...
  databasePath: string;
  variantIdentityKey: Digest;
  skuIdentityKey: Digest;
  createIntentKey: Digest;
  createManifestDigest: Digest;
};
...
function seedUnresolvedCreate(): SeededCreate {
  tick = 0;
  ...
  for (const ownership of [
    { version: 1, owner: 'paused' as const, atUtc: at(3) },
    { version: 2, owner: 'product_pipeline' as const, atUtc: at(4) },
  ]) {
...
function seedRecoveryCeremony(seeded: SeededCreate): {
  recoveryIntentKey: Digest;
...
function recordRemovalRun(seeded: SeededCreate, input: {
  runId: string;
  intentKey: Digest;
  effect?: 'effect_observed' | 'effect_absent' | 'effect_residue_removed';
  startedAtUtc: string;
  ...
  it('admits a production recovery intent only against an unresolved create job on the exact target', () => {
    ...
    try {
      ...
      expect(() => raw.prepare(
        ...
        ) VALUES (?, ?, 'listingCreate', 'recover_create_ebay_listing', ?, ?, ?, ?, ?, ?)`,
      ).run(
        intentKey,
        deriveScopeKey(PRODUCTION_SCOPE),
        seeded.variantIdentityKey,
        foreignSku.identityKey,
        foreignSku.identityKey,
        digest('raw-speculative-recovery'),
        at(12),
        Date.parse(at(12)),
      )).toThrow(/recovery intent requires an unresolved create job on the exact target/);
      ...
      expect(() => raw.prepare(
        `INSERT INTO idempotency_intents (
          ...
          target_identity_key, approval_target_identity_key, desired_state_digest,
          created_at_utc, created_epoch_ms
        ) VALUES (?, ?, 'mapping', 'update_mapping', ?, ?, ?, ?, ?, ?)`,
      ).run(
        digest('raw-mapping-intent'),
        deriveScopeKey(PRODUCTION_SCOPE),
        seeded.variantIdentityKey,
        seeded.skuIdentityKey,
        seeded.skuIdentityKey,
        digest('raw-mapping-manifest'),
        at(13),
        Date.parse(at(13)),
    ...
    try {
      ...
      expect(() => raw.prepare(
        `INSERT INTO target_effect_observations (
          ...
          'price', 'effect_residue_removed', ?, ?, ?)`,
      ).run(
        seeded.createIntentKey,
        seeded.skuIdentityKey,
        digest('raw-price-observed'),
        at(27),
        Date.parse(at(27)),
      )).toThrow(/CHECK|listingCreate|binding/);
      ...
      ).run(digest('forged'), at(28), Date.parse(at(28))))
      ...
      expect(() => raw.prepare(
        `INSERT INTO job_events (
          ...
          'resolved_residue_removed', ?, ?, ?)`,
      ).run(digest('forged2'), at(28), Date.parse(at(28))))
  ...
  it('keeps the rebuilt tables append-only and replay-denied after the v5 rebuild', () => {
    ...
    try {
      ...
      for (const [table, mutation] of [
        ...
        expect(() => raw.prepare(mutation).run(), mutation).toThrow(
          new RegExp(`${table} is append-only`),
        );
      }
      expect(() => raw.prepare(
        'INSERT INTO idempotency_intents SELECT * FROM idempotency_intents',
      ).run()).toThrow(/replay or replacement denied/);
      // Replaying a resolution is denied — by the replay trigger or the
      // authoritative-reconciliation gate, whichever fires first.
      expect(() => raw.prepare(
        'INSERT INTO attempt_resolutions SELECT * FROM attempt_resolutions',
      ).run()).toThrow(/replay or replacement denied|authoritative target reconciliation/);
    } finally {
      raw.close();
    }
  });
  ...
  it('upgrades a verified v4 store to v5 explicitly and leaves it operable', () => {
    ...
    const populate = database.transaction(() => {
      database.prepare(
        ...
        Date.parse('2026-08-27T16:00:00.000Z'),
      );
      database.prepare(
