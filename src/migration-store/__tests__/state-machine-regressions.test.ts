...
type ReservedOrder = {
  ...
  ebayOrder: ExternalIdentity;
  intentKey: Digest;
  approvalToken: string;
  approvalEvidenceDigest: Digest;
};

function createReservedPostWatermarkOrder(): ReservedOrder {
...
describe('migration-store order incident state machine', () => {
  it('permanently denies pre-watermark and equality-boundary order intents', () => {
    const store = createStore();
    const registerOrder = (suffix: string, occurredAtUtc: string): ExternalIdentity =>
    ...
    for (const [identity, eventId] of [[before, 'before'], [equal, 'equal']] as const) {
  ...
  it('denies dispatch when an observed Shopify link appears after reservation', () => {
    const { store, intentKey, approvalToken, approvalEvidenceDigest } =
      createReservedPostWatermarkOrder();
    const ebayOrder = store.getIntent(intentKey);
    if (!ebayOrder) throw new Error('reserved order intent fixture was not created');
  ...
  it('requires post-dispatch reconciliation before one terminal linked outcome', () => {
    const { store, ebayOrder, intentKey, approvalToken, approvalEvidenceDigest } =
      createReservedPostWatermarkOrder();
