...
const insertConflictPredicates: Readonly<Record<string, string>> = {
  ...
  order_observation_resolutions:
    'resolution_id = NEW.resolution_id OR observation_id = NEW.observation_id',
...
const actionResponsibilitySql = Object.entries(INTENT_ACTION_RESPONSIBILITY)
  .map(([action, responsibility]) =>
    `(NEW.action = '${action}' AND NEW.responsibility = '${responsibility}')`)
  .join('\n    OR ');
...
/**
 * Schema version 4 — the inert G17 fulfillment/tracking slice.
 *
 * This widens production capability for exactly one previously denied
 * Class-B responsibility: fulfillment. Marketplace Connect remains the
 * truthful genesis owner and ProductPipeline ownership still requires the
 * staged MC -> paused -> ProductPipeline evidence chain. The existing
 * sync_fulfillment action, production-canary reconciliation, and durable
 * target-effect observation machinery are extended without enabling mapping
 * or feedback.
 */
  ...
  WHERE scope.scope_key = NEW.scope_key
    ...
    AND NOT (
      (
        NEW.version = 1
        AND NEW.owner = 'marketplace_connect'
        AND NEW.responsibility IN ('orderImport', 'price', 'inventory', 'fulfillment')
      )
      OR NEW.responsibility IN ('listingCreate', 'listingRevise', 'listingEndRelist')
      OR (
        NEW.responsibility IN ('orderImport', 'price', 'inventory', 'fulfillment')
        AND NEW.version > 1
        AND EXISTS (
          SELECT 1 FROM ownership_versions previous
          WHERE previous.scope_key = NEW.scope_key
  ...
  WHERE scope.scope_key = NEW.scope_key
    ...
    AND NOT (
      ...
      OR (
        NEW.mode = 'production_canary'
        AND NEW.responsibility IN (
          'orderImport', 'price', 'inventory', 'fulfillment',
          'listingCreate', 'listingRevise', 'listingEndRelist'
        )
        AND NEW.external_writes_observed = 0
      )
...
ALTER TABLE target_effect_observations RENAME TO target_effect_observations_v3;
CREATE TABLE target_effect_observations (
  observation_id TEXT PRIMARY KEY,
  run_id TEXT NOT NULL UNIQUE REFERENCES reconciliation_runs(run_id),
  intent_key TEXT NOT NULL REFERENCES idempotency_intents(intent_key),
...
INSERT INTO target_effect_observations SELECT * FROM target_effect_observations_v3;
DROP TABLE target_effect_observations_v3;
 ...
 */
function applyMigrationsWithDeferredForeignKeys(
  database: InstanceType<typeof Database>,
  migrate: () => void,
): void {
...
function installSchema(
  database: InstanceType<typeof Database>,
  appliedAtUtc: string,
  throughVersion: number = CURRENT_SCHEMA_VERSION,
): void {
...
function canonicalCatalog(database: InstanceType<typeof Database>): Array<{
  type: string;
  name: string;
  tableName: string;
  sql: string;
...
function expectedCatalogDigest(throughVersion: number = CURRENT_SCHEMA_VERSION): string {
...
const expectedCatalogDigestByVersion = new Map<number, string>(
  SCHEMA_MIGRATIONS.map((migration) => [migration.version, expectedCatalogDigest(migration.version)]),
);
...
export function verifySchemaAtExactVersion(
  database: InstanceType<typeof Database>,
  requiredVersion: number,
): void {
  const expectedDigest = expectedCatalogDigestByVersion.get(requiredVersion);
  const expectedMigrations = SCHEMA_MIGRATIONS.filter(
    (migration) => migration.version <= requiredVersion,
  );
  ...
  if (userVersion !== requiredVersion) {
    throw new Error(
      `Migration store schema version ${String(userVersion)} does not match required version ${requiredVersion}`,
    );
  }
...
export function verifySchema(database: InstanceType<typeof Database>): void {
 ...
 */
export function verifiedStoredSchemaVersion(database: InstanceType<typeof Database>): number {
  const userVersion = database.pragma('user_version', { simple: true });
 ...
 */
export function upgradeSchemaToCurrent(
  database: InstanceType<typeof Database>,
  appliedAtUtc: string,
