...
import {
  ALL_INTENT_ACTIONS_V6,
  ALL_INTENT_ACTION_RESPONSIBILITY_V6,
...
class MigrationStoreImpl {
  ...
  createIdempotencyIntent(input: {
    ...
    if (
      input.action === 'recover_orphaned_artifact_ebay_listing'
      && this.database.prepare(
        `SELECT 1
         FROM execution_jobs job
         JOIN idempotency_intents source_intent ON source_intent.intent_key = job.intent_key
      ...
      throw new MigrationStoreError(
        'OWNERSHIP_DENIED',
        'Orphan recovery intent requires a resolved create job on the exact target',
      );
    }
  ...
  getOperationalMonitoring(nowUtc: string): OperationalStoreMonitoring {
    ...
    const currentRows = this.database.prepare(
      `SELECT latest.to_state AS state, COUNT(*) AS count
       FROM execution_jobs job
       JOIN job_events latest ON latest.job_id = job.job_id
       WHERE job.scope_key = ?
         AND latest.sequence = (
       ...
       GROUP BY latest.to_state`,
    ).all(this.scopeKey) as Array<{ state: string; count: number }>;
    const current = Object.fromEntries(currentRows.map((row) => [row.state, row.count]));
    const count = (query: string, ...parameters: unknown[]): number => {
      const row = this.database.prepare(query).get(...parameters) as { count: number };
      return row.count;
    };
    const between = [this.scopeKey, startEpochMs, endEpochMs] as const;
    ...
    return Object.freeze({
      ...
      previousUtcDay: Object.freeze({
        ...
        writes: Object.freeze({
          performed: count(
            `SELECT COUNT(*) AS count FROM intent_attempts attempt
             JOIN execution_jobs job ON job.job_id = attempt.job_id
             WHERE job.scope_key = ?
          ...
          unresolved: count(
            `SELECT COUNT(*) AS count FROM intent_attempts attempt
             ...
             WHERE job.scope_key = ? AND resolution.attempt_id IS NULL
               AND attempt.recorded_epoch_ms >= ? AND attempt.recorded_epoch_ms < ?`,
            endEpochMs, this.scopeKey, startEpochMs, endEpochMs,
          ),
        }),
        ...
        exceptions: Object.freeze({
          info: count(
            `SELECT COUNT(*) AS count FROM reconciliation_exceptions exception
