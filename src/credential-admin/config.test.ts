...
function environment(overrides: Record<string, string | undefined> = {}) {
  return {
    ...
    SHOPIFY_CREDENTIAL_ROTATION_SINGLE_WRITER_ACK:
      PRODUCT_PIPELINE_PRODUCTION_RUNTIME.singleWriterAck,
...
function load(overrides: Record<string, string | undefined> = {}, requireRefreshToken = true) {
...
describe('Shopify credential rotation Production binding', () => {
  it('allows the read-only database diagnostic with only exact nonsecret binding', () => {
    ...
    expect(() => assertShopifyCredentialDatabaseDiagnosticRuntimeBinding(
  ...
  it('allows permission repair only with exact topology proof and no writer or rotation authority', () => {
    ...
    expect(() => assertShopifyCredentialDatabasePermissionRepairRuntimeBinding(
      repairEnvironment,
    )).not.toThrow();
  });
  ...
  it('revalidates the carried authorization at the exact dispatch/CAS cutoff', () => {
    const loaded = load();
    expect(() => assertShopifyCredentialRotationAuthorizationActive(
      loaded,
      loaded.authorizationExpiresAtEpochMs - 1,
    )).not.toThrow();
    expect(() => assertShopifyCredentialRotationAuthorizationActive(
      loaded,
      loaded.authorizationExpiresAtEpochMs,
    )).toThrow(expect.objectContaining({ code: 'configuration-denied' }));
  });
  ...
  it('requires both ACK and old/new verifier overlap to retain the fixed dispatch window', () => {
    ...
    expect(() => assertShopifyCredentialRotationDispatchAuthorized(
      loaded,
      Math.min(
        loaded.authorizationExpiresAtEpochMs,
        loaded.previousClientSecretExpiresAtEpochMs ?? 0,
      ) - minimum,
    )).not.toThrow();
    expect(() => assertShopifyCredentialRotationDispatchAuthorized(
      loaded,
      loaded.authorizationExpiresAtEpochMs - minimum + 1,
    )).toThrow(expect.objectContaining({ code: 'configuration-denied' }));
  });
});
