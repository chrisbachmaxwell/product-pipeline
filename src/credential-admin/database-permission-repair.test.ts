...
import {
  ...
  SHOPIFY_DATABASE_PERMISSION_REPAIR_STAGES,
...
function fixture(mode = ORIGINAL_MODE): Readonly<{ root: string; databasePath: string }> {
...
function mapPath(loaded: Readonly<{ root: string; databasePath: string }>, value: fs.PathLike) {
  const text = String(value);
  if (text === FIXED_PATH) return loaded.databasePath;
  if (text === path.dirname(FIXED_PATH)) return loaded.root;
  if (text.startsWith(FIXED_PATH)) return `${loaded.databasePath}${text.slice(FIXED_PATH.length)}`;
  throw Object.assign(new Error('unexpected fixed path'), { code: 'EPERM' });
}

function dependencies(
  loaded: Readonly<{ root: string; databasePath: string }>,
): ShopifyDatabasePermissionRepairDependencies {
  return {
    filesystem: {
      lstatSync: ((value: fs.PathLike) => fs.lstatSync(mapPath(loaded, value))) as typeof fs.lstatSync,
      openSync: ((value: fs.PathLike, flags: fs.OpenMode) =>
        fs.openSync(mapPath(loaded, value), flags)) as typeof fs.openSync,
...
function run(
  loaded: Readonly<{ root: string; databasePath: string }>,
  dependencyOverrides: ShopifyDatabasePermissionRepairDependencies = {},
  environment: Readonly<Record<string, string | undefined>> = REPAIR_ENVIRONMENT,
) {
...
describe('fixed Production Shopify database permission repair', () => {
  ...
  it('accepts the exact root runtime contract without emitting either uid', () => {
    ...
    const differentOwner = (stat: fs.Stats) => new Proxy(stat, {
      get(target, property, receiver) {
        if (property === 'uid') return target.uid + 1000;
    ...
    const result = run(loaded, {
      getEffectiveUid: () => 0,
      filesystem: {
        lstatSync: ((value: fs.PathLike) => differentOwner(
          fs.lstatSync(mapPath(loaded, value)),
        )) as typeof fs.lstatSync,
        fstatSync: ((descriptor: number) => differentOwner(
  ...
  it('reports parent descriptor open, inspection, and identity failures before fchmod', () => {
    ...
    const inspectResult = run(inspect, {
      filesystem: {
        openSync: ((value: fs.PathLike, flags: fs.OpenMode) => {
          ...
          if (String(value) === path.dirname(FIXED_PATH)) parentDescriptor = descriptor;
          return descriptor;
        }) as typeof fs.openSync,
    ...
    let identityParentDescriptor: number | null = null;
    const identityResult = run(identity, {
      filesystem: {
        openSync: ((value: fs.PathLike, flags: fs.OpenMode) => {
          const descriptor = fs.openSync(mapPath(identity, value), flags);
          if (String(value) === path.dirname(FIXED_PATH)) identityParentDescriptor = descriptor;
          return descriptor;
        }) as typeof fs.openSync,
        fstatSync: ((descriptor: number) => {
          const actual = fs.fstatSync(descriptor);
          if (descriptor !== identityParentDescriptor) return actual;
  ...
  it.each([0o666, 0o400])(
    'does not overwrite concurrent same-inode mode drift to %s after repair',
    (concurrentMode) => {
      const loaded = fixture();
      let fchmodCalls = 0;
      let reads = 0;
      const result = run(loaded, {
        filesystem: {
          fchmodSync: ((descriptor: number, mode: number) => {
            ...
            if (fchmodCalls === 1) fs.fchmodSync(descriptor, concurrentMode);
          }) as typeof fs.fchmodSync,
          readSync: ((...args: Parameters<typeof fs.readSync>) => {
            reads += 1;
            return (fs.readSync as (...parameters: Parameters<typeof fs.readSync>) => number)(...args);
  ...
  it.each(['database', 'parent'] as const)(
    'binds %s gid after fchmod and never writes again after concurrent ownership drift',
    (target) => {
      ...
      const result = run(loaded, {
        filesystem: {
          openSync: ((value: fs.PathLike, flags: fs.OpenMode) => {
            ...
            if (String(value) === FIXED_PATH) databaseDescriptor = descriptor;
            if (String(value) === path.dirname(FIXED_PATH)) parentDescriptor = descriptor;
            return descriptor;
          }) as typeof fs.openSync,
          fstatSync: ((descriptor: number) => {
            const actual = fs.fstatSync(descriptor);
            const shouldDrift = drifted && (
              (target === 'database' && descriptor === databaseDescriptor)
              || (target === 'parent' && descriptor === parentDescriptor)
            );
            if (!shouldDrift) return actual;
            return new Proxy(actual, {
              get(stat, property, receiver) {
                if (property === 'gid') return stat.gid + 1;
  ...
  it('detects post-fchmod content drift with masked mtime and never writes again', () => {
    ...
    let fchmodCalls = 0;
    let reads = 0;
    const stableMtime = (stat: fs.Stats) => new Proxy(stat, {
      get(target, property, receiver) {
        if (drifted && property === 'mtimeMs') return initialMtime;
        return Reflect.get(target, property, receiver);
      },
    });
    const result = run(loaded, {
    ...
    const result = run(loaded, {
      filesystem: {
        fchmodSync: ((descriptor: number, mode: number) => {
          calls += 1;
          fs.fchmodSync(descriptor, mode);
          if (calls === 1) throw Object.assign(new Error('private detail'), { code: 'EIO' });
        }) as typeof fs.fchmodSync,
      } as never,
    });
    expect(result).toMatchObject({
  ...
  it.each([
    ...
    const result = run(loaded, {
      filesystem: {
        ...
        fsyncSync: ((descriptor: number) => {
          calls += 1;
          if (calls === failingCall) throw Object.assign(new Error('private detail'), { code: 'EIO' });
          fs.fsyncSync(descriptor);
        }) as typeof fs.fsyncSync,
      } as never,
    });
  ...
  it('fails before fchmod on an initial read error and reports unknown after a postchange read error', () => {
    ...
    const result = run(after, {
      filesystem: {
        readSync: ((...args: Parameters<typeof fs.readSync>) => {
          reads += 1;
          if (reads === 2) throw Object.assign(new Error('private detail'), { code: 'EIO' });
          return (fs.readSync as (...parameters: Parameters<typeof fs.readSync>) => number)(...args);
        }) as typeof fs.readSync,
      } as never,
    });
    ...
    const closeResult = run(close, {
      filesystem: {
        ...
        closeSync: ((descriptor: number) => {
          closeCalls += 1;
          fs.closeSync(descriptor);
          if (closeCalls === 1) throw Object.assign(new Error('private detail'), { code: 'EIO' });
        }) as typeof fs.closeSync,
      } as never,
    });
