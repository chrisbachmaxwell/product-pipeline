...
import {
  assertShopifyCredentialDatabasePermissionRepairRuntimeBinding,
...
export type ShopifyDatabasePermissionRepairStage =
  (typeof SHOPIFY_DATABASE_PERMISSION_REPAIR_STAGES)[number];
...
export type ShopifyDatabasePermissionRepairReport = Readonly<{
  status:
    | 'permission_repair_verified'
    | 'permission_repair_failed_closed'
    | 'permission_repair_outcome_unknown';
...
type MutableChecks = {
  -readonly [Key in keyof ShopifyDatabasePermissionRepairChecks]: boolean
};
...
function fileMetadataRepresentable(stat: fs.Stats): boolean {
...
function sameOwnerAndMtime(left: fs.Stats, right: fs.Stats): boolean {
...
function ownerCompatible(ownerUid: number, effectiveUid: number): boolean {
...
function frozenReport(input: Readonly<{
  stage: ShopifyDatabasePermissionRepairStage;
  checks: MutableChecks;
  permissionMetadataWritesPerformed: 0 | 1 | 'unknown';
}>): ShopifyDatabasePermissionRepairReport {
  let status: ShopifyDatabasePermissionRepairReport['status'];
  if (input.stage === 'verified') status = 'permission_repair_verified';
  else if (input.checks.descriptorPermissionChangeInvoked) {
    status = 'permission_repair_outcome_unknown';
  } else status = 'permission_repair_failed_closed';
...
export function repairFixedProductionShopifyDatabasePermissions(
  ...
  let parentInitial: fs.Stats | null = null;
  let originalMode: number | null = null;
  let beforeDigest: Buffer | null = null;
  let permissionMetadataWritesPerformed: 0 | 1 | 'unknown' = 0;
  ...
  if (!checks.fileRegular || !checks.fileSymlinkAbsent) return finish('file-type-denied');
  if (!fileMetadataRepresentable(initial)) return finish('file-inspection-denied');
  checks.fileSingleLink = initial.nlink === 1;
  if (!checks.fileSingleLink) return finish('file-link-denied');
  checks.fileNonEmpty = initial.size > 0;
  if (!checks.fileNonEmpty) return finish('file-empty-denied');
  checks.fileWithinSizeLimit = Number.isSafeInteger(initial.size)
    && initial.size <= MAXIMUM_DATABASE_BYTES;
  if (!checks.fileWithinSizeLimit) return finish('file-size-denied');
  originalMode = modeOf(initial);
  checks.repairRequired = originalMode !== REQUIRED_DATABASE_MODE;
  if (!checks.repairRequired) return finish('file-mode-already-0600');
  checks.originalModeSafe = (originalMode & SPECIAL_MODE_BITS) === 0;
  if (!checks.originalModeSafe) return finish('file-mode-denied');
  ...
  if (!checks.parentDirectory || !checks.parentSymlinkAbsent) return finish('parent-type-denied');
  if (!fileMetadataRepresentable(parentInitial)) return finish('parent-inspection-denied');
  checks.parentGroupWorldWritableAbsent = (parentInitial.mode & 0o022) === 0;
  if (!checks.parentGroupWorldWritableAbsent) return finish('parent-permissions-denied');

  const beforeSidecars = inspectSidecars(filesystem, databasePath);
  if (beforeSidecars === 'denied') return finish('sidecar-inspection-denied');
  checks.sqliteSidecarsAbsentBeforeRepair = beforeSidecars === 'absent';
  if (!checks.sqliteSidecarsAbsentBeforeRepair) return finish('sidecar-present');
  ...
  if (!checks.descriptorIdentityStableBeforeRepair) return finish('descriptor-identity-denied');

  let effectiveUid: number;
  try {
  ...
  if (!checks.fileOwnerCompatibleWithProcess) return finish('owner-compatibility-denied');

  try {
    parentDescriptor = filesystem.openSync(
      parentPath,
  ...
  if (!checks.parentDescriptorIdentityStable) return finish('parent-descriptor-identity-denied');
  checks.parentOwnerCompatibleWithProcess = ownerCompatible(openedParent.uid, effectiveUid);
  if (!checks.parentOwnerCompatibleWithProcess) return finish('owner-compatibility-denied');
  ...
  if (!checks.mtimeCapturedBeforeRepair) return finish('content-proof-denied');

  let immediatelyBefore: fs.Stats;
  try {
  ...
  if (!checks.descriptorStableImmediatelyBeforeRepair) return finish('prechange-identity-denied');
  let parentImmediatelyBefore: fs.Stats;
  try {
    parentImmediatelyBefore = filesystem.fstatSync(parentDescriptor);
  } catch {
    return finish('prechange-inspection-denied');
  }
  checks.parentDescriptorStableImmediatelyBeforeRepair = sameDirectoryIdentity(
    openedParent,
    parentImmediatelyBefore,
  );
  if (!checks.parentDescriptorStableImmediatelyBeforeRepair) {
    return finish('prechange-identity-denied');
  }
  let pathImmediatelyBefore: fs.Stats;
  try {
  ...
  if (!checks.pathStableImmediatelyBeforeRepair) return finish('prechange-path-identity-denied');
  let parentPathImmediatelyBefore: fs.Stats;
  try {
    parentPathImmediatelyBefore = filesystem.lstatSync(parentPath);
  } catch {
    return finish('prechange-path-inspection-denied');
  }
  checks.parentPathStableImmediatelyBeforeRepair = sameDirectoryIdentity(
    openedParent,
    parentPathImmediatelyBefore,
  );
  ...
  if (prechangeSidecars === 'denied') return finish('prechange-sidecar-inspection-denied');
  ...
  if (
    !checks.descriptorIdentityStableAfterRepair
  ...
  if (!checks.mode0600VerifiedAfterRepair) return finish('postchange-mode-denied');
  ...
  let afterDigest: Buffer;
  let postDigestStat: fs.Stats;
  try {
  ...
  if (!checks.mtimeUnchangedAfterRepair) return finish('postchange-mtime-changed');
  ...
  if (!checks.pathIdentityStableAfterRepair) return finish('postchange-path-identity-denied');
  let afterParentDescriptor: fs.Stats;
  let afterParentPath: fs.Stats;
  try {
  ...
  checks.parentIdentityStableAfterRepair = sameDirectoryIdentity(
    openedParent,
    afterParentDescriptor,
  ) && sameDirectoryIdentity(openedParent, afterParentPath);
  ...
  if (afterSidecars === 'denied') return finish('postchange-sidecar-inspection-denied');
  checks.sqliteSidecarsAbsentAfterRepair = afterSidecars === 'absent';
  if (!checks.sqliteSidecarsAbsentAfterRepair) return finish('postchange-sidecar-present');
