...
import {
  ...
  SHOPIFY_DATABASE_DIAGNOSTIC_STAGES,
...
function fixture(options: Readonly<{
  schema?: string;
  includeShopifyRow?: boolean;
  walHeader?: boolean;
}> = {}): Readonly<{ root: string; databasePath: string }> {
  ...
  if (options.walHeader === true) database.pragma('journal_mode = WAL');
  ...
  if (options.includeShopifyRow !== false) {
    database.prepare(
      `INSERT INTO auth_tokens
       ...
       VALUES ('shopify', 'shopify-token-value-never-output', NULL, NULL, NULL, 100, 200)`,
    ).run();
  }
  if (options.walHeader === true) database.pragma('wal_checkpoint(TRUNCATE)');
  database.close();
...
function mappedDependencies(
  ...
  transformDatabase?: (database: Database.Database) => Database.Database,
): ShopifyDatabaseDiagnosticDependencies {
  const mapped = (fixedPath: fs.PathLike): string => {
    const value = String(fixedPath);
    if (value === FIXED_DATABASE_PATH) return loaded.databasePath;
    if (value === path.dirname(FIXED_DATABASE_PATH)) return loaded.root;
  ...
  return Object.freeze({
    filesystem: {
      lstatSync: ((fixedPath: fs.PathLike) => fs.lstatSync(mapped(fixedPath))) as typeof fs.lstatSync,
      openSync: ((fixedPath: fs.PathLike, flags: fs.OpenMode) =>
        fs.openSync(mapped(fixedPath), flags)) as typeof fs.openSync,
...
function proxyPragma(
  database: Database.Database,
  override: (source: string, options?: Database.PragmaOptions) => unknown,
): Database.Database {
  return new Proxy(database, {
    get(target, property) {
      if (property === 'pragma') {
        return (source: string, options?: Database.PragmaOptions) => override(source, options);
      }
...
function runDiagnostic(dependencies: ShopifyDatabaseDiagnosticDependencies) {
...
describe('fixed Production Shopify database diagnostic', () => {
  ...
  it('identifies connection-safety failures without reflecting driver errors', () => {
    const loaded = fixture();
    const tempStore = mappedDependencies(loaded, (database) => proxyPragma(
      database,
      (source, options) => source === 'temp_store'
        ? 0
        : database.pragma(source, options as never),
    ));
    expect(runDiagnostic(tempStore).stage)
      .toBe('sqlite-temp-store-denied');

    const queryOnly = mappedDependencies(loaded, (database) => proxyPragma(
      database,
      (source, options) => source === 'query_only'
        ? 0
        : database.pragma(source, options as never),
    ));
  ...
  it('maps shared storage, column, foreign-key, and mutation-shape proof failures precisely', () => {
    ...
    expect(runDiagnostic(mappedDependencies(tableStorage, (database) => proxyPragma(
      database,
      (source, options) => {
        const value = database.pragma(source, options as never);
        if (source !== 'table_list') return value;
        return (value as Array<Record<string, unknown>>).map((entry) =>
          entry.name === 'auth_tokens' ? { ...entry, strict: 1 } : entry);
      },
    ))).stage).toBe('schema-table-storage-denied');

    const columns = fixture();
    expect(runDiagnostic(mappedDependencies(columns, (database) => proxyPragma(
      database,
      (source, options) => {
        const value = database.pragma(source, options as never);
        if (source !== 'table_xinfo(auth_tokens)') return value;
    ...
    expect(runDiagnostic(mappedDependencies(foreignKey, (database) => proxyPragma(
      database,
      (source, options) => source === 'foreign_key_list(auth_tokens)'
        ? [{ id: 0 }]
        : database.pragma(source, options as never),
    ))).stage).toBe('schema-foreign-key-denied');
  ...
  it('reports a fixed integrity stage and never emits paths, contents, digests, or sidecar suffixes', () => {
    const loaded = fixture();
    const dependencies = mappedDependencies(loaded, (database) => proxyPragma(
      database,
      (source, options) => source === 'integrity_check'
