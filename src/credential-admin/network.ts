...
export const CANONICAL_SHOPIFY_SCOPE_TEXT =
  PRODUCT_PIPELINE_SHOPIFY_IDENTITY.canonicalReadScopes.join(',');

export type CredentialRotationFetch = (
...
function exactToken(value: unknown, code: 'provider-denied' | 'verification-denied'): string {
  if (
    typeof value !== 'string'
    || value.length < 16
...
async function boundedJson(response: Response, maximumBytes: number): Promise<unknown> {
  ...
  if (response.body === null) return rotationDenied('provider-denied');
...
async function boundedFetch(
  ...
  try {
    ...
    if (!response.ok || response.status !== 200) return rotationDenied('provider-denied');
...
function exactScopes(value: unknown): typeof PRODUCT_PIPELINE_SHOPIFY_IDENTITY.canonicalReadScopes {
...
function exactScopeText(value: unknown): void {
  if (typeof value !== 'string') return rotationDenied('provider-denied');
...
export async function verifyShopifyAccessToken(
  accessToken: string,
  dependencies: ShopifyCredentialRotationNetworkDependencies = {},
): Promise<VerifiedShopifyAuthority> {
  ...
  if (payload === null || payload.errors !== undefined) return rotationDenied('verification-denied');
  const data = record(payload.data);
  const shop = record(data?.shop);
  const installation = record(data?.currentAppInstallation);
  const app = record(installation?.app);
  if (
    shop?.id !== PRODUCT_PIPELINE_SHOPIFY_IDENTITY.shopGid
...
export async function requestRotatedShopifyAccessToken(input: Readonly<{
  config: ShopifyCredentialRotationConfig;
  currentAccessToken: string;
  dependencies?: ShopifyCredentialRotationNetworkDependencies;
}>): Promise<RotatedShopifyAccessToken> {
  if (input.config.refreshToken === null) return rotationDenied('configuration-denied');
  ...
  if (payload === null) return rotationDenied('provider-denied');
  const keys = Object.keys(payload).sort();
  if (keys.some((key) => key !== 'access_token' && key !== 'scope')
    || !keys.includes('access_token')) return rotationDenied('provider-denied');
  const freshAccessToken = exactToken(payload.access_token, 'provider-denied');
  if (sameToken(freshAccessToken, currentAccessToken)) return rotationDenied('provider-denied');
  if (payload.scope !== undefined) exactScopeText(payload.scope);
  return Object.freeze({
