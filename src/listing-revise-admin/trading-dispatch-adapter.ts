...
export class TradingDispatchError extends Error {
  constructor(readonly code:
    | 'TRADING_DISPATCH_AUTHORITY_UNAVAILABLE'
    | 'TRADING_DISPATCH_TARGET_INVALID'
    | 'TRADING_DISPATCH_PAYLOAD_INVALID'
    | 'TRADING_DISPATCH_PAYLOAD_TOO_LARGE'
    | 'TRADING_DISPATCH_WRITE_FAILED'
    | 'TRADING_DISPATCH_REJECTED') {
    super('Trading listing revise dispatch adapter failed');
    this.name = 'TradingDispatchError';
  }
}

const deny = (code: ConstructorParameters<typeof TradingDispatchError>[0]): never => {
...
function parseImageList(serialized: string): string[] {
  try {
    ...
    if (error instanceof TradingDispatchError) throw error;
    return deny('TRADING_DISPATCH_PAYLOAD_INVALID');
  }
}
 ...
 */
export function buildReviseFixedPriceItemXml(input: TradingReviseFixedPriceItemInput): string {
  if (!EXACT_ITEM_ID.test(input.listingId)) deny('TRADING_DISPATCH_TARGET_INVALID');
  if (input.changes.length === 0) deny('TRADING_DISPATCH_PAYLOAD_INVALID');
  const values = new Map<ListingFieldName, string>();
  ...
  const elements: string[] = [`<ItemID>${escapeXml(input.listingId)}</ItemID>`];
  ...
  if (profiles.length > 0) elements.push(`<SellerProfiles>${profiles.join('')}</SellerProfiles>`);
...
export function createTradingDispatchAdapter(dependencies: Readonly<{
  fetchImpl?: FetchLike;
  ...
  async function boundedPost(body: string): Promise<string> {
    ...
    try {
      ...
      if (response.status !== 200) deny('TRADING_DISPATCH_WRITE_FAILED');
      return text;
    } catch (error) {
      if (error instanceof TradingDispatchError) throw error;
  ...
  async function reviseFixedPriceItem(input: TradingReviseFixedPriceItemInput): Promise<void> {
    ...
    if (/<!DOCTYPE|<!ENTITY/iu.test(text)) deny('TRADING_DISPATCH_REJECTED');
    let parsed: unknown;
    try {
    ...
    if (ack !== 'Success' && ack !== 'Warning') deny('TRADING_DISPATCH_REJECTED');
  }
