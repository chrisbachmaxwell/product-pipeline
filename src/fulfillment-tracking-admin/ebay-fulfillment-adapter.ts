...
export class EbayFulfillmentAdapterError extends Error {
  constructor(readonly code:
    | 'FULFILLMENT_EBAY_AUTHORITY_UNAVAILABLE'
    | 'FULFILLMENT_EBAY_TARGET_INVALID'
    | 'FULFILLMENT_EBAY_READ_FAILED'
    | 'FULFILLMENT_EBAY_WRITE_FAILED') {
    super('eBay fulfillment adapter denied');
    this.name = 'EbayFulfillmentAdapterError';
  }
}

const deny = (code: ConstructorParameters<typeof EbayFulfillmentAdapterError>[0]): never => {
...
export function buildShippingFulfillmentBody(manifest: FulfillmentManifest): string {
...
export function createEbayFulfillmentAdapter(dependencies: Readonly<{
  fetchImpl?: FetchLike;
  ...
  async function request(
    orderId: string,
    suffix: '' | '/shipping_fulfillment',
    init: RequestInit,
    failure: 'FULFILLMENT_EBAY_READ_FAILED' | 'FULFILLMENT_EBAY_WRITE_FAILED',
  ): Promise<{ status: number; body: string }> {
    if (!SAFE_ORDER_ID.test(orderId)) deny('FULFILLMENT_EBAY_TARGET_INVALID');
    let token = '';
    try {
    ...
    if (!token || token.length > 4_096) deny('FULFILLMENT_EBAY_AUTHORITY_UNAVAILABLE');
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS);
    try {
      const response = await fetchImpl(
        `${EBAY_API_HOST}/sell/fulfillment/v1/order/${encodeURIComponent(orderId)}${suffix}`,
        {
          ...init,
          headers: {
      ...
      if (Number.isFinite(declared) && declared > MAX_RESPONSE_BYTES) deny(failure);
      const body = await response.text();
      if (Buffer.byteLength(body, 'utf8') > MAX_RESPONSE_BYTES) deny(failure);
      return { status: response.status, body };
    } catch (error) {
      if (error instanceof EbayFulfillmentAdapterError) throw error;
      return deny(failure);
    } finally {
  ...
  async function getOrder(orderId: string): Promise<EbayFulfillmentOrder> {
    ...
    try {
      ...
      if (order.orderId !== orderId) deny('FULFILLMENT_EBAY_READ_FAILED');
      ...
      const rawFulfillments = Array.isArray(fulfillmentsBody.fulfillments)
        ...
        : [];
      const shippingFulfillments = rawFulfillments.map((raw: unknown) => {
        const fulfillment = record(raw);
        const fulfillmentLineItems = Array.isArray(fulfillment.lineItems)
          ? fulfillment.lineItems.map((rawLine: unknown) => {
  ...
  return Object.freeze({
    getOrder,
    createShippingFulfillment: async (manifest: FulfillmentManifest): Promise<void> => {
      const response = await request(
        manifest.ebayOrderId,
        '/shipping_fulfillment',
        { method: 'POST', body: buildShippingFulfillmentBody(manifest) },
        'FULFILLMENT_EBAY_WRITE_FAILED',
      );
      if (response.status !== 201) deny('FULFILLMENT_EBAY_WRITE_FAILED');
    },
  });
}

export function createProductionEbayFulfillmentAdapter(): EbayFulfillmentAdapter {
