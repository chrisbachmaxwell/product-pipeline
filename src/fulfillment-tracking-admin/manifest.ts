import { sha256Digest, type Digest } from '../migration-store/index.js';
...
export class FulfillmentManifestError extends Error {
  constructor(readonly code:
    ...
    | 'FULFILLMENT_PARTIAL_DENIED'
    | 'FULFILLMENT_MULTIPLE_DENIED'
    | 'FULFILLMENT_TRACKING_REQUIRED'
    | 'FULFILLMENT_ALREADY_RECORDED'
    | 'FULFILLMENT_SOURCE_INVALID'
    | 'FULFILLMENT_TARGET_INVALID') {
    super('Fulfillment manifest derivation denied');
    this.name = 'FulfillmentManifestError';
  }
}

const deny = (code: ConstructorParameters<typeof FulfillmentManifestError>[0]): never => {
  throw new FulfillmentManifestError(code);
};

export type ShopifyFulfillmentOrder = Readonly<{
  orderGid: string;
  lineItems: readonly Readonly<{ lineItemGid: string; quantity: number }>[];
  fulfillments: readonly Readonly<{
    fulfillmentGid: string;
    status: string;
    createdAtUtc: string;
    tracking: readonly Readonly<{ company: string | null; number: string }>[];
    lineItems: readonly Readonly<{ lineItemGid: string; quantity: number }>[];
  }>[];
}>;
...
function canonicalUtc(value: string): string {
  ...
  if (!Number.isSafeInteger(epoch)) return deny('FULFILLMENT_SOURCE_INVALID');
  return new Date(epoch).toISOString();
}

export function mapCarrierCode(company: string | null): string {
  const normalized = (company ?? '').trim().toLowerCase();
  if (normalized.includes('usps')) return 'USPS';
  if (normalized === 'ups' || normalized.includes('united parcel')) return 'UPS';
...
function quantityMap(
  values: readonly Readonly<{ lineItemGid: string; quantity: number }>[],
): Map<string, number> {
...
export function deriveFulfillmentManifest(input: {
  ...
  if (successful.length === 0) deny('FULFILLMENT_NOT_COMPLETE');
