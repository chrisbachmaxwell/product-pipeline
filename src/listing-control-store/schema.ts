...
const LISTING_CONTROL_V2_EXPECTED_CATALOG_DIGEST = expectedCatalogDigest(
  LISTING_CONTROL_MIGRATIONS.slice(0, 2),
);
...
export function verifyListingControlSchemaV2(database: InstanceType<typeof Database>): void {
  if (database.pragma('application_id', { simple: true }) !== LISTING_CONTROL_APPLICATION_ID
...
/** Explicit admin-only upgrade. Runtime open paths never invoke this function. */
export function upgradeListingControlSchemaV2ToV3(
  database: InstanceType<typeof Database>,
  appliedAtUtc: string,
): void {
  verifyListingControlSchemaV2(database);
