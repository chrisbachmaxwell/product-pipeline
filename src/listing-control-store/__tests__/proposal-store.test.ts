...
const editableValues: Partial<Record<(typeof LISTING_FIELD_NAMES)[number], string>> = {
...
function revisionFields(): ListingFieldInput[] {
...
function job(overrides: Partial<CreateListingProposalJobInput> = {}): CreateListingProposalJobInput {
...
function decisions(): ListingProposalFieldDecisionInput[] {
...
function approvalRevision(): ListingRevisionInput {
...
function ready(store: ReturnType<typeof initializeListingControlStore>) {
...
describe('listing AI proposal store', () => {
  ...
  it('runs first proposal through atomic local approval without provider capability', () => {
    ...
    expect(store.getLatestProposalForCatalog(
      identity.shopifyVariantGid, 'catalog-2026-08-14',
    )?.job.jobId).toBe(job().jobId);
    expect(store.getLatestProposalForCatalog(
      identity.shopifyVariantGid, 'catalog-not-present',
    )).toBeNull();
    expect(store.countProposalJobsForSubjectSince(
      identity.shopifyVariantGid, '2026-08-14T15:00:00.000Z',
    )).toBe(1);
  ...
  it('never approves a needs_human result even with a syntactically valid revision', () => {
    ...
    const persisted = raw.prepare(
      `SELECT job.scope_key, job.subject_key, event.event_digest
       FROM listing_proposal_jobs job
       JOIN listing_proposal_events event ON event.job_id = job.job_id
       WHERE job.job_id = ? ORDER BY event.sequence DESC LIMIT 1`,
    ).get(queued.job.jobId) as {
      scope_key: string; subject_key: string; event_digest: string;
    };
    const approvalBindingTrigger = raw.prepare(
      `SELECT sql FROM sqlite_schema WHERE type = 'trigger'
       AND name = 'listing_proposal_events_approved_revision'`,
    ).get() as { sql: string };
    ...
    expect(() => raw.prepare(
      ...
      ) VALUES (?, ?, 4, ?, ?, 'approved', ?, ?, ?, ?, ?, ?, ?, 'accepted', ?)`,
    ).run(
      'direct-illegal-approval', queued.job.jobId, persisted.scope_key, persisted.subject_key,
      sha256Digest('direct-illegal-approval-event'), persisted.event_digest, 'operator-chris',
      '2026-08-14T16:00:03.000Z', Date.parse('2026-08-14T16:00:03.000Z'),
      completed.result.resultDigest, sha256Digest('nonexistent-reviewed-revision'),
      sha256Digest('direct-illegal-approval-payload'),
    )).toThrow(/proposal event chain mismatch/);
  ...
  it('rejects a ready event with a low-confidence decision at the SQL boundary', () => {
    ...
    raw.prepare(
      ...
      ) VALUES (?, ?, ?, ?, ?, 'ready', ?, NULL, 1, 1, 2, ?, ?, ?)`,
    ).run(
      'direct-ready-low-result', resultDigest, queued.job.jobId, persisted.scope_key,
      persisted.subject_key, sha256Digest('direct-ready-low-output'), 'listing-agent-v1',
      '2026-08-14T16:00:02.000Z', Date.parse('2026-08-14T16:00:02.000Z'),
    );
    raw.prepare(
      `INSERT INTO listing_proposal_field_decisions (
        ...
        'conflicting_sources', 'low_confidence', '[]', ?)`,
    ).run(
      'direct-ready-low-result', persisted.scope_key, persisted.subject_key,
      sha256Digest('unsafe'), sha256Digest('empty-evidence'),
    );
    expect(() => raw.prepare(
      `INSERT INTO listing_proposal_events (
        event_id, job_id, sequence, scope_key, subject_key, event_type, event_digest,
        previous_event_digest, actor, occurred_at_utc, occurred_epoch_ms, result_digest,
        reviewed_revision_digest, review_reason_code, payload_digest
      ) VALUES (?, ?, 3, ?, ?, 'ready', ?, ?, ?, ?, ?, ?, NULL, NULL, ?)`,
    ).run(
      'direct-ready-low-event', queued.job.jobId, persisted.scope_key, persisted.subject_key,
      sha256Digest('direct-ready-low-event'), persisted.event_digest, 'listing-agent-v1',
      '2026-08-14T16:00:02.000Z', Date.parse('2026-08-14T16:00:02.000Z'),
  ...
  it('rejects protected-field model decisions, credential material, and bad usage', () => {
    ...
    }] as ListingProposalFieldDecisionInput[];
    ...
    const inventedOverride = decisions().map((decision, index) => index === 0 ? {
      ...decision, proposedSource: 'override' as const,
    } : decision);
    expectCode(() => store.completeProposal({
    ...
    const lowConfidence = decisions().map((decision, index) => index === 0
      ? { ...decision, confidence: 'low' as const, warningCode: 'low_confidence' as const }
      : decision);
    expectCode(() => store.completeProposal({
  ...
  it('detects proposal tampering after append-only triggers are restored', () => {
    ...
    const raw = new Database(databasePath);
    expect(() => raw.prepare(
      "UPDATE listing_proposal_results SET actor = 'tampered' WHERE result_id = 'proposal-result-1'",
    ).run()).toThrow(/append-only/);
    const trigger = raw.prepare(
      "SELECT sql FROM sqlite_schema WHERE type = 'trigger' AND name = 'listing_proposal_results_deny_update'",
    ).get() as { sql: string };
    ...
    raw.close();
    expectCode(() => openListingControlStoreReadOnly({ databasePath, expectedScope: scope }),
      'SCHEMA_MISMATCH');
  });
});

function canonicalV2(): { databasePath: string; auditHead: string; revisionDigest: string } {
  ...
  try {
    ...
    for (const migration of LISTING_CONTROL_MIGRATIONS.slice(0, 2)) {
      ...
      ).run(migration.version, migration.name, migration.checksum, '2026-08-14T15:59:50.000Z');
