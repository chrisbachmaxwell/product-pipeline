...
import {
  AUTOMATION_CEILINGS,
  AUTOMATION_RESPONSIBILITIES,
  AutomationContractError,
  buildAutomationPolicy,
  digestAutomationPolicy,
  isAutomationResponsibility,
  MAXIMUM_AUTOMATION_AUTHORIZATION_MS,
  serializeAutomationPolicy,
  type AutomationPolicyInput,
  type AutomationResponsibility,
} from './contracts.js';

const digest = (character: string) => `sha256:${character.repeat(64)}` as const;

function validInput(
  responsibility: AutomationResponsibility = 'inventory',
...
describe('G18 automation policy contract', () => {
  ...
  it.each(AUTOMATION_RESPONSIBILITIES)(
    'canonicalizes and deterministically digests the bounded %s policy',
    (responsibility) => {
      ...
      expect(digestAutomationPolicy(policy)).toBe(digestAutomationPolicy(
        buildAutomationPolicy({ ...validInput(responsibility) }),
      ));
    },
  );
  ...
  it.each(AUTOMATION_RESPONSIBILITIES)(
    'denies every compiled ceiling violation for %s',
    (responsibility) => {
      const ceiling = AUTOMATION_CEILINGS[responsibility];
      const invalid = [
  ...
  it('requires explicit Lightspeed cascade acceptance only for order import', () => {
    ...
    for (const responsibility of ['inventory', 'price', 'fulfillment'] as const) {
      expect(() => buildAutomationPolicy({
  ...
  it('denies noncanonical, nonpositive, overlong, and structurally widened inputs', () => {
    ...
    const invalid: unknown[] = [
      ...
      { ...base, expiresAtUtc: base.authorizedAtUtc },
      {
        ...
        expiresAtUtc: new Date(
          Date.parse(base.authorizedAtUtc) + MAXIMUM_AUTOMATION_AUTHORIZATION_MS + 1,
        ).toISOString(),
      },
    ...
    for (const value of invalid) {
      expect(() => buildAutomationPolicy(value as AutomationPolicyInput)).toThrow(
        AutomationContractError,
      );
    }
  });
