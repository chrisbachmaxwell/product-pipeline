...
const deny = (code: string): never => { throw new SandboxManifestError(code); };
...
export function validateTarget(input: {
  storeDomain: string; productGid: string; variantGid: string; sku: string; shopifyEvidenceDigest: string;
}): SandboxTarget {
  if (!/^[a-z0-9][a-z0-9-]*\.myshopify\.com$/.test(input.storeDomain)
    || !PRODUCT_GID.test(input.productGid)
    || !VARIANT_GID.test(input.variantGid)
    || !SAFE.test(input.sku)
    || !DIGEST.test(input.shopifyEvidenceDigest)) deny('TARGET_INVALID');
...
export function readSandboxManifest(filePath: string, exactTarget: SandboxTarget): {
  manifest: SandboxListingManifest; digest: Digest;
} {
  if (!filePath.startsWith('/') || fs.lstatSync(filePath).isSymbolicLink()) deny('MANIFEST_PATH_DENIED');
  const stat = fs.statSync(filePath);
  if (!stat.isFile() || stat.size > 64 * 1024 || (stat.mode & 0o077) !== 0) deny('MANIFEST_PATH_DENIED');
  let raw: unknown;
  try { raw = JSON.parse(fs.readFileSync(filePath, 'utf8')); } catch { deny('MANIFEST_INVALID'); }
  ...
  if (!exactKeys(target, ['storeDomain', 'productGid', 'variantGid', 'sku', 'shopifyEvidenceDigest'])) deny('MANIFEST_INVALID');
  ...
  if (!exactKeys(listing, ['title', 'description', 'imageUrls', 'categoryId', 'condition', 'conditionDescription', 'quantity', 'price', 'merchantLocationKey', 'fulfillmentPolicyId', 'paymentPolicyId', 'returnPolicyId'])) deny('MANIFEST_INVALID');
  const title = text(listing.title, 80);
  const description = text(listing.description, 50_000);
  if (!title.startsWith(SANDBOX_MARKER) || !description.includes(SANDBOX_MARKER)) deny('MARKER_REQUIRED');
  const images = listing.imageUrls;
  if (!Array.isArray(images) || images.length < 1 || images.length > 12 || images.some((v) => {
    if(typeof v!=='string'||v.length>2048)return true;try{const u=new URL(v);return u.protocol!=='https:'||u.username!==''||u.password!==''||u.port!==''||u.search!==''||u.hash!==''||!IMAGE_HOSTS.has(u.hostname.toLowerCase());}catch{return true;}
  })) deny('MANIFEST_INVALID');
  const price = record(listing.price);
  if (!exactKeys(price, ['currency', 'value']) || price.currency !== 'USD' || price.value !== '1.00' || listing.quantity !== 1) deny('CANARY_COMMERCE_LIMIT');
  const manifest: SandboxListingManifest = Object.freeze({
    ...
    listing: Object.freeze({
      title, description, imageUrls: Object.freeze([...(images as string[])]),
  ...
  const digest = `sha256:${createHash('sha256').update(JSON.stringify(manifest)).digest('hex')}` as Digest;
  return { manifest, digest };
}

export function buildPayloads(manifest: SandboxListingManifest): {
  inventory: Record<string, unknown>; offer: Record<string, unknown>;
} {
