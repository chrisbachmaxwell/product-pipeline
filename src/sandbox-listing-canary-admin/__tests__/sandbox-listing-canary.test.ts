...
import {
  assertSandboxCreatedState,
  buildSandboxListingCanaryProgram,
  type SandboxCanaryIo,
} from '../program.js';
...
function createdSnapshot(offerId = 'OFFER-100', listingId = '123456789012'): any {
  const m = manifest() as SandboxListingManifest;
  const payloads = buildPayloads(m);
  const { format: _format, listingDuration: _duration, ...offer } = payloads.offer;
  return {
    inventory: { sku: target.sku, ...payloads.inventory },
    offers: [{ offerId, ...offer, status: 'PUBLISHED', listingId }],
    tradingListings: [
      {
        itemId: listingId,
        sku: target.sku,
  ...
  it('rejects a 200 inventory echo for another SKU and unsupported offer status', async () => {
    ...
    const wrongItem = createSandboxAdapter({
      ...
      fetchImpl: async (input) =>
        String(input).startsWith(SANDBOX_IDENTITY_ORIGIN)
    ...
    const invalidOffer = createSandboxAdapter({
      ...
      fetchImpl: async (input) => {
        ...
        if (url.includes('/inventory_item/')) return new Response('', { status: 404 });
  ...
  it('constructs only immutable Sandbox URLs and rejects ambiguous pagination', async () => {
    ...
    const adapter = createSandboxAdapter({
      ...
      fetchImpl: async (input) => {
        ...
        return new Response(
          '<GetSellerListResponse><Ack>Success</Ack><HasMoreItems>false</HasMoreItems><PaginationResult><TotalNumberOfEntries>0</TotalNumberOfEntries></PaginationResult></GetSellerListResponse>',
          { status: 200 },
        );
      },
    });
    ...
    expect(
      calls.every(
        (url) => url.startsWith(SANDBOX_API_ORIGIN) || url.startsWith(SANDBOX_IDENTITY_ORIGIN),
      ),
    ).toBe(true);
  ...
  it('requires every approved create effect and exact Trading listing binding', () => {
    ...
    expect(() =>
      assertSandboxCreatedState(
        ...
        'OFFER-100',
        '123456789012',
        manifest() as SandboxListingManifest,
      ),
    ).not.toThrow();
    const drifts = [
      {
...
describe('standalone ceremony', () => {
  it('initializes separate state, contains partial outcomes, reconciles, cleans up, and denies replay', async () => {
    const f = fixture();
    let state: 'absent' | 'item' | 'offer' | 'published' = 'absent';
    let listingStatus: 'none' | 'Active' | 'Ended' = 'none';
    ...
    const fetchImpl: typeof fetch = async (input, init) => {
      ...
      if (url.endsWith('/ws/api.dll')) {
        ...
        const item = count
          ...
          : '';
        return new Response(
          `<GetSellerListResponse><Ack>Success</Ack><HasMoreItems>false</HasMoreItems><PaginationResult><TotalNumberOfEntries>${count}</TotalNumberOfEntries></PaginationResult>${item}</GetSellerListResponse>`,
          { status: 200 },
        );
      }
      if (url.includes('/inventory_item/')) {
        if (method === 'GET')
          return new Response(
            state === 'absent' ? '' : JSON.stringify({ sku: target.sku, ...payloads.inventory }),
            { status: state === 'absent' ? 404 : 200 },
          );
        if (method === 'PUT') {
          state = 'item';
      ...
      if (url.includes('/offer?')) {
        const { format: _format, listingDuration: _duration, ...offerPayload } = payloads.offer;
        const offers =
          state === 'offer' || state === 'published'
            ? [
                {
                  offerId,
                  ...offerPayload,
    ...
    const writesBeforeApproval = calls.filter(
    ...
    ).toHaveLength(writesBeforeApproval);
    ...
    const beforeWrong = calls.filter(
      (v) => /^(PUT|POST|DELETE) /.test(v) && !v.includes('/ws/api.dll'),
    ).length;
    ...
    const writesAfterUnknown = calls.filter(
      (v) => /^(PUT|POST|DELETE) /.test(v) && !v.includes('/ws/api.dll'),
    ).length;
    ...
    expect(
      calls.filter((v) => /^(PUT|POST|DELETE) /.test(v) && !v.includes('/ws/api.dll')),
    ).toHaveLength(writesAfterUnknown);
    state = 'published';
    listingStatus = 'Active';
    const recovered = await execute('reconcile-create', [
      '--state',
      f.state,
      '--manifest-digest',
      pre.json.manifestDigest,
    ...
    const auditBeforeReplay = (await execute('verify-state', ['--state', f.state])).json.audit
      .recordCount;
    const terminalReplay = await execute('reconcile-create', [
      '--state',
      f.state,
      '--manifest-digest',
      pre.json.manifestDigest,
    ...
    expect(dispatchCalls.findIndex((v) => v.startsWith('GET '))).toBeLessThan(
      dispatchCalls.findIndex((v) => v.startsWith('PUT ')),
    );
    ...
    expect(state).toBe('offer');
    const cleanupWrites = calls.filter(
      (v) => /^(PUT|POST|DELETE) /.test(v) && !v.includes('/ws/api.dll'),
    ).length;
    ...
    state = 'absent';
    const cleanupRecovered = await execute('reconcile-cleanup', [
      '--state',
      f.state,
      '--offer-id',
      offerId,
