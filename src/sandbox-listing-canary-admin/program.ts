...
export function discoverSandboxRecovery(
  snapshot: SandboxSnapshot,
  manifest: SandboxListingManifest,
): RecoveryDiscovery {
  const payloads = buildPayloads(manifest);
...
export function buildSandboxListingCanaryProgram(deps: Dependencies = {}): Command {
  ...
  targetOptions(
    ...
    run('approve-cleanup', async (o) => {
      ...
      try {
        ...
        if (
          !store.hasResolvedExistingEffect(
            createIntentKey(t, s),
            'listingCreate',
            createObservedDigest,
          )
        )
  ...
  const recoverySourceOptions = (c: Command) =>
    c
  ...
  recoverySourceOptions(
    targetOptions(
      program.command('preflight-recovery-cleanup').requiredOption('--state <absolute-path>'),
    ),
  ).action(
    run(
      'preflight-recovery-cleanup',
      async (o) => {
        const t = target(o);
        const parsed = readSandboxManifest(o.manifestFile, t);
        const a = await authority(deps, now());
        ...
        try {
          ...
          if (['drift', 'absent', 'cleaned'].includes(d.stage)) deny('RECOVERY_RESIDUE_NOT_EXACT');
  ...
  recoverySourceOptions(
    ...
    run('approve-recovery-cleanup', async (o) => {
      const t = target(o);
      const parsed = readSandboxManifest(o.manifestFile, t);
      const a = await authority(deps, now());
      const s = scope(t, a.sellerHash);
  ...
  recoverySourceOptions(
    ...
    run('dispatch-recovery-cleanup', async (o) => {
      const t = target(o);
      const parsed = readSandboxManifest(o.manifestFile, t);
      const a = await authority(deps, now());
      const s = scope(t, a.sellerHash);
      ...
      try {
        ...
        const source = recoverySource(o, t, s, d.listingId);
        assertOutstanding(store, source.ceremony, source.responsibility, source.evidenceDigest);
        const targetKey = deriveExternalIdentityKey(
          d.listingId ? listingIdentity(d.listingId, s) : skuIdentity(t, s),
        );
        const c = reserve({
          store,
          responsibility: 'listingEndRelist',
  ...
  targetOptions(
    ...
    run('reconcile-recovery-cleanup', async (o) => {
      if (
        !DIGEST.test(o.recoveryDigest) ||
        !DIGEST.test(o.intentKey) ||
        !SAFE.test(o.jobId) ||
        !SAFE.test(o.attemptId) ||
      ...
      try {
        const c: Ceremony = {
          intentKey: o.intentKey,
          targetIdentityKey: deriveExternalIdentityKey(
            listingId ? listingIdentity(listingId, s) : skuIdentity(t, s),
          ),
          jobId: o.jobId,
          attemptId: o.attemptId,
          markDispatching: () => deny('RECONCILE_WRITE_DENIED'),
