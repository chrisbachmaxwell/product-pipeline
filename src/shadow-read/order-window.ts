...
const WINDOW_KEYS = ['creationDateEndUtc', 'creationDateStartUtc'] as const;
const VALIDATED_WINDOWS = new WeakSet<object>();
...
function canonicalUtc(value: unknown): number | null {
 ...
 */
export function validateBoundedOrderReadWindow(
  rawWindow: unknown,
  nowUtc: string,
): BoundedOrderReadWindow {
  ...
  const window = Object.freeze({
    kind: 'bounded-order-observation-window' as const,
    eventTimeField: 'creationDate' as const,
    lowerBoundInclusiveUtc: rawWindow.creationDateStartUtc as string,
    upperBoundExclusiveUtc: rawWindow.creationDateEndUtc as string,
    durationMs: endMs - startMs,
    notCutoverWatermark: true as const,
    historicalBackfillAuthorized: false as const,
    fixtureBoundarySemantics: 'normalized-half-open' as const,
    liveEbayBoundarySemanticsVerified: false as const,
  });
...
export function orderWindowQueryForTransport(
  window: BoundedOrderReadWindow,
  source: 'shopify' | 'ebay',
): Readonly<Record<string, string>> {
  if (!window || typeof window !== 'object' || !VALIDATED_WINDOWS.has(window as object)) {
    denyShadowRead('order-window-denied');
  }
  if (source !== 'shopify' && source !== 'ebay') denyShadowRead('order-window-denied');
  if (source === 'shopify') {
