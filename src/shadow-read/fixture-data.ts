...
type FrozenJson = JsonPrimitive | FrozenJsonArray | FrozenJsonObject;
interface FrozenJsonArray extends ReadonlyArray<FrozenJson> {}
interface FrozenJsonObject { readonly [key: string]: FrozenJson }
...
function canonicalClone(
  ...
  state.nodes += 1;
  if (state.nodes > MAX_NODES || depth > MAX_DEPTH) denyShadowRead('fixture-payload-denied');
  if (value === null || typeof value === 'boolean') return value;
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) denyShadowRead('fixture-payload-denied');
    return value;
  }
  ...
  if (seen.has(value)) denyShadowRead('fixture-payload-denied');
  seen.add(value);
 ...
 */
export function sanitizeFixtureRecords<T>(
  rawRecords: unknown,
  maxRecords: number,
  maxBytes: number,
): SanitizedFixtureRecords<T> {
  if (!Array.isArray(rawRecords)) denyShadowRead('fixture-payload-denied');
  if (rawRecords.length > maxRecords) denyShadowRead('record-cap-exceeded');
  const cloned = canonicalClone(rawRecords, new WeakSet(), { nodes: 0 }, 0);
  if (!Array.isArray(cloned)) denyShadowRead('fixture-payload-denied');
  const canonical = JSON.stringify(cloned);
  const responseBytes = Buffer.byteLength(canonical, 'utf8');
  if (responseBytes > maxBytes) denyShadowRead('response-byte-cap-exceeded');
  const datasetDigest = `sha256:${createHash('sha256').update(canonical).digest('hex')}`;
  return Object.freeze({
    records: cloned as readonly T[],
