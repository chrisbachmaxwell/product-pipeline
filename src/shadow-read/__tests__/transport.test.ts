...
import {
  ...
  type FixtureReadRequest,
  type FixtureReadTransportConfig,
  type InjectedFixtureReadRequest,
} from '../transport.js';
...
function productRequest(overrides: Partial<FixtureReadRequest> = {}): FixtureReadRequest {
  return {
    source: 'shopify',
    method: 'GET',
...
describe('fixture-only read transport', () => {
  ...
  it('constructs only the exact configured HTTPS GET and exposes no body or token to fixtures', async () => {
    let dispatched: InjectedFixtureReadRequest | undefined;
    const transport = createFixtureReadTransport(transportConfig(), {
      clock: fixedClock,
      dispatcher: async (request: InjectedFixtureReadRequest) => {
        dispatched = request;
        return { status: 200, records: [{ id: 'product-1', sku: 'SAFE-SKU-1' }] };
      },
    });
    const result = await transport.request<{ id: string; sku: string }>(productRequest());
  ...
  it('denies POST and unknown request fields before dispatch', async () => {
    ...
    await expect(transport.request(productRequest({ method: 'POST' as 'GET' }))).rejects.toMatchObject({
      code: 'method-denied',
    });
    await expect(transport.request({
      ...productRequest(),
      unreviewed: true,
    } as unknown as FixtureReadRequest)).rejects.toMatchObject({ code: 'configuration-denied' });
  ...
  it('requires an opaque validated creationDate window for every Shopify order read', async () => {
    ...
    } as FixtureReadRequest)).rejects.toMatchObject({ code: 'order-window-denied' });
  ...
  it('requires and injects the bounded creationdate filter for eBay fulfillment orders', async () => {
    ...
    expect(url.searchParams.get('filter')).toBe(
      `creationdate:[2026-08-10T18:00:00.000Z..${NOW_UTC}]`,
    );
  ...
  it('computes record and byte caps locally and deep-freezes the returned fixture', async () => {
    ...
    const result = await transport.request<{ id: string; nested: { state: string } }>(productRequest());
  ...
  it('denies an unrecognized dispatcher response shape', async () => {
    const transport = createFixtureReadTransport(transportConfig(), {
      clock: fixedClock,
      dispatcher: async () => ({ status: 200, records: [], unreviewed: true } as never),
    });
    await expect(transport.request(productRequest())).rejects.toMatchObject({
