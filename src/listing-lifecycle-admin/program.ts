...
import {
  createListingRecoverDispatchAdapter,
...
function safeError(error: unknown): { code: string; field?: string } {
  ...
  if (error instanceof ListingCreateRecoveryError) return { code: error.code };
  if (error instanceof ListingRecoverDispatchError) return { code: error.code };
...
function verifyRecoverySourceBindings(input: {
  ...
  if (boundJob.state !== 'reconciliation_required') deny('RECOVER_STATE_MISMATCH');
  ...
  if (boundAttempt.resolution !== null) deny('RECOVER_ATTEMPT_ALREADY_RESOLVED');
...
function recoveryUnresolvedCode(kind: LifecycleOutcome['kind']): string {
  if (kind === 'artifact') return 'RECOVER_RESIDUE_STILL_PRESENT';
  if (kind === 'observed') return 'RECOVER_UNEXPECTED_ACTIVE_LISTING';
...
function recordResidueRemovalReconciliation(input: {
  store: MigrationStore;
  intentKey: MigrationDigest;
  intentRole: 'recovery' | 'source_create' | 'prior_recovery';
  targetIdentityKey: MigrationDigest;
  jobId: string;
  attemptId: string;
  ...
  outcome: LifecycleOutcome;
  /**
   * True only when direct provider reads proved the offer AND inventory item
   * gone. A fresh capture's absence alone must not terminalize (the slice's
   * propagation-delay rule): removal resolves only with both proofs.
   */
...
export function buildListingLifecycleAdminProgram(
  ...
  const createRecoverAdapter = dependencies.createRecoverAdapter ?? (() =>
    createListingRecoverDispatchAdapter({
      getAccessToken: createProductionDispatchTokenProvider(),
    }));
  ...
  const withRecoverBindingOptions = (command: Command): Command => command
    .requiredOption('--migration-store <path>', 'Absolute migration-state database path')
    .requiredOption(
      '--confirm-scope <sha256>',
      'Exact migration scope key confirming the one store',
    )
    ...
    .requiredOption(
      ...
      "Exact approval evidence digest of the job (the create's manifest digest)",
    )
    .requiredOption(
      '--offer-id <id>',
      'Exact unpublished eBay offer id recorded by reconciliation',
    )
    .option(
      '--prior-recovery-job-id <id>',
      'Chained retry only: the exact unresolved prior recovery job id (L29)',
    )
    .option(
      '--prior-recovery-attempt-id <id>',
      'Chained retry only: the exact unresolved prior recovery attempt id',
    );

  type RecoverBindingOptions = {
    migrationStore: string;
    confirmScope: string;
    catalogId: string;
    sku: string;
   ...
   */
  const verifyPriorRecoveryBinding = (
    store: MigrationStore,
    priorJobId: string,
    ...
    const boundPriorJob = priorJob as NonNullable<typeof priorJob>;
    const priorIntent = store.getIntent(boundPriorJob.intentKey);
    if (!priorIntent
      || (priorIntent as NonNullable<typeof priorIntent>).action
        !== 'recover_create_ebay_listing') {
      deny('RECOVER_PRIOR_RECOVERY_MISMATCH');
    }
    const priorAttempt = store.getAttemptStatus(priorJobId, priorAttemptId);
    if (!priorAttempt
      || (priorAttempt as NonNullable<typeof priorAttempt>).resolution !== null) {
      deny('RECOVER_PRIOR_RECOVERY_MISMATCH');
    }
  ...
  withRecoverBindingOptions(program
    ...
    .description(
      'One-action exact-target recovery cleanup for ONE unresolved create job whose dispatch '
    ...
    .action(async (options: RecoverBindingOptions) => {
      ...
      try {
        ...
        const recovery: DerivedListingCreateRecoveryManifest =
          deriveListingCreateRecoveryManifest({
        ...
        try {
          ...
          let priorBinding: { intentKey: MigrationDigest } | null = null;
          if (options.priorRecoveryJobId !== undefined
            || options.priorRecoveryAttemptId !== undefined) {
            priorBinding = verifyPriorRecoveryBinding(
              store,
              options.priorRecoveryJobId ?? '',
              options.priorRecoveryAttemptId ?? '',
              bindings.targetIdentityKey,
            );
          }
          ...
          let sourceResult: ReturnType<typeof recordResidueRemovalReconciliation> | null = null;
          if (recoveryResult.resolved) {
            sourceResult = recordResidueRemovalReconciliation({
              store,
              intentKey: options.intentKey as MigrationDigest,
            ...
            if (priorBinding !== null) {
              recordResidueRemovalReconciliation({
                ...
                targetIdentityKey: ceremony.targetIdentityKey,
                jobId: options.priorRecoveryJobId as string,
                attemptId: options.priorRecoveryAttemptId as string,
                recoveryDigest: recovery.manifestDigest,
                outcome,
                providerRemovalVerified: !dispatchFailed,
                startedAtUtc,
  ...
  withRecoverBindingOptions(program
    ...
    .description(
      'Zero-provider-write re-verification for an outstanding recover-create ceremony: '
    ...
    .requiredOption(
      ...
      'Exact recovery attempt id printed by recover-create',
    )
    .action(async (options: RecoverBindingOptions & {
      ...
      try {
        ...
        try {
          ...
          const recoveryAlreadyResolved =
            boundRecoveryJob.state === 'resolved_residue_removed';
          ...
          let recoveryResult: ReturnType<typeof recordResidueRemovalReconciliation> | null = null;
          if (!recoveryAlreadyResolved) {
