...
export class ListingEndDispatchError extends Error {
  constructor(readonly code:
    | 'END_DISPATCH_AUTHORITY_UNAVAILABLE'
    | 'END_DISPATCH_TARGET_INVALID'
    | 'END_DISPATCH_PAYLOAD_INVALID'
    | 'END_DISPATCH_WRITE_FAILED'
    | 'END_DISPATCH_REJECTED') {
    super('Listing end dispatch adapter failed');
    this.name = 'ListingEndDispatchError';
  }
}

const deny = (code: ConstructorParameters<typeof ListingEndDispatchError>[0]): never => {
...
export type TradingEndDispatchAdapter = Readonly<{
  endFixedPriceItem: (input: { listingId: string }) => Promise<void>;
}>;
 ...
 */
export function buildEndFixedPriceItemXml(listingId: string): string {
  if (!EXACT_ITEM_ID.test(listingId)) deny('END_DISPATCH_TARGET_INVALID');
...
async function requireToken(getAccessToken: () => Promise<string>): Promise<string> {
  let token = '';
  try {
...
export function createTradingEndDispatchAdapter(dependencies: Readonly<{
  fetchImpl?: FetchLike;
  getAccessToken: () => Promise<string>;
}>): TradingEndDispatchAdapter {
  const fetchImpl = dependencies.fetchImpl ?? fetch;

  async function endFixedPriceItem(input: { listingId: string }): Promise<void> {
    ...
    try {
      ...
      if (response.status !== 200) deny('END_DISPATCH_WRITE_FAILED');
    } catch (error) {
      if (error instanceof ListingEndDispatchError) throw error;
      return deny('END_DISPATCH_WRITE_FAILED');
    } finally {
      clearTimeout(timeout);
    }
    if (/<!DOCTYPE|<!ENTITY/iu.test(text)) deny('END_DISPATCH_REJECTED');
    let parsed: unknown;
    try {
    ...
    if (ack !== 'Success' && ack !== 'Warning') deny('END_DISPATCH_REJECTED');
  }

  return Object.freeze({ endFixedPriceItem });
}

export function createInventoryWithdrawDispatchAdapter(dependencies: Readonly<{
  fetchImpl?: FetchLike;
  getAccessToken: () => Promise<string>;
  ...
  async function withdrawOffer(offerId: string): Promise<void> {
    if (!SAFE_SEGMENT.test(offerId)) deny('END_DISPATCH_TARGET_INVALID');
    const token = await requireToken(dependencies.getAccessToken);
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS);
    try {
      const response = await fetchImpl(
        `${EBAY_API_HOST}/sell/inventory/v1/offer/${encodeURIComponent(offerId)}/withdraw`,
        {
          method: 'POST',
          headers: {
