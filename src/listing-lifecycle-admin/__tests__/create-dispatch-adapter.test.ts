...
describe('listing-create dispatch outcome classification', () => {
  ...
  it('classifies a known Inventory PUT HTTP rejection as definite no effect', async () => {
    ...
    const error = await expectFailure(
      dispatch.putInventoryItem(SKU, payload),
      'definite_no_effect',
    );
  ...
  it('keeps a known create-offer rejection outcome unknown while exposing safe diagnostics',
    async () => {
      const dispatch = adapter(async () => new Response(
        '{"errors":[{"errorId":25710,"message":"VERY_SECRET"}]}',
        { status: 503 },
      ));
  ...
  it('keeps an Inventory PUT 5xx outcome unknown even with valid safe diagnostics', async () => {
    const dispatch = adapter(async () => new Response(
      '{"errors":[{"errorId":25001,"message":"VERY_SECRET"}]}',
      { status: 503 },
    ));
    const error = await expectFailure(
      dispatch.putInventoryItem(SKU, payload),
  ...
  it.each([
    ...
    const dispatch = adapter(fetchImpl as typeof fetch);
    const error = await expectFailure(
      dispatch.putInventoryItem(SKU, payload),
      'outcome_unknown',
    );
  ...
  it('classifies a local target denial before fetch as definite no effect', async () => {
    ...
    await expectFailure(
      dispatch.putInventoryItem('bad/sku', payload),
      'definite_no_effect',
    );
