...
function itemOnlyResidueWorkspace(): ListingWorkspaceDto {
  const base = notListedWorkspace();
  return {
    ...base,
    catalog: {
...
type World = {
  ...
  recoverCalls: string[];
  useProductionCaptureShape: (on: boolean) => void;
...
describe('listing-lifecycle operator CLI — recover-create', () => {
  ...
  it('binds the provider offer id into recorded evidence when the capture cannot (L40)', async () => {
    ...
    try {
      ...
      const bindsProviderOfferId = evidence.some((entry) =>
        recomputeUnpublishedArtifactResultDigest({
          sourceApprovalEvidenceDigest: source.manifestDigest as `sha256:${string}`,
          offerId: OFFER_ID,
...
describe('recorded unpublished-offer evidence (L40 second layer)', () => {
  const EVIDENCE = `sha256:${'5'.repeat(64)}` as const;
  const SNAPSHOT = `sha256:${'6'.repeat(64)}` as const;
  const run = (offerId: string | null) => ({
