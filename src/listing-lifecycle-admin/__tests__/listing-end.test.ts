...
function tradingWorkspace(): ListingWorkspaceDto {
  return {
    schemaVersion: 1,
    evidence: {
...
function inventoryWorkspace(): ListingWorkspaceDto {
...
function endedWorkspace(): ListingWorkspaceDto {
  return {
    schemaVersion: 1,
    evidence: {
...
type EndWorld = {
  ...
  tradingRequests: CapturedRequest[];
  withdrawCalls: string[];
  setResponseAck: (ack: string) => void;
  setPropagationDelay: (delayed: boolean) => void;
  stdout: string[];
  stderr: string[];
  exitCodes: number[];
...
function createEndWorld(initial: ListingWorkspaceDto): EndWorld {
  ...
  const tradingRequests: CapturedRequest[] = [];
  const fakeFetch: typeof fetch = async (input, init) => {
    ...
    if (responseAck === 'Success' && !propagationDelay) current = endedWorkspace();
    return new Response(
  ...
  const tradingAdapter: TradingEndDispatchAdapter = createTradingEndDispatchAdapter({
    fetchImpl: fakeFetch,
    getAccessToken: async () => 'test-iaf-token',
  });

  const withdrawCalls: string[] = [];
  const withdrawAdapter: InventoryWithdrawDispatchAdapter = Object.freeze({
    withdrawOffer: async (offerId: string) => {
      withdrawCalls.push(offerId);
      if (!propagationDelay) current = endedWorkspace();
    },
  });
...
function establishArguments(world: EndWorld): string[] {
...
function tradingTargetArguments(): string[] {
  return [
...
function inventoryTargetArguments(): string[] {
  return [
    '--catalog-id', CATALOG_ID,
    '--sku', SKU,
