...
import {
  runEvidenceCapturePreflight,
  runEvidenceCollection,
  verifyLocalEvidenceArtifact,
  type EvidenceCapturePreflight,
  type EvidenceCollectionResult,
  type EvidenceVerificationResult,
  type EvidenceCaptureRuntimeBuildIdentity,
} from './capture.js';
...
export type EvidenceCaptureGitExecutor = (
  executable: string,
  arguments_: readonly string[],
  options: ExecFileSyncOptionsWithStringEncoding,
) => string;

const defaultIo: EvidenceCaptureIo = Object.freeze({
...
const defaultDependencies: EvidenceCaptureProgramDependencies = Object.freeze({
...
export function inspectEvidenceCaptureRuntimeBuild(
  repositoryRoot: string,
  execute: EvidenceCaptureGitExecutor = execFileSync,
): EvidenceCaptureRuntimeBuildIdentity {
  ...
  const gitPrefix = ['-c', 'core.fsmonitor=false', '-c', 'core.hooksPath=/dev/null'] as const;
...
function printPreflight(
  result: EvidenceCapturePreflight,
  json: boolean,
  io: EvidenceCaptureIo,
): void {
  if (json) {
  ...
  io.stdout(
    `Local authority: Shopify=${result.sourceReadiness.shopify ? 'valid' : 'blocked'}; eBay=${result.sourceReadiness.ebay ? 'valid' : 'blocked'}`,
  );
...
function printCollection(
  ...
  io.stdout(
    `Counts: primary=${result.counts.primary}; secondary=${result.counts.secondary}; orders=${result.counts.orders}`,
  );
...
function printVerification(
  ...
  io.stdout(
    `Current read evidence: ${result.currentReadEvidence ? 'YES' : 'NO'}; parity use allowed: NO`,
  );
  io.stdout('Safety: zero external writes; no import/backfill; parity and cutover remain false');
}

export function buildEvidenceCaptureProgram(
  io: EvidenceCaptureIo = defaultIo,
  dependencies: EvidenceCaptureProgramDependencies = defaultDependencies,
): Command {
  ...
  program
    ...
    .description(
      'Bounded authoritative-read evidence capture. It cannot import orders, mutate commerce, refresh OAuth, or claim cutover parity.',
    )
    .version('0.1.0')
    .showHelpAfterError();
  ...
  program
    ...
    .action((options: { repoRoot: string; json?: boolean }) => {
      try {
  ...
  program
    ...
    .requiredOption(
      '--artifact <path>',
      'Exact .local/evidence-capture artifact path emitted by collect',
    )
    .option('--repo-root <path>', 'ProductPipeline repository root', '.')
    .option('--json', 'Emit one safe summary object')
    .action((options: { artifact: string; repoRoot: string; json?: boolean }) => {
      try {
        const result = verify({
          repositoryRoot: options.repoRoot,
        ...
        if (!result.currentReadEvidence) io.setExitCode(2);
      } catch (error) {
        io.stderr(options.json
