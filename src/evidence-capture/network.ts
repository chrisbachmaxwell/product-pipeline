...
import {
  EBAY_READ_SCOPES,
  type EbayAuthorizationAttestation,
  type EbayGetRequest,
  type EbayInjectedGetTransport,
} from './ebay.js';
import {
  SHOPIFY_ADMIN_API_VERSION,
  SHOPIFY_GRAPHQL_DOCUMENTS,
  type InjectedShopifyGraphqlRequest,
  type ShopifyGraphqlDispatcher,
} from './shopify.js';
...
const SAFE_FILTER_PATTERN =
  /^creationdate:\[[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9:.]+Z\.\.[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9:.]+Z\]$/;

function canonicalUtc(value: string | undefined): { text: string; epochMs: number } {
  if (typeof value !== 'string') throw new EvidenceNetworkError('authority-unavailable');
...
function assertShopifyVariables(request: InjectedShopifyGraphqlRequest): void {
  ...
  if (request.body.operationName === 'ProductPipelineShopifyPreflight') {
    if (!exactObjectKeys(variables, [])) throw new EvidenceNetworkError('request-denied');
    return;
  }
  if (request.body.operationName === 'ProductPipelineShopifyVariants') {
    if (
      !exactObjectKeys(variables, ['after', 'first'])
  ...
  if (
    !exactObjectKeys(variables, ['after', 'first', 'query'])
    || !Number.isInteger(variables.first)
    || Number(variables.first) < 1
    || Number(variables.first) > 100
    || (variables.after !== null && (typeof variables.after !== 'string' || variables.after.length > 2_048))
...
function assertEbayUrl(request: EbayGetRequest, loaded: LoadedEvidenceCaptureConfig): void {
  let url: URL;
  try {
  ...
  if (url.host !== sellHost) throw new EvidenceNetworkError('request-denied');
  ...
  if (url.pathname === '/sell/inventory/v1/inventory_item') {
    if (
      request.requiredScope !== EBAY_READ_SCOPES.inventory
  ...
  if (url.pathname === '/sell/fulfillment/v1/order') {
    if (
      request.requiredScope !== EBAY_READ_SCOPES.fulfillment
...
async function readJsonResponse(response: Response, maximumBytes: number): Promise<unknown> {
  ...
  if (response.body === null) throw new EvidenceNetworkError('response-denied');
  ...
  try {
    ...
    if (error instanceof EvidenceNetworkError) throw error;
...
function linkedSignal(signal: AbortSignal, timeoutMs: number): AbortSignal {
...
export function createShopifyNetworkDispatcher(input: {
  ...
  const expectedUrl =
    `https://${input.loaded.config.identities.shopifyStoreDomain}/admin/api/${SHOPIFY_ADMIN_API_VERSION}/graphql.json`;
  return async (request) => {
    ...
    return {
      ...
      body: await readJsonResponse(
        response,
        Math.min(input.loaded.config.limits.maxResponseBytes, MAX_RESPONSE_BYTES),
      ),
    };
  };
}
...
export function createEbayNetworkTransport(input: {
  ...
  fetch: EvidenceFetch;
  nowUtc: string;
}): { transport: EbayInjectedGetTransport; authorization: EbayAuthorizationAttestation } {
  ...
  if (
    input.environment[EVIDENCE_AUTHORITY_ENVIRONMENT.ebayScopes]
    !== scopes.join(' ')
  ) {
    throw new EvidenceNetworkError('authority-invalid');
  }
  ...
  if (expiresAt.epochMs <= now.epochMs) throw new EvidenceNetworkError('authority-expired');
  if (
    expiresAt.epochMs - now.epochMs
    < input.loaded.config.limits.minimumEbayAccessValiditySeconds * 1_000
  ) throw new EvidenceNetworkError('authority-near-expiry');
  const authorization: EbayAuthorizationAttestation = Object.freeze({
  ...
  const transport: EbayInjectedGetTransport = Object.freeze({
    provenance: Object.freeze({
      kind: 'direct-ebay-api' as const,
      captureSessionId: randomUUID(),
    }),
    get: async (request: EbayGetRequest) => {
...
export function inspectEvidenceAuthorityAvailability(
  environment: Readonly<Record<string, string | undefined>>,
): Readonly<{
  ...
  return Object.freeze({
    shopifyAccessPresent:
      typeof environment[EVIDENCE_AUTHORITY_ENVIRONMENT.shopifyAccess] === 'string',
    ebayAccessPresent:
      typeof environment[EVIDENCE_AUTHORITY_ENVIRONMENT.ebayAccess] === 'string',
    ebayScopeMetadataPresent:
      typeof environment[EVIDENCE_AUTHORITY_ENVIRONMENT.ebayScopes] === 'string',
    ebayExpiryMetadataPresent:
      typeof environment[EVIDENCE_AUTHORITY_ENVIRONMENT.ebayIssuedAt] === 'string'
      && typeof environment[EVIDENCE_AUTHORITY_ENVIRONMENT.ebayExpiresAt] === 'string',
    signingAuthorityPresent:
      typeof environment.PRODUCT_PIPELINE_EVIDENCE_SIGNING_KEY_PKCS8_B64 === 'string',
  });
}
