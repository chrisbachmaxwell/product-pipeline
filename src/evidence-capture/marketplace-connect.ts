...
const FORBIDDEN_KEY_PATTERN =
  /(?:token|secret|password|credential|cookie|authorization|email|phone|address|customer|buyer|lineitem|line_item|rawpayload|rawresponse|raworder|url|uri)/i;
...
function exactRecord(
  ...
  if (!isRecord(value)) deny(`${fieldPath} must be an object`);
  const actualKeys = Object.keys(value);
  const allowed = new Set(keys);
  for (const key of actualKeys) {
    if (!allowed.has(key)) deny(`${fieldPath}.${key} is not supported`);
  }
  for (const key of keys) {
    if (!Object.hasOwn(value, key)) deny(`${fieldPath}.${key} is required`);
  }
  return value;
}
...
function literal<T extends string | number | boolean | null>(
  ...
  if (value !== expected) deny(`${fieldPath} must use its fail-closed literal`);
  return expected;
}
...
function digest(value: unknown, fieldPath: string): Digest {
...
function canonicalUtc(value: unknown, fieldPath: string): string {
  if (typeof value !== 'string') deny(`${fieldPath} must be a canonical UTC instant`);
...
function boundedInteger(value: unknown, maximum: number, fieldPath: string): number {
...
function boundedArray(value: unknown, maximum: number, fieldPath: string): unknown[] {
  if (!Array.isArray(value)) deny(`${fieldPath} must be an array`);
  if (value.length > maximum) deny(`${fieldPath} exceeds its accepted bound`);
  return value;
}
...
function canonicalBase64(value: unknown, decodedBytes: number, fieldPath: string): string {
...
function scanUntrustedTree(value: unknown): void {
  let nodes = 0;
  const visit = (entry: unknown, fieldPath: string, depth: number): void => {
    nodes += 1;
    ...
    if (typeof entry === 'string') {
      ...
      if (URL_PATTERN.test(entry)) deny(`${fieldPath} contains a forbidden URL or URI`);
...
export function canonicalJson(value: unknown): string {
  ...
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) deny('canonical payload contains a non-finite number');
    return JSON.stringify(value);
  }
...
function sha256Bytes(value: string | Uint8Array): Digest {
...
function parseEvidenceIds(
  ...
  if (!allowEmpty && entries.length === 0) deny(`${fieldPath} must bind at least one evidence item`);
  const result = entries.map((entry, index) =>
    safeIdentifier(entry, EVIDENCE_ID_PATTERN, `${fieldPath}[${index}]`),
  );
  if (new Set(result).size !== result.length) deny(`${fieldPath} contains duplicate evidence IDs`);
  return result;
}

function parseSubject(value: unknown): MarketplaceConnectSubject {
  ...
  return {
    shopifyStoreDomainDigest: digest(
      source.shopifyStoreDomainDigest,
      'payload.subject.shopifyStoreDomainDigest',
    ),
    ebayEnvironment: enumeration(
      source.ebayEnvironment,
      ['sandbox', 'production'] as const,
      'payload.subject.ebayEnvironment',
    ),
    ebaySellerAccountDigest: digest(
      source.ebaySellerAccountDigest,
      'payload.subject.ebaySellerAccountDigest',
    ),
    ebayMarketplaceId: literal(
      source.ebayMarketplaceId,
      'EBAY_US',
      'payload.subject.ebayMarketplaceId',
    ),
  };
}

function parseCapture(value: unknown): MarketplaceConnectCapture {
  const source = exactRecord(
    value,
    ['capturedAtUtc', 'method', 'completeness'],
    'payload.capture',
  );
  return {
    ...
    method: enumeration(
      source.method,
      ['operator-ui', 'shopify-support-export'] as const,
      'payload.capture.method',
    ),
    completeness: enumeration(
      source.completeness,
      ['complete', 'partial'] as const,
      'payload.capture.completeness',
    ),
  };
}

function parseSettings(value: unknown): MarketplaceConnectSettings {
  const source = exactRecord(value, [
    'connection',
    'orderImport',
    'priceSync',
  ...
  const order = exactRecord(
    source.orderImport,
    ['productScope', 'fulfillmentScope', 'importWhen'],
    'payload.settings.orderImport',
  );
  const inventoryLocation = exactRecord(
    source.inventoryLocation,
    ['mode', 'locationSetDigest'],
    'payload.settings.inventoryLocation',
  );
  const mode = enumeration(
    inventoryLocation.mode,
    ['all-locations', 'selected-locations', 'per-product', 'unknown'] as const,
    'payload.settings.inventoryLocation.mode',
  );
  const locationSetDigest = inventoryLocation.locationSetDigest === null
    ? null
    : digest(
      inventoryLocation.locationSetDigest,
      'payload.settings.inventoryLocation.locationSetDigest',
    );
  ...
  const settings: MarketplaceConnectSettings = {
    connection: enumeration(
      source.connection,
      ['connected', 'disconnected', 'unknown'] as const,
      'payload.settings.connection',
    ),
    orderImport: {
      productScope: enumeration(
        order.productScope,
        ['all-orders', 'linked-products-only', 'no-orders', 'unknown'] as const,
        'payload.settings.orderImport.productScope',
      ),
      fulfillmentScope: enumeration(
        order.fulfillmentScope,
        [
          'all-orders',
          'marketplace-fulfilled-only',
          'merchant-fulfilled-only',
      ...
      importWhen: enumeration(
        order.importWhen,
        ['pending', 'complete', 'unknown'] as const,
        'payload.settings.orderImport.importWhen',
      ),
    },
    priceSync: enumeration(
      source.priceSync,
      ['enabled', 'disabled', 'unknown'] as const,
      'payload.settings.priceSync',
    ),
    inventorySync: enumeration(
      source.inventorySync,
      ['enabled', 'disabled', 'unknown'] as const,
      'payload.settings.inventorySync',
    ),
    autoListProducts: enumeration(
      source.autoListProducts,
      ['enabled', 'disabled', 'unknown'] as const,
      'payload.settings.autoListProducts',
    ),
    autoCategorization: enumeration(
      source.autoCategorization,
      ['enabled', 'disabled', 'unknown'] as const,
      'payload.settings.autoCategorization',
    ),
...
function parseAttachments(
  ...
  if (new Set(ids).size !== ids.length) deny('payload.evidenceAttachments has duplicate evidence IDs');
  return result;
}

function parseListingRecord(value: unknown, index: number): MarketplaceConnectListingRecord {
  ...
  const owners = exactRecord(
    source.fieldOwners,
    ['shipping', 'returns', 'title', 'description', 'priceTaxes'],
    `${fieldPath}.fieldOwners`,
  );
  const owner = (entry: unknown, name: string) => enumeration(
    entry,
    ['ebay', 'marketplace-connect', 'unknown'] as const,
    `${fieldPath}.fieldOwners.${name}`,
  );
  return {
    ...
    linkStatus: enumeration(
      source.linkStatus,
      ['linked', 'unlinked', 'suggested', 'unknown'] as const,
...
function parseListingCoverage(value: unknown): MarketplaceConnectListingCoverage {
  const source = exactRecord(value, [
    'status',
  ...
  const status = enumeration(
    ...
    'payload.listingCoverage.status',
  );
  const records = boundedArray(
    source.records,
    MAX_LISTING_RECORDS,
    'payload.listingCoverage.records',
  ).map(parseListingRecord);
  const normalizedRecordCount = boundedInteger(
    source.normalizedRecordCount,
    MAX_LISTING_RECORDS,
    'payload.listingCoverage.normalizedRecordCount',
  );
...
function parseClaims(value: unknown): MarketplaceConnectClaim[] {
  const entries = boundedArray(value, MAX_CLAIMS, 'payload.claims');
  if (entries.length === 0) deny('payload.claims must bind at least one responsibility observation');
  const result = entries.map((entry, index) => {
    const fieldPath = `payload.claims[${index}]`;
    const source = exactRecord(
      entry,
      ['responsibility', 'assertedOwner', 'evidenceClass', 'evidenceIds'],
      fieldPath,
    );
    return {
      ...
      assertedOwner: enumeration(
        source.assertedOwner,
        ['marketplace_connect', 'unverified'] as const,
        `${fieldPath}.assertedOwner`,
      ),
      evidenceClass: enumeration(
        source.evidenceClass,
        ['operator-attested-ui', 'shopify-support-export'] as const,
        `${fieldPath}.evidenceClass`,
      ),
      evidenceIds: parseEvidenceIds(source.evidenceIds, `${fieldPath}.evidenceIds`, false),
    } satisfies MarketplaceConnectClaim;
  });
...
function parseUnknowns(value: unknown): MarketplaceConnectUnknown[] {
  ...
  const result = entries.map((entry, index) => {
    ...
    const source = exactRecord(
      entry,
      ['unknownId', 'responsibility', 'detailsDigest', 'evidenceIds'],
      fieldPath,
    );
    return {
    ...
    } satisfies MarketplaceConnectUnknown;
  });
...
function parseLimitations(value: unknown): MarketplaceConnectAttestationPayload['limitations'] {
  const source = exactRecord(value, [
    'evidenceOnly',
  ...
  return {
    ...
    ownershipTransferAuthorized: literal(
      source.ownershipTransferAuthorized,
      false,
      'payload.limitations.ownershipTransferAuthorized',
    ),
    liveParityProven: literal(
      source.liveParityProven,
      false,
      'payload.limitations.liveParityProven',
    ),
    externalWritesObserved: literal(
      source.externalWritesObserved,
      0,
      'payload.limitations.externalWritesObserved',
    ),
    historicalBackfill: literal(
      source.historicalBackfill,
      false,
      'payload.limitations.historicalBackfill',
    ),
  };
}

function assertEvidenceBindings(payload: MarketplaceConnectAttestationPayload): void {
  const attachmentById = new Map(
    payload.evidenceAttachments.map((attachment) => [attachment.evidenceId, attachment]),
  );
  const referenced = new Set<string>();
  const assertReferences = (ids: string[], fieldPath: string) => {
    for (const evidenceId of ids) {
      if (!attachmentById.has(evidenceId)) deny(`${fieldPath} references missing evidence`);
      referenced.add(evidenceId);
    }
  };
  payload.listingCoverage.records.forEach((record, index) =>
    assertReferences(record.evidenceIds, `payload.listingCoverage.records[${index}].evidenceIds`),
  );
  ...
  payload.unknowns.forEach((unknown, index) =>
    assertReferences(unknown.evidenceIds, `payload.unknowns[${index}].evidenceIds`),
  );
...
function assertClaimSemantics(payload: MarketplaceConnectAttestationPayload): void {
  const attachmentById = new Map(
    payload.evidenceAttachments.map((attachment) => [attachment.evidenceId, attachment]),
  );
  for (const [index, claim] of payload.claims.entries()) {
    if (claim.assertedOwner !== 'marketplace_connect') continue;
    ...
    if (claim.responsibility === 'orderImport') {
      if (
        payload.settings.orderImport.productScope === 'unknown'
    ...
    if (claim.responsibility === 'price') {
      if (
        payload.settings.priceSync !== 'enabled'
        || claim.evidenceClass !== 'shopify-support-export'
    ...
    if (claim.responsibility === 'inventory') {
      if (
        payload.settings.inventorySync !== 'enabled'
        || claim.evidenceClass !== 'shopify-support-export'
        || !surfaces.includes('shopify-support-export')
      ) {
...
function parsePayload(value: unknown): MarketplaceConnectAttestationPayload {
  const source = exactRecord(value, [
    'subject',
    'capture',
    'settings',
  ...
  const payload: MarketplaceConnectAttestationPayload = {
  ...
  if (
    payload.capture.method === 'shopify-support-export'
  ...
  if (
    ...
    && !payload.evidenceAttachments.some((entry) =>
      entry.contentDigest === payload.listingCoverage.terminalPageDigest
...
function parseSignature(
  ...
  const source = exactRecord(
    value,
    ['role', 'signerId', 'keyId', 'algorithm', 'signatureBase64'],
    fieldPath,
  );
  return {
...
function parsePacket(value: unknown): MarketplaceConnectAttestationPacket {
  scanUntrustedTree(value);
  const source = exactRecord(
    value,
    ['schemaVersion', 'kind', 'payload', 'signatures'],
    'packet',
  );
  const signatureValues = boundedArray(source.signatures, 2, 'signatures');
  if (signatureValues.length !== 2) deny('signatures must contain collector and reviewer');
  const packet: MarketplaceConnectAttestationPacket = {
    schemaVersion: literal(source.schemaVersion, 1, 'schemaVersion'),
    kind: literal(
      source.kind,
      'marketplace-connect-readonly-attestation',
      'kind',
    ),
  ...
  if (
    packet.signatures[0].signerId === packet.signatures[1].signerId
...
function verifyDetachedSignature(
  signature: MarketplaceConnectDetachedSignature,
  trusted: TrustedMarketplaceConnectSigner,
  ...
  const publicKeyDer = canonicalBase64(
    trusted.publicKeySpkiBase64,
    44,
    `${signature.role} trusted public key`,
  );
  ...
  if (!verified) deny(`${signature.role} detached signature verification failed`);
}

/** Validate and canonicalize a payload before an external collector/reviewer signs it. */
export function canonicalizeMarketplaceConnectPayload(value: unknown): string {
  scanUntrustedTree(value);
 ...
 */
export function verifyMarketplaceConnectAttestation(
  value: unknown,
  trust: MarketplaceConnectTrust,
): VerifiedMarketplaceConnectAttestation {
  ...
  if (
    canonicalJson(packet.payload.subject) !== canonicalJson(expectedSubject)
  ...
  if (
    trust.collector.signerId === trust.reviewer.signerId
