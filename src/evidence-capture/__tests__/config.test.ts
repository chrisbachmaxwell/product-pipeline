...
import {
  ...
  EvidenceCaptureConfigError,
  loadEvidenceCaptureConfig,
  parseEvidenceCaptureConfig,
} from '../config.js';
...
describe('evidence capture configuration', () => {
  ...
  it('rejects lane/account mismatches, placeholders, and malformed keys', () => {
    ...
    const shippedExample = JSON.parse(fs.readFileSync(
      path.resolve('config/evidence-capture.example.json'),
      'utf8',
    ));
  ...
  it('loads only the fixed repository-local regular file and derives digests', () => {
    ...
    expect(loaded.outputDirectoryAbsolutePath).toBe(
      path.join(fs.realpathSync(root), '.local/evidence-capture'),
    );
  });
