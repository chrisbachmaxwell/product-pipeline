...
import {
  ...
  type InjectedShopifyGraphqlResponse,
  type ShopifyGraphqlDispatcher,
  type ShopifyReadCollectorConfig,
} from '../shopify.js';

const NOW = '2026-08-11T20:00:00.000Z';
const TOKEN = 'shpat_fixture_secret_never_log';

function config(overrides: Partial<ShopifyReadCollectorConfig> = {}): ShopifyReadCollectorConfig {
  return {
...
function response(data: Record<string, unknown>): InjectedShopifyGraphqlResponse {
  return {
    status: 200,
...
function preflight(overrides: Record<string, unknown> = {}): InjectedShopifyGraphqlResponse {
  return response({
    shop: {
...
function variant(id: number, extra: Record<string, unknown> = {}): Record<string, unknown> {
  return {
...
function order(id: number, extra: Record<string, unknown> = {}): Record<string, unknown> {
  return {
...
function terminalVariants(nodes: unknown[] = [variant(1)]): InjectedShopifyGraphqlResponse {
  return response({
    productVariants: {
      nodes,
      pageInfo: { hasNextPage: false, endCursor: null },
...
function terminalOrders(nodes: unknown[] = [order(1)]): InjectedShopifyGraphqlResponse {
  return response({
    orders: {
      nodes,
      pageInfo: { hasNextPage: false, endCursor: null },
...
function queueDispatcher(
  items: InjectedShopifyGraphqlResponse[],
  requests: InjectedShopifyGraphqlRequest[] = [],
...
describe('Shopify authoritative evidence collector', () => {
  it('uses only pinned static query documents and returns complete PII-minimal evidence', async () => {
    const requests: InjectedShopifyGraphqlRequest[] = [];
  ...
  it('fails before dispatch for invalid authority, expired credentials, and non-recent windows', async () => {
    const dispatcher = vi.fn<ShopifyGraphqlDispatcher>();
    const invalid: ShopifyReadCollectorConfig[] = [
  ...
  it('requires exact shop, domain, app, and least-privilege granted scopes', async () => {
    const cases: Array<{ response: InjectedShopifyGraphqlResponse; code: string }> = [
      {
        response: preflight({
          shop: {
  ...
  it('fails closed on HTTP, API-version, GraphQL, throttle, malformed, and dispatcher errors', async () => {
    const cases: Array<{ result: InjectedShopifyGraphqlResponse | Error; code: string }> = [
    ...
    for (const testCase of cases) {
      const dispatcher = async (): Promise<InjectedShopifyGraphqlResponse> => {
        if (testCase.result instanceof Error) throw testCase.result;
  ...
  it('rejects incomplete nested inventory, cursor loops, duplicate IDs, and out-of-window orders', async () => {
    ...
    const inventoryItem = nestedPartial.inventoryItem as Record<string, unknown>;
