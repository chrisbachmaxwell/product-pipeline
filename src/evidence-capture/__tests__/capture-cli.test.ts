...
import {
  EvidenceCaptureCommandError,
  runEvidenceCapturePreflight,
  runEvidenceCollection,
  verifyLocalEvidenceArtifact,
  type EbayCaptureEvidence,
  type ShopifyCaptureEvidence,
} from '../capture.js';
import {
...
import {
  buildEvidenceCaptureProgram,
  inspectEvidenceCaptureRuntimeBuild,
} from '../program.js';
...
function fixture() {
  ...
  fs.writeFileSync(
    path.join(root, EVIDENCE_CAPTURE_CONFIG_RELATIVE_PATH),
...
function shopifyEvidence(orders: ShopifyCaptureEvidence['orders'] = []): ShopifyCaptureEvidence {
  return {
    schemaVersion: 1,
...
describe('evidence capture CLI safety surface', () => {
  it('exposes exactly preflight, collect, and verify with no mutation flags', () => {
    ...
    const optionNames = program.commands.flatMap((command) =>
  ...
  it('round-trips nonempty canonical eBay evidence with an explicit end before capture time', () => {
    ...
    const session = { kind: 'direct-ebay-api', captureSessionId: 'session-1' } as const;
    const identity = { userId: 'immutable-seller-id', registrationMarketplaceId: 'EBAY_US' } as const;
    const evidence: EbayCaptureEvidence = {
      schemaVersion: 1,
    ...
    const artifact = createEvidenceArtifactSigner({ loaded, environment }).sign(
      buildEvidencePayload({
        loaded,
        source: 'ebay',
        evidence,
