...
import type { openMigrationStore } from '../../migration-store/index.js';
import type { openOrderImportStateReader } from '../store-reader.js';
...
type World = {
  ...
  ebayBehavior: { mode: 'success' | 'transport_error' };
  ebayUrls: string[];
  shopifyBehavior: { mode: 'success' | 'transport_error' };
  shopifyHasNextPage: boolean;
  shopifySourceEchoOverride: string | null;
  shopifyOperations: string[];
  ordersByTag: Map<string, string[]>;
  ordersBySourceIdentifier: Map<string, string[]>;
  migrationOpenCalls: number;
  stateReaderOpenCalls: number;
  stdout: string[];
  stderr: string[];
  exitCodes: number[];
...
function shadowArgv(extra: string[] = [],
  maxOrders = '10', lookbackHours = '24'): string[] {
...
describe('shadow-poll (read-only shadow parity mode)', () => {
  ...
  it('blocks non-exact Shopify search echoes and unexpected pagination', async () => {
    for (const mode of ['nonexact', 'pagination'] as const) {
      const world = createWorld();
      world.ebayListOrders = [
        ebayOrderFixture(MATCHED_ORDER_ID, '2026-08-20T10:00:00.000Z'),
      ];
      world.ordersBySourceIdentifier.set(MATCHED_ORDER_ID, [MATCHED_GID]);
      if (mode === 'nonexact') world.shopifySourceEchoOverride = UNMATCHED_ORDER_ID;
      if (mode === 'pagination') world.shopifyHasNextPage = true;
  ...
  it('denies a report path whose parent directory does not exist', async () => {
    const world = createWorld();
    await world.run(shadowArgv(
      ['--report-file', path.join(makeRoot(), 'missing-parent', 'report.json')],
    ));
  ...
  it('never opens the migration store on any path, including failures', async () => {
    ...
    for (const line of world.stderr) {
      expect((JSON.parse(line) as { code: string }).code).toMatch(/^SHADOW_POLL_/);
    }
  });
});
