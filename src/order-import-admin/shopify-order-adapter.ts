...
const deny = (code: ConstructorParameters<typeof ShopifyOrderAdapterError>[0]): never => {
...
function asRecord(
  value: unknown,
  code: ConstructorParameters<typeof ShopifyOrderAdapterError>[0],
): Record<string, any> {
...
export function createShopifyOrderAdapter(dependencies: Readonly<{
  fetchImpl?: FetchLike;
  ...
  async function graphql(
    ...
    failure: 'SHOPIFY_READ_FAILED' | 'SHOPIFY_WRITE_FAILED',
    ...
    if (Buffer.byteLength(body, 'utf8') > MAX_PAYLOAD_BYTES) deny(failure);
    ...
    try {
      ...
      if (!response.ok) deny(failure);
      const parsed = asRecord(JSON.parse(text), failure);
      if (parsed.errors !== undefined || !parsed.data) deny(failure);
      return asRecord(parsed.data, failure);
    } catch (error) {
      if (error instanceof ShopifyOrderAdapterError) throw error;
      return deny(failure);
    } finally {
  ...
  return Object.freeze({
    getInstallationScopes: async (): Promise<string[]> => {
      ...
      return scopes.map((scope: unknown) => {
    ...
    findOrderGidsByTag: async (tag: string): Promise<string[]> => {
      if (typeof tag !== 'string' || !SAFE_TAG.test(tag)) deny('SHOPIFY_TARGET_INVALID');
      const data = await graphql(
        'OrderImportOrdersByTag',
        ORDERS_BY_TAG_QUERY,
        { query: `tag:'${tag}'` },
        'SHOPIFY_READ_FAILED',
      );
    ...
    findVariantGidBySku: async (sku: string): Promise<string | null> => {
      ...
      const gid = node.id;
      if (typeof gid !== 'string' || !VARIANT_GID.test(gid)) return deny('SHOPIFY_READ_FAILED');
      return gid;
    },
    ...
    createOrder: async (
      ...
      const orderGidValue = result.order !== null && typeof result.order === 'object'
        ? (result.order as Record<string, unknown>).id
        : null;
 ...
 */
export function createProductionShopifyOrderTokenProvider(): () => Promise<string> {
  return async (): Promise<string> => {
    ...
    try {
      ...
      if (!row?.access_token) throw new ShopifyOrderAdapterError('SHOPIFY_AUTHORITY_UNAVAILABLE');
