...
export type OrderImportStateReader = Readonly<{
  ...
  getOrderLinkByIdentity: (
    ebayOrderIdentityKey: string,
  ) => Readonly<{ linkId: string; linkKind: string }> | null;
  getCurrentCursor: () => Readonly<{ ordinal: number; cursorValue: string }> | null;
  getUnadvancedPage: () => UnadvancedPageState | null;
  close: () => void;
}>;

export function openOrderImportStateReader(input: Readonly<{
  databasePath: string;
  scopeKey: string;
  ...
  return Object.freeze({
    getObservationByIdentity: (ebayOrderIdentityKey: string): ObservedOrderState | null => {
      const row = database.prepare(
        `SELECT observation.observation_id AS observationId,
      ...
      ).get(scopeKey, ebayOrderIdentityKey) as
        | {
            observationId: string;
            eligible: number;
    ...
    getOrderLinkByIdentity: (ebayOrderIdentityKey: string) => {
      const row = database.prepare(
        `SELECT link_id AS linkId, link_kind AS linkKind FROM order_links
         WHERE scope_key = ? AND ebay_order_identity_key = ? LIMIT 1`,
      ).get(scopeKey, ebayOrderIdentityKey) as
        | { linkId: string; linkKind: string }
        | undefined;
    ...
    getCurrentCursor: () => {
      const row = database.prepare(
        `SELECT ordinal, cursor_value AS cursorValue FROM cursor_advances
         WHERE scope_key = ? ORDER BY ordinal DESC LIMIT 1`,
      ).get(scopeKey) as { ordinal: number; cursorValue: string } | undefined;
      return row ? Object.freeze({ ordinal: row.ordinal, cursorValue: row.cursorValue }) : null;
    },

    getUnadvancedPage: (): UnadvancedPageState | null => {
      const row = database.prepare(
        `SELECT page.page_id AS pageId, page.cursor_after AS cursorAfter,
        ...
        | { pageId: string; cursorAfter: string; observedCount: number; resolvedCount: number }
        | undefined;
      return row
        ? Object.freeze({
            pageId: row.pageId,
