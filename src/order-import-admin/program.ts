...
async function findShopifyOrderGids(
  shopify: ShopifyOrderAdapter,
  orderId: string,
): Promise<{
  sourceIdentifierGids: string[];
  tagGids: string[];
  unionGids: string[];
}> {
...
async function runOrderPostVerify(input: {
  ...
  let identityMatches: Awaited<ReturnType<typeof findShopifyOrderGids>>;
      ...
      try {
        ...
        const observed: Array<{
          ...
          shopifyMatch: {
            ...
            matchedBy?: 'source_identifier' | 'tag' | 'both';
            lookupFailed?: true;
            ambiguous?: true;
          };
        }> = [];
        ...
        for (const order of fetched) {
          ...
          try {
            ...
            const matchedBy = onlyGid !== null
              ? matches.sourceIdentifierGids.includes(onlyGid)
                ? matches.tagGids.includes(onlyGid) ? 'both' as const : 'source_identifier' as const
                : 'tag' as const
              : null;
          ...
          if (shopifyMatch.lookupFailed === true) lookupFailedCount += 1;
          if (shopifyMatch.ambiguous === true) ambiguousCount += 1;
        ...
        if (lookupFailedCount > 0 || ambiguousCount > 0) io.setExitCode(1);
      } catch (error) {
    ...
    .action(async (options: {
      ...
      try {
        ...
        try {
          ...
          if (existingGids.length > 1) deny('IMPORT_SHOPIFY_DUPLICATE_AMBIGUOUS');
