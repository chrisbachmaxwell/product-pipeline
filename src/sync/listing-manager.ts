 ...
 */
export async function republishStaleListings(
  ebayToken: string,
  maxAgeDays = 30,
  ...
  info(
    `[ListingManager] Republish complete: ${result.republished} republished, ${result.skipped} skipped, ${result.failed} failed`,
  );
  return result;
}

// ---------------------------------------------------------------------------
// 2. Price Drop Schedule
// ---------------------------------------------------------------------------
 ...
 */
export async function applyPriceDropSchedule(
  ebayToken: string,
  ...
  // Read settings
  const settingsRows = rawDb.prepare(`SELECT key, value FROM settings WHERE key IN ('price_drop_after_days', 'price_drop_percent')`).all() as any[];
  ...
  for (const row of eligibleRows) {
    ...
    try {
      ...
      const hasSale = rawDb
        ...
        .get(row.ebay_listing_id) as any;
      ...
      await updateOffer(ebayToken, offerId!, offerData as any);

      // Save original price so we don't compound
      const now = new Date();
      rawDb
        .prepare(
          `UPDATE product_mappings SET original_price = ?, updated_at = ? WHERE id = ?`,
        )
  ...
  info(
    `[ListingManager] Price drops complete: ${result.dropped} dropped, ${result.skipped} skipped, ${result.failed} failed`,
  );
  return result;
}
 ...
 */
export async function getStaleListings(maxAgeDays = 14): Promise<any[]> {
  const rawDb = await getRawDb();
  const cutoff = new Date(Date.now() - maxAgeDays * 24 * 60 * 60 * 1000);
  const cutoffUnix = Math.floor(cutoff.getTime() / 1000);
...
export interface ListingHealthData {
  ...
  republished: number;
  promoted: number;
  revenue: number; // Total revenue from synced eBay orders
}

export async function getListingHealth(): Promise<ListingHealthData> {
  const rawDb = await getRawDb();
 ...
 */
export async function enablePromotedListings(
  ebayToken: string,
  listingIds: string[],
  adRate = 2.0,
  ...
  try {
    ...
    for (const listingId of listingIds) {
      ...
      try {
        ...
        rawDb
          .prepare(
            `UPDATE product_mappings SET promoted_at = ?, ad_rate = ?, updated_at = ? WHERE ebay_listing_id = ?`,
          )
  ...
  info(
    `[ListingManager] Promote complete: ${result.promoted} promoted, ${result.failed} failed`,
  );
  return result;
}
 ...
 */
async function findOrCreateCampaign(ebayToken: string, adRate: number): Promise<string> {
  // Try to find existing active PLS campaign
  try {
    const campaigns = await ebayRequest<any>({
  ...
  const created = await ebayRequest<any>({
    method: 'POST',
 ...
 */
async function addListingToCampaign(
  ebayToken: string,
  campaignId: string,
  listingId: string,
  adRate: number,
 ...
 */
export async function runListingManagement(ebayToken: string): Promise<{
