...
import {
  ...
  previewMigrationStoreInitialization,
  verifyMigrationStore,
} from '../program.js';
...
function temporaryRepository(config: unknown = validConfig()): {
  root: string;
...
function createDatabaseParent(root: string): void {
...
function entries(root: string): string[] {
  return (fs.readdirSync(root, { recursive: true, encoding: 'utf8' }) as string[]).sort();
}
...
describe('migration-admin strict local boundary', () => {
  it('parses only the explicit inert schema and rejects unsafe lanes or safety flags', () => {
    ...
    unsafe.safety.externalWrites = true as false;
