...
import { buildPriceInventoryAdminProgram, type PriceInventoryAdminIo } from '../program.js';
...
function emptySnapshot() {
  return { observedAtUtc: '2026-09-02T00:00:00.000Z', rows: [] } as never;
}

function createWorld(snapshot: () => unknown = emptySnapshot) {
  ...
  const run = (argv: string[]) => buildPriceInventoryAdminProgram({
...
function sweepArgs(world: { migrationDatabasePath: string }, overrides: string[] = []) {
  return [
    'align-sweep',
