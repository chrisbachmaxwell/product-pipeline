...
import { createSnapshot, previewSnapshot, rehearseRestore, verifySnapshot, type BackupPlatform } from './core.js';

type Io = { out: (value: string) => void; error: (value: string) => void; exit: (code: number) => void };
const defaultIo: Io = { out: console.log, error: console.error, exit: (code) => { process.exitCode = code; } };
const safe = { providerAccess: false, commerceWrites: false, sourceMutation: false, liveRestore: false } as const;

export function buildControlStateBackupProgram(io: Io = defaultIo, platform?: BackupPlatform): Command {
  ...
  program.command('verify').requiredOption('--snapshot <path>').action((options) => {
    try { const result = verifySnapshot(options.snapshot); io.out(JSON.stringify({ command: 'verify', status: 'verified', files: result.totals.files, bytes: result.totals.bytes, safety: safe })); } catch { fail(); }
  });
  program.command('rehearse-restore').requiredOption('--snapshot <path>').requiredOption('--destination <path>').action((options) => {
    try { const result = rehearseRestore({ snapshotPath: options.snapshot, destinationPath: options.destination }); io.out(JSON.stringify({ command: 'rehearse-restore', status: 'verified', files: result.totals.files, bytes: result.totals.bytes, safety: safe })); } catch { fail(); }
  });
  return program;
}
