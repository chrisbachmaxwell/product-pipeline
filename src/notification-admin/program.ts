...
import { deriveScopeKey, type IntegrationScope } from '../migration-store/index.js';
...
const defaultIo: NotificationAdminIo = Object.freeze({
...
const deny = (code: string): never => { throw new NotificationAdminError(code); };
...
function buildSetPreferencesXml(input: {
  ...
  const events = input.enable
    ? `<UserDeliveryPreferenceArray>${SUBSCRIBED_EVENTS.map((event) =>
      `<NotificationEnable><EventType>${event}</EventType><EventEnable>Enable</EventEnable></NotificationEnable>`).join('')
...
export function buildNotificationAdminProgram(dependencies: Readonly<{
  fetchImpl?: typeof fetch;
  getAccessToken?: () => Promise<string>;
  getAppCredentials?: () => Promise<Readonly<{ devId: string; appId: string; certId: string }>>;
  ...
  async function boundedTradingCall(callName: string, body: string): Promise<string> {
    ...
    try {
      ...
      if (error instanceof NotificationAdminError) throw error;
  ...
  const requireScope = (supplied: string): void => {
  ...
  program
    ...
    .action(async () => {
      try {
        const text = await boundedTradingCall(
        ...
        const events = [...text.matchAll(
          /<EventType>([A-Za-z]{1,64})<\/EventType>\s*<EventEnable>Enable<\/EventEnable>/gu,
        )].map((match) => match[1]);
  ...
  program
    ...
    .requiredOption(
      ...
      'Exact https receiver URL ending in /webhooks/ebay/notifications',
    )
    .requiredOption(
      '--confirm-subscribe',
      'Literal acknowledgement that this enables push delivery to the URL',
    )
    .action(async (options: { confirmScope: string; notificationUrl: string }) => {
      try {
        requireScope(options.confirmScope);
        if (!SAFE_URL.test(options.notificationUrl)) deny('NOTIFICATION_URL_INVALID');
        await boundedTradingCall(
          'SetNotificationPreferences',
          buildSetPreferencesXml({ enable: true, notificationUrl: options.notificationUrl }),
        );
        io.stdout(JSON.stringify({
  ...
  program
    ...
    .action(async (options: { confirmScope: string }) => {
      try {
        requireScope(options.confirmScope);
        await boundedTradingCall(
          'SetNotificationPreferences',
          buildSetPreferencesXml({ enable: false, notificationUrl: null }),
        );
        io.stdout(JSON.stringify({
