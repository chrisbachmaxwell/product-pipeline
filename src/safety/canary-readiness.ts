...
export const CANARY_AUDIT_DESTINATION = 'local-append-only-canary-audit-v1' as const;
...
const APPROVAL_ACTION_BY_RESPONSIBILITY: Record<CanaryResponsibility, CanaryApprovalAction> = {
...
function addIf(blockers: string[], condition: boolean, blocker: string): void {
  if (condition) blockers.push(blocker);
}

export function evaluateCanaryReadiness(input: CanaryReadinessInput): CanaryReadinessResult {
  ...
  if (target) addIf(blockers, input.evidence.targetKey !== target.targetKey, 'evidence.target-mismatch');
  if (responsibility) addIf(blockers, input.evidence.responsibility !== responsibility, 'evidence.responsibility-mismatch');
  ...
  if (target) addIf(blockers, input.singleWriter.targetKey !== target.targetKey, 'single-writer.target-mismatch');
  if (responsibility) addIf(blockers, input.singleWriter.responsibility !== responsibility, 'single-writer.responsibility-mismatch');
  ...
  if (target) addIf(blockers, input.approval.targetKey !== target.targetKey, 'approval.target-mismatch');
  if (responsibility) addIf(blockers, input.approval.responsibility !== responsibility, 'approval.responsibility-mismatch');
  ...
  } else if (!canonicalTimestamp(input.nowUtc)) blockers.push('evaluation.time-invalid');
  ...
  if (target) addIf(blockers, input.idempotency.targetKey !== target.targetKey, 'idempotency.target-mismatch');
  if (responsibility) addIf(blockers, input.idempotency.responsibility !== responsibility, 'idempotency.responsibility-mismatch');
  ...
  if (target) addIf(blockers, input.audit.targetKey !== target.targetKey, 'audit.target-mismatch');
  if (responsibility) addIf(blockers, input.audit.responsibility !== responsibility, 'audit.responsibility-mismatch');
  ...
  if (target) addIf(blockers, input.reconciliation.targetKey !== target.targetKey, 'reconciliation.target-mismatch');
  if (responsibility) addIf(blockers, input.reconciliation.responsibility !== responsibility, 'reconciliation.responsibility-mismatch');
  ...
  if (target) addIf(blockers, input.rollback.targetKey !== target.targetKey, 'rollback.target-mismatch');
  if (responsibility) addIf(blockers, input.rollback.responsibility !== responsibility, 'rollback.responsibility-mismatch');
  ...
  if (responsibility === 'orderImport') {
    ...
    if (
      canonicalTimestamp(input.orderSafety.cutoverWatermarkUtc) &&
      canonicalTimestamp(input.orderSafety.sourceOrderCreatedAtUtc)
    ) {
      addIf(
        blockers,
        Date.parse(input.orderSafety.sourceOrderCreatedAtUtc) <= Date.parse(input.orderSafety.cutoverWatermarkUtc),
        'orders.source-not-after-cutover-watermark',
      );
    }
  ...
  if (
    canonicalTimestamp(input.evidence.observationWindow.startUtc) &&
    canonicalTimestamp(input.evidence.observationWindow.endUtc)
  ) {
    addIf(
      blockers,
      Date.parse(input.evidence.observationWindow.startUtc) > Date.parse(input.evidence.observationWindow.endUtc),
  ...
  if (
    ...
    addIf(
      blockers,
      Date.parse(input.evidence.observationWindow.endUtc) > Date.parse(input.evidence.acceptedAtUtc),
  ...
  if (canonicalTimestamp(input.evidence.acceptedAtUtc) && canonicalTimestamp(input.approval.approvedAtUtc)) {
    addIf(
      blockers,
      Date.parse(input.evidence.acceptedAtUtc) > Date.parse(input.approval.approvedAtUtc),
      'approval.precedes-evidence-acceptance',
    );
  }
