...
export type SandboxAlignmentAdapters = Readonly<{
  ...
  readEbayState: (target: { sku: string; offerId: string; listingId: string }) => Promise<SandboxEbayState>;
  updatePrice: (target: { sku: string; offerId: string }, price: { value: string; currency: 'USD' }) => Promise<void>;
  updateQuantity: (target: { sku: string; offerId: string }, quantity: number) => Promise<void>;
}>;

async function boundedShopifyFetch(fetchImpl: typeof fetch, url: string, init: RequestInit): Promise<{
  ...
  if (parsedUrl.protocol !== 'https:' || parsedUrl.host !== SANDBOX_ALIGNMENT_SCOPE.shopify.storeDomain
    ...
    || parsedUrl.search !== '' || parsedUrl.hash !== '') deny('TRANSPORT_TARGET_DENIED');
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), TIMEOUT_MS);
  try {
    const response = await fetchImpl(url, { ...init, redirect: 'error', signal: controller.signal });
    const declared = Number(response.headers.get('content-length') ?? '0');
    if (Number.isFinite(declared) && declared > MAX_RESPONSE_BYTES) deny('PROVIDER_RESPONSE_INVALID');
    const textBody = await response.text();
    if (Buffer.byteLength(textBody, 'utf8') > MAX_RESPONSE_BYTES) deny('PROVIDER_RESPONSE_INVALID');
    let body: unknown = null;
    if (textBody !== '') {
      try { body = JSON.parse(textBody) as unknown; } catch { deny('PROVIDER_RESPONSE_INVALID'); }
    }
    return { status: response.status, body };
  } catch (error) {
    if (error instanceof Error && error.name === 'SandboxAlignmentError') throw error;
    return deny('PROVIDER_TRANSPORT_FAILED');
  } finally { clearTimeout(timeout); }
}

function object(value: unknown): Record<string, unknown> {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) deny('PROVIDER_RESPONSE_INVALID');
  return value as Record<string, unknown>;
}
function exactText(value: unknown, max = 512): string {
  if (typeof value !== 'string' || value.length === 0 || value.length > max) deny('PROVIDER_RESPONSE_INVALID');
  return value as string;
}
function exactInteger(value: unknown): number {
  if (!Number.isSafeInteger(value) || (value as number) < 0) deny('PROVIDER_RESPONSE_INVALID');
  return value as number;
}

function readShopifyToken(databasePath?: string): string {
  ...
  try {
    ...
    if (database.pragma('query_only', { simple: true }) !== 1) deny('SHOPIFY_AUTHORITY_UNAVAILABLE');
    const rows = database.prepare(`SELECT access_token FROM auth_tokens WHERE platform = 'shopify'`)
      .all() as Array<{ access_token: string }>;
    const row = rows[0];
...
function toEbayState(snapshot: Awaited<ReturnType<SandboxAdapter['snapshot']>>, target: {
  ...
  return Object.freeze({
    ...
    listingId: offer.listingId as string, marketplaceId: offer.marketplaceId,
    merchantLocationKey: offer.merchantLocationKey as 'pp-test-lane', format: offer.format,
    listingDuration: offer.listingDuration, status: offer.status as 'PUBLISHED', listingStatus: 'ACTIVE',
    ...
    price: Object.freeze({ currency: offer.pricingSummary.price.currency as 'USD', value: offer.pricingSummary.price.value }),
    tradingPrice: Object.freeze({ currency: trading.currency as 'USD', value: trading.price }),
  });
}

export async function createSandboxAlignmentAdapters(dependencies: Readonly<{
  ...
  if (packet.sellerId !== SANDBOX_ALIGNMENT_SCOPE.ebay.sellerId) deny('EBAY_SANDBOX_STATE_MISMATCH');
  ...
  async function readShopifySource(): Promise<SandboxSourceState> {
    const token = readShopifyToken(dependencies.databasePath);
    const scope = SANDBOX_ALIGNMENT_SCOPE.shopify;
    const response = await boundedShopifyFetch(fetchImpl,
      `https://${scope.storeDomain}/admin/api/${scope.apiVersion}/graphql.json`, {
    ...
    if (response.status !== 200) deny('SHOPIFY_READ_FAILED');
    const root = object(response.body);
    if (root.errors !== undefined) deny('SHOPIFY_READ_FAILED');
    ...
    if (!Array.isArray(install.accessScopes) || !Array.isArray(product.tags)) deny('SHOPIFY_READ_FAILED');
    return Object.freeze({
      storeDomain: exactText(shop.myshopifyDomain), shopId: exactText(shop.id), appClientId: exactText(app.apiKey),
      scopes: Object.freeze((install.accessScopes as unknown[]).map((entry) => exactText(object(entry).handle)).sort()),
      productId: exactText(product.id), variantId: exactText(variant.id), title: exactText(product.title),
      status: exactText(product.status) as 'ACTIVE',
      tags: Object.freeze((product.tags as unknown[]).map((tag) => exactText(tag)).sort()),
      publishedAt: product.publishedAt as null, sku: exactText(variant.sku),
      currency: exactText(shop.currencyCode) as 'USD', price: exactText(variant.price) as '99.99',
      quantity: exactInteger(variant.inventoryQuantity) as 1,
    });
  }

  async function readEbayState(target: { sku: string; offerId: string; listingId: string }): Promise<SandboxEbayState> {
