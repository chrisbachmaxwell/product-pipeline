...
import { runSandboxPriceInventoryAdmin, type SandboxAlignmentIo } from '../program.js';
...
function world(options: { throwAfterWrite?: boolean; throwWithoutWrite?: boolean } = {}) {
  ...
  let ebay: SandboxEbayState = Object.freeze({
  ...
  let writes = 0;
  const adapter: SandboxAlignmentAdapters = Object.freeze({
    readShopifySource: async () => source,
    readEbayState: async () => ebay,
    updatePrice: async (_target, price) => {
      writes += 1;
      if (!options.throwWithoutWrite) ebay = Object.freeze({ ...ebay, price: Object.freeze(price),
        tradingPrice: Object.freeze(price) });
      if (options.throwAfterWrite || options.throwWithoutWrite) throw new Error('redacted');
    },
    updateQuantity: async (_target, quantity) => {
      writes += 1;
      if (!options.throwWithoutWrite) ebay = Object.freeze({ ...ebay, itemQuantity: quantity,
  ...
  const run = async (...args: string[]) => {
    ...
    return JSON.parse(output.at(-1) as string) as Record<string, unknown>;
  };
  ...
  return { store, run, target, listingArgs, source, adapter, get ebay() { return ebay; },
    ...
    setEbay: (next: SandboxEbayState) => { ebay = next; }, get writes() { return writes; } };
}

async function initialize(candidate: ReturnType<typeof world>) {
...
type Approved = Readonly<{ manifestDigest: string; approvalToken: string; approvalDigest: string }>;
const approvalArgs = (approved: Approved) => ['--manifest-digest', approved.manifestDigest,
  '--approval-token', approved.approvalToken, '--approval-digest', approved.approvalDigest];

async function preflightAndApprove(candidate: ReturnType<typeof world>, action: string): Promise<Approved> {
  const preflight = await candidate.run('preflight', ...candidate.target, '--action', action,
    ...candidate.listingArgs);
  expect(preflight).toMatchObject({ status: 'approval-required', action, externalWritesPerformed: 0 });
  const manifestDigest = preflight.manifestDigest as string;
  const approve = await candidate.run('approve', '--store', candidate.store,
    '--manifest-digest', manifestDigest, '--confirm-action', action);
  expect(approve).toMatchObject({ status: 'approved', manifestDigest, externalWritesPerformed: 0 });
  return Object.freeze({ manifestDigest, approvalToken: approve.approvalToken as string,
    approvalDigest: approve.approvalDigest as string });
}
...
describe('sandbox price/inventory ceremony', () => {
  ...
  it('resolves a response-lost write by observation and never retries it', async () => {
    ...
    expect(await candidate.run('dispatch', ...candidate.target, '--manifest-digest', manifest.manifestDigest,
      '--approval-token', `${manifest.approvalToken}-wrong`, '--approval-digest', manifest.approvalDigest))
  ...
  it('recovers a crash after approval consumption without redispatch', async () => {
    ...
    store.beginDispatch(approved.manifestDigest, approved.approvalToken, approved.approvalDigest,
  ...
  it('denies wrong source, account, target, and action ordering before a write', async () => {
    ...
    expect(await candidate.run('preflight', ...wrongTarget, '--action', 'price-align',
      ...candidate.listingArgs)).toMatchObject({ status: 'denied', code: 'EXACT_TARGET_INVALID' });
    ...
    expect(await candidate.run('preflight', ...candidate.target, '--action', 'price-align',
      ...wrongProvenance)).toMatchObject({ status: 'denied', code: 'LISTING_PROVENANCE_MISMATCH' });

    expect(await candidate.run('preflight', ...candidate.target, '--action', 'quantity-seed',
      ...candidate.listingArgs)).toMatchObject({ status: 'denied', code: 'ACTION_SEQUENCE_INVALID' });

    candidate.setEbay(Object.freeze({ ...candidate.ebay, sellerId: 'production-seller' }));
    expect(await candidate.run('preflight', ...candidate.target, '--action', 'price-align',
      ...candidate.listingArgs)).toMatchObject({ status: 'denied', code: 'EBAY_SANDBOX_STATE_MISMATCH' });
    expect(candidate.writes).toBe(0);
  });
  ...
  it('detects manifest tampering independently of SQLite integrity', async () => {
    const candidate = world();
    await initialize(candidate);
    await candidate.run('preflight', ...candidate.target, '--action', 'price-align',
      ...candidate.listingArgs);
    ...
    await forged.run('preflight', ...forged.target, '--action', 'price-align',
      ...forged.listingArgs);
...
describe('sandbox bulk update body', () => {
  it('has exactly one price-only or quantity-only request', () => {
    const price = JSON.parse(buildSandboxBulkUpdateBody({ field: 'price', sku: SANDBOX_ALIGNMENT_SCOPE.ebay.sku,
      offerId: '123', price: { currency: 'USD', value: '99.99' } })) as Record<string, unknown>;
    ...
    const quantity = JSON.parse(buildSandboxBulkUpdateBody({ field: 'quantity', sku: SANDBOX_ALIGNMENT_SCOPE.ebay.sku,
      offerId: '123', quantity: 2 })) as Record<string, unknown>;
