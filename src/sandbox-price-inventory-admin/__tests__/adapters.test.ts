...
describe('bounded live adapters', () => {
  it('reuses the listing canary Sandbox transport for exact reads and one bulk write', async () => {
    ...
    const fetchImpl = async (input: string | URL | Request, init?: RequestInit): Promise<Response> => {
      ...
      if (parsed.host === 'usedcameragear.myshopify.com') return json({ data: {
      ...
      if (parsed.host === 'apiz.sandbox.ebay.com') return json({ userId: 'testuser_ppcanary-3c55629b', registrationMarketplaceId: 'EBAY_US' });
      if (parsed.pathname.includes('/inventory_item/')) return json({ sku: 'PIPELINE-TEST-20260826',
      ...
      if (parsed.pathname.endsWith('/offer')) return json({ total: 1, offers: [{ offerId: '123',
      ...
      if (parsed.pathname === '/ws/api.dll') return new Response(`<?xml version="1.0"?><GetSellerListResponse>
      ...
      if (parsed.pathname.endsWith('/bulk_update_price_quantity')) return json({ responses: [{ statusCode: 200 }] });
      throw new Error('unexpected target');
    };
    const adapters = await createSandboxAlignmentAdapters({ fetchImpl: fetchImpl as typeof fetch, databasePath,
  ...
  it('consumes the shared bulk-write capability even when the response is rejected', async () => {
    ...
    const action = { field: 'quantity' as const, sku: 'PIPELINE-TEST-20260826', offerId: '123', quantity: 2 };
