...
import {
  ...
  refreshReconciliationSource,
  validConfig,
  validReconciliationSnapshot,
} from './fixtures.js';
...
async function tempRepo(
  snapshot = validReconciliationSnapshot(),
  config: OperatorConfig = validConfig(),
): Promise<string> {
...
describe('reconciliation snapshot schema v2', () => {
  ...
  it('requires total and terminal pagination evidence for every complete capture', () => {
    ...
    expect(() => parseReconciliationSnapshot(missingTotal)).toThrow(
      /reportedTotal is required when availability is complete/,
    );

    const missingTerminal = cloneSnapshot();
    missingTerminal.sources.shopify.provenance.terminalCursorDigest = null;
    expect(() => parseReconciliationSnapshot(missingTerminal)).toThrow(
      /terminalCursorDigest is required when availability is complete/,
    );

    const mismatchedTotal = cloneSnapshot();
    mismatchedTotal.sources.shopify.provenance.reportedTotal = 1_000;
    expect(() => parseReconciliationSnapshot(mismatchedTotal)).toThrow(
      /reportedTotal must equal recordCount when availability is complete/,
    );
  });
  ...
  it('requires API version provenance for available direct reads', () => {
    ...
    expect(() => parseReconciliationSnapshot(snapshot)).toThrow(
      /apiVersion is required for an available direct API read/,
    );
  });
  ...
  it('denies treating Marketplace Connect UI observation as a direct API read', () => {
    const snapshot = cloneSnapshot() as unknown as Record<string, any>;
...
describe('snapshot reconciliation evidence and safety', () => {
  it('compares complete snapshots without network or application database access', async () => {
    ...
    const operationalEvidence = result.responsibilityEvidence.filter(
      (item) => item.responsibility !== 'reconciliation',
    );
    ...
    for (const item of operationalEvidence) {
      expect(item.blockers).toContain(
        `responsibility.${item.responsibility}.model-coverage-incomplete`,
      );
  ...
  it('blocks a snapshot generation timestamp beyond the allowed clock skew', async () => {
    ...
    expect(result.responsibilityEvidence.find(
      (item) => item.responsibility === 'reconciliation',
    )?.state).toBe('blocked');
  });
  ...
  it('keeps an empty partial self-asserted capture blocked without terminal evidence', async () => {
    ...
    expect(result.sourceEvidence.find((item) => item.source === 'shopify')?.blockers).toEqual(
  ...
  it('records dataset/snapshot/result digests, not commerce identifiers, in audit evidence', async () => {
    ...
    for (const source of result.sourceEvidence) expect(auditText).toContain(source.datasetDigest!.slice('sha256:'.length));
