...
function event(command: 'preflight' | 'ownership' = 'preflight'): AuditEventInput {
  return {
    command,
...
describe('operator audit chain', () => {
  ...
  it('rejects credential-like material before writing', async () => {
    ...
    const unsafe = {
      ...event(),
      checks: [{ id: 'secret.value', result: 'pass' as const }],
    };

    await expect(appendAuditRecord(root, '.local/operator-audit/operator-cli.jsonl', unsafe)).rejects.toThrow(
      /credential-like/,
    );
  ...
  it('does not follow a symlinked local audit directory', async () => {
    ...
    await expect(
      appendAuditRecord(root, '.local/operator-audit/operator-cli.jsonl', event()),
