 ...
 */
function sanitizedProductTags(value: unknown): readonly string[] {
...
export function createLiveListingCatalogCache(
   ...
   */
  const refreshIfStale = async (maxAgeMs: number): Promise<LiveListingCatalogSnapshot> => {
    if (cached && now() - cached.refreshedAt < maxAgeMs) return cached.value;
    return refresh();
  };
...
export function startLiveListingCatalogRefresher(
  ...
  if (!Number.isSafeInteger(intervalMs) || intervalMs < 15_000 || intervalMs > 3_600_000) deny();
