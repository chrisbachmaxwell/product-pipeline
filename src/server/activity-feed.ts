...
const RESPONSIBILITY_EVENT: Readonly<
  Record<string, { kind: ActivityEventKind; label: string }>
> = Object.freeze({
...
function safeIdentifier(value: unknown): string | undefined {
  ...
  if (value.length === 0 || value.length > MAX_IDENTIFIER_LENGTH) return undefined;
  if (!/^[\x21-\x7e][\x20-\x7e]*$/.test(value)) return undefined;
  return value;
}
...
type Sortable = { epochMs: number; event: ActivityEvent };

function unavailableFeed(windowHours: number): ActivityFeedProjection {
...
export async function readActivityFeed(options: {
  ...
  let databaseAbsolutePath: string;
  try {
  ...
  try {
    ...
    if (!Number.isSafeInteger(nowEpochMs)) return unavailableFeed(windowHours);
    ...
    let dispatchRows: Array<Record<string, unknown>>;
    try {
      orderRows = database
        .prepare(ORDER_LINK_SQL)
        .all(sinceEpochMs, untilEpochMs, limit) as Array<Record<string, unknown>>;
    ...
    const collected: Sortable[] = [];

    for (const row of orderRows) {
      if (!isExactUtc(row.atUtc) || !Number.isSafeInteger(row.atEpochMs)) continue;
      ...
      if (ebayOrderId !== undefined) event.ebayOrderId = ebayOrderId;
      collected.push({ epochMs: row.atEpochMs as number, event });
    }
    ...
    for (const row of dispatchRows) {
      ...
      if (identifier !== undefined) {
        ...
        if (row.targetKind === 'inventory_sku') event.sku = identifier;
        else if (row.targetKind === 'listing') event.listingId = identifier;
        else if (row.targetKind === 'order') event.ebayOrderId = identifier;
      }
      collected.push({ epochMs: row.atEpochMs as number, event });
    }
