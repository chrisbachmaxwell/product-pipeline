...
function row(overrides: Partial<LiveListingCatalogRow> = {}): LiveListingCatalogRow {
  return {
    id: rowId,
    shopify: {
      productId,
...
function snapshot(rows: readonly LiveListingCatalogRow[] = [row()]): LiveListingCatalogSnapshot {
  return {
    observedAtUtc,
    rows,
    summary: {
...
function detail(input: EnrichedListingDetailRequest): EnrichedListingDetail {
  return {
    schemaVersion: 1,
    evidence: {
...
function dependencies(source: LiveListingCatalogSnapshot = snapshot()) {
  const getEbayAccessToken = vi.fn(async () => 'secret-transient-token');
  const readEbayDetail = vi.fn(async (input: EnrichedListingDetailRequest) => detail(input));
  return {
    dependencies: {
...
describe('read-only listing workspace reader', () => {
  ...
  ] as const)('rejects an absent or inexact row ID %j as %s', async (requested, kind) => {
    const harness = dependencies();
  ...
  ] as const)('fails unavailable for %s before requesting a token', async (_name, source) => {
