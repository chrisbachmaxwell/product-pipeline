...
function unavailable(categoryId: string): EbayCategoryAspectsDto {
  return Object.freeze({ available: false, categoryId, aspects: Object.freeze([]) });
}

type FetchLike = typeof fetch;

async function applicationToken(fetchImpl: FetchLike): Promise<string | null> {
...
async function fetchAspects(
  ...
  if (!Array.isArray(body.aspects)) return unavailable(categoryId);

  const aspects: EbayCategoryAspect[] = [];
  // Required aspects first, in eBay's order; then recommended.
  const ordered = [...body.aspects].sort((left, right) =>
    Number(right.aspectConstraint?.aspectRequired === true)
  ...
  for (const raw of ordered) {
    ...
    for (const value of raw.aspectValues ?? []) {
      if (values.length >= MAX_VALUES_PER_ASPECT) break;
...
export function createEbayCategoryAspectsReader(dependencies: Readonly<{
  fetchImpl?: FetchLike;
  now?: () => number;
}> = {}): (categoryId: string) => Promise<EbayCategoryAspectsDto> {
  const fetchImpl = dependencies.fetchImpl ?? fetch;
  const now = dependencies.now ?? Date.now;
  const cache = new Map<string, { at: number; ttlMs: number; value: EbayCategoryAspectsDto }>();
  const flights = new Map<string, Promise<EbayCategoryAspectsDto>>();
  return async (categoryId: string) => {
    if (!CATEGORY_ID.test(categoryId)) return unavailable(String(categoryId).slice(0, 10));
    const hit = cache.get(categoryId);
    if (hit && now() - hit.at < hit.ttlMs) return hit.value;
    const inFlight = flights.get(categoryId);
    if (inFlight) return inFlight;
    const flight = (async () => {
      let value: EbayCategoryAspectsDto;
      try {
        value = await fetchAspects(fetchImpl, categoryId);
      } catch {
        value = unavailable(categoryId);
      }
      if (cache.size >= MAX_CACHED_CATEGORIES) cache.clear();
