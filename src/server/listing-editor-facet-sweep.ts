...
const NO_OBSERVATIONS: readonly ListingEditorFacetObservation[] = Object.freeze([]);
 ...
 * facet at all.
 */
function observationFromWorkspace(dto: unknown): ListingEditorFacetObservation | null {
...
export function createEditorFacetSweep(dependencies: Readonly<{
  ...
  let cache: {
    observations: readonly ListingEditorFacetObservation[];
    expiresAt: number;
  } | null = null;
  ...
  async function runSweep(): Promise<void> {
    ...
    const observations: ListingEditorFacetObservation[] = [];
    let nextIndex = 0;
    const workers = Array.from(
      { length: Math.min(SWEEP_CONCURRENCY, identities.length) },
      async () => {
        while (nextIndex < identities.length) {
          const identity = identities[nextIndex]!;
          nextIndex += 1;
          try {
            const observation = observationFromWorkspace(
              await dependencies.readListingDetail(identity),
            );
  ...
  return Object.freeze({
    ...
    settle: async (): Promise<void> => {
      while (sweepInFlight !== null) await sweepInFlight;
    },
  });
}
 ...
 */
export const editorFacetSweep: EditorFacetSweep = createEditorFacetSweep({
