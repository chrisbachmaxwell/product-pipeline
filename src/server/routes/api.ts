...
router.post('/api/products/:productId/apply-ai-description', async (req: Request, res: Response) => {
  try {
    const db = await getRawDb();
    const productId = Array.isArray(req.params.productId) ? req.params.productId[0] : req.params.productId;
    ...
    const row = db
      ...
      .get(productId) as { ai_description: string | null } | undefined;
    ...
    const response = await fetch(
      `https://${creds.storeDomain}/admin/api/2024-01/products/${productId}.json`,
      {
        method: 'PUT',
        headers: {
