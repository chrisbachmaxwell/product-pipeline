...
import {
  LISTING_DRAFT_ROUTE_TESTING,
  createListingDraftRouter,
  listingDraftJsonErrorHandler,
  listingDraftJsonParser,
} from './listing-drafts.js';
...
let currentWorkspace: ListingWorkspaceDto = workspace();

function mintSessionToken(input: {
  secret: string; subject?: string; audience?: string; destination?: string;
}): string {
...
function runAdmin(command: 'init' | 'verify'): Record<string, unknown> {
  const lines: string[] = [];
...
beforeAll(() => {
  for (const key of ENVIRONMENT_KEYS) savedEnvironment.set(key, process.env[key]);
...
describe('G3 exercise: authenticated bounded local draft append', () => {
  it('performs one bounded append with revision, stale-base, and admin verification proof', async () => {
    ...
    const openedDto = await opened.json() as {
      base: { sourceDigest: string; ebayDigest: string };
    ...
    const savedDto = await saved.json() as {
      revision: { revisionId: string; revisionNumber: number; revisionDigest: string };
      sections: { listing: { title: { draft: string | null } } };
    };
    ...
    const reopenedDto = await reopened.json() as {
      base: { sourceDigest: string; ebayDigest: string };
    ...
    const secondDto = await second.json() as { revision: { revisionNumber: number } };
