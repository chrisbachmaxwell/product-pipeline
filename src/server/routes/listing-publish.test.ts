...
function harness(options: {
  ...
  reconcileJson?: Record<string, unknown> | null;
  recoverJson?: Record<string, unknown> | null;
  recoverReconcileJson?: Record<string, unknown> | null;
  unresolvedCreate?: {
  ...
  app.use(createListingPublishRouter({
    ...
    runStep: async (argv) => {
      ...
      if (argv[1] === 'reconcile') return { json: options.reconcileJson ?? null };
      if (argv[1] === 'recover-create') return { json: options.recoverJson ?? null };
      if (argv[1] === 'recover-reconcile') return { json: options.recoverReconcileJson ?? null };
...
describe('listing publish route', () => {
  ...
  it('recovery retry: ledger lookup feeds the chain, offer id comes from reconcile', async () => {
    ...
    const response = await post(h.app, '/api/listing-recovery',
      { catalogId: CATALOG_ID, sku: '2882A001-U002' });
    expect(response.status).toBe(200);
