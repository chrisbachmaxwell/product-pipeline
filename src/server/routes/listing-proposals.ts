...
const STATUS: Readonly<Record<ListingProposalFailureCode, number>> = Object.freeze({
...
const MESSAGE: Readonly<Record<ListingProposalFailureCode, string>> = Object.freeze({
...
function verifiedPrincipal(req: Request) {
...
function allowProposalAttempt(subject: string, now = Date.now()): boolean {
  const key = createHash('sha256').update(subject, 'utf8').digest('hex');
  for (const [candidate, entry] of requestRates) {
    if (now - entry.startedAt >= REQUEST_WINDOW_MS) requestRates.delete(candidate);
  }
  const current = requestRates.get(key);
  if (!current || now - current.startedAt >= REQUEST_WINDOW_MS) {
    if (requestRates.size >= MAX_RATE_BUCKETS) return false;
...
export function createListingProposalRouter(
  service: ListingProposalService = createListingProposalService(),
): Router {
  const router = Router();
  ...
  router.get(EXACT_ROUTE, async (req, res) => {
    try {
      ...
      try { verifiedPrincipal(req); authorized = true; }
...
export function listingProposalJsonErrorHandler(
  err: unknown,
  req: Request,
  res: Response,
  next: NextFunction,
