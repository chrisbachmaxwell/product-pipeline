...
async function requestHealth(port: number): Promise<Record<string, any>> {
  return new Promise((resolve, reject) => {
    http.get({ hostname: '127.0.0.1', port, path: '/health' }, (response) => {
      let raw = '';
      response.setEncoding('utf8');
      response.on('data', (chunk) => { raw += chunk; });
      response.on('end', () => resolve(JSON.parse(raw) as Record<string, any>));
    }).on('error', reject);
  });
}
...
describe('health operational counters', () => {
  ...
  it('has no request-time store or full monitoring reader in the public health route', () => {
    const source = fs.readFileSync(path.join(
      path.dirname(fileURLToPath(import.meta.url)), 'health.ts',
    ), 'utf8');
    expect(source).not.toMatch(
      /readOperationalMonitoring|readConfiguredMigrationState|inspectMigrationStoreReadOnly|better-sqlite3/,
    );
