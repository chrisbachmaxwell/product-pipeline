...
function findNotificationType(value: unknown): string | null {
  if (!value || typeof value !== 'object') return null;
  const record = value as Record<string, unknown>;
  if (typeof record.NotificationType === 'string') return record.NotificationType;
...
router.post('/webhooks/ebay/notifications', async (req: Request, res: Response) => {
  ...
  try {
    ...
    ).run('ebay', notificationType, evidence);
