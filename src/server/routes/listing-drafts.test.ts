...
function harness(kind: ApiPrincipal['kind'], service: { get: ReturnType<typeof vi.fn>; save: ReturnType<typeof vi.fn> }) {
  const app = express();
  app.use((req, _res, next) => {
    (req as Request & { apiPrincipal?: ApiPrincipal }).apiPrincipal = principal(kind);
    next();
  });
  ...
  app.use(createListingDraftRouter(service as never));
  return app;
}
...
describe('local listing draft route boundary', () => {
  it('allows only an exact verified Shopify session principal and derives actor server-side', async () => {
    ...
    expect(service.save).toHaveBeenCalledWith(expect.objectContaining({ catalogId: 'row-1' }),
      'shopify-user:123');
  ...
  it('authenticates before parsing and sanitizes authenticated parser failures', async () => {
    ...
    app.use('/api', (req, res, next) => {
      ...
      (req as Request & { apiPrincipal?: ApiPrincipal }).apiPrincipal = principal('shopify_session');
      next();
    });
    app.use('/api', writerQuarantineMiddleware);
    app.post('/api/listing-draft', listingDraftJsonParser);
