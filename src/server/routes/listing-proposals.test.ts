...
import {
  LISTING_PROPOSAL_ROUTE_TESTING,
  createListingProposalRouter,
  listingProposalJsonErrorHandler,
  listingProposalJsonParser,
} from './listing-proposals.js';
...
function harness(kind: ApiPrincipal['kind'], service: {
  ...
  app.use(createListingProposalRouter(service as never));
  return app;
}
...
describe('local AI listing proposal route boundary', () => {
  it('dispatches generation and approval only for an exact Shopify principal', async () => {
    ...
    expect(service.generate).toHaveBeenCalledWith(
      expect.objectContaining({ action: 'generate_local_proposal' }),
      'shopify-user:123',
    );
    ...
    expect(service.approve).toHaveBeenCalledWith(
      expect.objectContaining({ action: 'approve_local_proposal' }),
      'shopify-user:123',
    );
