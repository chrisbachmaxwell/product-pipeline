...
describe('shadow API allowlist', () => {
  ...
  it('filters schema-v3 live rows to the operator ready queue and rejects invalid ready values', async () => {
    ...
    const readyVariant = (suffix: string, sku: string, productTags: readonly string[]) => ({
  ...
  it('mounts the narrow live reader without legacy routers or commerce writers', async () => {
    ...
    expect(server).toMatch(
      /app\.use\('\/api', apiKeyAuth\)[\s\S]*app\.use\('\/api', writerQuarantineMiddleware\)[\s\S]*app\.post\('\/api\/listing-draft', listingDraftJsonParser\)/,
    );
