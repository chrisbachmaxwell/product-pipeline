...
import {
  buildListingDescriptionPreviewInput,
...
function field(input: Partial<{
  shopify: string | null; ebay: string | null; draft: string | null;
}> = {}): { shopify: string | null; ebay: string | null; draft: string | null; editable: boolean } {
...
function draftDto(overrides: {
  ...
  descriptionObserved?: string | null;
  conditionDraft?: string | null;
  conditionNoteDraft?: string | null;
  imagesObserved?: string | null;
} = {}): ListingDraftDto {
  return {
    schemaVersion: 1,
    ...
    sections: {
      listing: {
        ...
        price: field({ editable: false } as never),
        quantity: field({ editable: false } as never),
      },
      content: {
        description: field({
        ...
        itemSpecifics: field({ editable: false } as never),
        identifiers: field({ editable: false } as never),
      },
      delivery: {
...
async function requestPreview(
  router: Router,
  pathname = `/api/listing-description-preview?id=${encodeURIComponent(CATALOG_ID)}`,
...
function router(getListingDraft: (catalogId: string) => Promise<ListingDraftDto>): Router {
