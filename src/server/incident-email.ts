...
export function buildIncidentEmail(input: {
  ...
  const subject = headerSafe(
    `[ProductPipeline ${input.payload.severity.toUpperCase()}] ${input.payload.title}`,
  );
  const bodyLines = [
    input.payload.title,
    '',
...
export async function sendIncidentEmail(
  payload: IncidentEmailPayload,
  config: SmtpConfig | null = readSmtpConfigFromEnv(),
): Promise<boolean> {
  ...
  return new Promise<boolean>((resolve) => {
    ...
    const steps: Array<{ expect: RegExp; send: string | null }> = [
      ...
      ...config.to.map((address) => (
        { expect: /^250 /m, send: `RCPT TO:<${address}>\r\n` }
      )),
    ...
    socket.on('data', (chunk: Buffer) => {
      ...
      while (step < steps.length) {
        ...
        if (!current.expect.test(buffer)) {
          if (/^[45]\d\d[ -]/m.test(buffer)) finish(false);
          return;
        }
        buffer = '';
        step += 1;
        if (current.send !== null) socket.write(current.send);
        if (step === steps.length) finish(true);
      }
    });
