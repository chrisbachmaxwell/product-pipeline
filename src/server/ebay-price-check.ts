...
export type PriceCheckComp = Readonly<{
  title: string;
  price: Readonly<{ value: number; currency: string }>;
...
function fail(code: PriceCheckFailureCode): never {
 ...
 */
export function derivePriceCheckQuery(title: string): string {
  const stripped = title
    ...
    .replace(/\s+/gu, ' ')
    .trim();
  if (stripped.length <= MAX_QUERY_CHARACTERS) return stripped;
 ...
 */
export function derivePriceCheckCondition(hint: string | null | undefined): PriceCheckCondition {
  if (typeof hint !== 'string') return 'USED';
  const normalized = hint.trim().toLocaleLowerCase('en-US');
  return normalized === 'new' || normalized === 'brand new' ? 'NEW' : 'USED';
}

function safeCompText(value: unknown, maximum: number): string | null {
...
function toComp(raw: unknown): PriceCheckComp | null {
...
export function medianPrice(values: readonly number[]): number {
  if (values.length === 0) return fail('PRICE_CHECK_NO_RESULTS');
...
function toResult(
  ...
  const comps: PriceCheckComp[] = [];
  for (const raw of summaries) {
    if (comps.length >= SEARCH_LIMIT) break;
    const comp = toComp(raw);
    if (comp !== null) comps.push(comp);
  }
  if (comps.length === 0) return fail('PRICE_CHECK_NO_RESULTS');
...
async function boundedBrowseGet(
  fetchImpl: FetchLike,
  path: string,
  accessToken: string,
): Promise<UnknownRecord> {
  if (!path.startsWith(EBAY_BROWSE_SEARCH_PATH)) return fail('PRICE_CHECK_READ_FAILED');
  ...
  try {
    ...
    if (Buffer.byteLength(text, 'utf8') > MAX_RESPONSE_BYTES) return fail('PRICE_CHECK_READ_FAILED');
    status = response.status;
  } catch (error) {
    if (error instanceof PriceCheckError) throw error;
    return fail('PRICE_CHECK_READ_FAILED');
  } finally {
    clearTimeout(timeout);
  }
  if (status !== 200) return fail('PRICE_CHECK_READ_FAILED');
  try {
 ...
 * logged or persisted.
 */
export function createApplicationEbayTokenProvider(dependencies: Readonly<{
  loadCredentials?: () => Promise<Readonly<{ appId?: string; certId?: string }>>;
  fetchImpl?: FetchLike;
  now?: () => number;
}> = {}) {
  ...
  return async (): Promise<string> => {
    ...
    flight = (async () => {
      ...
      if (!appId || !certId) return fail('PRICE_CHECK_AUTH_UNAVAILABLE');
...
export function createPriceCheck(dependencies: Readonly<{
  ...
  const cache = new Map<string, { result: PriceCheckResult; expiresAt: number }>();
  const inFlight = new Map<string, Promise<PriceCheckResult>>();
  ...
  return async (title: string, conditionHint?: string | null): Promise<PriceCheckResult> => {
    if (typeof title !== 'string') return fail('PRICE_CHECK_NO_RESULTS');
    const query = derivePriceCheckQuery(title);
    if (query.length < MIN_QUERY_CHARACTERS) return fail('PRICE_CHECK_NO_RESULTS');
    ...
    const lookup = (async () => {
      ...
      const body = await boundedBrowseGet(
        fetchImpl,
        `${EBAY_BROWSE_SEARCH_PATH}?${search.toString()}`,
        await accessToken(),
      );
...
export const checkListingPrice: PriceCheck = createPriceCheck({
