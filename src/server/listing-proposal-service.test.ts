...
import {
  LISTING_PROPOSAL_POLICY_DIGEST,
  LISTING_PROPOSAL_SERVICE_TESTING,
  ListingProposalServiceError,
  createListingProposalService,
  parseListingProposalRequest,
} from './listing-proposal-service.js';

const digest = `sha256:${'a'.repeat(64)}` as const;
const sourceDigest = `sha256:${'b'.repeat(64)}` as const;
const ebayDigest = `sha256:${'c'.repeat(64)}` as const;
const roots: string[] = [];
...
function draftDto(): ListingDraftDto {
  const candidate: ListingDraftDto = {
    schemaVersion: 1,
...
function modelOutput() {
  return {
    ...
    fields: LISTING_PROPOSAL_FIELDS.map((fieldName) => fieldName === 'title'
      ? { field: fieldName, choice: 'use_shopify' as const,
        reasonCode: 'use_verified_shopify' as const, riskCodes: [] }
      : { field: fieldName, choice: 'keep_ebay' as const,
        reasonCode: 'keep_verified_ebay' as const, riskCodes: [] }),
  };
}

function seedInFlightProposal(
  databasePath: string,
  dto: ListingDraftDto,
  ...
  try {
    ...
    if (state === 'generating') {
      ...
      if (!queued) throw new Error('Seeded proposal job is missing');
...
describe('listing proposal browser contract', () => {
  ...
  it('automatically prepares a verified-value proposal and atomically records local approval', async () => {
    ...
    let currentDto = dto;
    const agent = {
      readiness: vi.fn(() => ({ ready: true as const, code: 'ready' as const,
        model: 'gpt-5.6-terra' as const })),
      generate: vi.fn(async (current: ListingDraftDto) => ({
        decision: resolveListingProposalOutput(
          modelOutput(), buildListingProposalEvidence(current), current,
        ),
        generator: {
    ...
    const service = createListingProposalService({
      ...
      draftService: { get: vi.fn(async () => currentDto), save: vi.fn() } as never,
      agent,
      provider: 'fixture',
  ...
  it('rejects stale semantic facts before any AI request', async () => {
    const dto = draftDto();
    const agent = { readiness: vi.fn(() => ({ ready: true as const, code: 'ready' as const,
      model: 'gpt-5.6-terra' as const })), generate: vi.fn() };
    const service = createListingProposalService({
      databasePath: () => temporaryStore(),
      draftService: { get: vi.fn(async () => dto), save: vi.fn() } as never,
  ...
  it.each(['queued', 'generating'] as const)(
    'projects an abandoned %s job as failed and durably recovers it before a manual retry',
    async (state) => {
      const databasePath = temporaryStore();
      const dto = draftDto();
