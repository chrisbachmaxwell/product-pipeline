 ...
 */
const CONDITION_DISPLAY_OVERRIDES: Readonly<Record<string, string>> = Object.freeze({
  '1500': 'New other (open box)',
});

const CONDITION_LABELS: ReadonlyMap<string, string> = new Map(
  EBAY_CONDITIONS.map((option) => [
    option.id,
...
export class ListingDescriptionTemplateError extends Error {
  constructor(readonly code: 'INVALID_INPUT' | 'OUTPUT_TOO_LARGE') {
    super('Listing description template rendering failed');
    this.name = 'ListingDescriptionTemplateError';
  }
}

const invalid = (): never => { throw new ListingDescriptionTemplateError('INVALID_INPUT'); };
...
function validate(value: unknown): ListingDescriptionTemplateInput {
  if (!exactKeys(value, [
    'templateVersion', 'title', 'bodyHtml', 'conditionId', 'conditionNote', 'imageUrls', 'sku',
  ])) return invalid();
  if (value.templateVersion !== LISTING_DESCRIPTION_TEMPLATE_VERSION) return invalid();
  ...
  if (typeof title !== 'string' || title.length === 0 || title.length > MAX_TITLE_LENGTH
    || FORBIDDEN_CONTROL.test(title)) return invalid();
  if (typeof bodyHtml !== 'string' || bodyHtml.length > MAX_BODY_HTML_LENGTH
    || FORBIDDEN_CONTROL.test(bodyHtml) || !isAllowlistedListingHtml(bodyHtml)) return invalid();
  if (conditionId !== null
    && (typeof conditionId !== 'string' || !CONDITION_LABELS.has(conditionId))) return invalid();
  if (conditionNote !== null
    && (typeof conditionNote !== 'string' || conditionNote.length === 0
      || conditionNote.length > MAX_CONDITION_NOTE_LENGTH
      || FORBIDDEN_CONTROL.test(conditionNote))) return invalid();
  if (!Array.isArray(imageUrls) || imageUrls.length > MAX_IMAGE_URLS) return invalid();
  for (const entry of imageUrls) {
    if (typeof entry !== 'string' || entry.length > MAX_IMAGE_URL_LENGTH) return invalid();
    let url: URL;
  ...
  return Object.freeze({
    ...
    title,
    bodyHtml,
    conditionId: conditionId as string | null,
    conditionNote: conditionNote as string | null,
    imageUrls: Object.freeze([...imageUrls] as string[]),
    sku,
  });
}
 ...
 */
function sizedImageUrl(rawUrl: string, params: string): string {
  let host = '';
...
const PAYMENT_TEXT =
  '<p>Immediate payment is required upon selecting &#39;Buy It Now&#39; or upon checking out '
  + 'through the cart.</p>'
...
function conditionNoteHtml(note: string): string {
 ...
 */
function galleryHtml(imageUrls: readonly string[], title: string): string {
  ...
  const thumbs = imageUrls.map((url, index) =>
    `<a href="#ucg-img-${index + 1}">`
 ...
 */
export function renderListingDescription(input: unknown): string {
  const checked = validate(input);
  const title = escapeHtml(checked.title);
  const conditionLabel = checked.conditionId === null
    ? null
    : CONDITION_LABELS.get(checked.conditionId) as string;
