...
export type UnavailableMigrationStateProjection = {
  ...
  errorCode:
    ...
    | 'MIGRATION_STATE_CONFIG_INVALID'
  ...
  | {
      ...
      monitoring: {
        ...
        previousUtcDay: {
          ...
          windowEndUtc: string;
          writes: { performed: number; succeeded: number; failed: number; unresolved: number };
...
function unavailableProjection(
  status: UnavailableMigrationStateProjection['status'],
  errorCode?: UnavailableMigrationStateProjection['errorCode'],
): UnavailableMigrationStateProjection {
  const blocker = {
...
function normalizeVerifiedProjection(
  projection: MigrationStoreProjection,
): MigrationStateApiProjection {
  if (projection.status === 'unavailable') return unavailableProjection('unavailable');
  if (projection.status !== 'verified') return unavailableProjection('invalid');
  ...
  const ownershipValid =
    ownership.length === expectedResponsibilities.length &&
    ownership.every((entry, index) =>
      entry.responsibility === expectedResponsibilities[index] &&
  ...
  const countsValid = counts !== null && COUNT_KEYS.every(
    (key) => Number.isSafeInteger(counts[key]) && counts[key] >= 0,
  );
  const watermarkValid = orders.watermarkUtc === null || (
    typeof orders.watermarkUtc === 'string' &&
    isExactUtc(orders.watermarkUtc)
  );
  const blockersValid = readiness.blockers.every(
  ...
  const productionWatermarkValid =
...
export async function readConfiguredMigrationState(options: {
  ...
  let loaded: Awaited<ReturnType<ConfigLoader>>;
  try {
 ...
 */
export function findUnresolvedListingCreateFromArgv(
