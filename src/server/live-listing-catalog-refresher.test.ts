...
describe('live listing catalog background refresher', () => {
  it('starts one read-only refresh, schedules bounded refreshes, and stops cleanly', async () => {
    ...
    const scheduledCallback = scheduled as (() => void) | null;
    scheduledCallback?.();
  ...
  it('contains a failed refresh and rejects unsafe intervals', async () => {
    ...
    const setIntervalImpl = (
      vi.fn(() => ({ unref: vi.fn() }) as unknown as NodeJS.Timeout)
    ) as unknown as typeof setInterval;
    expect(() => startLiveListingCatalogRefresher(
      { refresh },
      { intervalMs: 1_000, setIntervalImpl },
    )).toThrow('Live listing catalog is unavailable');
