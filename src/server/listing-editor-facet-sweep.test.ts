...
import {
  createEditorFacetSweep,
  EDITOR_FACET_SWEEP_TESTING,
} from './listing-editor-facet-sweep.js';
...
function activeRow(rowId: string, listingId: string, lifecycleStatus = 'active'): unknown {
  return { id: rowId, lifecycleStatus, ebay: { sku: `SKU-${rowId}`, listingId } };
}

function workspaceDto(listingId: string, facets: Readonly<{
  ...
  } as unknown as ListingWorkspaceDto;
}

function facetObservation(listingId: string, overrides: Record<string, unknown> = {}): unknown {
  return {
    listingId,
    categoryId: null,
    categoryName: null,
...
describe('listing editor used-facet enrichment sweep', () => {
  ...
  it('sweeps only active rows bound to a listing, deduplicated, capped at 150', async () => {
    const rows = [
      ...Array.from({ length: 160 }, (_value, index) =>
        activeRow(`row-${index}`, `listing-${index}`)),
    ...
    const identities = EDITOR_FACET_SWEEP_TESTING.collectSweepIdentities(
      snapshotWith({ rows }),
    );
  ...
  it('keeps at most three detail reads in flight and reads at most 150 listings', async () => {
    ...
    const sweep = createEditorFacetSweep({
      getSnapshot: async () => snapshotWith({
        rows: Array.from({ length: 160 }, (_value, index) =>
          activeRow(`row-${index}`, `listing-${index}`)),
      }),
      readListingDetail: async (rowId) => {
        reads += 1;
  ...
  it('skips per-listing failures and unusable details without failing the sweep', async () => {
    const sweep = createEditorFacetSweep({
      ...
      readListingDetail: async (rowId) => {
        if (rowId === 'row-throws') throw new Error('Bearer secret detail failure');
        if (rowId === 'row-no-detail') return { ebayDetail: null } as unknown as ListingWorkspaceDto;
        if (rowId === 'row-no-facets') return workspaceDto('400');
  ...
  it('caches the aggregate for six hours, then refreshes in the background', async () => {
    ...
    const sweep = createEditorFacetSweep({
      ...
      readListingDetail: async () =>
        workspaceDto('100', { categoryId: '30088', categoryName }),
      now: () => epoch,
    });
  ...
  it('does not cache a failed snapshot read, so the next request retries', async () => {
    ...
    const sweep = createEditorFacetSweep({
      getSnapshot: async () => {
        snapshotReads += 1;
        if (snapshotReads === 1) throw new Error('snapshot unavailable');
        return snapshotWith({ rows: [activeRow('row-1', '100')] });
      },
