 ...
 */
const ALIGNMENT_EVENTS: readonly string[] = Object.freeze([
...
export function verifyEbayNotification(input: Readonly<{
  body: unknown;
  credentials: Readonly<{ devId: string; appId: string; certId: string }> | null;
  nowMs: number;
}>): EbayNotificationVerdict {
  const reject = (reason: EbayNotificationVerdict['reason']): EbayNotificationVerdict =>
    Object.freeze({
      outcome: 'rejected' as const, reason, eventName: null, itemId: null,
  ...
  if (input.credentials === null) return reject('credentials_unavailable');

  const timestamp = TIMESTAMP_PATTERN.exec(input.body)?.[1];
  if (!timestamp) return reject('timestamp_missing');
  ...
  if (!suppliedSignature) return reject('signature_missing');
