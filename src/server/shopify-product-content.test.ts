...
import {
  ...
  ShopifyProductContentError,
  SHOPIFY_PRODUCT_CONTENT_TESTING,
...
function mediaNodes(urls: readonly string[]): unknown[] {
  ...
  return {
    ...
    read: createShopifyProductContentReader({
      getAccessToken: async () => {
        if (overrides.tokenThrows) throw new Error('no token');
...
describe('shopify product content reader', () => {
  ...
  it('posts exactly one request to the pinned Shopify admin endpoint', async () => {
    ...
    expect(calls[0]!.url).toBe(
      SHOPIFY_PRODUCT_CONTENT_TESTING.SHOPIFY_ADMIN_ORIGIN
  ...
  it('never leaks the token through a failure', async () => {
    ...
    try {
      ...
      expect(String((error as Error).message)).not.toMatch(/shpat|token/i);
    }
  });
