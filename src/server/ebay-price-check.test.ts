...
import {
  createPriceCheck,
  derivePriceCheckCondition,
  derivePriceCheckQuery,
  EBAY_PRICE_CHECK_TESTING,
  medianPrice,
  PriceCheckError,
...
function browseBody(): Record<string, unknown> {
  const item = (title: string, value: string, condition = 'Used') => ({
...
async function failureCode(operation: Promise<unknown>): Promise<string> {
  try {
    ...
    return (error as PriceCheckError).code;
  }
