...
export function isInventoryTopic(rawTopic: string | undefined): boolean {
 ...
 */
function parseArgv(raw: string | undefined): readonly string[] | null {
...
export function configuredSweepArgv(
...
export function configuredFullSweepArgv(
  env: NodeJS.ProcessEnv = process.env,
): readonly string[] | null {
  return parseArgv(env.INVENTORY_FULL_SWEEP_ARGV);
}

export function configuredFullSweepIntervalMs(
  env: NodeJS.ProcessEnv = process.env,
): number {
...
export type SweepRunner = () => Promise<{ ok: boolean; summary: string }>;
 ...
 */
export function summarizeSweepStdout(stdout: string): string | null {
  try {
    ...
    const failures = (Array.isArray(parsed.results) ? parsed.results : [])
      .filter((entry) => typeof (entry as { code?: unknown })?.code === 'string')
      .slice(0, MAX_LOGGED_FAILURES)
      .map((entry) => {
        const { sku, code } = entry as { sku?: unknown; code: string };
 ...
 */
export function deniedCode(stderr: string): string {
  try {
    const line = stderr.split('\n').reverse().find((entry) => entry.startsWith('{'));
    if (!line) return stderr.trim().length > 0 ? 'no-code' : 'no-output';
...
export function createConfiguredRunner(argv: readonly string[]): SweepRunner {
  return () => new Promise((resolve) => {
    execFile(
      ...
      (error, stdout, stderr) => {
        ...
        const reason = error && (error as { killed?: boolean }).killed
          ? 'timed out'
 ...
 */
export function createInventorySweepTrigger(dependencies: Readonly<{
  ...
  const setTimer = dependencies.setTimer
    ?? ((callback: () => void, ms: number) => setTimeout(callback, ms).unref?.());
  ...
  function requestFast(): boolean {
    if (runSweep === null) return false;
    if (running) {
      pendingFast = true;
      return true;
    }
  ...
  function fullSweepDue(): boolean {
    if (runFullSweep === null) return false;
  ...
  function startFullSweepSchedule(): void {
    if (runFullSweep === null) return;
    setTicker(() => {
      if (!fullSweepDue()) return;
      pendingFull = true;
      if (!running) void drain();
    }, FULL_SWEEP_TICK_MS);
  }
