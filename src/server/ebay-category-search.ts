...
export type EbayCategorySearchFailureCode =
  (typeof EBAY_CATEGORY_SEARCH_FAILURE_CODES)[number];
...
function fail(code: EbayCategorySearchFailureCode): never {
 ...
 */
export function validateEbayCategoryQuery(value: unknown): string {
  if (typeof value !== 'string') return fail('INVALID_QUERY');
  const query = value.trim();
  if (query.length < MIN_QUERY_CHARACTERS
    || query.length > MAX_QUERY_CHARACTERS
    || /[\u0000-\u001F\u007F-\u009F\u2028\u2029]/u.test(query)) return fail('INVALID_QUERY');
  return query;
}
...
function safeCategoryId(value: unknown): string | null {
...
async function boundedTaxonomyGet(
  fetchImpl: FetchLike,
  path: string,
  accessToken: string,
  ...
  if (status !== 200) return fail('REMOTE_READ_FAILED');
  try {
 ...
 */
function toSuggestion(raw: unknown): EbayCategorySuggestion | null {
  ...
  if (ancestors === null || ancestors.length > MAX_ANCESTORS) return null;
  ...
  if (rawLeaf !== undefined && rawLeaf !== null && typeof rawLeaf !== 'boolean') return null;
...
function toSearchDto(body: UnknownRecord): EbayCategorySearchDto {
  ...
  for (const raw of suggestions) {
    if (categories.length >= MAX_SUGGESTIONS) break;
    const suggestion = toSuggestion(raw);
    if (suggestion !== null) categories.push(suggestion);
  }
...
export function createEbayCategorySearch(dependencies: Readonly<{
  ...
  let treeIdFlight: Promise<string> | null = null;
  /** Insertion-ordered LRU: re-inserted on hit, oldest evicted at capacity. */
  const queryCache = new Map<string, { dto: EbayCategorySearchDto; expiresAt: number }>();
  const inFlight = new Map<string, Promise<EbayCategorySearchDto>>();
  ...
  async function defaultTreeId(): Promise<string> {
    if (cachedTreeId !== null) return cachedTreeId;
    if (treeIdFlight) return treeIdFlight;
    treeIdFlight = (async () => {
  ...
  return async (rawQuery: unknown): Promise<EbayCategorySearchDto> => {
    ...
    const lookup = (async () => {
      ...
      const body = await boundedTaxonomyGet(
        fetchImpl,
