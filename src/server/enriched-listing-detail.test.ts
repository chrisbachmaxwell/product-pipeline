...
import {
  ...
  ENRICHED_LISTING_DETAIL_TESTING,
  EnrichedListingDetailError,
  parseInventoryOfferControl,
  parseTradingItemDetail,
...
function getUserXml(userId = 'usedcameragear'): string {
...
function getItemXml(overrides: Readonly<{
  itemId?: string;
  itemSku?: string;
  sellerId?: string;
  imageUrl?: string;
...
function legacyRequest(): EnrichedListingDetailRequest {
...
describe('enriched eBay listing detail reader', () => {
  ...
  it('captures Inventory item and exact offer control without replacing public Trading actuals', async () => {
    ...
    const fetchImpl = vi.fn(async (url: string | URL | Request, init?: RequestInit) => {
      calls.push({ url: String(url), init });
      if (String(url).endsWith(`/inventory_item/${sku}`)) return jsonResponse(inventoryItem());
      if (String(url).includes('/sell/inventory/v1/offer/')) return jsonResponse(offer());
      const callName = new Headers(init?.headers).get('X-EBAY-API-CALL-NAME');
      return callName === 'GetUser' ? xmlResponse(getUserXml()) : xmlResponse(getItemXml());
    }) as unknown as typeof fetch;
  ...
  it.each([
    ['SELLER_MISMATCH', () => parseTradingItemDetail(
      { Item: { ItemID: listingId, SKU: sku, Seller: { UserID: 'wrong-seller' } } },
      { sellerId: 'usedcameragear', listingId, sku },
    )],
    ['LISTING_MISMATCH', () => parseTradingItemDetail(
      { Item: { ItemID: '999', SKU: sku, Seller: { UserID: 'usedcameragear' } } },
      { sellerId: 'usedcameragear', listingId, sku },
    )],
    ['SKU_MISMATCH', () => parseTradingItemDetail(
      { Item: { ItemID: listingId, SKU: `${sku} `, Seller: { UserID: 'usedcameragear' } } },
      { sellerId: 'usedcameragear', listingId, sku },
    )],
    ['OFFER_MISMATCH', () => parseInventoryOfferControl(
      offer({ offerId: 'wrong-offer' }),
      { offerId, sku, listingId, marketplaceId: 'EBAY_US' },
    )],
    ['LISTING_MISMATCH', () => parseInventoryOfferControl(
      offer({ listing: { listingId: '999', listingStatus: 'ACTIVE' } }),
      { offerId, sku, listingId, marketplaceId: 'EBAY_US' },
    )],
  ] as const)('fails closed with %s on an exact binding adversary', (code, operation) => {
  ...
  ] as const)('rejects unsafe %s responses without reflecting upstream content', async (mode, body, code) => {
    const fetchImpl = vi.fn(async () => mode === 'redirect'
      ? new Response('', { status: 302, headers: { Location: 'https://evil.example/' } })
      : xmlResponse(body)) as unknown as typeof fetch;
  ...
  it('enforces declared and streamed response caps and rejects unsafe render URLs', async () => {
    ...
    const unsafeImageFetch = (async (_url: string | URL | Request, init?: RequestInit) => {
  ...
  it('rejects 500,001 description characters', async () => {
    const description = 'X'.repeat(
      ENRICHED_LISTING_DETAIL_TESTING.MAX_DESCRIPTION_CHARACTERS + 1,
    );
    const fetchImpl = (async (_url: string | URL | Request, init?: RequestInit) => {
      return new Headers(init?.headers).get('X-EBAY-API-CALL-NAME') === 'GetUser'
        ? xmlResponse(getUserXml())
  ...
  it('rejects a multibyte description above the separate UTF-8 safety cap', async () => {
    ...
    const description = '€'.repeat(
      Math.floor(ENRICHED_LISTING_DETAIL_TESTING.MAX_DESCRIPTION_UTF8_BYTES / bytesPerCharacter) + 1,
    );
  ...
  it('rejects prohibited control characters in a decoded eBay description', () => {
    expect(() => parseInventoryOfferControl(
      offer({ listingDescription: 'safe\u0001unsafe' }),
      { offerId, sku, listingId, marketplaceId: 'EBAY_US' },
