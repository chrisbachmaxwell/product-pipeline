...
const SCOPE = {
  ...
  ebayEnvironment: 'sandbox' as const,
...
const PRODUCTION_SCOPE = {
  shopifyStoreDomain: 'usedcameragear.myshopify.com',
  ebayEnvironment: 'production' as const,
  ebaySellerId: 'usedcameragear',
  ebayMarketplaceId: 'EBAY_US',
};

function createProductionDatabase(): string {
...
async function readProductionDatabase(databasePath: string) {
...
function populateResolvedG10(databasePath: string): void {
...
describe('request-time durable migration-state reader', () => {
  it('does not load a config or inspect a store when the explicit environment path is absent', async () => {
    ...
    const result = await readConfiguredMigrationState({
      ...
      inspectStore: inspectStore as never,
    });
  ...
  it('loads and inspects only at invocation, then returns one normalized inert projection', async () => {
    ...
    expect(JSON.stringify(result)).not.toMatch(
      /databaseAbsolutePath|configDigest|scopeDigest|must-not-escape|\.local\/migration-state/,
    );
  ...
  it('accepts configured fulfillment as the fourth Production Class-B responsibility', async () => {
    ...
    if (result.status !== 'verified') throw new Error('verified result required');
  ...
  it('deep-allowlists the projection so future internal or injected details cannot escape', async () => {
    ...
    expect(serialized).not.toMatch(
      /must-not-escape|accessToken|databasePath|ebaySellerId|internalSecrets|buyerEmail|customerPayload|rawSqliteError|internalApproval|scopeDigest|configDigest/i,
    );
  ...
  it('accepts a production watermark only with ProductPipeline single-writer orderImport ownership', async () => {
    const watermarked = (orderImportOwner: 'marketplace_connect' | 'product_pipeline') => ({
      ...VERIFIED,
      scope: { ...VERIFIED.scope!, ebayEnvironment: 'production' as const },
      counts: { ...VERIFIED.counts!, orderWatermarks: 1, ownershipVersions: 3 },
      ownership: VERIFIED.ownership.map((entry) =>
        entry.responsibility === 'orderImport'
          ? {
              ...entry,
              configured: true,
              version: 3,
    ...
    const denied = await readConfiguredMigrationState({
      environment: { MIGRATION_STATE_CONFIG_PATH: 'config/migration-state.json' },
      loadConfig: loadConfig as never,
      inspectStore: vi.fn(() => watermarked('marketplace_connect')) as never,
    });
    ...
    const accepted = await readConfiguredMigrationState({
      environment: { MIGRATION_STATE_CONFIG_PATH: 'config/migration-state.json' },
      loadConfig: loadConfig as never,
      inspectStore: vi.fn(() => watermarked('product_pipeline')) as never,
    });
  ...
  it('collapses config failures to one redacted non-authorizing response', async () => {
    ...
    const result = await readConfiguredMigrationState({
      environment: { MIGRATION_STATE_CONFIG_PATH: 'config/migration-state.json' },
      loadConfig: vi.fn(async () => { throw new Error(secret); }) as never,
      inspectStore: vi.fn(() => { throw new Error(secret); }) as never,
    });
  ...
  it('has no startup, writer, platform, credential, or default-store boundary', async () => {
    ...
    expect(reader).not.toMatch(
      /createMigrationStore|openMigrationStore(?:ReadOnly)?|migration-store\/(?:index|store)\.js|\bfetch\s*\(|token|credential/i,
    );
    ...
    const shadowRouter = await fs.readFile(
      path.join(sourceRoot, 'server/routes/shadow-api.ts'),
      'utf8',
    );
