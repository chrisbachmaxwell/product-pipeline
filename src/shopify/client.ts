...
class ShopifyClientCredentialsTokenError extends Error {
  readonly code = 'shopify-token-denied';
...
function denyClientCredentialsToken(): never {
...
async function readBoundedJson(response: Response): Promise<unknown> {
  ...
  if (response.body === null) return denyClientCredentialsToken();
...
export const requestShopifyClientCredentialsToken = async (
  dependencies: ShopifyClientCredentialsTokenDependencies = {},
): Promise<string> => {
  ...
  try {
    ...
    if (
      shopify.storeDomain !== PRODUCT_PIPELINE_SHOPIFY_IDENTITY.storeDomain
    ...
    const response = await (dependencies.fetchImpl ?? fetch)(
      `https://${shopify.storeDomain}/admin/oauth/access_token`,
      {
        method: 'POST',
        redirect: 'error',
    ...
    if (!response.ok || response.status !== 200) return denyClientCredentialsToken();
