...
- Focused G17/migration verification: 6 files / 57 tests passed and TypeScript no-emit passed. The hardened dispatch requires the existing durable Shopify/eBay order link and schema v4 admits only one fulfillment intent per exact linked order pair; outcome-unknown reconciliation binds the complete observed eBay effect rather than mutable Shopify tracking. Full-suite and production-build results are recorded in the PR after execution.
