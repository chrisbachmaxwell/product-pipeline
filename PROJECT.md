...
### 2026-08-14: Combined Shopify and eBay Credential Administrator (Integration Candidate)

- Replayed the reviewed eBay administrator onto Shopify credential-maintenance merge `579cc077a6ca4930fbfa88d415b80cc04c12d963` without combining either provider's configuration, authority, network, database-update, or recovery policy.
- Preserved the three option-free Shopify commands at the compiled root and moved the eBay parser behind the exact `credential-admin ebay ...` family. The shared dispatcher selects only that literal family token; rejected Shopify and eBay arguments retain their provider-specific fixed, value-free failure output without reflecting operator input.
- Preserved both direct compiled help families while removing the npm argument-forwarding wrapper: npm can echo appended raw arguments before either provider parser can reject them, so the only supported invocation is direct `node dist/credential-admin/index.js ...`. Added a sentinel regression proving the package exposes no credential-maintenance wrapper and an attempted missing-script invocation does not reflect appended input. Shopify implementation remains in its existing modules; eBay implementation is isolated in `ebay-program.ts` and `ebay-rotation.ts`. Neither family is imported by the mounted server or commerce writers.
- Added a mutual maintenance fence for the shared legacy ledger: Shopify rotation denies any present `EBAY_ROTATION_NEW_CERT_ID`, while eBay Railway commands deny any present Shopify rotation acknowledgement or temporary refresh-token variable. Local eBay consent preparation remains local-only and has no provider or database effect.
- Final local integration verification passed the focused 10-file / 135-test credential and inbound-security suite, the full 55-file / 625-test repository suite, TypeScript no-emit, Production build, direct compiled help, malformed-argument and absent-package-wrapper redaction, fresh-compile source/dist parity, whitespace, runtime-import/process-launch, and credential-pattern scans.
- This integration is source/build work only. It does not authorize either runbook and performed no provider, Railway, Production database, secret, deployment, Marketplace Connect, Lightspeed, or commerce mutation.
...
- This documentation authorizes no execution. Merge `579cc077a6ca4930fbfa88d415b80cc04c12d963` does not satisfy deployment or live-verification gates; a direct deployment still requires a fresh canonical cutoff no more than one hour ahead and every exact release, authentication, webhook, topology, quarantine, and dispatch-window proof in the incident runbook.
...
- Local verification passed the focused 8-file / 88-test security suite, full 54-file / 582-test suite, TypeScript no-emit check, Production build, compiled command help and malformed-argument redaction checks, and whitespace checks. Source commit `3309dfd` plus incident-documentation commit `8187878` merged through PR #15 as `579cc077a6ca4930fbfa88d415b80cc04c12d963`. That merge is source identity only; deployment and live verification remain separate. The source work performed no provider, Railway, Production database, Marketplace Connect, or Lightspeed mutation.

### 2026-08-14: Fixed-Purpose eBay Production Credential Administrator (Source Candidate)

- Added an isolated `credential-admin ebay` entrypoint for one reviewed Production reset/re-consent window. It is not imported by the mounted server, legacy CLI, scheduler, webhook handlers, catalog readers, or commerce writers and has no route or process/shell launcher.
- Split consent across a local private raw-state artifact and digest-only Railway registration. The completed authorization result is accepted only through no-echo stdin; command output is fixed and value-free.
- Pinned the exact Railway project/environment/service and `/data` database/work/backup paths. Remote commands accept no path argument, require private mode/link boundaries, and deny whenever `LISTING_CONTROL_SINGLE_WRITER_ACK` is present.
- Pinned Production seller `usedcameragear`, Trading Site ID `0`, and exactly the base plus `sell.inventory` scopes. Grant installation uses only the dedicated new Cert, introspects access and refresh tokens, performs read-only Trading/Inventory proof, rejects Commerce Identity and any missing/extra scope, and auto-revokes an unused invalid grant.
- Added exact `auth_tokens` DDL/index/trigger preflight, a verified private backup before code exchange, a full baseline, and one `BEGIN IMMEDIATE` compare-and-swap update/insert of only the eBay row without delete-first. Unrelated rows, integrity, and independent read-back must remain valid.
- Added credential-free access- and refresh-token digests before the ledger CAS. The digest-bearing commit intent is file- and parent-directory-synced before CAS; stored-token verification and exact-bound new-grant revocation require the update timestamp, scope, and both token digests, so a token swap with unchanged metadata fails before provider access. An exact commit-pending row can be durably reconciled after a crash immediately following COMMIT.
- Added terminal no-replay handling for ambiguous exchange plus an explicitly confirmed provider-reconciliation reset. Reset archives the complete old private evidence without deletion and creates a fresh private pending state; a COMMIT-error state is eligible only after provider reconciliation and exact proof that the fixed ledger matches its named private pre-install backup. It cannot erase or reset a committed database effect.
- Added truthful COMMIT-error classification after the SQLite connection closes. Exact installed, exact baseline, and unreadable/mismatched outcomes report respectively one, zero, or unknown database-row effect; all preserve digest-bound commit evidence, report the provider mutation, require exit-`5` reconciliation, and never automatically revoke or replay. Exact-bound `verify` reconciles committed state.
- Added truthful post-commit state failure handling and owner/time/expiry/live-PID stale-lock recovery that archives rather than deletes the lock. Evidence and lock renames durably sync the target directory before the source. Streamed response limits, hostile XML rejection, and adversarial regressions remain enforced. See `docs/EBAY_CREDENTIAL_ROTATION.md` and incident `ECM-2026-08-14-001` in `docs/CREDENTIAL_MAINTENANCE_INCIDENTS.md`.
- Original-branch local verification passed TypeScript, the production build, direct compiled CLI help, whitespace checks, and the complete 49-file / 543-test suite. The later combined integration removes the npm wrapper after review proved npm itself can echo forwarded arguments before CLI redaction. These checks use fake provider transports and temporary databases only.
- The original candidate was committed and pushed as `665e9ca` on PR #16 from base `234e0cb`; its conflict-resolution integration onto current `origin/main` remains uncommitted and unpushed pending independent review. No provider request, Railway change, live database access, secret creation/storage, deployment, credential rotation, or commerce write occurred; human provider closure and a separately authorized maintenance window remain outstanding.
