...
Live evidence, zero external writes: the Production catalog row for variant `55519196250403` reports `activeMatchCount: 0`, `inventoryItemCount: 1`, `offerCount: 1`, `unpublishedArtifactCount: 2`, `offerId: null`, and a zero-write `reconcile` returns `offer_unpublished` / `CREATE_OFFER_UNPUBLISHED` with `offerId: null`. The residue is still present; only its id is unobservable through the capture. The unit tests missed this because `listing-create-recovery.test.ts` builds `offerPendingWorkspace` with `listingId: null`, `activeMatchCount: 0` and a populated `offerId` — a DTO the real capture cannot emit.

Proposed fix, not implemented pending review because it relaxes a safety guard on a destructive ceremony: require the live capture to prove residue presence only (`kind === 'artifact'`, unpublished artifacts present, exact SKU bound), and take the offer id from the store's recorded `CREATE_OFFER_UNPUBLISHED` reconciliation evidence as the runbook already describes, with the adapter's bounded `GET offer` independently confirming UNPUBLISHED and exact SKU before any DELETE. G16 cannot close and Phase 1 cannot exit until this lands.

### 2026-08-27: Phase 1 Executed From a Local Terminal — Steps 1, 2, 5, 6 Done; Recovery Denies on Changed Residue (L39)

Teleported the cloud session into a local terminal (L38 path 1) and executed Phase 1 against Production. Transport confirmed: port 22 reachable, SSH negotiating, `npx -y @railway/cli@latest` giving v5.45.3 with no sudo and no global install; a dedicated ed25519 key was registered rather than reusing an unrelated vendor key. **Completed:** off-volume backup (remote/local sha256 identical `7faec83e...3160`, `integrity_check ok`, and no `-wal`/`-shm` on the volume so the copy was consistent); schema v4 to v5 upgrade under the exact scope digest, returning `fromVersion 4 / toVersion 5` with the post-upgrade `verify` reporting `verified`, schemaVersion 5 and a valid audit chain over 42 records; G10 listing `147232036779` confirmed publicly at `US $164.95` / `5 available` so Marketplace Connect price/quantity sync still behaves; and the first fully clean post-fix `shadow-poll` into a fresh mode-0700 `/data/shadow-reports/` — 7 observed, 7 matched by `source_identifier`, zero unmatched, blocked, ambiguous, and zero external writes, which is clean day 1 of the required 7 to 14.

**Recovery ceremony ran once and denied safely with `RECOVER_RESIDUE_STATE_MISMATCH`, performing zero provider calls.** The denial is a state finding: it fires after the exact-target gate, so the catalog id `shopify-variant:gid://shopify/ProductVariant/55519196250403` and SKU `PIPELINE-TEST-20260826` are proven correct, and it fails where a fresh capture must still show the unpublished artifact bound to offer `247267392011`. It no longer does, and the listing-control store holds no eBay artifact binding for that offer. The next action is the zero-write `reconcile` to establish current truth; `--accept-absent` must not be used to force closure. The L34 intentKey gap is closed by method — a verified off-volume backup is a queryable read-only replica, so `intent_attempts` yields the intent key, `external_identities` the raw SKU, and the catalog row id is mechanically `shopify-variant:<variant gid>`. Also corrected: the pre-upgrade `verify` prints the generic `Migration-state operation denied`, not `SCHEMA_MISMATCH` as `docs/MIGRATION_ADMIN.md` implies, because `safeError` masks non-config errors; confirm the cause by comparing deployed `CURRENT_SCHEMA_VERSION` against the store `user_version` before upgrading.

### 2026-08-27: Correction — "Full" Network Access Does Not Unblock Railway SSH (L38)

Corrected the resolution path recorded in L36/L37. Both said Phase 1 could be unblocked by running the agent session in an environment whose network policy allows all outbound traffic. That is wrong for Anthropic-hosted cloud environments, and this session was already running under exactly that setting. Cloud environment network access levels (`None`, `Trusted`, `Full`, `Custom`) are a **domain** dimension only; all Anthropic-hosted outbound traffic passes through an HTTP/HTTPS security proxy, and that proxy carries no WebSocket upgrades, no non-443 ports, and no raw TCP. `Full` widens which hosts are reachable, never which protocols or ports, so raw SSH on port 22 cannot traverse it at any level. Confirmed empirically: the `Default` environment reached `example.com`, `www.wikipedia.org`, `ebay.com`, `railway.com`, and `1.1.1.1:443` — none on the Trusted allowlist, proving it is already `Full` — while `ssh.railway.com:22` still timed out. The viable paths are now running Claude Code locally in a terminal (its own network and files; `claude --teleport <session-id>` carries this session's history across), a self-hosted environment whose egress leaves through the operator's own network boundary, or the prepared Railway Console blocks. `docs/PHASE1_CONSOLE_BLOCKS.md` carries a banner pointing at L38 so no future session repeats the "just switch it to Full" suggestion.

### 2026-08-27: Phase 1 Execution Attempt — SSH Transport Re-Confirmed Blocked; Operator Console Blocks Prepared

A delegated agent session attempted hands-off execution of Phase 1 (backup -> schema v4->v5 upgrade -> `recover-create` for offer `247267392011` -> fresh create proof -> G10 MC-sync confirmation -> shadow-poll). No step ran and no provider write occurred: Railway shell access is unreachable from the remote agent environment, re-confirming Brain L36. Newly pinned mechanics are recorded as L37 — Railway CLI 5.45.3 installs and authenticates normally (browserless login and project link both succeeded over HTTPS), but `railway ssh` is not a WebSocket tunnel; it shells out to OpenSSH against `ssh.railway.com` port 22. DNS resolves and port 443 to that host connects, while port 22 to it (and to any host) times out, so a raw `ssh -v` fails at `connect ... port 22: Connection timed out` before authentication — registering an SSH key would not help. The agent proxy README independently confirms WebSocket upgrades, non-443 ports, and raw TCP are unsupported and must be reported rather than worked around; the L36 deploy-transport channel was not re-attempted. Delivered instead: `docs/PHASE1_CONSOLE_BLOCKS.md`, paste-ready in-order Railway Console blocks for all six steps with the exact v4->v5 scope digest, the bound G16 recovery identifiers, target-selection and failure-branch guidance, and a corrected canonical store path (`docs/ORDER_IMPORT.md` line 61 carries a stale example path). One value only in-container access can supply: the Draft 5 `intentKey` required by `recover-create` was never recorded in the repo and must come from the saved dispatch output or the store's intent row; future dispatch runs must record the printed intentKey here at the time of the run.

### 2026-08-27: G16 Production Recovery-Cleanup Ceremony (schema v5) and Pre-Publish Validation

Built the separately approved Production recovery-cleanup ceremony for the unpublished Draft 5 residue. Migration-store schema v5 admits exactly one new production action, `recover_create_ebay_listing`, structurally bound to an outstanding unresolved create job on the identical inventory-SKU target, and adds the truthful terminal vocabulary `resolved_residue_removed` / `effect_residue_removed` (listingCreate only, paired exclusively, reachable only from `reconciliation_required`); every other denial is recreated with identical predicates and the immutable v1 SQL checksums are untouched. The standalone `listing-lifecycle-admin recover-create` command binds the exact source job, attempt, intent key, approval evidence digest (L29), SKU, and the exact offer id re-derived from the recorded `CREATE_OFFER_UNPUBLISHED` reconciliation evidence; its bounded adapter can reach only GET/DELETE offer and GET/DELETE inventory-item on api.ebay.com (publish/create are structurally impossible), refuses a published offer, verifies each deletion gone, and resolves the recovery job and the original job as removed residue — never a fabricated `confirmed_missing`. A zero-write `recover-reconcile` completes a delayed capture, and an explicitly chained retry (`--prior-recovery-job-id`) is the only path past a failed delete. The create path now also prevalidates eBay's documented publish prerequisites locally with fixed `CREATE_PREVALIDATION_*` codes (category shape, condition mapping, policy ids, merchant location key, aspects, GTC, split description bounds) before any provider write. Nothing dispatched: the Production store remains v4 until the operator backs up and runs `migration-admin upgrade`, and the incident job/attempt remain unresolved until the operator executes the reviewed ceremony.
...
- Hardened the lane after review: the exact action digest is now required from approval through recovery; an expired unused approval can be replaced on the same intent without clock rollback, while any attempted intent remains terminally one-shot. Create reconciliation exact-compares all approved Inventory/Offer fields and Trading ItemID/SKU/content/commerce/status, and drift remains unresolved. Cleanup requires same-store reconciled-create provenance and proves the exact Trading listing ended while Inventory artifacts are absent. Manifest reads use one owner-bound, mode-0600, single-link, no-follow descriptor; offer payloads include `GTC`; preflight verifies the enabled Sandbox location and exact business policies. No provider write or deployment occurred.
- Added unknown-outcome recovery without create replay: `recover-create` discovers response-lost offer/listing IDs and resolves only an exact complete effect; fixed partial stages flow through a new separately preflighted/approved recovery-cleanup ceremony bound to the original source job/attempt/intent. Partial cleanup uses the same chained recovery instead of repeating a consumed dispatch. Manifest mode is now exactly `0600`, offer reads bind `FIXED_PRICE` and `GTC`, and preflight truthfully reports `prerequisites-partial` because category-condition compatibility is final-provider validated. No provider write or deployment occurred.
- Bound every reconciliation to the reserved job's exact immutable `approvalEvidenceDigest` before provider evidence can be appended. Recovery-cleanup now requires and digest-binds the source job's approval evidence, so a wrong action, cleanup, or recovery digest cannot cross an otherwise matching job/attempt/intent/target seam. Added adversarial wrong-digest replay coverage and reformatted the recovery commands to the established readable TypeScript style. No provider write or deployment occurred.
- Rebuilt the complete tracked distribution from a clean `npm ci` install after the final rebase. Two consecutive builds produced the identical declaration and web-asset diff, so the source-consistent TypeScript union ordering and hashed frontend bundles replace the stale pre-rebase artifacts.
