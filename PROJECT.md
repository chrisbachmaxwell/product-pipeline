...
- Local remediation verification passes the complete 5-file / 98-test credential-admin matrix, the full 55-file / 623-test repository suite, TypeScript no-emit, the Production build, compiled direct-entrypoint help/malformed-argv redaction, absent npm-wrapper sentinel, and whitespace checks. Independent review confirmed those gates plus source/dist parity. These remain local source/build/review proofs only; commit, push, PR, merge, deployment, and one fixed Production diagnostic run remain pending.
- Production maintenance remains stopped: deployed merge `579cc077a6ca4930fbfa88d415b80cc04c12d963` returned only fixed `database-denied` from direct preflight on temporary-ACK deployment `77c18d72-e757-41be-8692-284d77f2490c`, before any provider request. The ACK variables were removed; deployment `d44e6238-0072-4c8c-bbd3-df6929f6164d` is Active with one replica and served healthy shadow-read-only policy at `2026-08-14T20:52:35.738Z`. The diagnostic is a local source candidate only: it was not committed, pushed, deployed, or run against Production, and it authorizes no database repair or token rotation.
