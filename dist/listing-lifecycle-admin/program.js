...
import { deriveListingCreateRecoveryManifest, deriveOrphanedArtifactRecoveryManifest, ListingCreateRecoveryError, requireRecordedUnpublishedOffer, } from './recovery.js';
...
export function buildListingLifecycleAdminProgram(dependencies = {}) {
    ...
    const verifyOrphanedSourceBindings = (input) => {
        ...
        if (!job || job.responsibility !== 'listingCreate')
            deny('RECOVER_STATE_MISMATCH');
        const boundJob = job;
        if (boundJob.state !== 'resolved_existing')
    ...
    withRecoverBindingOptions(program
        ...
        .description('One-action exact-target cleanup for ONE resolved create job whose published listing '
        ...
        .requiredOption('--superseded-by-listing-id <id>', 'Exact LIVE eBay listing id that superseded the create (verified against the capture)')
        .action(async (options) => {
    ...
    withRecoverBindingOptions(program
        ...
        .description('Zero-provider-write re-verification for an outstanding recover-orphaned-artifact '
        ...
        .requiredOption('--superseded-by-listing-id <id>', 'Exact live listing id bound into the recovery manifest by recover-orphaned-artifact')
        .requiredOption('--recovery-job-id <id>', 'Exact recovery job id printed by recover-orphaned-artifact')
        .requiredOption('--recovery-attempt-id <id>', 'Exact recovery attempt id printed by recover-orphaned-artifact')
        .action(async (options) => {
