...
function listedWorkspace(descriptionHtml) {
...
function orphanedWorkspace(liveListingId = NEW_LISTING_ID) {
    const base = baseWorkspace();
    return {
        ...base,
        catalog: {
...
function orphanCleanWorkspace(liveListingId = NEW_LISTING_ID) {
    const base = baseWorkspace();
    return {
        ...base,
        catalog: {
...
async function createWorld() {
    ...
    let offer = {
        ...
        status: 'PUBLISHED', listingId: OLD_LISTING_ID,
    };
...
async function dispatchResolvedCreate(world) {
...
async function recordOrphanEvidence(world, source) {
...
function orphanArguments(world, source, overrides = {}) {
    return [
        '--migration-store', world.migrationDatabasePath,
        '--confirm-scope', deriveScopeKey(MIGRATION_SCOPE),
...
function attemptResolution(world, jobId, attemptId) {
...
describe('listing-lifecycle operator CLI — recover-orphaned-artifact', () => {
    it('removes the orphaned pair end to end and never touches the source truth', async () => {
        ...
        expect(attemptResolution(world, recovered.recoveryJobId, recovered.recoveryAttemptId)).toBe('resolved_residue_removed');
    });
    it('refuses an UNRESOLVED source create — that is recover-create’s job', async () => {
        ...
        const failed = {
            ...
            attemptId: dispatched.attemptId,
            intentKey: dispatched.intentKey,
            manifestDigest: dispatched.manifestDigest,
        };
        world.setWorkspace(orphanedWorkspace());
    ...
    it('leaves a delayed capture unresolved and closes later via recover-orphaned-reconcile', async () => {
        const world = await createWorld();
        const source = await dispatchResolvedCreate(world);
        await recordOrphanEvidence(world, source);
        ...
        await world.run(['recover-orphaned-reconcile', ...orphanArguments(world, source),
            '--recovery-job-id', recovered.recoveryJobId,
            '--recovery-attempt-id', recovered.recoveryAttemptId,
        ]);
        expect(lastJson(world.stdout)).toMatchObject({
