...
import { ListingLifecycleManifestError, MAX_INVENTORY_PRODUCT_DESCRIPTION_LENGTH, MAX_OFFER_LISTING_DESCRIPTION_LENGTH, prevalidateListingCreateManifest, } from '../manifest.js';
...
function withProposed(overrides) {
    const manifest = validManifest();
    return { ...manifest, proposed: { ...manifest.proposed, ...overrides } };
}
function expectDenied(manifest, code, field) {
    try {
        ...
        if (field !== undefined)
            expect(denial.field).toBe(field);
        return;
    }
...
describe('create pre-publish validation matrix', () => {
    ...
    it('requires a numeric-leaf-shaped category id', () => {
        for (const categoryId of ['', 'abc', '0', '033', '3323x', '3323 ', '1'.repeat(11)]) {
            expectDenied(withProposed({ categoryId }), 'CREATE_PREVALIDATION_CATEGORY_ID', 'category');
        }
    ...
    it('requires the condition id to map through the fixed table to the exact enum', () => {
        expectDenied(withProposed({ conditionId: '2010' }), 'CREATE_PREVALIDATION_CONDITION', 'condition');
        expectDenied(withProposed({ conditionEnum: 'USED_GOOD' }), 'CREATE_PREVALIDATION_CONDITION', 'condition');
    });
    it('requires the three policy ids to be shape-valid, naming the family member', () => {
        expectDenied(withProposed({ fulfillmentPolicyId: 'policy-1' }), 'CREATE_PREVALIDATION_POLICY_IDS', 'fulfillment_policy');
        expectDenied(withProposed({ paymentPolicyId: '' }), 'CREATE_PREVALIDATION_POLICY_IDS', 'payment_policy');
        expectDenied(withProposed({ returnPolicyId: '0333' }), 'CREATE_PREVALIDATION_POLICY_IDS', 'return_policy');
    });
    it('requires a shape-valid merchant location key', () => {
        for (const merchantLocationKey of ['', 'ware house', 'x'.repeat(37), 'käse']) {
            expectDenied(withProposed({ merchantLocationKey }), 'CREATE_PREVALIDATION_MERCHANT_LOCATION', 'merchant_location');
        }
    });
    it('requires non-empty product aspects with non-empty values', () => {
        expectDenied(withProposed({ aspects: {} }), 'CREATE_PREVALIDATION_ASPECTS', 'item_specifics');
        expectDenied(withProposed({ aspects: { Brand: [] } }), 'CREATE_PREVALIDATION_ASPECTS', 'item_specifics');
    });
    it('requires the fixed GTC listing duration', () => {
        expectDenied(withProposed({ listingDuration: 'DAYS_7' }), 'CREATE_PREVALIDATION_LISTING_DURATION');
    });
    it('enforces the split description bounds per L30', () => {
        expectDenied(withProposed({ inventoryProductDescription: '' }), 'CREATE_PREVALIDATION_INVENTORY_DESCRIPTION', 'description');
        expectDenied(withProposed({
            inventoryProductDescription: 'x'.repeat(MAX_INVENTORY_PRODUCT_DESCRIPTION_LENGTH + 1),
        }), 'CREATE_PREVALIDATION_INVENTORY_DESCRIPTION', 'description');
        expectDenied(withProposed({ description: '' }), 'CREATE_PREVALIDATION_LISTING_DESCRIPTION', 'description');
        expectDenied(withProposed({ description: 'x'.repeat(MAX_OFFER_LISTING_DESCRIPTION_LENGTH + 1) }), 'CREATE_PREVALIDATION_LISTING_DESCRIPTION', 'description');
