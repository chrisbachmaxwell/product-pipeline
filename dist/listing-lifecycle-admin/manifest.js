 ...
 */
export const CREATE_CONDITION_ENUM_BY_ID = Object.freeze({
    '1000': 'NEW',
    '1500': 'NEW_OTHER',
...
function proposedValue(revision, field) {
    return revisionField(revision, field)?.proposedValue ?? null;
}
function requireProposed(revision, field) {
    const value = proposedValue(revision, field);
    if (value === null || value.length === 0)
        deny('CREATE_REQUIRED_FIELD_MISSING', field);
    return value;
}
...
function parseQuantity(serialized) {
    if (!/^[0-9]{1,9}$/.test(serialized))
 ...
 * already-listed.
 */
export function deriveListingCreateManifest(revision) {
...
export function deriveListingEndManifest(input) {
    if (input.reason !== END_SUPPORTED_REASON)
        deny('END_REASON_UNSUPPORTED');
...
function outcome(workspace, kind) {
    return Object.freeze({
        kind,
...
function captureBound(workspace, sku) {
    const row = workspace.catalog;
...
export function classifyCreateOutcome(input) {
    ...
    if (!captureBound(workspace, input.sku))
...
export function classifyEndOutcome(input) {
    ...
    if (!captureBound(workspace, input.sku))
        return outcome(workspace, 'unverified');
    const row = workspace.catalog;
    if (row.lifecycleStatus === 'unknown')
        return outcome(workspace, 'unverified');
    if (row.lifecycleStatus === 'active') {
