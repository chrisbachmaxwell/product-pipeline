...
import { assertEphemeralReadAuthorizedForTransport, validateEphemeralReadToken, } from '../token.js';
import { EBAY_SCOPE, FIXTURE_SECRET, NOW_UTC, SHOPIFY_SCOPE, ebayPolicy, shopifyPolicy, tokenFor, } from './fixtures.js';
...
function rawShopifyToken(overrides = {}) {
    return {
        provider: 'shopify',
...
describe('ephemeral no-refresh read authority', () => {
    ...
    it('re-checks provider, exact scope, and expiry at the transport seam', () => {
        ...
        expectCode(() => assertEphemeralReadAuthorizedForTransport(token, 'ebay', [EBAY_SCOPE], NOW_UTC), 'token-denied');
        expectCode(() => assertEphemeralReadAuthorizedForTransport(token, 'shopify', ['read_orders'], NOW_UTC), 'token-scope-denied');
        expectCode(() => assertEphemeralReadAuthorizedForTransport(token, 'shopify', [SHOPIFY_SCOPE], '2026-08-11T18:40:00.000Z'), 'token-near-expiry');
    });
