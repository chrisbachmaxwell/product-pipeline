...
describe('listing control store', () => {
    ...
    it('serializes simultaneous processes so exactly one consumes the same base', async () => {
        ...
        const children = inputs.map((input) => spawn(process.execPath, ['--import', 'tsx', runner, databasePath, JSON.stringify(scope), JSON.stringify(input)], { stdio: ['pipe', 'pipe', 'pipe'] }));
        const outputs = children.map((child) => new Promise((resolve, reject) => {
            ...
            child.once('close', (code) => {
                if (code === 0)
                    ...
                    reject(new Error(`Concurrent writer exited ${code}: ${errors}`));
            });
        }));
        ...
        const results = (await Promise.all(outputs)).map((output) => {
            ...
            if (!line)
                throw new Error(`Missing writer result: ${output}`);
    ...
    it('blocks direct mutation and detects content tampering even after the trigger is restored', () => {
        ...
        expect(() => raw.prepare("UPDATE listing_revision_fields SET proposed_value = 'tampered' WHERE revision_id = 'revision-1' AND field_name = 'title'").run()).toThrow(/append-only/);
        expect(() => raw.prepare(`INSERT OR REPLACE INTO listing_revision_fields
       SELECT * FROM listing_revision_fields WHERE revision_id = 'revision-1' AND field_name = 'title'`).run()).toThrow(/replay or replacement/);
        const trigger = raw.prepare("SELECT sql FROM sqlite_schema WHERE type = 'trigger' AND name = 'listing_revision_fields_deny_update'").get();
        raw.exec('DROP TRIGGER listing_revision_fields_deny_update');
        raw.prepare(`UPDATE listing_revision_fields
       SET source_value = 'tampered', proposed_value = 'tampered'
       WHERE revision_id = 'revision-1' AND field_name = 'title'`).run();
    ...
    it('rejects over-limit and control-bearing descriptions without widening other large fields', () => {
        const { store } = initialized();
        const descriptionField = (value) => fields({
            description: {
                observedValue: value,
    ...
    it('detects orphan subjects and timestamp-pair tampering', () => {
        ...
        expect(() => raw.prepare(`INSERT INTO listing_subjects (
         subject_key, scope_key, shopify_product_gid, shopify_variant_gid,
         created_at_utc, created_epoch_ms
       ) SELECT ?, scope_key, ?, ?, created_at_utc, created_epoch_ms FROM control_scope`).run(orphanKey, 'gid://shopify/Product/9998', 'gid://shopify/ProductVariant/9999')).not.toThrow();
        raw.close();
        expectCode(() => openListingControlStore({ databasePath, expectedScope: scope }), 'SCHEMA_MISMATCH');
    });
    it('detects timestamp epoch tampering even with the canonical trigger restored', () => {
        ...
        store.close();
        const raw = new Database(databasePath);
        const trigger = raw.prepare("SELECT sql FROM sqlite_schema WHERE type = 'trigger' AND name = 'listing_revisions_deny_update'").get();
        raw.exec('DROP TRIGGER listing_revisions_deny_update');
        raw.pragma('ignore_check_constraints = ON');
        raw.prepare("UPDATE listing_revisions SET created_epoch_ms = created_epoch_ms + 1 WHERE revision_id = 'revision-1'").run();
