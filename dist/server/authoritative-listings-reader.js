...
import { PRODUCTION_CAN3570_LISTING_EVIDENCE, PRODUCTION_CAN3570_LISTING_EVIDENCE_EXPECTED_DIGEST, } from './evidence/production-can3570-authoritative-listing.v1.js';
...
export function digestAuthoritativeListingEvidence(value) {
...
function hasForbiddenMaterial(value, key = '') {
    if (CREDENTIAL_KEY_PATTERN.test(key))
        return true;
    if (typeof value === 'string')
        return CREDENTIAL_VALUE_PATTERN.test(value);
    if (Array.isArray(value))
        return value.some((entry) => hasForbiddenMaterial(entry));
    if (!isRecord(value))
        return false;
 ...
 */
export function verifyAuthoritativeListingEvidence(evidence) {
...
export function readAuthoritativeListingsPage(input) {
