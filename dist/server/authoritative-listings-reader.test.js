...
import { PRODUCTION_CAN3570_LISTING_EVIDENCE, PRODUCTION_CAN3570_LISTING_EVIDENCE_BODY, PRODUCTION_CAN3570_LISTING_EVIDENCE_EXPECTED_DIGEST, } from './evidence/production-can3570-authoritative-listing.v1.js';
import { digestAuthoritativeListingEvidence, readAuthoritativeListingsPage, verifyAuthoritativeListingEvidence, } from './authoritative-listings-reader.js';
...
describe('authoritative listing evidence projection', () => {
    ...
    it('supports all four lifecycle filters without inventing rows', () => {
        ...
        for (const status of ['attention', 'ready', 'ended']) {
    ...
    it('pins the asset digest and fails closed on every material tamper', () => {
        ...
        expect(() => verifyAuthoritativeListingEvidence(PRODUCTION_CAN3570_LISTING_EVIDENCE))
            .not.toThrow();
        for (const mutate of [
            (value) => { value.listing.ebay.listingId = 'different'; },
            (value) => { value.listing.ebay.offerId = 'different'; },
            (value) => { value.source.sourceAuditHead = `sha256:${'0'.repeat(64)}`; },
            (value) => { value.source.publishedEffectProofDigest = `sha256:${'0'.repeat(64)}`; },
            (value) => { value.listing.audit.unresolvedCount = 1; },
            (value) => { value.accessToken = 'Bearer must-not-escape'; },
        ]) {
    ...
    it('contains no credential, seller, customer, description, or policy material', () => {
        ...
        expect(JSON.stringify(projectedEvidence)).not.toMatch(/access.?token|refresh.?token|authorization|sellerUser|buyerUsername|customerEmail|shipping.?address|listingDescription|policyId|merchantLocation|password|cookie|credential/i);
