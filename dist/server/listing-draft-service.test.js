...
function body(base, title = 'Operator Title') {
    return {
...
describe('local listing draft service', () => {
    ...
    it('normalizes eBay HTML safely without truncating semantic suffixes', () => {
        expect(LISTING_DRAFT_SERVICE_TESTING.htmlToPlainText('<style>bad</style><script>alert(1)</script><p>Safe &amp; clean</p>')).toBe('Safe & clean');
    ...
    it('saves one immutable local override, rejects replay/no-op, and ignores timestamp-only refresh', async () => {
        ...
        const second = { ...first, evidence: { ...first.evidence,
            ...
            ebayDetail: first.ebayDetail && { ...first.ebayDetail,
                evidence: { ...first.ebayDetail.evidence, observedAtUtc: '2026-08-13T22:01:01.000Z' } } };
        let current = first;
    ...
    it('keeps admin/config and the local route free of commerce writers and provider credentials', () => {
        ...
        expect(`${service}\n${route}`).not.toMatch(/from ['"][^'"]*(?:shopify|ebay|sync|inventory|order|token-manager)[^'"]*['"]|\b(?:createOffer|updateOffer|publishOffer|createShopifyOrder)\b/);
    });
});
