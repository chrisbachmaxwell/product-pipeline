...
export function cacheOperationalMonitoringSnapshot(projection, cachedAtEpochMs) {
    if (!Number.isFinite(cachedAtEpochMs))
        return;
    cachedHealth = { projection, cachedAtEpochMs };
}
/** Cheap public-health projection. It never opens a database or filesystem report. */
export function getCachedOperationalHealth(nowEpochMs = Date.now()) {
...
function unavailableShadow(status) {
    return Object.freeze({
        status,
        arrivedAtUtc: null,
        observedCount: 0,
...
export function readLatestShadowSummary(input) {
    ...
    try {
        ...
        for (const entry of entries) {
            if (!/^[a-zA-Z0-9][a-zA-Z0-9._-]{0,127}\.json$/.test(entry))
                continue;
        ...
        if (![observedCount, matchedCount, unmatchedCount, blockedCount,
            ...
            || matchedCount + unmatchedCount !== observedCount
            || lookupFailedCount + ambiguousCount !== blockedCount
            || blockedCount > unmatchedCount) {
            return unavailableShadow('unavailable');
        }
        ...
        const status = stale
            ? 'stale'
            : unmatchedCount === 0 && blockedCount === 0
                ? 'clean'
                : 'attention';
        return Object.freeze({
            status,
