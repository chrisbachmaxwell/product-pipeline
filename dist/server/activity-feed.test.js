...
const at = (seconds) => new Date(BASE_EPOCH + seconds * 1000).toISOString();
const digest = (label) => sha256Digest(`activity-fixture:${label}`);
...
function buildFixtureStore() {
    ...
    const ownershipChain = (responsibility, firstSecond) => {
    ...
    const finishOwnershipChain = (responsibility, firstSecond) => {
        store.recordOwnershipVersion({
    ...
    const runResolvedCeremony = (input) => {
...
function feedOptions(databasePath, overrides = {}) {
    return {
...
describe('activity feed', () => {
    ...
    it('degrades to an unavailable empty feed without a configured store', async () => {
        ...
        const configRejected = await readActivityFeed({
            environment: { MIGRATION_STATE_CONFIG_PATH: 'config/migration-state.json' },
            loadConfig: vi.fn(async () => { throw new Error('denied'); }),
        });
        expect(configRejected).toEqual({ available: false, windowHours: 24, events: [] });
        const missingDatabase = await readActivityFeed(feedOptions(path.join(os.tmpdir(), 'activity-feed-missing', 'no-such.sqlite')));
    ...
    it('serves the unavailable path over HTTP without a 500', async () => {
        ...
        const res = {
            ...
            status(code) { recorded.statusCode = code; return res; },
        ...
        await unavailableHandler({ query: {} }, res);
        ...
        await throwingHandler({ query: {} }, res);
        expect(recorded.statusCode).toBe(200);
        expect(recorded.body).toMatchObject({
            schemaVersion: 1,
            available: false,
