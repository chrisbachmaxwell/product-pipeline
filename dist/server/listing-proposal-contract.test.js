...
import { LISTING_PROPOSAL_CONTRACT_TESTING, LISTING_PROPOSAL_FIELDS, ListingProposalContractError, buildListingProposalEvidence, digestListingProposalDecision, resolveListingProposalOutput, } from './listing-proposal-contract.js';
...
function dto(options = {}) {
    ...
    return {
        ...
        sections: {
            listing: {
                title: field(options.title === undefined ? 'Canon lens' : options.title, options.ebayTitle === undefined ? 'Canon lens' : options.ebayTitle, options.draftTitle ?? null),
...
function choiceFor(fieldName) {
    if (fieldName === 'title') {
        return { field: fieldName, choice: 'use_shopify',
            reasonCode: 'use_verified_shopify', riskCodes: [] };
    }
    return { field: fieldName, choice: 'keep_ebay',
...
describe('listing proposal contract', () => {
    ...
    it('uses digested previews so a long verified description remains selectable', () => {
        const current = dto();
        current.sections.content.description.ebay = 'x'.repeat(300_000);
        const evidence = buildListingProposalEvidence(current);
        const description = evidence.fields.find((entry) => entry.field === 'description');
        expect(description.candidates.ebay.preview).toHaveLength(LISTING_PROPOSAL_CONTRACT_TESTING.maximumPreviewCodePoints);
    ...
    it('allows only optional omissions and downgrades required omissions', () => {
        ...
        const proposal = { ...original, fields: original.fields.map((entry) => {
                if (entry.field === 'title' || entry.field === 'conditionDescription') {
                    return { field: entry.field, choice: 'omit',
                        reasonCode: 'omit_optional_field', riskCodes: [] };
                }
                return entry;
            }) };
        const decision = resolveListingProposalOutput(proposal, evidence, current);
    ...
    it('rejects output against evidence from a different verified base', () => {
        ...
        expect(() => resolveListingProposalOutput(output(), buildListingProposalEvidence(first), second)).toThrowError(ListingProposalContractError);
    });
});
