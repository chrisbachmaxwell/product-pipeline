...
describe('readSmtpConfigFromEnv', () => {
    ...
    it('arms with the full set and defaults FROM to the SMTP user', () => {
        const config = readSmtpConfigFromEnv(ARMED);
    ...
    it('stays unarmed on any missing or malformed piece', () => {
        ...
        expect(readSmtpConfigFromEnv({ ...ARMED, INCIDENT_EMAIL_TO: 'not-an-email' })).toBeNull();
        expect(readSmtpConfigFromEnv({ ...ARMED, INCIDENT_SMTP_PASS: '' })).toBeNull();
    });
});
