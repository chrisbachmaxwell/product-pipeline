...
export function configuredOrderPollArgv(env = process.env) {
    return parseArgvEnv('ORDER_POLL_ARGV', env);
}
export function configuredOrderImportArgv(env = process.env) {
    return parseArgvEnv('ORDER_IMPORT_ARGV', env);
}
export function configuredOrderReconcileArgv(env = process.env) {
    return parseArgvEnv('ORDER_RECONCILE_ARGV', env);
}
export function configuredPollIntervalMs(env = process.env) {
...
export function substituteArgv(template, values) {
    return template.map((entry) => entry.replace(/\{([a-zA-Z]+)\}/gu, (whole, key) => {
        const value = values[key];
        if (value === undefined)
            throw new Error(`ORDER_TRIGGER_PLACEHOLDER_${key}`);
        return value;
    }));
}
export function createProcessStepRunner() {
    return (argv) => new Promise((resolve) => {
        execFile(process.execPath, [...argv], { timeout: RUN_TIMEOUT_MS, maxBuffer: MAX_OUTPUT_BYTES }, (_error, stdout, stderr) => {
            for (const stream of [stdout, stderr]) {
                const line = stream.split('\n').reverse().find((entry) => entry.startsWith('{'));
                if (line) {
                    try {
                        resolve({ json: JSON.parse(line) });
                        return;
                    }
                    catch { /* fall through */ }
                }
...
export function createOrderImportTrigger(dependencies = {}) {
    ...
    async function runCycle(reason) {
        ...
        try {
            const poll = await runStep(pollArgv);
            const eligible = Array.isArray(poll.json?.eligibleOrders)
                ? poll.json.eligibleOrders
                : [];
            const orderIds = eligible
                .map((entry) => entry?.orderId)
                .filter((value) => typeof value === 'string' && ORDER_ID.test(value))
            ...
            for (const orderId of orderIds) {
                const imported = await runStep(substituteArgv(importArgv, { orderId }));
                const status = typeof imported.json?.status === 'string' ? imported.json.status : 'no-summary';
                if (imported.json === null || status === 'denied') {
                    warn(`ORDER_IMPORT_FAILED: ${orderId} ${typeof imported.json?.code === 'string' ? imported.json.code : 'no-summary'}`);
                    continue;
                }
                ...
                const reconciled = await runStep(substituteArgv(reconcileArgv, { orderId, jobId, attemptId }));
    ...
    return {
        ...
        notifySale() {
            if (!armed)
                return false;
            if (now() - lastCycleStartedMs < NOTIFY_MIN_INTERVAL_MS)
                return true;
