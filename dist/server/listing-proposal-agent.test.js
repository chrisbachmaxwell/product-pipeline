...
import { LISTING_PROPOSAL_AGENT_METADATA, LISTING_PROPOSAL_DEFAULT_MODEL, LISTING_PROPOSAL_MAX_OUTPUT_TOKENS, LISTING_PROPOSAL_MAX_RETRIES, LISTING_PROPOSAL_TIMEOUT_MS, ListingProposalAgentError, createListingProposalAgent, getListingProposalAgentReadiness, } from './listing-proposal-agent.js';
...
function field(shopify, ebay, editable = true) {
    return { shopify, ebay, draft: null, editable };
}
function dto(title = 'Canon lens') {
    return {
        schemaVersion: 1,
        ...
        sections: {
            listing: {
                ...
                price: field('{"amount":"39.95","currency":"USD"}', '{"amount":"39.95","currency":"USD"}', false),
...
function fieldChoice(fieldName) {
    if (fieldName === 'title')
        return {
            field: fieldName,
            choice: 'use_shopify',
            reasonCode: 'use_verified_shopify',
            riskCodes: [],
        };
    return {
        field: fieldName,
        choice: 'keep_ebay',
        reasonCode: 'keep_verified_ebay',
        riskCodes: [],
    };
}
...
function successfulTransport(inspect) {
...
describe('listing proposal agent', () => {
    ...
    ])('returns a fixed redacted code for %s', async (_name, status, code) => {
        const logged = [];
