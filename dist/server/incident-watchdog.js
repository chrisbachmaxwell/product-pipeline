...
function loadState(stateFile) {
    try {
        ...
        if (Array.isArray(parsed.incidents) && parsed.pendingSince)
            return parsed;
    }
    catch { /* fresh */ }
...
function defaultLearningsContext() {
    try {
...
async function defaultDiagnose(incident, context) {
    ...
    if (response.status !== 200)
        throw new Error(`diagnosis http ${response.status}`);
...
async function defaultNotify(incident) {
...
export async function runWatchdogOnce(dependencies = {}) {
    ...
    const getLedgerSignals = dependencies.getLedgerSignals
        ?? ((sinceUtc) => readIncidentLedgerSignalsFromArgv(parseArgvEnv('PUBLISH_RECONCILE_ARGV') ?? parseArgvEnv('INVENTORY_SWEEP_ARGV'), sinceUtc));
    ...
    const needsDiagnosis = active.find((incident) => incident.severity === 'critical' && incident.diagnosisState === 'none');
    if (needsDiagnosis) {
    ...
    for (const incident of active) {
        if (incident.severity !== 'critical' || incident.notifiedAtUtc !== null)
            continue;
        if (incident.diagnosisState === 'pending')
            continue;
...
export function initIncidentWatchdog(dependencies = {}) {
