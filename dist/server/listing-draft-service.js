...
const invalid = () => { throw new ListingDraftServiceError('LISTING_DRAFT_INVALID'); };
const unavailable = () => { throw new ListingDraftServiceError('LISTING_DRAFT_UNAVAILABLE'); };
...
function boundedNullable(value, maximum) {
    ...
    if (typeof value !== 'string' || value.length > maximum
        || /[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/u.test(value)
        || PROHIBITED.some((pattern) => pattern.test(value)))
        return invalid();
    return value;
}
function exactOverrideText(value, maximum) {
    const checked = boundedNullable(value, maximum);
    if (checked !== null && (checked.length === 0 || checked.trim() !== checked))
        return invalid();
    return checked;
}
...
function exactSafeIdentifier(value) {
    ...
    if (checked !== null && !/^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/.test(checked))
        return invalid();
    return checked;
}
...
export function parseSaveListingDraftRequest(value) {
    ...
    return Object.freeze({
        ...
        base: Object.freeze({
            sourceDigest: value.base.sourceDigest,
            ebayDigest: value.base.ebayDigest,
        }),
        draft: Object.freeze({
...
function eligibleBasis(workspace) {
    ...
    if (workspace.schemaVersion !== 1 || workspace.evidence.freshness !== 'live'
        ...
        || (notListed && (row.ebay.listingId !== null || row.ebay.offerId !== null
            ...
            || row.ebay.offerCount !== 0 || row.ebay.unpublishedArtifactCount !== 0)))
        unavailable();
    ...
    if (detail && (detail.identity.sellerId !== LISTING_DRAFT_SCOPE.ebaySellerId
        ...
        || !detail.management.exactBindings.sku || !detail.management.lifecycleAligned))
        unavailable();
    const managementModel = workspace.mapping.managementModel === 'inventory_offer'
        ? 'inventory_api'
    ...
    if (managementModel === 'inventory_api' && (!detail
        ...
        || detail.management.offer.marketplaceId !== LISTING_DRAFT_SCOPE.ebayMarketplaceId))
        unavailable();
...
function fieldsForRevision(basis, draft) {
    return Object.keys(EDITABLE).map((field) => {
 ...
 */
export function deriveListingDraftBasis(workspaceDto) {
...
export function createListingDraftService(dependencies = {}) {
    ...
    return Object.freeze({
        async get(catalogId, saveAuthorized = false) {
            ...
            try {
                ...
                const store = openReadOnly({ databasePath: path(), expectedScope: LISTING_DRAFT_SCOPE });
                try {
                    return dto(basis, store.getLatestRevision(basis.identity.shopifyVariantGid), saveAuthorized && writerInstanceReady());
                }
                finally {
                    store.close();
                }
