...
import { createEbayCategorySearch, EbayCategorySearchError, EBAY_CATEGORY_SEARCH_TESTING, validateEbayCategoryQuery, } from '../ebay-category-search.js';
    ...
    return {
        categorySuggestions: [
            ...
            { category: { categoryId: '3', categoryName: 'Deep' },
                categoryTreeNodeAncestors: Array.from({ length: 11 }, (_v, i) => ({ categoryId: String(i), categoryName: `Level ${i}` })) },
...
function fakeFetch(calls, respond) {
...
function standardRespond(url) {
    if (url === TREE_URL)
        return jsonResponse({ categoryTreeId: '0', categoryTreeVersion: '129' });
    if (url.startsWith(SUGGESTIONS_PREFIX))
...
describe('eBay category suggestion search', () => {
    ...
    it.each([
        ...
        ['oversized body', () => new Response(`{"pad":"${'x'.repeat(EBAY_CATEGORY_SEARCH_TESTING.MAX_RESPONSE_BYTES)}"}`, { status: 200 })],
    ])('redacts %s to a fixed failure code', async (_label, respond) => {
        const search = createSearch({
            respond: (url) => url === TREE_URL ? jsonResponse({ categoryTreeId: '0' }) : respond(),
        });
        const failure = await search('studio light').catch((error) => error);
        expect(failure).toBeInstanceOf(EbayCategorySearchError);
        expect(failure.message).toBe('eBay category search is unavailable');
        expect(JSON.stringify({ ...failure })).not.toMatch(/secret|upstream detail|Bearer/);
    });
...
describe('eBay category browse route', () => {
    ...
    it('maps a bad id to 400 and every other failure to one generic 503', async () => {
        const router = createShadowApiRouter({
            ...
            browseEbayCategories: async (parentId) => {
                if (parentId === 'bogus')
                    throw new EbayCategorySearchError('INVALID_QUERY');
