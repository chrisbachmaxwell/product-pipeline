...
router.get('/api/ebay/orders/stats', async (_req, res) => {
    try {
        const db = await getRawDb();
        const total = db.prepare(`SELECT COUNT(*) as count FROM ebay_orders`).get()?.count ?? 0;
        const byFulfillment = db.prepare(`SELECT fulfillment_status, COUNT(*) as count FROM ebay_orders GROUP BY fulfillment_status`).all();
        const byPayment = db.prepare(`SELECT payment_status, COUNT(*) as count FROM ebay_orders GROUP BY payment_status`).all();
        const syncedCount = db.prepare(`SELECT COUNT(*) as count FROM ebay_orders WHERE synced_to_shopify = 1`).get()?.count ?? 0;
        const lastImport = db.prepare(`SELECT MAX(imported_at) as ts FROM ebay_orders`).get()?.ts ?? null;
        res.json({
            total,
            synced: syncedCount,
...
router.get('/api/ebay/orders', async (req, res) => {
    try {
        const db = await getRawDb();
        ...
        const countRow = db.prepare(`SELECT COUNT(*) as count FROM ebay_orders ${where}`).get(...params);
        const rows = db.prepare(`SELECT id, ebay_order_id, legacy_order_id, buyer_username, order_status, fulfillment_status, payment_status, total_amount, currency, item_count, line_items_json, shipping_address_json, ebay_created_at, ebay_modified_at, synced_to_shopify, shopify_order_id, imported_at FROM ebay_orders ${where} ORDER BY imported_at DESC LIMIT ? OFFSET ?`).all(...params, limit, offset);
 ...
 */
router.get('/api/ebay/orders/import-status', async (_req, res) => {
    try {
        const db = await getRawDb();
...
router.get('/api/ebay/orders/:id', async (req, res) => {
    try {
        const db = await getRawDb();
 ...
 */
router.post('/api/ebay/orders/import', async (req, res) => {
    try {
    ...
    `);
        ...
        const insertMany = db.transaction(() => {
            for (const order of orders) {
                ...
                const result = upsert.run(order.orderId, order.legacyOrderId ?? null, order.buyer?.username ?? null, order.cancelStatus?.cancelState ?? 'NONE', order.orderFulfillmentStatus, order.orderPaymentStatus, parseFloat(order.pricingSummary?.total?.value ?? '0'), order.pricingSummary?.total?.currency ?? 'USD', order.lineItems?.length ?? 0, JSON.stringify(order.lineItems ?? []), shippingAddress ? JSON.stringify(shippingAddress) : null, order.creationDate, order.lastModifiedDate, JSON.stringify(order));
                if (result.changes > 0) {
 ...
 */
router.post('/api/ebay/orders/enable-import', async (req, res) => {
    try {
        const { confirm, cutoff } = req.body || {};
        if (confirm !== true) {
            res.status(400).json({
                error: 'confirm must be true',
                warning: 'Enabling import sets the go-live cutoff and starts creating real Shopify orders for new eBay orders. ' +
        ...
        const db = await getRawDb();
        const upsert = db.prepare(`INSERT INTO settings (key, value, updatedAt) VALUES (?, ?, datetime('now'))
 ...
 */
router.post('/api/ebay/orders/disable-import', async (_req, res) => {
    try {
        const db = await getRawDb();
        db.prepare(`INSERT INTO settings (key, value, updatedAt) VALUES ('auto_sync_enabled', 'false', datetime('now'))
       ON CONFLICT(key) DO UPDATE SET value = 'false', updatedAt = datetime('now')`).run();
 ...
 */
router.post('/api/ebay/orders/sync-to-shopify', async (req, res) => {
    try {
