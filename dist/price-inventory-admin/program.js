...
async function runReconciliation(input) {
    ...
    if (input.expectEnded === true) {
        ...
        comparison = Object.freeze({
            effect: ended ? 'effect_observed' : 'effect_absent',
...
export function buildPriceInventoryAdminProgram(dependencies = {}) {
    ...
    program
        .command('align-sweep')
        .description('Batched delta-only alignment of ONE field across every drifting listing, using the '
                    ...
                    try {
                        ...
                        if (result.providerDispatchReported === false) {
                            ...
                            results.push({
                                ...
                                code: result.providerFailureCode ?? 'PROVIDER_DISPATCH_REJECTED',
                            });
