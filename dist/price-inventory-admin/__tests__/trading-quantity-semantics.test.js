...
describe('trading quantity semantics', () => {
    it('adds the sold count so AVAILABLE converges, not total', () => {
        ...
        expect(tradingQuantityToWrite(target({ soldQuantity: 2, availableQuantityBasis: 'total_minus_sold' }), 106)).toBe(108);
    });
    it('is a no-op when nothing has sold', () => {
        expect(tradingQuantityToWrite(target({ soldQuantity: 0, availableQuantityBasis: 'total_minus_sold' }), 5)).toBe(5);
    });
    it('does not adjust when eBay reported available directly', () => {
        ...
        expect(tradingQuantityToWrite(target({ soldQuantity: 4, availableQuantityBasis: 'reported' }), 10)).toBe(10);
    });
    it('does not adjust when the basis is unavailable or the detail is missing', () => {
        expect(tradingQuantityToWrite(target({ soldQuantity: 3, availableQuantityBasis: 'unavailable' }), 7)).toBe(7);
        expect(tradingQuantityToWrite(target(null), 7)).toBe(7);
        expect(tradingQuantityToWrite({ basis: {} }, 7)).toBe(7);
    });
    it('ignores a nonsensical sold count rather than corrupting the write', () => {
        for (const sold of [-1, 1.5, Number.NaN, null, undefined, '2']) {
            expect(tradingQuantityToWrite(target({ soldQuantity: sold, availableQuantityBasis: 'total_minus_sold' }), 9), String(sold)).toBe(9);
        }
    });
    it('zeroing stock still zeroes AVAILABLE on a listing with sales', () => {
        ...
        expect(tradingQuantityToWrite(target({ soldQuantity: 2, availableQuantityBasis: 'total_minus_sold' }), 0)).toBe(2);
    });
});
