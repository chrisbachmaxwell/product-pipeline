...
import { buildBulkUpdateBody, createPriceInventoryDispatchAdapter, AlignDispatchError, } from '../dispatch-adapter.js';
    ...
    return {
        ...
        catalog: {
            ...
            shopify: {
                ...
                imageCount: 1, available: 1, price: { amount: shopifyPrice, currency: 'USD' },
            },
            ebay: {
        ...
        ebayDetail: {
            ...
            actual: {
                ...
                commerce: { price: { value: ebayPrice, currency: 'USD' }, totalQuantity: 1,
...
function createWorld(initial = workspace()) {
    ...
    // successful provider entry flips the observed eBay price to the Shopify
    // source price, simulating the remote effect landing.
    const requests = [];
    ...
    const fakeFetch = async (input, init) => {
        ...
        if (entryStatusCode === 200) {
            const shopifyPrice = current.catalog.shopify
                .price.amount;
            current = workspace({ shopifyPrice, ebayPrice: shopifyPrice });
        }
        return new Response(JSON.stringify({ responses: [{ statusCode: entryStatusCode }] }), { status: 200, headers: { 'Content-Type': 'application/json' } });
    };
...
describe('price/inventory alignment operator CLI (inventory model)', () => {
    ...
    it('fails closed on missing ownership, moved drift, no drift, and wrong targets', async () => {
        ...
        quantityWorld.catalog.shopify.available = 3;
        world.setWorkspace(quantityWorld);
        await world.run(['plan', ...targetArguments('quantity')]);
        const quantityDigest = lastJson(world.stdout).manifestDigest;
    ...
    it('derives manifests only for valid drift and builds contamination-free bodies', () => {
        ...
        noPrice.catalog.shopify.price = null;
        let caught;
        try {
        ...
        expect(caught.code).toBe('PLAN_SOURCE_VALUE_INVALID');
        ...
        const parsedPrice = JSON.parse(priceBody);
    ...
    it('keeps the CLI free of server-mount, direct-fetch, and legacy writer imports', () => {
        ...
        const program = fs.readFileSync(path.join(sourceRoot, 'price-inventory-admin/program.ts'), 'utf8');
        const manifestSource = fs.readFileSync(path.join(sourceRoot, 'price-inventory-admin/manifest.ts'), 'utf8');
        const adapterSource = fs.readFileSync(path.join(sourceRoot, 'price-inventory-admin/dispatch-adapter.ts'), 'utf8');
        const tradingAdapterSource = fs.readFileSync(path.join(sourceRoot, 'price-inventory-admin/trading-dispatch-adapter.ts'), 'utf8');
        const indexSource = fs.readFileSync(path.join(sourceRoot, 'price-inventory-admin/index.ts'), 'utf8');
        ...
        expect(`${program}\n${manifestSource}\n${adapterSource}\n${tradingAdapterSource}\n${indexSource}`).not.toMatch(/from ['"][^'"]*(?:\/sync\/|order-sync|product-sync|inventory-sync|price-sync|token-manager)[^'"]*['"]/);
