...
function basis(observedPrice, sourcePrice) {
    return {
...
const money = (amount) => JSON.stringify({ amount, currency: 'USD' });
function planPrice(observed, source) {
    return deriveAlignmentManifest({ basis: basis(observed, source), field: 'price' });
}
function expectNoDrift(observed, source) {
    try {
        ...
        expect(error.code).toBe('PLAN_NO_DRIFT');
    }
}
