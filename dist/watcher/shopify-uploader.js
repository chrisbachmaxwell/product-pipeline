 ...
 */
async function uploadSingleImage(accessToken, storeDomain, productId, filePath, position) {
 ...
 */
export async function uploadImagesToShopify(shopifyProductId, imagePaths) {
    ...
    for (let i = 0; i < imagePaths.length; i++) {
        ...
        try {
            ...
            const result = await uploadSingleImage(accessToken, creds.storeDomain, shopifyProductId, filePath, i);
            if (result) {
                uploaded++;
