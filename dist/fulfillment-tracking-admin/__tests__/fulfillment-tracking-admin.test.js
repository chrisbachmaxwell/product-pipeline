...
function shopifyFixture(partial = false) {
    return {
...
async function preflight(world) {
    await world.run([
        'preflight',
...
describe('migration schema v4 fulfillment boundary', () => {
    it('publishes an explicit versioned migration', () => {
        ...
        expect(SCHEMA_MIGRATIONS.at(-3)).toMatchObject({
            version: 4,
    ...
    it('does not mount the ceremony in the server or weaken legacy writers', () => {
        ...
        const program = fs.readFileSync(path.join(sourceRoot, 'fulfillment-tracking-admin/program.ts'), 'utf8');
