...
import { assertShopifyCredentialDatabaseDiagnosticRuntimeBinding, PRODUCT_PIPELINE_PRODUCTION_RUNTIME, } from './config.js';
import { inspectCanonicalAuthTokensSchema, } from './database-contract.js';
...
function readExactDescriptor(filesystem, descriptor, size) {
...
function privateSqliteSnapshot(snapshot) {
    if (snapshot.length < SQLITE_HEADER_BYTES
        || !snapshot.subarray(0, SQLITE_MAGIC.length).equals(SQLITE_MAGIC))
        return null;
    ...
    if (writeVersion !== readVersion
        || (writeVersion !== SQLITE_ROLLBACK_JOURNAL_VERSION
            && writeVersion !== SQLITE_WAL_VERSION))
        return null;
...
function schemaFailureStage(inspection) {
...
function applySchemaChecks(checks, inspection) {
 ...
 */
export function diagnoseFixedProductionShopifyDatabase(environment = process.env, dependencies = {}) {
    ...
    const openPrivateSnapshotReadOnly = dependencies.openPrivateSnapshotReadOnly
        ?? ((snapshot) => new Database(snapshot, { readonly: true }));
    ...
    if (!checks.fileRegular)
        return report('file-type-denied', checks);
    if (!checks.fileSymlinkAbsent || !checks.fileSingleLink) {
        return report('file-link-denied', checks);
    }
    if (!checks.fileNonEmpty)
        return report('file-empty-denied', checks);
    if (!checks.fileWithinSnapshotLimit)
        return report('file-size-denied', checks);
    if (!checks.fileMode0600)
        return report('file-permissions-denied', checks);
    try {
    ...
    if (sidecarsBefore === 'denied')
        return report('sidecar-inspection-denied', checks);
    if (sidecarsBefore === 'present')
        return report('sidecar-present', checks);
    ...
    try {
        ...
        if (failure === null && openedBefore !== null) {
            ...
            if (!checks.descriptorIdentityStableBeforeSnapshot)
                failure = 'descriptor-identity-denied';
        }
        ...
        if (failure === null && snapshot !== null) {
            ...
            if (!checks.snapshotHeaderCanonical)
                failure = 'snapshot-header-denied';
        }
        ...
        if (failure === null && database !== null) {
            ...
            if (!checks.sqlitePrivateMemory)
                failure = 'sqlite-memory-denied';
        }
        if (failure === null && database !== null) {
            checks.sqliteOpenedReadOnly = database.readonly === true;
            if (!checks.sqliteOpenedReadOnly)
                failure = 'sqlite-readonly-denied';
        }
        if (failure === null && database !== null) {
            try {
        ...
        if (failure === null && database !== null) {
            ...
            if (failure === null && !checks.sqliteQueryOnly)
                failure = 'sqlite-query-only-denied';
        }
        if (failure === null && database !== null) {
        ...
        if (failure === null && database !== null) {
            ...
            if (failure === null && !checks.sqliteIntegrityOk)
                failure = 'integrity-check-denied';
        }
        if (failure === null && database !== null) {
            try {
                const cardinality = database.prepare(`SELECT COUNT(*) AS count FROM auth_tokens WHERE platform = 'shopify'`).get();
        ...
        if (descriptor !== null && openedBefore !== null) {
            ...
            if (snapshotDigest !== null) {
                try {
                    ...
                    if (failure === null)
                        failure = 'snapshot-post-read-denied';
                }
            }
            ...
            try {
                ...
                checks.descriptorIdentityStableAfterSnapshot = sameDescriptorIdentity(openedBefore, afterInspection);
            }
            catch {
                if (failure === null)
                    failure = 'descriptor-post-inspection-denied';
            }
            if (afterInspection !== null
                && !checks.descriptorIdentityStableAfterSnapshot
                && failure === null)
                failure = 'descriptor-post-identity-denied';
            ...
            try {
                ...
                if (failure === null)
                    failure = 'path-post-inspection-denied';
            }
        ...
        if (descriptor !== null) {
            try {
                ...
                if (failure === null)
                    failure = 'descriptor-close-denied';
            }
        }
        snapshot?.fill(0);
