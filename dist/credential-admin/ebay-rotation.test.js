...
import { archiveAndResetEbayProductionConsent, beginEbayProductionConsent, createBoundedEbayProviderTransport, EBAY_PRODUCTION_SCOPES, EBAY_RECONCILIATION_RESET_CONFIRMATION, EBAY_REVOKE_CONFIRMATION, EBAY_STALE_LOCK_RECOVERY_CONFIRMATION, EbayRotationError, installEbayProductionGrant, recoverStaleEbayOperationLock, registerEbayProductionConsent, revokeInstalledEbayGrant, verifyInstalledEbayGrant, } from './ebay-rotation.js';
import { assertEbayProductionRailwayBoundary, EBAY_ROTATION_RAILWAY_ENVIRONMENT_ID, EBAY_ROTATION_RAILWAY_PROJECT_ID, EBAY_ROTATION_RAILWAY_SERVICE_ID, credentialAdminExitCode, parseCredentialAdminArguments, runCredentialAdmin, } from './ebay-program.js';
...
async function fixture(includeEbay = true) {
  ...
  `);
    database.prepare(`INSERT INTO auth_tokens (
     ...
     ) VALUES ('shopify', ?, NULL, 'read_products', NULL, 100, 100)`).run('shopify-authority-must-not-change');
    if (includeEbay) {
        database.prepare(`INSERT INTO auth_tokens (
         platform, access_token, refresh_token, scope, expires_at, created_at, updated_at
       ) VALUES ('ebay', ?, ?, 'legacy-scope', 200, 100, 100)`).run(OLD_ACCESS_TOKEN, OLD_REFRESH_TOKEN);
    }
    database.close();
...
function rows(databasePath) {
    ...
    try {
        return database.prepare('SELECT * FROM auth_tokens ORDER BY id').all();
    }
    finally {
        database.close();
    }
}
async function callbackFor(workDirectory, stateOverride) {
    ...
    transport = async (request) => {
        ...
        if (request.url === 'https://api.ebay.com/identity/v1/oauth2/token') {
            ...
            if (body.get('grant_type') === 'authorization_code') {
                ...
                if (this.failExchange)
                    throw new Error('ambiguous token exchange transport failure');
        ...
        if (request.url === 'https://api.ebay.com/identity/v1/oauth2/token/introspect') {
            ...
            if (this.revoked.has(token))
                return { status: 200, bodyText: '{"active":false}' };
            return {
                status: 200,
                bodyText: JSON.stringify({
                    active: true,
...
function assertNewCertOnly(requests) {
...
async function install(target, provider) {
...
describe('eBay Production credential administrator', () => {
    ...
    it('registers only consent digests at the fixed-purpose remote boundary', async () => {
        const target = await fixture();
        const authorizationResult = await begin(target);
        const localStateText = await fs.readFile(path.join(target.workDirectory, 'consent-state.json'), 'utf8');
        ...
        const remoteStateText = await fs.readFile(path.join(remoteWorkDirectory, 'consent-state.json'), 'utf8');
        ...
        await installEbayProductionGrant({
            ...
            dependencies: {
                ...
                afterDirectorySync: async (_directory, phase) => {
                    if (phase !== 'state-replace')
                        return;
                    const state = JSON.parse(await fs.readFile(path.join(target.workDirectory, 'consent-state.json'), 'utf8'));
    ...
    it('fails closed before CAS when the commit-intent directory sync fails', async () => {
        ...
        await expect(installEbayProductionGrant({
            ...
            dependencies: {
                ...
                beforeDirectorySync: async (_directory, phase) => {
                    if (phase !== 'state-replace' || injected)
                        return;
                    const state = JSON.parse(await fs.readFile(path.join(target.workDirectory, 'consent-state.json'), 'utf8'));
        ...
        expect(JSON.parse(await fs.readFile(path.join(target.workDirectory, 'consent-state.json'), 'utf8'))).toMatchObject({
    ...
    it('archives terminal evidence and permits a fresh consent only after exact reconciliation confirmation', async () => {
        ...
        const freshLocalState = JSON.parse(await fs.readFile(path.join(localFreshDirectory, 'consent-state.json'), 'utf8'));
        ...
        const resetInput = {
            ...
            dependencies: {
                ...
                afterDirectorySync: (_directory, phase) => {
        ...
        const freshRemoteState = JSON.parse(await fs.readFile(path.join(target.workDirectory, 'consent-state.json'), 'utf8'));
        ...
        expect(JSON.parse(await fs.readFile(path.join(archivedEvidence, 'consent-state.json'), 'utf8'))).toMatchObject({
            status: 'failed-cleanup-required',
            terminalCode: 'EBAY_ROTATION_CLEANUP_REQUIRED',
        });
    ...
    it('never overwrites an existing backup candidate', async () => {
        ...
        const collision = path.join(target.backupDirectory, 'ebaysync-before-ebay-grant-2026-08-14T20-00-00-000Z-090909090909.sqlite');
    ...
    it('verifies the bound installed grant with no database write', async () => {
        ...
        await install(target, provider);
        const before = rows(target.databasePath);
        const tokenRequestCount = provider.requests.filter((request) => request.url === 'https://api.ebay.com/identity/v1/oauth2/token').length;
        ...
        expect(rows(target.databasePath)).toEqual(before);
        expect(provider.requests.filter((request) => request.url === 'https://api.ebay.com/identity/v1/oauth2/token')).toHaveLength(tokenRequestCount);
        assertNewCertOnly(provider.requests);
    });
    it('rejects a same-row same-time same-scope token swap before verify or revoke provider access', async () => {
        ...
        const requestCount = provider.requests.length;
        const database = new Database(target.databasePath);
        database.prepare("UPDATE auth_tokens SET access_token = ?, refresh_token = ? WHERE platform = 'ebay'").run('swapped-access-token', 'swapped-refresh-token');
        database.close();
        expect(rows(target.databasePath).find((row) => row.platform === 'ebay')).toMatchObject({
            id: installed.id,
    ...
    it('records a post-COMMIT state failure as committed reconciliation-required with exit 5 truth', async () => {
        ...
        try {
            await installEbayProductionGrant({
                ...
                dependencies: {
                    ...
                    transport: provider.transport,
                    beforeDirectorySync: async (_directory, phase) => {
                        if (phase !== 'state-replace' || installedDirectorySyncFailed)
                            return;
                        const state = JSON.parse(await fs.readFile(path.join(target.workDirectory, 'consent-state.json'), 'utf8'));
        ...
        expect(verified.code).toBe('EBAY_GRANT_VERIFIED');
        expect(JSON.parse(await fs.readFile(path.join(target.workDirectory, 'consent-state.json'), 'utf8'))).toMatchObject({ status: 'installed', databaseEffect: 'committed', terminalCode: null });
    });
    it('never revokes or erases evidence when COMMIT applies the row and then surfaces an error', async () => {
        ...
        expect(provider.requests.some((request) => request.url === 'https://api.ebay.com/identity/v1/oauth2/token/revoke')).toBe(false);
        expect(rows(target.databasePath).find((row) => row.platform === 'ebay')).toMatchObject({
            access_token: ACCESS_TOKEN,
            refresh_token: REFRESH_TOKEN,
            scope: SCOPE,
    ...
    it('preserves commit-pending evidence and reports an unknown database effect when readback is ambiguous', async () => {
            ...
            await installEbayProductionGrant({
                ...
                dependencies: {
                    ...
                    afterCommitAppliedBeforeResult: () => {
                        ...
                        ambiguous.prepare("UPDATE auth_tokens SET access_token = ? WHERE platform = 'ebay'").run('post-commit-ambiguous-authority');
                        ambiguous.close();
        ...
        expect(provider.requests).toHaveLength(requestCount);
        const preserved = JSON.parse(await fs.readFile(path.join(target.workDirectory, 'consent-state.json'), 'utf8'));
        ...
        const fresh = JSON.parse(await fs.readFile(path.join(freshLocalDirectory, 'consent-state.json'), 'utf8'));
        const archiveDirectory = path.join(target.root, 'evidence-archive');
        const reset = {
            workDirectory: target.workDirectory,
            archiveDirectory,
        ...
        await expect(archiveAndResetEbayProductionConsent(reset))
            .rejects.toMatchObject({ code: 'EBAY_ROTATION_RECONCILIATION_DENIED' });
        const backupPath = path.join(target.backupDirectory, preserved.installation.backupFileName);
        ...
        expect(JSON.parse(await fs.readFile(path.join(target.workDirectory, 'consent-state.json'), 'utf8'))).toMatchObject({ status: 'pending', databaseEffect: 'none' });
        const archivedContainers = await fs.readdir(archiveDirectory);
        expect(archivedContainers).toHaveLength(1);
        expect(JSON.parse(await fs.readFile(path.join(archiveDirectory, archivedContainers[0], 'evidence', 'consent-state.json'), 'utf8'))).toMatchObject({
            status: 'commit-outcome-reconciliation-required',
            databaseEffect: 'commit-pending',
            terminalCode: 'EBAY_ROTATION_CLEANUP_REQUIRED',
        });
            ...
            await installEbayProductionGrant({
                ...
                dependencies: {
                    ...
                    afterCommitAppliedBeforeResult: () => {
                        ...
                        baseline.prepare(`UPDATE auth_tokens
               SET access_token = ?, refresh_token = ?, scope = 'legacy-scope',
                   expires_at = 200, created_at = 100, updated_at = 100
               WHERE platform = 'ebay'`).run(OLD_ACCESS_TOKEN, OLD_REFRESH_TOKEN);
                        baseline.close();
    ...
    it('classifies an ambiguous bound-grant revocation as mandatory reconciliation', async () => {
        ...
        expect(JSON.parse(await fs.readFile(path.join(target.workDirectory, 'consent-state.json'), 'utf8'))).toMatchObject({ status: 'installed', databaseEffect: 'committed' });
    });
