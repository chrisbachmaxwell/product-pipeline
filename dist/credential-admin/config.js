...
export const SHOPIFY_ROTATION_ENVIRONMENT = Object.freeze({
    ...
    databasePermissionRepairReplicaCount: 'SHOPIFY_DATABASE_PERMISSION_REPAIR_REPLICA_COUNT',
    databasePermissionRepairVolumeMountCount: 'SHOPIFY_DATABASE_PERMISSION_REPAIR_VOLUME_MOUNT_COUNT',
});
 ...
 */
export function assertShopifyCredentialDatabasePermissionRepairRuntimeBinding(environment = process.env) {
    if (environment.NODE_ENV !== 'production'
        || environment[SHOPIFY_ROTATION_ENVIRONMENT.projectId]
            !== PRODUCT_PIPELINE_PRODUCTION_RUNTIME.projectId
        || environment[SHOPIFY_ROTATION_ENVIRONMENT.environmentId]
...
function requireRuntimeBinding(environment, now) {
    if (environment.NODE_ENV !== 'production'
...
function requireActivePreviousSecret(environment, currentSecret, now, required) {
    ...
    if (exactEqual(secret, currentSecret))
        return rotationDenied('configuration-denied');
    const expiresAt = Date.parse(expiresAtUtc);
    if (!Number.isSafeInteger(expiresAt)
        || new Date(expiresAt).toISOString() !== expiresAtUtc
...
export function assertLegacyDatabasePath(databasePath, expectedDatabasePath = PRODUCT_PIPELINE_PRODUCTION_RUNTIME.databasePath) {
...
export function assertLegacyDatabaseIdentity(databasePath, expected, expectedDatabasePath = PRODUCT_PIPELINE_PRODUCTION_RUNTIME.databasePath) {
    const actual = assertLegacyDatabasePath(databasePath, expectedDatabasePath);
    if (actual.dev !== expected.dev || actual.ino !== expected.ino) {
        return rotationDenied('database-denied');
    }
}
export function loadShopifyCredentialRotationConfig(input) {
    const environment = input.environment ?? process.env;
    const now = input.now ?? Date.now();
    const authorizationExpiresAtEpochMs = requireRuntimeBinding(environment, now);
    ...
    const refreshToken = input.requireRefreshToken
        ...
        : null;
    const previous = requireActivePreviousSecret(environment, clientSecret, now, input.requireRefreshToken);
    if (refreshToken !== null && exactEqual(clientSecret, refreshToken)) {
        return rotationDenied('configuration-denied');
    }
    (input.validateDatabasePath ?? assertLegacyDatabasePath)(PRODUCT_PIPELINE_PRODUCTION_RUNTIME.databasePath);
...
export function assertShopifyCredentialRotationAuthorizationActive(config, now) {
 ...
 */
export function assertShopifyCredentialRotationDispatchAuthorized(config, now) {
