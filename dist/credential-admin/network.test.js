...
import { CANONICAL_SHOPIFY_SCOPE_TEXT, requestRotatedShopifyAccessToken, SHOPIFY_CREDENTIAL_ROTATION_NETWORK_LIMITS, SHOPIFY_ROTATION_GRAPHQL_DOCUMENT, verifyShopifyAccessToken, } from './network.js';
...
describe('Shopify credential rotation network boundary', () => {
    it('uses the exact official one-shot rotation body, then verifies exact read-only authority', async () => {
        ...
        expect(tokenUrl).toBe('https://usedcameragear.myshopify.com/admin/oauth/access_token');
        ...
        expect(graphqlUrl).toBe('https://usedcameragear.myshopify.com/admin/api/2026-07/graphql.json');
        ...
        expect((graphqlInit?.headers)['X-Shopify-Access-Token'])
            .toBe(NEW_TOKEN);
    });
    it.each([
