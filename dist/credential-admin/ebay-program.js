import { archiveAndResetEbayProductionConsent, beginEbayProductionConsent, EBAY_PRODUCTION_BACKUP_DIRECTORY, EBAY_PRODUCTION_DATABASE_PATH, EBAY_PRODUCTION_EVIDENCE_ARCHIVE_DIRECTORY, EBAY_PRODUCTION_LOCK_ARCHIVE_DIRECTORY, EBAY_PRODUCTION_WORK_DIRECTORY, EBAY_RECONCILIATION_RESET_CONFIRMATION, EBAY_REVOKE_CONFIRMATION, EBAY_STALE_LOCK_RECOVERY_CONFIRMATION, EbayRotationError, ensureEbayProductionPrivateParents, installEbayProductionGrant, recoverStaleEbayOperationLock, registerEbayProductionConsent, revokeInstalledEbayGrant, verifyInstalledEbayGrant, } from './ebay-rotation.js';
export const EBAY_ROTATION_RAILWAY_PROJECT_ID = 'f8c050c9-11c3-4611-8805-092289941aa4';
export const EBAY_ROTATION_RAILWAY_ENVIRONMENT_ID = '544d8896-b900-48ad-b42e-95272e1ad397';
export const EBAY_ROTATION_RAILWAY_SERVICE_ID = '32ef14cc-2c85-447d-a890-53c422d81de1';
...
function parseOptions(values, expected) {
    ...
    if (expected.some((key) => !(key in result)))
        failArgument();
    return Object.freeze(result);
}
export function parseCredentialAdminArguments(argv) {
    if (argv.length === 0 || (argv.length === 1 && ['help', '--help', '-h'].includes(argv[0]))) {
        return Object.freeze({ command: 'help' });
    }
    if (argv[0] !== 'ebay' || !argv[1])
        failArgument();
    ...
    if (command === 'archive-reset-after-reconciliation') {
        const options = parseOptions(rest, ['--state-digest', '--request-digest', '--confirm']);
        return Object.freeze({
            command,
            stateDigest: options['--state-digest'],
            requestDigest: options['--request-digest'],
...
export function assertEbayProductionRailwayBoundary(environment) {
...
function beginCredentials(environment) {
    return Object.freeze({
        appId: environment.EBAY_APP_ID ?? '',
        ruName: environment.EBAY_RU_NAME ?? '',
    });
}
function rotationCredentials(environment) {
    return Object.freeze({
        appId: environment.EBAY_APP_ID ?? '',
        ruName: environment.EBAY_RU_NAME ?? '',
...
export function credentialAdminExitCode(code, reconciliationRequired = false) {
    if (reconciliationRequired)
        return 5;
    if (code.includes('CLEANUP'))
        return 5;
    if (code.includes('PROVIDER') || code.includes('REVOCATION'))
        return 3;
    if (code.includes('DATABASE') || code.includes('GRANT_BINDING'))
        return 4;
    return 2;
}
...
export async function defaultNoEchoSecretReader() {
...
export async function runCredentialAdmin(input) {
    const environment = input.environment ?? {
        ...
        SHOPIFY_CREDENTIAL_ROTATION_SINGLE_WRITER_ACK: process.env.SHOPIFY_CREDENTIAL_ROTATION_SINGLE_WRITER_ACK,
    ...
    try {
        ...
        io.setExitCode(credentialAdminExitCode(failure.code, failure.effects.reconciliationRequired));
    }
}
