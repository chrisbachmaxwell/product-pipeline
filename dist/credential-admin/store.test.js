...
function fixture(scope = null) {
  ...
  `);
    database.prepare(`INSERT INTO auth_tokens
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?)`).run('shopify', 'old-shopify-access-token-value', null, scope, null, 100, 200);
    database.prepare(`INSERT INTO auth_tokens
      (platform, access_token, refresh_token, scope, expires_at, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?)`).run('ebay', 'ebay-access-token-value', 'ebay-refresh-token-value', 'sell.inventory', 999, 101, 201);
...
describe('legacy Shopify token store maintenance boundary', () => {
    it('creates a whole consistent 0600 backup before a metadata-aware one-row CAS', async () => {
        ...
        store.close();
        const after = rawRows(loaded.databasePath);
        const beforeTyped = before;
    ...
    it('rejects and removes a backup whose unrelated content differs despite matching schema/counts', async () => {
        ...
        const backupSpy = vi.spyOn(Database.prototype, 'backup').mockImplementation(async function (destinationFile, options) {
    ...
    it('fails without mutation on noncanonical schema, triggers, or stale token metadata', () => {
        for (const mutate of [
            (db) => db.exec('CREATE INDEX extra_auth_index ON auth_tokens(updated_at)'),
            (db) => db.exec(`CREATE TRIGGER auth_touch AFTER UPDATE ON auth_tokens BEGIN UPDATE unrelated_state SET value = 'changed' WHERE id = 1; END`),
            (db) => db.prepare(`UPDATE auth_tokens SET refresh_token = 'stale-refresh' WHERE platform = 'shopify'`).run(),
            (db) => db.prepare(`UPDATE auth_tokens SET expires_at = 123 WHERE platform = 'shopify'`).run(),
            (db) => db.prepare(`UPDATE auth_tokens SET scope = 'read_products,write_products' WHERE platform = 'shopify'`).run(),
            (db) => db.prepare(`UPDATE auth_tokens SET scope = 'read_fulfillments,read_inventory,read_orders,read_products,read_products' WHERE platform = 'shopify'`).run(),
        ]) {
            const loaded = fixture();
    ...
    it('requires exactly one Shopify row while retaining the unrelated eBay row', () => {
        ...
        expect(rawRows(loaded.databasePath).tokens).toHaveLength(1);
    });
});
