...
export const EBAY_ROTATION_ENVIRONMENT = 'production';
export const EBAY_ROTATION_SELLER = 'usedcameragear';
export const EBAY_ROTATION_MARKETPLACE = 'EBAY_US';
export const EBAY_REVOKE_CONFIRMATION = 'revoke-productpipeline-ebay-grant';
export const EBAY_RECONCILIATION_RESET_CONFIRMATION = 'provider-reconciled-reset-ebay-consent';
export const EBAY_STALE_LOCK_RECOVERY_CONFIRMATION = 'recover-stale-ebay-credential-lock';
export const EBAY_PRODUCTION_DATABASE_PATH = '/data/ebaysync.db';
export const EBAY_PRODUCTION_WORK_DIRECTORY = '/data/product-pipeline/credential-maintenance/ebay';
export const EBAY_PRODUCTION_BACKUP_DIRECTORY = '/data/product-pipeline/credential-backups/ebay';
export const EBAY_PRODUCTION_EVIDENCE_ARCHIVE_DIRECTORY = '/data/product-pipeline/credential-maintenance/evidence-archive';
export const EBAY_PRODUCTION_LOCK_ARCHIVE_DIRECTORY = '/data/product-pipeline/credential-maintenance/lock-archive';
...
const NO_FAILURE_EFFECTS = Object.freeze({
...
export class EbayRotationError extends Error {
    code;
    effects;
    constructor(code, effects = NO_FAILURE_EFFECTS) {
        super(code);
...
function failWithEffects(code, effects) {
...
function requireConfiguredText(value, maximum) {
    if (typeof value !== 'string' || value.length === 0 || value.length > maximum
        || /\s|[\u0000-\u001f\u007f]/u.test(value)) {
...
function tokenDigest(kind, value) {
    return sha256(`product-pipeline-ebay-${kind}-token-v1\u0000${value}`);
}
function digestMatchesToken(kind, value, expectedDigest) {
...
function consentRequestDigest(credentials, stateDigest) {
...
function canonicalScopeString(scopes) {
    return [...scopes].sort().join(' ');
}
function exactScopeSet(value) {
...
function fixedResult(code, input = {}) {
...
function assertCredentials(credentials, requireCert) {
    requireConfiguredText(credentials.appId, 512);
    requireConfiguredText(credentials.ruName, 512);
    if (requireCert)
        requireConfiguredText(credentials.newCertId, 512);
    return credentials;
}
...
async function syncPrivateDirectory(directory, dependencies, phase) {
    const exact = assertAbsolute(directory);
...
async function syncPrivateRegularFile(filePath) {
...
async function createPrivateDirectory(directory, dependencies) {
    const exact = assertAbsolute(directory);
...
async function writeNewPrivateFile(filePath, value, dependencies, phase = 'new-private-file') {
...
async function replacePrivateFile(filePath, value, dependencies) {
    await assertPrivateRegularFile(filePath);
    const directory = path.dirname(filePath);
    const temporaryPath = path.join(directory, `.state-${randomBytes(12).toString('hex')}.tmp`);
    try {
        await writeNewPrivateFile(temporaryPath, value, dependencies, 'state-temporary-create');
...
async function durableRename(sourcePath, targetPath, dependencies, targetPhase, sourcePhase = targetPhase) {
...
function parseInstallation(value) {
    ...
    if (!Number.isInteger(value.expectedUpdatedAt) || value.expectedUpdatedAt <= 0
        || !(rowId === null || (Number.isInteger(rowId) && rowId > 0))
...
function parseConsentRecord(value) {
    ...
    if (value.schemaVersion !== 2
        ...
        || !['co-located', 'external'].includes(value.consentArtifact)
        ...
        || !['none', 'commit-pending', 'committed'].includes(value.databaseEffect)
    ...
    const databaseEffect = value.databaseEffect;
    ...
    return Object.freeze({
        ...
        consentArtifact: value.consentArtifact,
...
async function readConsentRecord(workDirectory) {
    ...
    if (Buffer.byteLength(textValue, 'utf8') > 32_768)
        fail('EBAY_ROTATION_STATE_INVALID');
    try {
...
async function writeConsentRecord(workDirectory, record, dependencies) {
    const filePath = path.join(workDirectory, STATE_FILE);
    await replacePrivateFile(filePath, `${JSON.stringify(record)}\n`, dependencies);
}
function parseOperationLock(value) {
...
async function readOperationLock(lockPath) {
...
function exactOpaqueValue(actual, expected) {
    const actualBytes = Buffer.from(actual, 'utf8');
    const expectedBytes = Buffer.from(expected, 'utf8');
    return actualBytes.length === expectedBytes.length
        && timingSafeEqual(actualBytes, expectedBytes);
}
async function withOperationLock(workDirectory, dependencies, operation) {
    await assertPrivateDirectory(workDirectory);
    ...
    const lockRecord = Object.freeze({
        schemaVersion: 1,
        kind: 'product-pipeline-ebay-credential-operation-lock',
        ownerId,
        pid: process.pid,
...
function callbackResult(secretInput) {
...
function stateMatches(actualState, expectedDigest) {
    const actual = Buffer.from(sha256(actualState).slice('sha256:'.length), 'hex');
    const expected = Buffer.from(expectedDigest.slice('sha256:'.length), 'hex');
    return actual.length === expected.length && timingSafeEqual(actual, expected);
}
export function createBoundedEbayProviderTransport(fetchImplementation = fetch) {
...
async function providerJson(transport, request, failureCode) {
    let response;
    try {
...
async function exchangeAuthorizationCode(transport, credentials, code, now) {
    ...
    if (response.token_type !== 'User Access Token'
        || !Number.isInteger(expiresIn) || expiresIn < 300 || expiresIn > 86_400) {
        fail('EBAY_ROTATION_PROVIDER_RESPONSE_INVALID');
    }
    ...
    return Object.freeze({
        accessToken,
        refreshToken,
        accessExpiresAt: Math.floor(now.getTime() / 1_000) + expiresIn,
        scope: canonicalScopeString(EBAY_PRODUCTION_SCOPES),
    });
}
async function introspect(transport, credentials, token, hint) {
...
function audienceIsProduction(value) {
    if (value === 'https://api.ebay.com')
        return true;
    return Array.isArray(value)
        && value.length > 0
        && value.every((entry) => entry === 'https://api.ebay.com');
}
function validateActiveIntrospection(value, credentials) {
    if (value.active !== true || value.client_id !== credentials.appId) {
        fail('EBAY_ROTATION_PROVIDER_ENVIRONMENT_MISMATCH');
    }
    if (!exactScopeSet(value.scope))
        fail('EBAY_ROTATION_PROVIDER_SCOPE_MISMATCH');
    if (!audienceIsProduction(value.aud))
        fail('EBAY_ROTATION_PROVIDER_ENVIRONMENT_MISMATCH');
...
async function validateTradingSeller(transport, accessToken) {
...
async function validateInventoryScope(transport, accessToken) {
...
async function validateGrant(transport, credentials, grant) {
...
async function revokeGrant(transport, credentials, refreshToken) {
    let response;
    try {
...
function validateAuthRow(value) {
    if (!isRecord(value)
...
function readAuthRows(database) {
    const raw = database.prepare(`SELECT id, platform, access_token, refresh_token, scope, expires_at, created_at, updated_at
     FROM auth_tokens ORDER BY id`).all();
    if (raw.length > 64)
        fail('EBAY_ROTATION_DATABASE_PREFLIGHT_FAILED');
...
function verifyDatabaseShape(database) {
    const quick = database.pragma('quick_check');
    if (quick.length !== 1 || quick[0]?.quick_check !== 'ok') {
        fail('EBAY_ROTATION_DATABASE_PREFLIGHT_FAILED');
    }
    const objects = database.prepare("SELECT type, name, sql FROM sqlite_master WHERE tbl_name = 'auth_tokens' ORDER BY type, name").all();
    if (objects.filter((entry) => entry.type === 'table' && entry.name === 'auth_tokens').length !== 1
        || objects.some((entry) => !['table', 'index'].includes(entry.type))) {
        fail('EBAY_ROTATION_DATABASE_PREFLIGHT_FAILED');
    }
    const tableSql = objects.find((entry) => entry.type === 'table' && entry.name === 'auth_tokens')?.sql;
    ...
    const indexColumns = database.pragma(`index_info(${JSON.stringify(platformIndex.name)})`);
    ...
    const triggers = database.prepare("SELECT name FROM sqlite_master WHERE type = 'trigger' AND tbl_name = 'auth_tokens'").all();
    if (triggers.length !== 0)
        fail('EBAY_ROTATION_DATABASE_PREFLIGHT_FAILED');
}
async function assertExistingDatabase(databasePath) {
...
async function assertLedgerMatchesPrivateBackup(input) {
    try {
        ...
        try {
            ...
            const currentForeignKeys = current.pragma('foreign_key_check');
...
function backupName(now, random) {
...
async function prepareLedgerInstallation(input) {
    ...
    try {
        ...
        if (error instanceof EbayRotationError
            && error.code === 'EBAY_ROTATION_DATABASE_BACKUP_FAILED')
            throw error;
        return fail('EBAY_ROTATION_DATABASE_BACKUP_FAILED');
    }
...
async function commitLedgerGrant(preparation, grant, beforeLedgerCas, afterCommitAppliedBeforeResult) {
    ...
    try {
        ...
        if (existing) {
            const result = database.prepare(`UPDATE auth_tokens
         SET access_token = ?, refresh_token = ?, scope = ?, expires_at = ?, updated_at = ?
         WHERE id = ? AND platform = 'ebay'`).run(grant.accessToken, grant.refreshToken, grant.scope, grant.accessExpiresAt, preparation.expectedUpdatedAt, existing.id);
            if (result.changes !== 1)
                fail('EBAY_ROTATION_DATABASE_CAS_FAILED');
            rowId = existing.id;
        }
        else {
            const result = database.prepare(`INSERT INTO auth_tokens (
           platform, access_token, refresh_token, scope, expires_at, created_at, updated_at
         ) VALUES ('ebay', ?, ?, ?, ?, ?, ?)`).run(grant.accessToken, grant.refreshToken, grant.scope, grant.accessExpiresAt, preparation.expectedUpdatedAt, preparation.expectedUpdatedAt);
    ...
    try {
        ...
        if (commitAttempted)
            ...
            operationError = new Error('database close failed');
    }
    if (operationError !== undefined) {
        if (operationError instanceof EbayRotationError)
            throw operationError;
...
async function classifyCommitError(preparation, grant, rowId) {
    try {
...
function requireBoundRow(databasePath, binding, allowCommitPendingRowId = false) {
...
async function verifyCommittedReadback(preparation, grant, rowId) {
    const database = openReadOnlyDatabase(preparation.databasePath);
    try {
        const rows = readAuthRows(database);
        const installed = rows.find((row) => row.platform === 'ebay');
...
function recordWith(record, update) {
...
export async function beginEbayProductionConsent(input) {
    ...
    try {
        await writeNewPrivateFile(path.join(workDirectory, STATE_FILE), `${JSON.stringify(record)}\n`, input.dependencies);
        await writeNewPrivateFile(path.join(workDirectory, CONSENT_FILE), `${consent.toString()}\n`, input.dependencies);
    }
    catch (error) {
...
export async function archiveAndResetEbayProductionConsent(input) {
    ...
    return withOperationLock(workDirectory, input.dependencies, async () => {
        ...
        if (!/^[0-9a-f]{24}$/u.test(suffix))
            fail('EBAY_ROTATION_STATE_INVALID');
        const archiveContainer = path.join(archiveDirectory, `ebay-consent-${now.toISOString().replace(/[:.]/gu, '-')}-${suffix}`);
        ...
        await writeNewPrivateFile(path.join(stagingDirectory, STATE_FILE), `${JSON.stringify(fresh)}\n`, input.dependencies);
        try {
            const archivedEvidence = path.join(archiveContainer, 'evidence');
            await durableRename(workDirectory, archivedEvidence, input.dependencies, 'evidence-archive-target', 'evidence-archive-source');
            await durableRename(stagingDirectory, workDirectory, input.dependencies, 'reset-publish');
...
function defaultIsLockOwnerAlive(pid) {
    try {
        process.kill(pid, 0);
        return true;
    }
...
export async function recoverStaleEbayOperationLock(input) {
    ...
    if (!/^[0-9a-f]{24}$/u.test(suffix))
        fail('EBAY_ROTATION_STATE_INVALID');
    const archiveContainer = path.join(archiveDirectory, `ebay-lock-${now.toISOString().replace(/[:.]/gu, '-')}-${suffix}`);
    await createPrivateDirectory(archiveContainer, input.dependencies);
    const archivedLock = path.join(archiveContainer, 'operation-lock.json');
    try {
        await durableRename(lockPath, archivedLock, input.dependencies, 'stale-lock-archive-target', 'stale-lock-archive-source');
...
export async function ensureEbayProductionPrivateParents() {
...
export async function installEbayProductionGrant(input) {
    ...
    return withOperationLock(workDirectory, input.dependencies, async () => {
        ...
        if (record.status !== 'pending')
            fail('EBAY_ROTATION_STATE_ALREADY_USED');
        const now = nowProvider();
        ...
        let committedRowId = null;
        let commitOutcomeEffects = null;
        ...
        try {
            ...
            await writeConsentRecord(workDirectory, record, input.dependencies);
            const commitResult = await commitLedgerGrant(preparation, grant, input.dependencies?.beforeLedgerCas, input.dependencies?.afterCommitAppliedBeforeResult);
        ...
        if (commitPending) {
            record = recordWith(record, {
                ...
                installation: Object.freeze({
                    ...record.installation,
                    rowId: row.id,
                    committedAtUtc: canonicalTime((input.dependencies?.now ?? (() => new Date()))()),
                }),
            });
        }
        const metadata = await introspect(transport, input.credentials, row.refresh_token, 'refresh_token');
