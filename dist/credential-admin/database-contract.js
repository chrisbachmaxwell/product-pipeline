export const EXPECTED_AUTH_TOKEN_COLUMNS = Object.freeze([
    Object.freeze({
        cid: 0,
        name: 'id',
        type: 'INTEGER',
...
function result(stage, inspection) {
...
function columnsAreCanonical(columns) {
 ...
 */
export function inspectCanonicalAuthTokensSchema(database) {
    const inspection = {
    ...
    try {
        const tables = database.prepare(`SELECT type, name, tbl_name, sql
       FROM sqlite_schema WHERE name = 'auth_tokens'`).all();
    ...
    if (!inspection.tableDefinitionCanonical)
        return result('table-definition', inspection);
    try {
        const entries = database.pragma('table_list').filter((entry) => entry.schema === 'main' && entry.name === 'auth_tokens');
    ...
    if (!inspection.tableStorageCanonical)
        return result('table-storage', inspection);
    try {
        const columns = database.pragma('table_xinfo(auth_tokens)');
        inspection.columnsCanonical = columnsAreCanonical(columns);
    }
    catch {
        return result('columns', inspection);
    }
    if (!inspection.columnsCanonical)
        return result('columns', inspection);
    try {
        ...
        if (indexes.length === 1
            ...
            && index.origin === 'u'
            && index.partial === 0) {
            const indexSchema = database.prepare(`SELECT type, name, tbl_name, sql
         FROM sqlite_schema WHERE name = 'sqlite_autoindex_auth_tokens_1'`).all();
            const columns = database.pragma(`index_xinfo('sqlite_autoindex_auth_tokens_1')`);
    ...
    if (!inspection.uniquePlatformIndexCanonical)
        return result('index', inspection);
    try {
    ...
    if (!inspection.foreignKeysAbsent)
        return result('foreign-keys', inspection);
    try {
        database.prepare(`EXPLAIN ${SHOPIFY_ACCESS_TOKEN_COMPARE_AND_SWAP_SQL}`).all('replacement-token-shape', 'read_fulfillments,read_inventory,read_orders,read_products', 2, 1, 'current-token-shape', null, 1, 1);
