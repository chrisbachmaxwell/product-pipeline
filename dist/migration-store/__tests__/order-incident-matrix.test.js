...
import { INTENT_ACTIONS, INTENT_ACTION_RESPONSIBILITY, WRITER_RESPONSIBILITIES, createMigrationStore, deriveIdempotencyKey, sha256Digest, } from '../index.js';
const SANDBOX_SCOPE = {
...
const at = (seconds) => new Date(baseEpoch + seconds * 1000).toISOString();
const digest = (label) => sha256Digest(`fixture:${label}`);
const audit = (eventId, seconds) => ({ eventId, occurredAtUtc: at(seconds) });
async function fixtureStore(scope = SANDBOX_SCOPE) {
...
function createOrderIntent(lane) {
...
function approveOrder(lane, intentKey, expiresAtSeconds = 30) {
...
function reserveOrder(lane, intentKey, approval) {
...
describe('duplicate-order incident safeguards', () => {
    ...
    it('requires the immutable creationDate cutoff and never admits history at or before it', async () => {
        ...
        expect(() => lane.store.advanceOrderCursor({
    ...
    it('makes one order intent natural and reserves approval plus observation exactly once', async () => {
        ...
        expect(() => lane.store.reserveExecutionJob({
        ...
        expect(() => reserveOrder(lane, intentKey, approval)).toThrow(/historical, resolved, linked|durable constraints/i);
    ...
    it('denies dispatch when Marketplace Connect linkage appears after reservation', async () => {
        ...
        expect(() => lane.store.markDispatchingOutcomeUnknown({
            jobId: 'job-order-new',
        ...
        try {
          ...
          ) VALUES (?, 'job-order-new', 2, 'reserved', 'dispatching', ?, ?, ?)`).run('raw-dispatch', digest('raw-dispatch'), at(19), Date.parse(at(19)))).toThrow(/already linked|dispatch lacks authority/i);
        }
        finally {
            raw.close();
        }
    ...
    it('persists outcome_unknown before dispatch and requires fresh authoritative reconciliation', async () => {
        ...
        expect(() => lane.store.resolveUnknownAttempt({
            jobId: 'job-order-new',
            attemptId: 'attempt-order-new',
    ...
    it('blocks raw replacement on alternate unique order identity and natural intent keys', async () => {
        ...
        try {
            ...
            FROM order_links WHERE link_id = 'link-one'`).run('link-two', lane.newOrderKey)).toThrow(/replacement denied/i);
            ...
            FROM idempotency_intents WHERE intent_key = ?`).run(digest('alternate-intent-key'), digest('alternate-state'), intentKey)).toThrow(/replacement denied/i);
            expect(() => raw.prepare('INSERT OR REPLACE INTO audit_events SELECT * FROM audit_events WHERE sequence = 1')
                .run()).toThrow(/replacement denied/i);
        }
        finally {
            raw.close();
        }
