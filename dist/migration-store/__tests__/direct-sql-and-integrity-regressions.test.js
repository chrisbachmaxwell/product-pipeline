...
function recordMarketplaceConnectBaseline(store) {
...
function establishWatermark(store, ownershipEvidenceDigest) {
...
function registerEbayOrder(store, suffix, occurredAtUtc) {
...
function registerShopifyOrder(store, suffix, occurredAtUtc) {
    return store.registerIdentity({
        platform: 'shopify',
        kind: 'order',
...
function seedEligibleOrderIntent(store) {
...
function transferOrderOwnershipToProductPipeline(store) {
    store.recordOwnershipVersion({
        responsibility: 'orderImport',
        version: 2,
        owner: 'paused',
...
describe('migration-store direct SQL and integrity regressions', () => {
    it('keeps the fixed watermark immutable and treats boundary equality as permanently ineligible', () => {
        ...
        try {
            ...
            expect(() => raw.prepare('UPDATE order_watermarks SET boundary_exclusive_epoch_ms = boundary_exclusive_epoch_ms + 1').run()).toThrow(/append-only/);
            expect(() => raw.prepare('DELETE FROM order_watermarks').run()).toThrow(/append-only/);
            expect(() => raw.prepare('INSERT OR REPLACE INTO order_watermarks SELECT * FROM order_watermarks').run()).toThrow(/already established|replacement denied/);
    ...
    it('rejects replace-style collisions on alternate identity, link, intent, job, and audit keys', () => {
        ...
        try {
            ...
            const countsBefore = {
                identities: raw.prepare('SELECT COUNT(*) AS count FROM external_identities').get().count,
                links: raw.prepare('SELECT COUNT(*) AS count FROM order_links').get().count,
                intents: raw.prepare('SELECT COUNT(*) AS count FROM idempotency_intents').get().count,
                jobs: raw.prepare('SELECT COUNT(*) AS count FROM execution_jobs').get().count,
                audit: raw.prepare('SELECT COUNT(*) AS count FROM audit_events').get().count,
            };
            expect(() => raw.prepare(`INSERT OR REPLACE INTO external_identities
         ...
         FROM external_identities WHERE identity_key = ?`).run(sha256Digest('replacement-external-identity'), otherEbayOrder.identityKey))
                .toThrow(/replacement denied/);
            expect(() => raw.prepare(`INSERT OR REPLACE INTO order_links (
         ...
         FROM order_links WHERE link_id = 'observed-link-one'`).run(source.identityKey)).toThrow(/replacement denied/);
            raw.prepare(`INSERT INTO order_pages (
         ...
         ) VALUES ('raw-repeat-page', ?, NULL, NULL, 'raw-repeat-cursor', ?, ?, ?, ?)`).run(source.scopeKey, sha256Digest('raw-repeat-cursor'), '2026-08-11T20:00:13.000Z', Date.parse('2026-08-11T20:00:13.000Z'), sha256Digest('raw-repeat-page'));
            raw.prepare(`INSERT INTO order_observations (
         ...
         ) VALUES ('raw-repeat-observation', 'raw-repeat-page', ?, ?, ?, ?, ?, 1, ?, ?)`).run(source.scopeKey, source.identityKey, '2026-08-11T20:00:00.001Z', Date.parse('2026-08-11T20:00:00.001Z'), Date.parse(WATERMARK_UTC), '2026-08-11T20:00:13.000Z', Date.parse('2026-08-11T20:00:13.000Z'));
            expect(() => raw.prepare(`INSERT OR REPLACE INTO idempotency_intents (
         ...
         FROM idempotency_intents WHERE intent_key = ?`).run(sha256Digest('replacement-order-intent'), sha256Digest('different-order-payload-must-not-create-new-intent'), intentKey)).toThrow(/replacement denied/);
            expect(() => raw.prepare(`INSERT OR REPLACE INTO execution_jobs (
           ...
           target_identity_key, ownership_version, approval_evidence_digest,
           order_observation_id, reserved_at_utc, reserved_epoch_ms
         FROM execution_jobs WHERE job_id = 'job-order-one'`).run()).toThrow(/replacement denied/);
            const auditHead = raw.prepare('SELECT sequence, scope_key, event_hash, occurred_at_utc, occurred_epoch_ms FROM audit_events ORDER BY sequence DESC LIMIT 1').get();
            expect(() => raw.prepare(`INSERT OR REPLACE INTO audit_events (
           sequence, scope_key, event_id, event_type, occurred_at_utc, occurred_epoch_ms,
           payload_digest, previous_hash, event_hash
         ) VALUES (?, ?, 'raw-audit-replacement', 'raw.test', ?, ?, ?, ?, ?)`).run(auditHead.sequence + 1, auditHead.scope_key, auditHead.occurred_at_utc, auditHead.occurred_epoch_ms, sha256Digest('raw-audit-payload'), auditHead.event_hash, auditHead.event_hash)).toThrow(/replacement denied/);
            expect({
                identities: raw.prepare('SELECT COUNT(*) AS count FROM external_identities').get().count,
                links: raw.prepare('SELECT COUNT(*) AS count FROM order_links').get().count,
                intents: raw.prepare('SELECT COUNT(*) AS count FROM idempotency_intents').get().count,
    ...
    it('rejects malformed sha256 values at the SQL boundary', () => {
        ...
        const raw = new Database(databasePath);
        try {
            const head = raw.prepare('SELECT sequence, scope_key, event_hash, occurred_at_utc, occurred_epoch_ms FROM audit_events').get();
            expect(() => raw.prepare(`INSERT INTO audit_events (
           sequence, scope_key, event_id, event_type, occurred_at_utc, occurred_epoch_ms,
           payload_digest, previous_hash, event_hash
         ) VALUES (?, ?, 'malformed-digest', 'raw.test', ?, ?, ?, ?, ?)`).run(head.sequence + 1, head.scope_key, head.occurred_at_utc, head.occurred_epoch_ms, `sha256:${'g'.repeat(64)}`, head.event_hash, sha256Digest('otherwise-valid-event-hash'))).toThrow(/CHECK constraint failed/);
    ...
    it('denies order-creation relationship forgery even if a raw client disables foreign keys', () => {
        ...
        try {
            ...
            expect(() => raw.prepare(`INSERT INTO action_approvals (
         ...
         ) VALUES (?, ?, ?, 'orderImport', ?, 3, ?, ?, ?, ?, ?)`).run(approvalDigest, source.scopeKey, intentKey, wrongTarget.identityKey, '2026-08-11T20:00:08.000Z', Date.parse('2026-08-11T20:00:08.000Z'), '2026-08-11T20:10:08.000Z', Date.parse('2026-08-11T20:10:08.000Z'), sha256Digest('forged-approval-evidence'))).toThrow(/intent|target|binding/i);
            expect(() => raw.prepare(`INSERT INTO order_links (
           link_id, scope_key, ebay_order_identity_key, shopify_order_identity_key,
           link_kind, idempotency_intent_key, evidence_digest, linked_at_utc, linked_epoch_ms
         ) VALUES ('forged-created-link', ?, ?, ?, 'product_pipeline_created', ?, ?, ?, ?)`).run(source.scopeKey, source.identityKey, wrongTarget.identityKey, sha256Digest('nonexistent-order-intent'), sha256Digest('forged-link-evidence'), '2026-08-11T20:00:08.000Z', Date.parse('2026-08-11T20:00:08.000Z'))).toThrow(/intent mismatch/);
        }
        finally {
            raw.close();
        }
