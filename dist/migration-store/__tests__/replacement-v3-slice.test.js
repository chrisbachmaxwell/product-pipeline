...
function registerVariant(store, tag, gid) {
...
function registerEbayIdentity(store, kind, externalId, occurredAtUtc = '2026-08-19T18:00:02.000Z') {
    return store.registerIdentity({
        platform: 'ebay',
        kind,
...
function recordNoIncumbentChain(store, responsibility) {
...
function recordVerifiedIncumbentChain(store, responsibility) {
    store.recordOwnershipVersion({
        responsibility,
        version: 1,
    ...
    it('permits the watermark only under ProductPipeline ownership and the one-hour clamp', () => {
        ...
        try {
            ...
            boundary_exclusive_epoch_ms, created_at_utc, created_epoch_ms
                ...
                .run(digest('raw-backfill-watermark'), scopeKey, productPipelineEvidence, digest('watermark-packet'), '2026-08-19T16:30:00.000Z', Date.parse('2026-08-19T16:30:00.000Z'), '2026-08-19T18:30:00.000Z', Date.parse('2026-08-19T18:30:00.000Z'))).toThrow(/no-backfill clamp denied/);
        }
        finally {
            raw.close();
        }
        ...
        for (const [identity, tag] of [[preOrder, 'pre'], [equalOrder, 'equal']]) {
            ...
            expect(reopened.isOrderEligible(tag === 'pre' ? '2026-08-19T10:00:00.000Z' : '2026-08-19T18:00:00.000Z')).toBe(false);
        }
...
describe('schema v3 keeps mapping, fulfillment, and feedback fully denied', () => {
    it('denies their production intents and ownership records', () => {
        ...
        for (const action of ['update_mapping', 'sync_fulfillment', 'sync_feedback']) {
...
describe('schema v3 upgrade path', () => {
    function createVersionedStoreFile(scope, createdAtUtc, throughVersion) {
    ...
    it('upgrades a verified v2 store to v3 and leaves it fully operable', () => {
        const databasePath = createVersionedStoreFile(PRODUCTION_SCOPE, '2026-08-19T17:00:00.000Z', 2);
    ...
    it('upgrades a verified v1 store directly to v3', () => {
        const databasePath = createVersionedStoreFile(PRODUCTION_SCOPE, '2026-08-19T17:00:00.000Z', 1);
        expect(upgradeMigrationStore({
            databasePath,
            expectedScope: PRODUCTION_SCOPE,
            appliedAtUtc: '2026-08-19T18:00:00.000Z',
