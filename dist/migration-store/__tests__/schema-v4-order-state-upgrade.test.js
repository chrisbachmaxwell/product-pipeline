...
function registerEbayIdentity(store, kind, externalId, occurredAtUtc) {
...
function registerShopifyIdentity(store, kind, externalId, occurredAtUtc) {
    return store.registerIdentity({
        platform: 'shopify',
        kind,
        bindingKey: `${kind}:${externalId}`,
...
function createPopulatedV3Store() {
    ...
    const populate = database.transaction(() => {
      ...
      ) VALUES (1, ?, ?, ?, ?, ?, ?, ?)`).run(scopeKey, PRODUCTION_SCOPE.shopifyStoreDomain, PRODUCTION_SCOPE.ebayEnvironment, PRODUCTION_SCOPE.ebaySellerId, PRODUCTION_SCOPE.ebayMarketplaceId, '2026-08-25T16:00:00.000Z', Date.parse('2026-08-25T16:00:00.000Z'));
      ...
      ) VALUES (1, ?, ?, 'scope.established', ?, ?, ?, 'GENESIS', ?)`).run(scopeKey, `scope:${scopeKey}`, '2026-08-25T16:00:00.000Z', Date.parse('2026-08-25T16:00:00.000Z'), genesisPayload, genesisHash);
        const insertOwnership = database.prepare(`INSERT INTO ownership_versions (
        ...
        for (const ownership of [
            ...
            insertOwnership.run(ownership.id, scopeKey, ownership.version, ownership.owner, ownership.evidence, ownership.at, Date.parse(ownership.at), ownership.at, Date.parse(ownership.at));
        }
        database.prepare(`INSERT INTO order_watermarks (
        ...
        '2026-08-25T17:00:00.000Z', ?)`).run(digest('order-import-watermark'), scopeKey, productPipelineEvidence, digest('accepted-watermark-evidence'), watermarkEpochMs, establishEpochMs);
        const insertIdentity = database.prepare(`INSERT INTO external_identities (
        ...
        insertIdentity.run(ebayOrderIdentityKey, scopeKey, 'ebay', 'order:ORDER-SAFE-001', 'ORDER-SAFE-001', null, PRODUCTION_SCOPE.ebayEnvironment, PRODUCTION_SCOPE.ebaySellerId, PRODUCTION_SCOPE.ebayMarketplaceId, '2026-08-25T17:01:00.000Z', Date.parse('2026-08-25T17:01:00.000Z'));
        insertIdentity.run(shopifyOrderIdentityKey, scopeKey, 'shopify', 'order:6000000000001', 'gid://shopify/Order/6000000000001', PRODUCTION_SCOPE.shopifyStoreDomain, null, null, null, '2026-08-25T17:02:00.000Z', Date.parse('2026-08-25T17:02:00.000Z'));
        database.prepare(`INSERT INTO order_pages (
        ...
        '2026-08-25T17:05:00.000Z', ?, ?)`).run(scopeKey, digest('cursor-safe-001'), observedEpochMs, digest('order-page-safe-001'));
        database.prepare(`INSERT INTO order_observations (
        ...
        '2026-08-25T17:05:00.000Z', ?)`).run(scopeKey, ebayOrderIdentityKey, Date.parse('2026-08-25T16:45:00.000Z'), watermarkEpochMs, observedEpochMs);
        database.prepare(`INSERT INTO order_links (
        ...
        '2026-08-25T17:06:00.000Z', ?)`).run(scopeKey, ebayOrderIdentityKey, shopifyOrderIdentityKey, digest('existing-order-dedup-evidence'), Date.parse('2026-08-25T17:06:00.000Z'));
        database.prepare(`INSERT INTO order_observation_resolutions (
        ...
        '2026-08-25T17:07:00.000Z', ?)`).run(digest('linked-existing-resolution'), Date.parse('2026-08-25T17:07:00.000Z'));
    });
    populate.immediate();
    database.close();
...
function seedCompletedCeremoniesIntoV3(databasePath, directory) {
...
function snapshotOrderSafetyState(databasePath) {
...
describe('schema v3 to v4 populated order-state upgrade', () => {
    it('preserves order safety, cursor, and dedup rows while retaining permanent clamps', () => {
        ...
        })).toEqual({ fromVersion: 3, toVersion: 6 });
        ...
        try {
            ...
            expect(() => raw.prepare(`INSERT INTO order_watermarks SELECT * FROM order_watermarks`).run()).toThrow(/already established/);
            expect(() => raw.prepare(`INSERT INTO order_links SELECT
         ...
         FROM order_links WHERE link_id = 'link:ORDER-SAFE-001'`).run()).toThrow();
        }
        finally {
            raw.close();
        }
    ...
    it('leaves the populated v3 store untouched when integrity preflight fails', () => {
        ...
        try {
            const head = corrupt.prepare('SELECT sequence, event_hash FROM audit_events ORDER BY sequence DESC LIMIT 1').get();
            corrupt.prepare(`INSERT INTO audit_events (
          sequence, scope_key, event_id, event_type, occurred_at_utc,
          occurred_epoch_ms, payload_digest, previous_hash, event_hash
        ) VALUES (?, ?, 'corrupt-audit', 'test.corruption',
          '2026-08-25T17:30:00.000Z', ?, ?, ?, ?)`).run(head.sequence + 1, deriveScopeKey(PRODUCTION_SCOPE), Date.parse('2026-08-25T17:30:00.000Z'), digest('corrupt-payload'), head.event_hash, digest('deliberately-wrong-event-hash'));
        }
        finally {
            corrupt.close();
        }
        ...
        try {
            ...
            expect(unchanged.prepare('SELECT version FROM schema_migrations ORDER BY version').all()).toEqual([{ version: 1 }, { version: 2 }, { version: 3 }]);
        }
        finally {
            unchanged.close();
        }
