...
import { createMigrationStore, deriveExternalIdentityKey, deriveScopeKey, MigrationStoreError, openMigrationStore, openMigrationStoreReadOnly, sha256Digest, } from '../../migration-store/index.js';
...
import { createEbayOrderReadAdapter, exchangeOrderImportEbayToken, EbayOrderReadError, EBAY_ORDER_TOKEN_SCOPES, } from '../ebay-order-adapter.js';
...
function createWorld() {
    ...
    const now = () => new Date(tickMs += 1_000);
    const setClock = (iso) => {
    ...
    const shopifyFetch = async (input, init) => {
        ...
        if (body.operationName === 'OrderImportCreate') {
            world.orderCreateCalls.push(body.variables.order);
            ...
            if (mode === 'success') {
                ...
                const sourceIdentifier = body.variables.order
                    .sourceIdentifier;
...
async function establishWatermark(world, boundary = BOUNDARY_UTC) {
...
function ebayOrderIdentityKey(orderId) {
...
describe('order-import operator CLI', () => {
    ...
    it('poll records eligible and permanently-ineligible observations and never a PII byte', async () => {
        ...
        expect(world.lastStdout().skipped
    ...
    it('imports exactly one order end to end and persists no PII byte', async () => {
        ...
        try {
            expect(store.getJobStatus(imported.jobId)).toMatchObject({
    ...
    it('requires the literal --confirm-lightspeed acknowledgement', async () => {
        ...
        await expect(program.parseAsync(['import', '--migration-store', world.migrationDatabasePath, '--order-id', ORDER_ID], { from: 'user' })).rejects.toThrowError(/confirm-lightspeed/);
        expect(world.orderCreateCalls).toHaveLength(0);
    });
    it('leaves orderCreate userErrors unresolved; only --accept-absent terminalizes', async () => {
        ...
        expect(world.exitCodes.at(-1)).toBe(1);
        const jobId = imported.jobId;
        const attemptId = imported.attemptId;
        const readState = () => {
            const store = openMigrationStoreReadOnly({
                databasePath: world.migrationDatabasePath,
                expectedScope: MIGRATION_SCOPE,
    ...
    it('resolves a created-but-not-yet-indexed order through reconcile once it appears', async () => {
        ...
        await world.run(['reconcile', '--migration-store', world.migrationDatabasePath,
            '--order-id', ORDER_ID,
            '--job-id', imported.jobId,
            '--attempt-id', imported.attemptId]);
        expect(world.lastStdout()).toMatchObject({
            command: 'reconcile',
            status: 'reconciled',
            outcome: 'resolved_existing',
    ...
    it('exchanges the transient token with exactly the fulfillment scope pair, fail closed', async () => {
        ...
        const exchangeFetch = (scopeEcho) => async (input, init) => {
        ...
        const token = await exchangeOrderImportEbayToken(auth, exchangeFetch(EBAY_ORDER_TOKEN_SCOPES.join(' ')));
        expect(token).toEqual({ accessToken: 'transient-user-token', expiresIn: 7200 });
        expect(requestedScopes[0]).toBe('https://api.ebay.com/oauth/api_scope https://api.ebay.com/oauth/api_scope/sell.fulfillment');
        // A broader or different scope echo fails closed: the token is unusable.
        await expect(exchangeOrderImportEbayToken(auth, exchangeFetch(`${EBAY_ORDER_TOKEN_SCOPES.join(' ')} https://api.ebay.com/oauth/api_scope/sell.inventory`))).rejects.toMatchObject({ code: 'ORDER_READ_AUTHORITY_UNAVAILABLE' });
        let caught;
        try {
    ...
    it('keeps the CLI free of server-mount, legacy-order, and out-of-adapter fetch usage', () => {
        ...
        expect(sliceSources).not.toMatch(/from ['"][^'"]*(?:\/sync\/|order-sync|order-safety|product-sync|inventory-sync|price-sync|sync-helper|shopify\/orders\.js|token-manager)[^'"]*['"]/);
