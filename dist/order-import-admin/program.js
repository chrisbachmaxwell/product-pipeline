...
import { createEbayOrderReadAdapter, createProductionOrderReadTokenProvider, EbayOrderReadError, } from './ebay-order-adapter.js';
import { createShopifyOrderAdapter, createProductionShopifyOrderTokenProvider, ShopifyOrderAdapterError, } from './shopify-order-adapter.js';
...
function safeErrorCode(error) {
    if (error instanceof OrderImportAdminError)
        return error.code;
    if (error instanceof EbayOrderReadError)
        return error.code;
...
function ebayOrderIdentityInput(orderId) {
    return {
        platform: 'ebay',
        kind: 'order',
...
function shopifyOrderIdentityInput(orderGid) {
    return {
        platform: 'shopify',
        kind: 'order',
...
async function runOrderPostVerify(input) {
    ...
    if (matchedGid !== null) {
        const shopifyOrderIdentityKey = ensureIdentity(input.store, shopifyOrderIdentityInput(matchedGid), input.clock());
...
export function buildOrderImportAdminProgram(dependencies = {}) {
    ...
    const createEbayAdapter = dependencies.createEbayAdapter ?? (() => createEbayOrderReadAdapter({
        getAccessToken: createProductionOrderReadTokenProvider(),
    }));
    const createShopifyAdapter = dependencies.createShopifyAdapter ?? (() => createShopifyOrderAdapter({
    ...
    program
        ...
        .description('Record the orderImport ownership chain once: marketplace_connect genesis, '
        ...
        .requiredOption('--baseline-evidence <sha256>', 'Digest of the verified Marketplace Connect incumbent baseline evidence')
        .requiredOption('--mc-disabled-evidence <sha256>', 'Digest of the captured proof that Marketplace Connect order import is OFF')
        .action((options) => {
        try {
            ...
            const baselineEvidence = normalizedDigest(options.baselineEvidence, 'OWNERSHIP_EVIDENCE_DIGEST_INVALID');
            const mcDisabledEvidence = normalizedDigest(options.mcDisabledEvidence, 'OWNERSHIP_EVIDENCE_DIGEST_INVALID');
    ...
    program
        ...
        .description('Establish the one immutable production order watermark (one per scope, forever). '
        ...
        .requiredOption('--migration-store <path>', 'Absolute migration-state database path')
        .requiredOption('--confirm-scope <sha256>', 'Exact migration scope key confirming the store')
        .requiredOption('--boundary <iso8601-utc>', 'Exclusive boundary instant, canonical UTC (e.g. 2026-08-19T18:00:00.000Z)')
        .requiredOption('--accepted-evidence <sha256>', 'Digest of the reviewed and accepted watermark packet')
        .action((options) => {
        try {
            ...
            const acceptedEvidence = normalizedDigest(options.acceptedEvidence, 'WATERMARK_EVIDENCE_DIGEST_INVALID');
            const store = openMigration({
                databasePath: options.migrationStore,
                expectedScope: MIGRATION_SCOPE,
            });
    ...
    program
        .command('poll')
        .description('READ-ONLY eBay poll starting exactly at the immutable watermark: record new order '
        ...
        try {
            ...
            try {
                ...
                if (!watermark)
                    deny('POLL_WATERMARK_REQUIRED');
                const boundary = watermark;
                const ebay = createEbayAdapter();
                const fetched = await ebay.listOrdersCreatedSince(boundary.boundaryExclusiveUtc, maxOrders);
                const skipped = [];
                const fresh = [];
                ...
                if (pendingPage) {
                    if (pendingPage.observedCount === pendingPage.resolvedCount) {
                        ...
                        io.stdout(JSON.stringify({
                            ...
                            pendingUnresolvedObservations: pendingPage.observedCount - pendingPage.resolvedCount,
    ...
    program
        ...
        .requiredOption('--confirm-lightspeed', 'Literal acknowledgement that this Shopify order creation cascades into Lightspeed POS')
        .action(async (options) => {
        try {
            if (options.confirmLightspeed !== true)
                deny('IMPORT_LIGHTSPEED_NOT_CONFIRMED');
            ...
            try {
                ...
                if (!observation)
                    deny('IMPORT_OBSERVATION_NOT_FOUND');
                ...
                const ebay = createEbayAdapter();
                const fresh = await ebay.getOrder(orderId);
                if (!store.isOrderEligible(fresh.creationDateUtc))
                    deny('IMPORT_ORDER_NOT_ELIGIBLE');
                ...
                for (const line of fresh.lineItems) {
                    if (line.sku === null)
                        deny('IMPORT_SKU_UNRESOLVED');
                    const variantGid = await shopify.findVariantGidBySku(line.sku);
                    if (variantGid === null)
                        deny('IMPORT_SKU_UNRESOLVED');
                    resolvedLines.push({
                        sku: line.sku,
                ...
                if (shipping !== null) {
                    ...
                    if (shipping.email !== null)
                        orderInput.email = shipping.email;
                }
                ...
                if (!scopes.includes('write_orders'))
                    deny('IMPORT_SHOPIFY_WRITE_SCOPE_MISSING');
                ...
                if (store.getIntent(intentKey) !== null)
                    deny('IMPORT_INTENT_ALREADY_RECORDED');
                const createdAtUtc = clock();
                ...
                io.stdout(JSON.stringify({
                    ...
                    ...(verification.outcome === 'unresolved' || verification.outcome === 'verify_failed'
                        ? {
                            exceptionCode: verification.exceptionCode,
    ...
    program
        .command('reconcile')
        .description('Re-run the zero-write post-dispatch verification for an outstanding '
        ...
        .option('--accept-absent', 'Explicitly accept a still-absent Shopify order as the terminal confirmed_missing outcome')
        .action(async (options) => {
        try {
            const orderId = requireOrderId(options.orderId);
            const store = openMigration({
            ...
            try {
                ...
                io.stdout(JSON.stringify({
                    ...
                    attemptId: options.attemptId,
                    outcome: verification.outcome,
                    reconciliationRunId: verification.runId,
