 ...
 */
import { createTransientEbayTokenProvider, } from '../server/live-listing-catalog-source.js';
...
function optionalPassthroughText(value, maximum = 512) {
...
function money(value) {
    if (value === null || value === undefined || typeof value !== 'object')
        return null;
    const record = value;
    if (typeof record.value !== 'string' || typeof record.currency !== 'string')
        return null;
 ...
 */
function extractOrder(raw, includeShipping) {
...
export function createEbayOrderReadAdapter(dependencies) {
    ...
    return Object.freeze({
        listOrdersCreatedSince: async (sinceUtc, maxOrders) => {
        ...
        getOrder: async (orderId) => {
            ...
            const headers = await authorizedHeaders();
            const body = await boundedFetchJson(fetchImpl, `${EBAY_API_HOST}/sell/fulfillment/v1/order/${encodeURIComponent(orderId)}`, { method: 'GET', headers });
...
function safeTokenText(value, maximum) {
    return typeof value === 'string' && value.length > 0 && value.length <= maximum
        ? value
        : deny('ORDER_READ_AUTHORITY_UNAVAILABLE');
}
function safeTokenInteger(value, minimum) {
 ...
 */
export async function exchangeOrderImportEbayToken(auth, fetchImpl = fetch) {
...
async function readOrderImportAuthMaterial() {
 ...
 */
export function createProductionOrderReadTokenProvider() {
