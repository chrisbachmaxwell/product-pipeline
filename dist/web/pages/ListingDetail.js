...
const ListingDetail = () => {
    ...
    const publishReady = Boolean(currentCatalog?.lifecycleStatus === 'not_listed'
        && currentEditEligible
        && draftRevision
        && currentCatalog?.shopify);
    const runPublish = async () => {
        if (!currentCatalog?.shopify || !draftRevision || !id)
            return;
        setPublishing(true);
        setPublishResult(null);
        try {
            const result = await apiClient.post('/listing-publish', {
                catalogId: id,
    ...
    return (_jsx(Page, { title: title || 'Listing', subtitle: listingSkuLabel(sku), backAction: { content: 'Listings', url: '/listings' }, titleMetadata: (_jsx(InlineStack, { gap: "200", blockAlign: "center", children: _jsx(Badge, { tone: listingStatusTone(catalog.lifecycleStatus), children: listingStatusLabel(catalog.lifecycleStatus) }) })), primaryAction: publishReady ? {
        ...
        ], children: _jsxs(BlockStack, { gap: "400", children: [_jsx(InlineStack, { align: "end", children: _jsxs(Text, { as: "span", variant: "bodySm", tone: "subdued", children: [formatVerifiedAt(observedAt), " \u00B7 refreshes every minute"] }) }), publishResult && (_jsx(Banner, { tone: publishResult.ok ? 'success' : 'critical', onDismiss: () => setPublishResult(null), children: publishResult.ok ? (_jsxs(Text, { as: "p", children: ["Published to eBay.", publishResult.listingId
                                ? ` Listing ${publishResult.listingId} is live — it will appear here on the next refresh.`
                                : ' It will appear here on the next refresh.'] })) : (_jsx(Text, { as: "p", children: publishResult.message })) })), publishConfirmOpen && (_jsx(Modal, { open: true, onClose: () => setPublishConfirmOpen(false), title: "Publish to eBay?", primaryAction: { content: 'Publish', loading: publishing, onAction: () => { void runPublish(); } }, secondaryActions: [{ content: 'Cancel', onAction: () => setPublishConfirmOpen(false) }], children: _jsx(Modal.Section, { children: _jsxs(BlockStack, { gap: "200", children: [_jsxs(Text, { as: "p", children: ["This creates a live eBay listing for \u201C", title, "\u201D using the saved draft", currentCatalog?.shopify
                                            ? ` at $${currentCatalog.shopify.price.amount} · ${currentCatalog.shopify.available ?? 0} available`
                                            : '', "."] }), _jsx(Text, { as: "p", tone: "subdued", children: "Buyers can purchase immediately. You can end the listing later from eBay." })] }) }) })), priceCheckOpen && (_jsx(Card, { children: _jsxs(BlockStack, { gap: "300", children: [_jsxs(InlineStack, { align: "space-between", blockAlign: "center", children: [_jsx(Text, { as: "h3", variant: "headingMd", children: "Market prices" }), _jsx(Button, { variant: "plain", onClick: () => setPriceCheckOpen(false), children: "Close" })] }), priceCheck.isLoading ? (_jsx(Text, { as: "p", tone: "subdued", children: "Checking current eBay listings\u2026" })) : priceCheck.error || !priceCheck.data ? (_jsx(Text, { as: "p", tone: "subdued", children: "Market prices are unavailable right now. Try again in a minute." })) : priceCheck.data.sampleSize === 0 ? (_jsx(Text, { as: "p", tone: "subdued", children: "No comparable active listings found." })) : (_jsxs(BlockStack, { gap: "200", children: [_jsxs(Text, { as: "p", children: [priceCheck.data.sampleSize, " comparable active listings \u00B7", ' ', "median ", priceCheck.data.median ? `$${priceCheck.data.median}` : '—'] }), priceCheck.data.comps.slice(0, 6).map((comp, index) => (_jsxs(InlineStack, { align: "space-between", blockAlign: "center", gap: "300", children: [_jsx(Text, { as: "span", variant: "bodySm", truncate: true, children: comp.title }), _jsxs(Text, { as: "span", variant: "bodySm", fontWeight: "semibold", children: ["$", comp.price.value] })] }, index))), _jsx(Text, { as: "p", variant: "bodySm", tone: "subdued", children: "Live eBay search for similar items \u2014 a sanity check, not an appraisal." })] }))] }) })), id && (_jsx(ListingDescriptionPreviewModal, { catalogId: id, open: descriptionPreviewOpen, hasUnsavedChanges: draftState.dirty, draftDescriptionHtml: draftState.descriptionHtml, onClose: () => setDescriptionPreviewOpen(false) })), editing && canEdit && validDraft ? (_jsx(ListingDraftEditor, { draft: validDraft, saving: saveDraft.isPending, onDraftStateChange: setDraftState, statusCard: (_jsx(Card, { children: _jsxs(InlineStack, { align: "space-between", blockAlign: "center", gap: "300", children: [_jsxs(BlockStack, { gap: "050", children: [_jsx(Text, { as: "h2", variant: "headingMd", children: publishReady ? 'Ready to publish' : 'Not on eBay yet' }), _jsx(Text, { as: "p", variant: "bodySm", tone: "subdued", children: publishReady
