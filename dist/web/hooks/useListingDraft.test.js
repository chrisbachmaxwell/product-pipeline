...
import { canonicalDraftImages, canonicalDraftItemSpecifics, effectiveDraftImages, isListingDraftBoundToWorkspace, isListingDraftResponse, isListingDraftSaveInput, parseDraftImages, verifiedDraftImageUrl, } from './useListingDraft';
import { buildListingDraftSaveInput, initialDraftValues, isSemanticImageChange, isSemanticScalarChange, } from '../components/ListingDraftEditor';
...
describe('local listing draft UI contract', () => {
    ...
    it('binds draft identity to the exact trusted workspace', () => {
        ...
        const tradingWorkspace = {
            ...workspace(),
            mapping: { ...workspace().mapping, managementModel: 'legacy_trading' },
        };
        const tradingDraft = {
            ...draft(),
            identity: {
                ...draft().identity,
                managementModel: 'trading_api',
    ...
    it('parses images without exposing arbitrary origins or tokenized URLs', () => {
        ...
        expect(verifiedDraftImageUrl('https://cdn.shopify.com/a.jpg')).toBe('https://cdn.shopify.com/a.jpg');
        expect(verifiedDraftImageUrl('https://user:pass@cdn.shopify.com/a.jpg')).toBeNull();
        expect(verifiedDraftImageUrl('https://cdn.shopify.com/a.jpg?v=123')).toBe('https://cdn.shopify.com/a.jpg?v=123');
    ...
    it('canonicalizes bounded item specifics for exact draft approval', () => {
        expect(canonicalDraftItemSpecifics('{"Type":["Lens"],"Brand":["Canon"]}')).toBe('{"Brand":["Canon"],"Type":["Lens"]}');
    ...
    it('freezes the edit CAS basis instead of advancing with a refetch', () => {
        ...
        const newerDigest = `sha256:${'b'.repeat(64)}`;
        const refetched = {
            ...editBase,
            ...
            revision: {
                ...
                state: 'draft', createdAtUtc: '2026-08-13T18:11:00.000Z',
            },
        };
    ...
    it('rejects unsafe HTML draft descriptions and keeps remote/local scope explicit', () => {
        ...
        const page = readFileSync(fileURLToPath(new URL('../pages/ListingDetail.tsx', import.meta.url)), 'utf8');
        const editor = readFileSync(fileURLToPath(new URL('../components/ListingDraftEditor.tsx', import.meta.url)), 'utf8');
    ...
    it('treats exact inherited scalar and image values as no change', () => {
        ...
        expect(isSemanticImageChange(inheritedImageField, true, canonicalDraftImages(['https://cdn.shopify.com/different.jpg']))).toBe(true);
