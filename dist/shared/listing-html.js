...
const ALLOWED_TAG_PATTERN = new RegExp('^(?:'
    ...
    + '|<\\/a>'
    + ')$', 'u');
export const isAllowlistedListingHtml = (value) => {
...
const HEADING_FOLD = Object.freeze({
    h1: 'h2', h4: 'h3', h5: 'h3', h6: 'h3',
});
const SAFE_HREF = /^https?:\/\/[^\s"<>]+$/u;
export const sanitizeListingHtml = (value) => {
    const sanitized = value
        .replace(/<!--[\s\S]*?-->/gu, ' ')
        .replace(DROP_WITH_CONTENT, ' ')
        .replace(/<\/?([a-zA-Z][a-zA-Z0-9-]*)((?:"[^"]*"|'[^']*'|[^>"'])*)>/gu, (tag, rawName, rawAttributes) => {
