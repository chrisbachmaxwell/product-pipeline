...
export async function uploadToShopify(productId, imageBuffers) {
    ...
    const result = await replaceProductImages(tokenRow.access_token, creds.storeDomain, productId, imageBuffers.map((buffer, i) => ({ buffer, filename: `processed-${productId}-${i + 1}.png` })));
    info(`[ImageProcessor] Uploaded ${result.uploaded}/${imageBuffers.length} processed images to product ${productId} (${result.failed} failed)`);
