...
import { createOrReplaceInventoryItem, createOffer, publishOffer, getInventoryItem, getLocation, createOrUpdateLocation, getOffersBySku, deleteOffer, getBusinessPolicies, } from '../ebay/inventory.js';
import { getDb } from '../db/client.js';
import { productMappings, syncLog } from '../db/schema.js';
import { eq } from 'drizzle-orm';
import { info, error as logError } from '../utils/logger.js';
import { getEbayCondition, getEbayUPC, getEbayTitle, getEbayDescription, getEbayHandlingTime, getMapping, resolveMapping } from '../sync/attribute-mapping-service.js';
 ...
 */
const mapShopifyProductToEbay = async (shopifyProduct, variant, settings) => {
 ...
 */
const syncProductToEbay = async (ebayToken, shopifyToken, productId, settings, options = {}) => {
    try {
...
export const syncProducts = async (ebayToken, shopifyToken, productIds, settings = {}, options = {}) => {
    ...
    for (const productId of productIds) {
        result.processed++;
        const syncResult = await syncProductToEbay(ebayToken, shopifyToken, productId, settings, options);
