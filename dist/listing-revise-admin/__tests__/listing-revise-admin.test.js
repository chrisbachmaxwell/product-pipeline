...
import { applyListingDescriptionTemplate, buildListingRevisePayloads, deriveListingReviseManifest, ListingReviseManifestError, ListingRevisePayloadError, } from '../manifest.js';
...
async function createWorld() {
    ...
    const adapterCalls = [];
    const putItemPayloads = [];
    const putOfferPayloads = [];
    let putsFail = false;
...
describe('listing-revise operator CLI', () => {
    ...
    it('applies opt-in description templating with a manifest digest binding the exact HTML', async () => {
        ...
        const plainDigest = plain.manifestDigest;
        ...
        const dispatchedDescription = world.putOfferPayloads[0].listingDescription;
        expect(dispatchedDescription.startsWith('<!-- template:ucg-branded-v2 -->')).toBe(true);
    ...
    it('passes manifests without a description change through templating unchanged', async () => {
        ...
        const untouched = applyListingDescriptionTemplate({
            derived, revision: world.revision, templateVersion: 'ucg-branded-v2',
        });
    ...
    it('keeps the CLI free of server-mount and legacy writer imports', () => {
        ...
        const adapterSource = fs.readFileSync(path.join(sourceRoot, 'listing-revise-admin/dispatch-adapter.ts'), 'utf8');
        const tradingAdapterSource = fs.readFileSync(path.join(sourceRoot, 'listing-revise-admin/trading-dispatch-adapter.ts'), 'utf8');
        const manifestSource = fs.readFileSync(path.join(sourceRoot, 'listing-revise-admin/manifest.ts'), 'utf8');
        ...
        expect(`${program}\n${adapterSource}\n${tradingAdapterSource}\n${manifestSource}`).not.toMatch(/from ['"][^'"]*(?:\/sync\/|order-sync|product-sync|inventory-sync|price-sync|token-manager)[^'"]*['"]/);
