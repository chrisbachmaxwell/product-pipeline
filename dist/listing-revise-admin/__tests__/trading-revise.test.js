...
import { buildReviseFixedPriceItemXml, createTradingDispatchAdapter, TradingDispatchError, } from '../trading-dispatch-adapter.js';
...
async function createTradingWorld(draft = { title: 'Trading Operator Title', merchantLocation: null }) {
    ...
    const revisedWorkspace = () => tradingWorkspace({ ebayTitle: draft.title ?? 'eBay Trading Old' });
    ...
    draftStore.close();
    if (!revision)
        throw new Error('trading revision fixture was not created');
    ...
    const fakeFetch = async (input, init) => {
        ...
        if (responseAck === 'Success')
            current = revisedWorkspace();
        return new Response('<?xml version="1.0" encoding="UTF-8"?>'
            + '<ReviseFixedPriceItemResponse xmlns="urn:ebay:apis:eBLBaseComponents">'
            + `<Ack>${responseAck}</Ack>`
            + '</ReviseFixedPriceItemResponse>', { status: 200, headers: { 'Content-Type': 'text/xml' } });
    };
...
describe('trading-model listing-revise dispatch', () => {
    ...
    it('escapes every XML text value and never serializes price or quantity', () => {
        ...
        expect(xml).toContain('<Title>Nikon &amp; Canon &lt;50mm&gt; &quot;prime&quot; &#39;lens&#39;</Title>');
        ...
        expect(xml).toContain('<SellerProfiles>'
        ...
        for (const field of ['price', 'quantity', 'merchant_location', 'condition']) {
