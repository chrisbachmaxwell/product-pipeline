...
const PRODUCTION_SCOPE_DIGEST = 'sha256:f1f798163d3f7c7042825d998c9f2b6f3f0ad5f75794a9d12dd887daa7e8f54c';
...
describe('migration-admin strict local boundary', () => {
    ...
    it('ships one exact nonsecret Railway production configuration and root marker', () => {
        const productionConfigPath = path.join(process.cwd(), 'config', 'migration-state.production.json');
        const production = parseMigrationAdminConfig(JSON.parse(fs.readFileSync(productionConfigPath, 'utf8')));
        ...
        expect(dockerfile).toContain('ENV MIGRATION_STATE_CONFIG_PATH=/app/config/migration-state.production.json');
    ...
    it('initializes and verifies a store on an absolute durable path outside the repository', () => {
        const volume = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'product-pipeline-durable-volume-')));
        ...
        const durableDatabasePath = path.join(durableParent, 'product-pipeline-migration-v1.sqlite');
        const repository = temporaryRepository({
            ...validConfig(),
    ...
    it('rejects unsafe durable database paths without creating state', () => {
        ...
        const insidePath = path.join(fs.realpathSync(insideRepository.root), 'migration-state', 'product-pipeline-migration-v1.sqlite');
    ...
    it('upgrades only with the exact scope confirmation and reports already-current idempotently', async () => {
        ...
        expect(JSON.parse(stdout[0])).toMatchObject({
            ...
            schemaUpgrade: { fromVersion: 5, toVersion: 5 },
        });
    });
