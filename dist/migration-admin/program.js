...
class MigrationAdminPostconditionError extends Error {
    constructor() {
        super('Migration state may have been created but did not pass verification; do not retry init and run verify before any further action');
        this.name = 'MigrationAdminPostconditionError';
    }
}
function scopeSummary(loaded) {
    return {
...
function assertInertInitialization(projection) {
...
function safeProjection(projection) {
    const counts = projection.counts
        ...
        : null;
    const safeHeadHash = typeof projection.audit.headHash === 'string'
    ...
    return {
        ...
        readiness: {
            canaryReady: false,
            cutoverReady: false,
            blockers: projection.readiness.blockers.filter((entry) => /^[A-Za-z0-9-]{1,96}$/.test(entry)),
        },
    };
}
...
function printResult(result, json, io) {
    ...
    io.stdout('Safety: local-only; no credentials, platform access, external writes, historical backfill, ownership transfer, canary, or cutover');
    io.stdout(`Scope: ${result.scope.shopifyStoreDomain} / ${result.scope.ebayEnvironment}:${result.scope.ebayMarketplaceId}`);
    ...
    if (result.status === 'preview') {
        ...
        io.stdout(`Local audit: ${result.projection.audit.valid ? 'valid' : 'invalid'} (${result.projection.audit.recordCount} record(s))`);
...
export function buildMigrationAdminProgram(io = defaultIo) {
    const program = new Command();
    program
        .name('product-pipeline-migration-admin')
        .description('Local migration-state initialization and verification only; no platform or commerce adapter exists.')
        .version('0.1.0')
        .showHelpAfterError();
    program
        .command('init')
