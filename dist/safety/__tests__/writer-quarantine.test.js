...
import { handleInventoryWebhook, syncAllInventory, updateEbayInventory, } from '../../sync/inventory-sync.js';
import { applyPriceDropSchedule, enablePromotedListings, republishStaleListings, runListingManagement, } from '../../sync/listing-manager.js';
import { syncOrders } from '../../sync/order-sync.js';
import { syncPrices } from '../../sync/price-sync.js';
import { autoSyncNewProducts, endEbayListing, syncProducts, updateProductOnEbay, } from '../../sync/product-sync.js';
import { getMigrationPolicyStatus, MARKETPLACE_CONNECT_BASELINE, WriterQuarantinedError, writerQuarantineMiddleware, } from '../writer-quarantine.js';
afterEach(() => {
    vi.unstubAllGlobals();
    vi.unstubAllEnvs();
});
describe('writer quarantine policy', () => {
    ...
    it('default-denies every state-changing API method with a stable 423 response', () => {
        ...
        writerQuarantineMiddleware({ method: 'POST', originalUrl: '/api/sync/trigger?dry=false', path: '/sync/trigger' }, { status }, next);
...
describe('writer entry points fail before network or database work', () => {
    it('blocks low-level platform writers before fetch', async () => {
        ...
        const attempts = [
            ...
            () => createShippingFulfillment('x', 'order', {}),
            () => createShopifyOrder('x', {}),
    ...
    it('keeps mapped automatic dispatches absent from server and webhook sources', async () => {
        ...
        const ebayWebhook = await fs.readFile(path.join(sourceRoot, 'server/routes/ebay-notifications.ts'), 'utf8');
        const shopifyWebhook = await fs.readFile(path.join(sourceRoot, 'server/routes/shopify-webhooks.ts'), 'utf8');
        ...
        expect(shopifyWebhook).not.toMatch(/syncProducts|updateProductOnEbay|updateEbayInventory|createShippingFulfillment|autoListProduct/);
    ...
    it('keeps tracked executable artifacts aligned with the source quarantine', async () => {
        ...
        expect(server).not.toMatch(/shopifyAuthRoutes|ebayAuthRoutes|startSyncScheduler|startCloudWatcher|chatRoutes|pipelineRoutes/);
        expect(server).not.toMatch(/apiRoutes|ebayOrderRoutes|ebayMetadataRoutes|helpRoutes|featureRoutes|getDb|getRawDb|seedHelpArticles/);
        ...
        expect(cli).not.toMatch(/\.command\(['"](?:sync|import|publish|inventory|orders|products|watcher|pipeline|drafts|images|listings|tim)['"]\)/);
    });
