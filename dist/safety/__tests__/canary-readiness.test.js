...
import { evaluateCanaryReadiness, } from '../canary-readiness.js';
...
function validOrderInput() {
    ...
    input.evidence.responsibility = 'orderImport';
    input.evidence.targetKey = 'order-canary-1';
    input.singleWriter.responsibility = 'orderImport';
    input.singleWriter.targetKey = 'order-canary-1';
    input.approval.responsibility = 'orderImport';
    input.approval.targetKey = 'order-canary-1';
    input.approval.action = 'import-order';
    ...
    input.audit.targetKey = 'order-canary-1';
    input.audit.responsibility = 'orderImport';
    input.reconciliation.targetKey = 'order-canary-1';
    input.reconciliation.responsibility = 'orderImport';
    input.rollback.targetKey = 'order-canary-1';
    input.rollback.responsibility = 'orderImport';
...
describe('future canary readiness evaluator', () => {
    ...
    it('requires an exact listing store, environment, seller, and marketplace scope', () => {
        ...
        if (target.kind !== 'listing')
            throw new Error('listing fixture expected');
        target.shopifyStoreDomain = 'not-a-shopify-store.example.com';
        target.ebayEnvironment = 'staging';
        target.ebaySellerAccount = 'x';
        target.marketplaceId = 'EBAY_GB';
        expect(evaluateCanaryReadiness(input).blockers).toEqual(expect.arrayContaining([
    ...
    it('binds audit, reconciliation, and rollback evidence to the exact target, responsibility, and ownership version', () => {
        ...
        input.audit.auditDestination = 'unsafe-destination';
    ...
    it('permits a null eBay listing ID only for listingCreate', () => {
        ...
        create.approval.responsibility = 'listingCreate';
        create.approval.action = 'create-listing';
    ...
    it('rejects replaying an order idempotency key into another seller account', () => {
        ...
        if (target.kind !== 'order')
            throw new Error('order fixture expected');
