...
export const EVIDENCE_SIGNING_KEY_ENV = 'PRODUCT_PIPELINE_EVIDENCE_SIGNING_KEY_PKCS8_B64';
...
const FORBIDDEN_KEY_PATTERN = /(?:authorization|cookie|access[_-]?token|refresh[_-]?token|token|secret|password|credential|api[_-]?key|private[_-]?key|email|phone|address|customer|buyer|first[_-]?name|last[_-]?name|full[_-]?name|line[_-]?items?|raw(?:[_-]?json|[_-]?payload)?|checkout[_-]?note)/i;
const FORBIDDEN_VALUE_PATTERN = /(?:Bearer\s+[A-Za-z0-9._~+/-]{8,}|shpat_[A-Za-z0-9_-]{8,}|shpca_[A-Za-z0-9_-]{8,}|shppa_[A-Za-z0-9_-]{8,}|gh[pousr]_[A-Za-z0-9_-]{8,}|sk-[A-Za-z0-9_-]{10,}|v\^1\.[^\s]{8,}|-----BEGIN [A-Z ]*PRIVATE KEY-----)/i;
const EMAIL_VALUE_PATTERN = /(?:^|[^A-Za-z0-9._%+-])[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}(?:$|[^A-Za-z0-9.-])/i;
const MAX_ARTIFACT_BYTES = 8 * 1024 * 1024;
const ARTIFACT_FILENAME_PATTERN = /^(?:shopify|ebay|marketplace-connect)-[0-9]{8}T[0-9]{9}Z-[a-f0-9]{12}\.json$/;
...
function assertNoForbiddenMaterial(value, pathLabel = 'artifact') {
    ...
    for (const [key, child] of Object.entries(value)) {
        if (key === 'rawPayloadPersistence' && child === false)
            continue;
...
export function verifyEvidenceArtifact(input) {
    ...
    if (artifact.payload.schemaVersion !== 1
    ...
    if (!verifyBytes(null, Buffer.from(canonicalJson(artifact.payload), 'utf8'), publicKey, signature)) {
        throw new EvidenceArtifactError('signature verification failed');
    }
}
function assertExistingPrivateDirectory(loaded) {
...
export function readEvidenceArtifact(input) {
    ...
    if (path.isAbsolute(input.requestedArtifactPath)
    ...
    const descriptor = fs.openSync(absolutePath, fsConstants.O_RDONLY | (fsConstants.O_NOFOLLOW ?? 0));
    let serialized;
    try {
    ...
    const expectedFilename = `${artifact.payload.source}-${timestamp}-${artifactDigest.slice(7, 19)}.json`;
    if (filename !== expectedFilename) {
        throw new EvidenceArtifactError('artifact filename does not match signed content');
    }
    return artifact;
}
function ensurePrivateDirectory(repositoryRoot, directory) {
...
export function writeEvidenceArtifact(input) {
    ...
    const temporaryPath = path.join(input.loaded.outputDirectoryAbsolutePath, `.pending-${input.artifact.payload.captureId}`);
    let descriptor = null;
    try {
        descriptor = fs.openSync(temporaryPath, fsConstants.O_CREAT
            | fsConstants.O_EXCL
