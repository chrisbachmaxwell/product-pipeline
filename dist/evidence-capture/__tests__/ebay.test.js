...
import { EBAY_READ_SCOPES, EbayEvidenceError, createEbayEvidenceCollector, createEbayOrderWindow, } from "../ebay.js";
...
function inventoryItem(sku, overrides = {}) {
    return {
        sku,
        locale: "en_US",
...
function inventoryItems(count, prefix = "SKU") {
    return Array.from({ length: count }, (_unused, index) => inventoryItem(`${prefix}-${String(index).padStart(3, "0")}`));
}
function offer(offerId, sku, overrides = {}) {
    return {
        offerId,
        sku,
...
function order(orderId, creationDate, overrides = {}) {
    return {
        orderId,
        creationDate,
...
function stubTransport(handler, provenance = {
    kind: "fixture",
...
function simpleInventoryTransport(environment = "production") {
...
function simpleOrderTransport(orders = [order("ORDER-1", "2026-08-11T12:00:00.000Z")]) {
    return stubTransport((request) => {
        const url = new URL(request.url);
...
describe("unwired GET-only boundary", () => {
    ...
    ])("uses only exact %s identity and sell hosts", async (environment, identityHost, sellHost) => {
        const stub = simpleInventoryTransport(environment);
        const result = await createEbayEvidenceCollector(baseConfig({ environment }), stub.transport).collectInventoryAndOffers();
    ...
    it("never upgrades fixture provenance into direct API evidence", async () => {
        const stub = simpleInventoryTransport();
        const result = await createEbayEvidenceCollector(baseConfig(), stub.transport).collectInventoryAndOffers();
    ...
    it("preserves an explicit direct transport label without inventing live completeness", async () => {
        ...
        const result = await createEbayEvidenceCollector(baseConfig(), direct).collectInventoryAndOffers();
    ...
    it("redacts thrown transport details and upstream status bodies", async () => {
        ...
        expect(JSON.stringify(caught)).toBe('{"name":"EbayEvidenceError","code":"transport-failure","message":"eBay evidence capture denied (transport-failure)."}');
    ...
    it("aborts and reports only a redacted timeout", async () => {
        vi.useFakeTimers();
        const stub = stubTransport(() => new Promise(() => undefined));
...
describe("authorization and exact commerce identity", () => {
    it("rejects write, generic, unknown, and duplicate scopes before transport", () => {
        for (const scopes of [
            ...
            expect(() => createEbayEvidenceCollector(baseConfig({
                authorization: {
    ...
    it("rejects embedded token material, refresh support, and secret-like provenance", () => {
        ...
        expect(() => createEbayEvidenceCollector(baseConfig({ authorization: authorizationWithToken }))).toThrowError(expect.objectContaining({ code: "invalid-authorization" }));
        expect(() => createEbayEvidenceCollector(baseConfig({
            authorization: { ...baseConfig().authorization, refreshSupported: true },
    ...
    it("preflights immutable user and registration marketplace before sell calls", async () => {
        ...
        await expect(createEbayEvidenceCollector(baseConfig(), stub.transport).collectInventoryAndOffers()).rejects.toMatchObject({ code: "identity-mismatch" });
    ...
    it("requires a method-specific readonly scope before identity or sell traffic", async () => {
        ...
        await expect(createEbayEvidenceCollector(config, stub.transport).collectRecentOrders(recentWindow())).rejects.toMatchObject({ code: "authorization-scope-denied" });
...
describe("Inventory API item and per-SKU offer enumeration", () => {
    ...
    it("accepts a provably empty Inventory API account without offer requests", async () => {
        const stub = stubTransport((request) => {
            ...
            if (path === "/commerce/identity/v1/user/")
                return { status: 200, body: identityBody() };
    ...
    it("uses Inventory page-number offsets and accepts a non-full terminal page", async () => {
        ...
        const result = await createEbayEvidenceCollector(config, stub.transport).collectInventoryAndOffers();
        ...
        expect(stub.requests
            ...
            .map((request) => new URL(request.url).searchParams.get("offset"))).toEqual(["0", "1"]);
    });
    it("uses per-SKU offer page-number offsets and accepts a non-full terminal page", async () => {
        const firstPage = Array.from({ length: 25 }, (_unused, index) => offer(`OFFER-${String(index).padStart(2, "0")}`, "SKU-1"));
        const stub = stubTransport((request) => {
            const url = new URL(request.url);
            if (url.pathname === "/commerce/identity/v1/user/") {
                return { status: 200, body: identityBody() };
    ...
    it("fails on a missing continuation before the declared total", async () => {
        ...
        await expect(createEbayEvidenceCollector(baseConfig(), stub.transport).collectInventoryAndOffers()).rejects.toMatchObject({ code: "incomplete-capture" });
    });
    it("fails on total drift across Inventory pages", async () => {
        ...
        await expect(createEbayEvidenceCollector(config, stub.transport).collectInventoryAndOffers()).rejects.toMatchObject({ code: "incomplete-capture" });
    });
    it("fails on a cursor loop before emitting complete", async () => {
        ...
        await expect(createEbayEvidenceCollector(config, stub.transport).collectInventoryAndOffers()).rejects.toMatchObject({ code: "pagination-loop" });
    });
    it("fails on duplicate stable SKUs", async () => {
        ...
        await expect(createEbayEvidenceCollector(config, stub.transport).collectInventoryAndOffers()).rejects.toMatchObject({ code: "duplicate-stable-id" });
    });
    it.each([
        ...
        await expect(createEbayEvidenceCollector(baseConfig(), stub.transport).collectInventoryAndOffers()).rejects.toMatchObject({ code });
    });
    it("fails when an offer ID is duplicated across enumerated SKUs", async () => {
        ...
        await expect(createEbayEvidenceCollector(baseConfig(), stub.transport).collectInventoryAndOffers()).rejects.toMatchObject({ code: "duplicate-stable-id" });
    });
    it("rejects a continuation that changes host, path, query, or page index", async () => {
        ...
        await expect(createEbayEvidenceCollector(config, stub.transport).collectInventoryAndOffers()).rejects.toMatchObject({ code: "pagination-invalid" });
    });
    it("enforces record, page, and locally computed response-byte caps", async () => {
        ...
        await expect(createEbayEvidenceCollector(recordCapConfig, tooMany.transport).collectInventoryAndOffers()).rejects.toMatchObject({ code: "response-limit-exceeded" });
        ...
        await expect(createEbayEvidenceCollector(byteCapConfig, oversized.transport).collectInventoryAndOffers()).rejects.toMatchObject({ code: "response-too-large" });
    });
});
describe("recent Fulfillment order evidence", () => {
    it.each([
        ...
        expect(() => createEbayOrderWindow(input)).toThrowError(expect.objectContaining({ code: "invalid-order-window" }));
    });
    it("rejects forged and capture-time-mismatched order windows before transport", async () => {
        ...
        await expect(collector.collectRecentOrders({
            startUtc: START,
            endUtc: END,
            asOfUtc: CAPTURED_AT,
        ...
        await expect(collector.collectRecentOrders(createEbayOrderWindow({
    ...
    it("queries eBay's inclusive range then post-filters the exclusive upper boundary", async () => {
        ...
        const result = await createEbayEvidenceCollector(baseConfig(), stub.transport).collectRecentOrders(recentWindow());
        const orderRequest = stub.requests.find((request) => new URL(request.url).pathname === "/sell/fulfillment/v1/order");
        ...
        expect(Object.keys(result.records[0]).sort()).toEqual(["orderId", "creationDate", "lastModifiedDate", "orderFulfillmentStatus"].sort());
    ...
    it("validates Fulfillment row-offset pagination and stable total", async () => {
        const raw = Array.from({ length: 201 }, (_unused, index) => order(`ORDER-${String(index).padStart(3, "0")}`, new Date(Date.parse(START) + index * 1_000).toISOString()));
        const stub = stubTransport((request) => {
            const url = new URL(request.url);
            if (url.pathname === "/commerce/identity/v1/user/") {
                return { status: 200, body: identityBody() };
        ...
        expect(stub.requests
            ...
            .map((request) => new URL(request.url).searchParams.get("offset"))).toEqual(["0", "200"]);
    });
    it("fails on duplicate orders, total mismatch, and non-empty warnings", async () => {
        ...
        for (const testCase of cases) {
            const stub = stubTransport((request) => new URL(request.url).pathname === "/commerce/identity/v1/user/"
                ? { status: 200, body: identityBody() }
                : { status: 200, body: testCase.body });
            await expect(createEbayEvidenceCollector(baseConfig(), stub.transport).collectRecentOrders(recentWindow())).rejects.toMatchObject({ code: testCase.code });
        }
    });
    it("fails when an upstream order falls outside the exact inclusive query", async () => {
        ...
        await expect(createEbayEvidenceCollector(baseConfig(), stub.transport).collectRecentOrders(recentWindow())).rejects.toMatchObject({ code: "incomplete-capture" });
    });
    it("rejects a Fulfillment next link that changes the mandatory creationdate filter", async () => {
        const firstPage = Array.from({ length: 200 }, (_unused, index) => order(`FILTER-${String(index).padStart(3, "0")}`, new Date(Date.parse(START) + index * 1_000).toISOString()));
        const stub = stubTransport((request) => {
            const url = new URL(request.url);
            if (url.pathname === "/commerce/identity/v1/user/") {
                return { status: 200, body: identityBody() };
            ...
            changed.searchParams.set("filter", "creationdate:[2026-01-01T00:00:00.000Z..2026-08-11T18:00:00.000Z]");
        ...
        await expect(createEbayEvidenceCollector(baseConfig(), stub.transport).collectRecentOrders(recentWindow())).rejects.toMatchObject({ code: "pagination-invalid" });
    });
});
