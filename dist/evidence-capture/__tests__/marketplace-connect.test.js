...
import { canonicalizeMarketplaceConnectPayload, verifyMarketplaceConnectAttestation, } from '../marketplace-connect.js';
const COLLECTOR_PRIVATE_KEY = 'MC4CAQAwBQYDK2VwBCIEIFy4K6lyZUn/2ZssokmVXIHjSrdFXTxf/p6XYw0lwkx7';
const COLLECTOR_PUBLIC_KEY = 'MCowBQYDK2VwAyEA9k/VdDM+2mAbEygTPW4qowe4sWmnv9Gjpum2WDBzeM0=';
const REVIEWER_PRIVATE_KEY = 'MC4CAQAwBQYDK2VwBCIEIDhgpoURdovJUUGNKSR5KBI7w/9YUakFTmmbwLV1FwOQ';
const REVIEWER_PUBLIC_KEY = 'MCowBQYDK2VwAyEAEk09B9GehQK+ADnr+p5NRT44EipesOKmg6mj3aOR2fc=';
...
const evidenceId = (value) => `evidence:${digest(value)}`;
const unknownId = (value) => `unknown:${digest(value)}`;
...
function detachedSignature(role, payload) {
...
function signedPacket(payload = validPayload()) {
    return {
        schemaVersion: 1,
...
describe('Marketplace Connect signed read-only evidence', () => {
    ...
    it('rejects a payload changed after signing', () => {
        ...
        expect(() => verifyMarketplaceConnectAttestation(packet, trust)).toThrow(/collector detached signature verification failed/);
    });
    it('rejects unknown keys and customer, raw-order, secret, or URL-shaped material', () => {
        const unknownKey = structuredClone(validPayload());
        unknownKey.extra = true;
        expect(() => canonicalizeMarketplaceConnectPayload(unknownKey)).toThrow(/extra is not supported/);
        const customerField = structuredClone(validPayload());
        customerField.listingCoverage.customerEmail = 'redacted';
        expect(() => canonicalizeMarketplaceConnectPayload(customerField)).toThrow(/forbidden secret, PII/);
        const rawOrder = structuredClone(validPayload());
        rawOrder.settings.rawOrder = { id: 'opaque' };
        ...
        urlValue.unknowns[0].detailsDigest = 'https://admin.shopify.com/private';
    ...
    it('requires canonical UTC and a four-hour bounded capture session', () => {
        ...
        expect(() => canonicalizeMarketplaceConnectPayload(oldAttachment)).toThrow(/outside the bounded capture session/);
    });
    it('requires terminal and dataset proof for complete listing coverage', () => {
        ...
        expect(() => canonicalizeMarketplaceConnectPayload(payload)).toThrow(/complete listing coverage requires terminal and dataset digests/);
    ...
    it('recomputes listing datasets and binds terminal proof to retained evidence', () => {
        ...
        expect(() => canonicalizeMarketplaceConnectPayload(mismatchedDataset)).toThrow(/dataset digest does not match/);
        ...
        expect(() => canonicalizeMarketplaceConnectPayload(unboundTerminal)).toThrow(/terminal digest is not bound/);
    });
    it('rejects mismatched counts, duplicate records, and unavailable observations', () => {
        ...
        expect(() => canonicalizeMarketplaceConnectPayload(unavailable)).toThrow(/unavailable listing coverage cannot contain observations/);
    });
    ...
    it('allows incumbent claims only for evidenced order, price, and inventory settings', () => {
        ...
        expect(() => canonicalizeMarketplaceConnectPayload(listingOwner)).toThrow(/cannot infer listing or operational ownership/);
        const uiPriceOwner = validPayload();
        uiPriceOwner.claims[1].assertedOwner = 'marketplace_connect';
        expect(() => canonicalizeMarketplaceConnectPayload(uiPriceOwner)).toThrow(/cannot infer price ownership from UI settings alone/);
        const missingAttribution = validPayload();
        missingAttribution.claims[0].evidenceIds = [ORDER_EVIDENCE_ID];
        missingAttribution.evidenceAttachments = missingAttribution.evidenceAttachments.filter((entry) => entry.evidenceId !== ORDER_ATTRIBUTION_EVIDENCE_ID);
        expect(() => canonicalizeMarketplaceConnectPayload(missingAttribution)).toThrow(/lacks enabled settings and order-attribution evidence/);
    });
    it('requires explicit limitations and at least one retained unknown', () => {
        const noUnknowns = validPayload();
        noUnknowns.unknowns = [];
        expect(() => canonicalizeMarketplaceConnectPayload(noUnknowns)).toThrow(/explicitly retain at least one evidence limitation/);
        const authorizing = validPayload();
        authorizing.limitations.ownershipTransferAuthorized = true;
    ...
    it('requires independent trusted collector and reviewer keys', () => {
        ...
        expect(() => verifyMarketplaceConnectAttestation(signedPacket(), sameTrust)).toThrow(/trusted collector and reviewer must be independent/);
        const wrongReviewerTrust = {
            ...trust,
            collector: trust.collector,
            reviewer: { ...trust.reviewer, signerId: digest('wrong-reviewer') },
        };
        expect(() => verifyMarketplaceConnectAttestation(signedPacket(), wrongReviewerTrust)).toThrow(/reviewer signature does not match the trusted signer/);
    });
    ...
    it('rejects noncanonical, truncated, or self-declared signatures', () => {
        ...
        expect(() => verifyMarketplaceConnectAttestation(truncated, trust)).toThrow(/expected byte length/);
        const selfDeclared = signedPacket();
        selfDeclared.signatures[0].keyId = digest('untrusted-key');
        expect(() => verifyMarketplaceConnectAttestation(selfDeclared, trust)).toThrow(/does not match the trusted signer/);
    });
});
