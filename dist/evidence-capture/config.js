...
const SENSITIVE_KEY_PATTERN = /(?:token|secret|password|credential|api[_-]?key|client[_-]?secret|access[_-]?token|refresh[_-]?token|private[_-]?key|authorization|cookie)/i;
...
const COLLECTOR_KEYS = ['name', 'version', 'buildCommit'];
const SIGNING_KEYS = ['keyId', 'publicKeySpkiDerBase64'];
const LIMIT_KEYS = [
    'requestTimeoutMs',
    'maxPagesPerSource',
    'maxRecordsPerSource',
...
function record(value, field, issues) {
    if (!isRecord(value)) {
...
function literal(value, expected, field, issues) {
    if (value !== expected) {
...
function enumeration(value, allowed, field, issues) {
...
function exactString(value, pattern, field, issues, maximumLength = 256) {
...
function boundedInteger(value, minimum, maximum, field, issues) {
...
function inspectSensitiveMaterial(value, field, issues) {
    if (typeof value === 'string') {
...
function validatePublicKey(value, issues) {
...
export function parseEvidenceCaptureConfig(value) {
    const issues = [];
    inspectSensitiveMaterial(value, 'config', issues);
    const root = record(value, 'config', issues);
    if (!root)
        throw new EvidenceCaptureConfigError(issues);
    ...
    const lane = enumeration(root.lane, ['sandbox', 'production-shadow'], 'config.lane', issues);
    const mode = literal(root.mode, 'authoritative-read-capture', 'config.mode', issues);
    const outputDirectory = literal(root.outputDirectory, EVIDENCE_CAPTURE_OUTPUT_DIRECTORY, 'config.outputDirectory', issues);
    ...
    if (identityValue) {
        ...
        const shopifyStoreDomain = exactString(identityValue.shopifyStoreDomain, STORE_DOMAIN_PATTERN, 'config.identities.shopifyStoreDomain', issues);
        const shopifyShopGid = exactString(identityValue.shopifyShopGid, SHOP_GID_PATTERN, 'config.identities.shopifyShopGid', issues);
        const shopifyAppGid = exactString(identityValue.shopifyAppGid, APP_GID_PATTERN, 'config.identities.shopifyAppGid', issues);
        const ebayEnvironment = enumeration(identityValue.ebayEnvironment, ['sandbox', 'production'], 'config.identities.ebayEnvironment', issues);
        const ebayUserId = exactString(identityValue.ebayUserId, EBAY_USER_ID_PATTERN, 'config.identities.ebayUserId', issues, 128);
        const ebayMarketplaceId = literal(identityValue.ebayMarketplaceId, 'EBAY_US', 'config.identities.ebayMarketplaceId', issues);
        const ebayRegistrationMarketplaceId = literal(identityValue.ebayRegistrationMarketplaceId, 'EBAY_US', 'config.identities.ebayRegistrationMarketplaceId', issues);
    ...
    if (collectorValue) {
        ...
        const name = literal(collectorValue.name, 'product-pipeline-evidence-capture', 'config.collector.name', issues);
        const version = literal(collectorValue.version, 1, 'config.collector.version', issues);
        const buildCommit = exactString(collectorValue.buildCommit, BUILD_COMMIT_PATTERN, 'config.collector.buildCommit', issues, 40);
        if (name && version === 1 && buildCommit)
    ...
    if (signingValue) {
        ...
        const keyId = exactString(signingValue.keyId, KEY_ID_PATTERN, 'config.signing.keyId', issues, 64);
        const publicKeySpkiDerBase64 = validatePublicKey(signingValue.publicKeySpkiDerBase64, issues);
        if (keyId && publicKeySpkiDerBase64)
            signing = { keyId, publicKeySpkiDerBase64 };
    }
    const limitValue = record(root.limits, 'config.limits', issues);
    let limits;
    if (limitValue) {
        exactKeys(limitValue, LIMIT_KEYS, 'config.limits', issues);
        const requestTimeoutMs = boundedInteger(limitValue.requestTimeoutMs, 1_000, 30_000, 'config.limits.requestTimeoutMs', issues);
        const maxPagesPerSource = boundedInteger(limitValue.maxPagesPerSource, 1, 200, 'config.limits.maxPagesPerSource', issues);
        const maxRecordsPerSource = boundedInteger(limitValue.maxRecordsPerSource, 1, 25_000, 'config.limits.maxRecordsPerSource', issues);
        const maxResponseBytes = boundedInteger(limitValue.maxResponseBytes, 1_024, 8 * 1024 * 1024, 'config.limits.maxResponseBytes', issues);
        const minimumEbayAccessValiditySeconds = boundedInteger(limitValue.minimumEbayAccessValiditySeconds, 300, 3_600, 'config.limits.minimumEbayAccessValiditySeconds', issues);
        const maxOrderWindowHours = literal(limitValue.maxOrderWindowHours, 168, 'config.limits.maxOrderWindowHours', issues);
        if (requestTimeoutMs
            && maxPagesPerSource
            && maxRecordsPerSource
            && maxResponseBytes
    ...
    if (safetyValue) {
        ...
        const externalPlatformReads = literal(safetyValue.externalPlatformReads, true, 'config.safety.externalPlatformReads', issues);
        const externalPlatformWrites = literal(safetyValue.externalPlatformWrites, false, 'config.safety.externalPlatformWrites', issues);
        const historicalBackfill = literal(safetyValue.historicalBackfill, false, 'config.safety.historicalBackfill', issues);
        const oauthAcquisition = literal(safetyValue.oauthAcquisition, false, 'config.safety.oauthAcquisition', issues);
        const accessRefresh = literal(safetyValue.accessRefresh, false, 'config.safety.accessRefresh', issues);
        const rawPayloadPersistence = literal(safetyValue.rawPayloadPersistence, false, 'config.safety.rawPayloadPersistence', issues);
        const personalDataPersistence = literal(safetyValue.personalDataPersistence, false, 'config.safety.personalDataPersistence', issues);
...
function readBoundedJsonFile(filePath) {
    const descriptor = fs.openSync(filePath, fsConstants.O_RDONLY | (fsConstants.O_NOFOLLOW ?? 0));
    try {
