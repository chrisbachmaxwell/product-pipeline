...
import { runSnapshotReconciliation, } from './reconciliation.js';
const defaultIo = {
...
function printInspection(result, json, io) {
    if (json) {
    ...
    io.stdout(`Declared target (not remotely verified): ${result.declaredIdentity.shopifyStoreDomain} / ${result.declaredIdentity.ebayEnvironment}:${result.declaredIdentity.ebaySellerAccount}`);
    io.stdout('Ownership:');
    for (const responsibility of RESPONSIBILITIES) {
        const entry = result.ownership[responsibility];
        io.stdout(`  ${responsibility}: owner=${entry.currentOwner}; ProductPipeline=${entry.productPipelineAccess}`);
    }
...
function addInspectionCommand(program, commandName, io) {
    program
        .command(commandName)
        .description(commandName === 'preflight'
...
function printReconciliation(result, json, io) {
    if (json) {
        io.stdout(JSON.stringify(result));
        return;
    }
    io.stdout(`Operator reconcile: ${result.status.toUpperCase()}`);
    io.stdout('Evidence: supplied snapshots only; no network, application database, external writes, order creation, or historical backfill');
    io.stdout('Live proof: NO; production parity: NO; order creation eligible: NO');
    io.stdout(`Generated: ${result.generatedAtUtc}`);
    io.stdout(`Observed: ${result.counts.shopifyVariants} Shopify variant(s), ${result.counts.ebayListings} eBay listing(s), ${result.counts.ebayOrders} eBay order(s)`);
    if (result.discrepancies.length > 0) {
        io.stdout(`Exceptions (${result.discrepancies.length}):`);
        result.discrepancies.forEach((item) => io.stdout(`  - [${item.severity}] ${item.code}: ${item.entityKey} (owner=${item.owner}) — ${item.summary}`));
    }
    io.stdout(`Snapshot digest: ${result.snapshot.digest}`);
    io.stdout(`Result digest: ${result.resultDigest}`);
    io.stdout(`Audit: ${result.audit.path} #${result.audit.sequence} ${result.audit.recordHash}`);
}
export function buildOperatorProgram(io = defaultIo) {
    const program = new Command();
    program
        .name('product-pipeline-operator')
        .description('Local-only ProductPipeline migration controls. This CLI has no network or commerce mutation adapter.')
        .version('0.1.0')
        .showHelpAfterError();
    ...
    program
        ...
        .requiredOption('--config <path>', 'Repository-local nonsecret operator config')
        .requiredOption('--snapshot <path>', 'Redacted JSON snapshot beneath .local/operator-reconciliation/')
        .option('--repo-root <path>', 'ProductPipeline repository root', '.')
        .option('--dry-run', 'Required safety posture; always enabled', true)
        .option('--json', 'Emit one JSON object')
        .action(async (options) => {
        ...
        try {
            ...
            if (result.status === 'exceptions-found')
                io.setExitCode(2);
        }
        catch (error) {
    ...
    audit
        ...
        try {
            ...
            if (options.json)
                ...
                io.stdout(`Audit valid: ${verification.recordCount} record(s); head=${verification.headHash ?? 'none'}`);
            }
            else {
