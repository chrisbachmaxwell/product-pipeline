import { RESPONSIBILITIES, sha256Digest } from '../config.js';
import { computeReconciliationDatasetDigest, } from '../reconciliation-schema.js';
...
export function refreshReconciliationSource(snapshot, source) {
...
export function markSourceUnavailable(snapshot, source, reason = 'not-collected') {
    switch (source) {
        case 'productPipeline':
...
export function validReconciliationSnapshot(overrides = {}) {
    ...
    for (const source of ['productPipeline', 'shopify', 'ebay', 'marketplaceConnect']) {
