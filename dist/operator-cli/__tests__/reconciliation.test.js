...
async function tempRepo(snapshot = validReconciliationSnapshot()) {
...
describe('snapshot reconciliation', () => {
    ...
    it('rejects personal-data and secret-like fields without echoing their values', () => {
        ...
        expect(() => parseReconciliationSnapshot(withPrivateData)).toThrow(ReconciliationSnapshotError);
        try {
    ...
    it('blocks stale supplied evidence without turning it into live proof', async () => {
        ...
        expect(result.discrepancies).toContainEqual(expect.objectContaining({ code: 'snapshot.stale' }));
