...
async function tempRepo(config = validConfig()) {
...
describe('operator preflight', () => {
    it('uses no network or application database and records a local audit result', async () => {
        ...
        await expect(fs.stat(path.join(root, '.local', 'operator-audit', 'operator-cli.jsonl'))).resolves.toBeDefined();
    });
    ...
    it('records a denial in the fallback audit when config validation fails', async () => {
        const root = await tempRepo({ ...validConfig(), writesEnabled: true });
        await expect(runOperatorInspection({
            command: 'preflight',
            repoRoot: root,
            configPath: 'config/operator.json',
    ...
    it('defaults to dry-run and rejects a no-dry-run flag', async () => {
        ...
        await program.parseAsync(['preflight', '--repo-root', root, '--config', 'config/operator.json', '--json'], { from: 'user' });
        ...
        await expect(unsafeProgram.parseAsync(['preflight', '--repo-root', root, '--config', 'config/operator.json', '--no-dry-run'], { from: 'user' })).rejects.toMatchObject({ code: 'commander.unknownOption' });
    });
});
