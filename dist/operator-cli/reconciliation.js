...
import { assertPathInsideRoot, canonicalJson, loadOperatorConfig, RESPONSIBILITIES, sha256Digest, validateRepositoryRoot, } from './config.js';
import { computeReconciliationDatasetDigest, parseReconciliationSnapshot, RECONCILIATION_SOURCES, ReconciliationSnapshotError, } from './reconciliation-schema.js';
...
const ACCEPTED_MC_BASELINE = new Set([
    'orderImport',
    'price',
    'inventory',
]);
async function loadReconciliationSnapshot(repoRoot, requestedPath) {
    ...
    const snapshot = parseReconciliationSnapshot(parsed, MAX_RECONCILIATION_RECORDS_PER_COLLECTION);
...
function uniqueMap(items, key) {
    ...
    return new Map(items.flatMap((item) => {
        const value = key(item);
        return value !== null && counts.get(value) === 1 ? [[value, item]] : [];
    }));
}
function sourceEvidence(snapshot, now) {
    const evidence = RECONCILIATION_SOURCES.map((source) => {
        ...
        const freshness = ageMs < -MAX_RECONCILIATION_CLOCK_SKEW_MS
            ? 'future'
        ...
        if (!provenance.paginationComplete)
            blockers.push(`source.${source}.pagination-incomplete`);
        if (provenance.pageCount < 1)
            blockers.push(`source.${source}.terminal-page-unavailable`);
        if (provenance.recordCount === null)
            blockers.push(`source.${source}.record-count-unavailable`);
        if (provenance.reportedTotal === null)
            blockers.push(`source.${source}.reported-total-unavailable`);
        ...
        if (provenance.datasetDigest === null)
            blockers.push(`source.${source}.dataset-digest-unavailable`);
        if (provenance.reportedTotal !== null &&
        ...
        if (freshness !== 'fresh')
            blockers.push(`source.${source}.${freshness}`);
        return {
            source,
...
function add(target, config, value) {
    target.push({ ...value, owner: config.ownership[value.responsibility].currentOwner });
}
function compareData(snapshot, config, discrepancies) {
    ...
    if (ppComplete && shopifyComplete && ebayComplete) {
        for (const local of pp.listings) {
            if (local.shopifyVariantGid === null) {
                ...
                if (!remote)
                    add(discrepancies, config, { code: 'listing.local-shopify-orphan', severity: 'critical', responsibility: 'mapping', entityType: 'listing', entityKey: local.shopifyVariantGid, summary: 'Local link has no matching Shopify variant.' });
                else {
                    const productGid = local.shopifyProductId.startsWith('gid://') ? local.shopifyProductId : `gid://shopify/Product/${local.shopifyProductId}`;
                    if (remote.shopifyProductGid !== productGid)
                        add(discrepancies, config, { code: 'listing.local-shopify-product-mismatch', severity: 'critical', responsibility: 'mapping', entityType: 'listing', entityKey: local.shopifyVariantGid, summary: 'Local product identity and Shopify variant parent do not agree.' });
                    if (remote.sku !== local.sku)
                        add(discrepancies, config, { code: 'listing.local-shopify-sku-mismatch', severity: 'warning', responsibility: 'mapping', entityType: 'listing', entityKey: local.shopifyVariantGid, summary: 'Local and Shopify SKUs do not agree.' });
                }
            }
            if (local.ebayListingId === null) {
                ...
                if (!remote)
                    add(discrepancies, config, { code: 'listing.local-ebay-orphan', severity: 'critical', responsibility: 'mapping', entityType: 'listing', entityKey: local.ebayListingId, summary: 'Local link has no matching eBay listing.' });
                else {
                    if (local.ebayInventoryItemSku !== null && local.ebayInventoryItemSku !== remote.inventoryItemSku)
                        add(discrepancies, config, { code: 'listing.inventory-item-sku-mismatch', severity: 'warning', responsibility: 'mapping', entityType: 'listing', entityKey: local.ebayListingId, summary: 'Local and eBay inventory SKUs do not agree.' });
                    if (local.ebayOfferId !== null && local.ebayOfferId !== remote.offerId)
                        add(discrepancies, config, { code: 'listing.offer-id-mismatch', severity: 'warning', responsibility: 'mapping', entityType: 'listing', entityKey: local.ebayListingId, summary: 'Local and eBay offer IDs do not agree.' });
                    const expectedStatus = local.status === 'active' ? 'published' : local.status === 'ended' ? 'ended' : local.status === 'draft' ? 'unpublished' : null;
                    if (expectedStatus !== null && remote.status !== expectedStatus)
                        add(discrepancies, config, { code: 'listing.status-mismatch', severity: 'warning', responsibility: 'listingRevise', entityType: 'listing', entityKey: local.ebayListingId, summary: 'ProductPipeline and eBay listing statuses do not agree.' });
                }
            }
        }
    }
    if (shopifyComplete && ebayComplete) {
        for (const [sku, shopifyVariant] of shopifyBySku) {
            ...
            if (!ebayListing)
                continue;
            if (shopifyVariant.priceMinor !== ebayListing.priceMinor || shopifyVariant.currency !== ebayListing.currency)
                add(discrepancies, config, { code: 'price.observed-difference', severity: 'warning', responsibility: 'price', entityType: 'listing', entityKey: sku, summary: 'Shopify and eBay prices differ; observation does not authorize a write.' });
            if (shopifyVariant.inventoryQuantity !== ebayListing.availableQuantity)
                add(discrepancies, config, { code: 'inventory.observed-difference', severity: 'warning', responsibility: 'inventory', entityType: 'listing', entityKey: sku, summary: 'Shopify and eBay quantities differ; observation does not authorize a write.' });
        }
    }
    if (ppComplete && shopifyComplete && ebayComplete) {
        const ebayOrderIds = new Set(ebay.orders.map((x) => x.ebayOrderId));
        const shopifyByEbay = new Map();
        for (const order of shopify.orders) {
            if (order.ebayOrderId === null)
                continue;
            const matches = shopifyByEbay.get(order.ebayOrderId) ?? [];
            matches.push(order);
            shopifyByEbay.set(order.ebayOrderId, matches);
            if (!ebayOrderIds.has(order.ebayOrderId))
                add(discrepancies, config, { code: 'order.shopify-source-not-in-ebay-snapshot', severity: 'warning', responsibility: 'reconciliation', entityType: 'order', entityKey: order.shopifyOrderGid, summary: 'Shopify references an eBay order absent from the bounded eBay dataset.' });
        }
        ...
        for (const local of pp.orders) {
            if (!ebayOrderIds.has(local.ebayOrderId))
                add(discrepancies, config, { code: 'order.local-ebay-orphan', severity: 'warning', responsibility: 'reconciliation', entityType: 'order', entityKey: local.ebayOrderId, summary: 'Local order observation is absent from the bounded eBay dataset.' });
            if (local.state === 'mapped' && local.shopifyOrderGid === null)
                add(discrepancies, config, { code: 'order.local-mapped-without-shopify-id', severity: 'critical', responsibility: 'reconciliation', entityType: 'order', entityKey: local.ebayOrderId, summary: 'Local mapped state lacks a stable Shopify order GID.' });
            if (local.shopifyOrderGid !== null) {
                const linked = shopifyByGid.get(local.shopifyOrderGid);
                if (!linked)
                    add(discrepancies, config, { code: 'order.local-shopify-orphan', severity: 'critical', responsibility: 'reconciliation', entityType: 'order', entityKey: local.ebayOrderId, summary: 'Local mapping points to an absent Shopify order.' });
                else if (linked.ebayOrderId !== local.ebayOrderId)
                    add(discrepancies, config, { code: 'order.local-shopify-link-mismatch', severity: 'critical', responsibility: 'reconciliation', entityType: 'order', entityKey: local.ebayOrderId, summary: 'Local and Shopify order-link identities do not agree.' });
            }
        }
        for (const order of ebay.orders) {
            const matches = shopifyByEbay.get(order.ebayOrderId) ?? [];
            if (matches.length === 0)
                add(discrepancies, config, { code: 'order.no-shopify-link-observed', severity: 'warning', responsibility: 'orderImport', entityType: 'order', entityKey: order.ebayOrderId, summary: 'No Shopify link is present; this is an incumbent-owned exception, never an import candidate.' });
            if (matches.length > 1)
                add(discrepancies, config, { code: 'order.duplicate-shopify-links', severity: 'critical', responsibility: 'orderImport', entityType: 'order', entityKey: order.ebayOrderId, summary: 'More than one Shopify order references the same eBay order.' });
            for (const match of matches) {
                if (match.importOwner === 'product-pipeline')
                    add(discrepancies, config, { code: 'order.product-pipeline-import-observed', severity: 'critical', responsibility: 'orderImport', entityType: 'order', entityKey: order.ebayOrderId, summary: 'An order is attributed to ProductPipeline during writer quarantine.' });
                if (match.importOwner === 'unknown')
                    add(discrepancies, config, { code: 'order.import-owner-unverified', severity: 'warning', responsibility: 'orderImport', entityType: 'order', entityKey: order.ebayOrderId, summary: 'Shopify order import ownership is not established.' });
            }
            if (!localByEbay.has(order.ebayOrderId))
                add(discrepancies, config, { code: 'order.missing-local-observation', severity: 'info', responsibility: 'reconciliation', entityType: 'order', entityKey: order.ebayOrderId, summary: 'eBay order is not represented in ProductPipeline observations.' });
        }
    }
}
function responsibilityEvidence(config, sources, discrepancies) {
    ...
    return RESPONSIBILITIES.map((responsibility) => {
        ...
        if (baselineAccepted && declaration.currentOwner !== 'marketplace-connect')
            blockers.push(`ownership.${responsibility}.baseline-not-accepted`);
        if (!baselineAccepted && declaration.currentOwner === 'unverified')
            blockers.push(`ownership.${responsibility}.unverified`);
...
function compareSnapshots(snapshot, config, now) {
    ...
    if (Date.parse(snapshot.generatedAtUtc) >
...
function auditChecks(snapshot, snapshotDigest, resultDigest, hasExceptions) {
    return [
        ...
        ...RECONCILIATION_SOURCES.map((source) => ({
...
export async function runSnapshotReconciliation(options) {
    ...
    const auditRecord = await appendAuditRecord(repoRoot, loadedConfig.config.audit.logPath, {
        ...
        checks: auditChecks(loadedSnapshot.snapshot, loadedSnapshot.digest, resultDigest, core.status === 'exceptions-found'),
