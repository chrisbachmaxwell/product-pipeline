...
function requireIdentity(value, pattern, fieldPath, issues) {
...
function requireOptionalIdentity(value, pattern, fieldPath, issues) {
    if (value === null)
        return null;
    return requireIdentity(value, pattern, fieldPath, issues);
}
function requireExactStringArray(value, fieldPath, issues) {
    if (!Array.isArray(value)) {
...
function requireResponsibilities(value, fieldPath, issues) {
...
function validateRelativeAuditPath(value, issues) {
...
export function parseOperatorConfig(value) {
    ...
    const lane = requireEnum(root.lane, ['development', 'sandbox', 'production-shadow'], 'config.lane', issues);
    ...
    const writesEnabled = requireLiteral(root.writesEnabled, false, 'config.writesEnabled', issues);
    const identitiesValue = requireRecord(root.identities, 'config.identities', issues);
    let identities;
    if (identitiesValue) {
        requireExactKeys(identitiesValue, [
            'shopifyStoreDomain',
            'ebayEnvironment',
            'ebaySellerAccount',
            'marketplaceConnectAccount',
        ], 'config.identities', issues);
        const shopifyStoreDomain = requireIdentity(identitiesValue.shopifyStoreDomain, STORE_DOMAIN_PATTERN, 'config.identities.shopifyStoreDomain', issues);
        const ebayEnvironment = requireEnum(identitiesValue.ebayEnvironment, ['sandbox', 'production'], 'config.identities.ebayEnvironment', issues);
        const ebaySellerAccount = requireIdentity(identitiesValue.ebaySellerAccount, ACCOUNT_PATTERN, 'config.identities.ebaySellerAccount', issues);
        const marketplaceConnectAccount = requireOptionalIdentity(identitiesValue.marketplaceConnectAccount, ACCOUNT_PATTERN, 'config.identities.marketplaceConnectAccount', issues);
    ...
    if (ownershipValue) {
        ...
        for (const responsibility of RESPONSIBILITIES) {
            const entry = requireRecord(ownershipValue[responsibility], `config.ownership.${responsibility}`, issues);
            if (!entry)
                continue;
            requireExactKeys(entry, ['currentOwner', 'productPipelineAccess'], `config.ownership.${responsibility}`, issues);
            const currentOwner = requireEnum(entry.currentOwner, ['marketplace-connect', 'manual', 'paused', 'product-pipeline', 'unverified'], `config.ownership.${responsibility}.currentOwner`, issues);
            const productPipelineAccess = requireEnum(entry.productPipelineAccess, ['disabled', 'read-only'], `config.ownership.${responsibility}.productPipelineAccess`, issues);
            if (currentOwner === 'product-pipeline' && responsibility !== 'reconciliation') {
                issues.push(`config.ownership.${responsibility}.currentOwner cannot be product-pipeline in shadow mode`);
            }
            if (currentOwner === 'product-pipeline' && productPipelineAccess === 'disabled') {
                issues.push(`config.ownership.${responsibility}.productPipelineAccess cannot be disabled when product-pipeline is the current owner`);
            }
    ...
    let orders;
    if (ordersValue) {
        requireExactKeys(ordersValue, ['importEnabled', 'historicalBackfill', 'cutoverWatermarkUtc'], 'config.orders', issues);
        const importEnabled = requireLiteral(ordersValue.importEnabled, false, 'config.orders.importEnabled', issues);
        const historicalBackfill = requireLiteral(ordersValue.historicalBackfill, false, 'config.orders.historicalBackfill', issues);
        const cutoverWatermarkUtc = requireLiteral(ordersValue.cutoverWatermarkUtc, null, 'config.orders.cutoverWatermarkUtc', issues);
    ...
    let testLane;
    if (testLaneValue) {
        requireExactKeys(testLaneValue, ['shopifyVariantGids', 'skus', 'ebayListingIds', 'responsibilities'], 'config.testLane', issues);
        const shopifyVariantGids = requireExactStringArray(testLaneValue.shopifyVariantGids, 'config.testLane.shopifyVariantGids', issues);
        const skus = requireExactStringArray(testLaneValue.skus, 'config.testLane.skus', issues);
        const ebayListingIds = requireExactStringArray(testLaneValue.ebayListingIds, 'config.testLane.ebayListingIds', issues);
        const responsibilities = requireResponsibilities(testLaneValue.responsibilities, 'config.testLane.responsibilities', issues);
        if (shopifyVariantGids && skus && ebayListingIds && responsibilities) {
            ...
            if (shopifyVariantGids.length > 0 ||
                skus.length > 0 ||
                ebayListingIds.length > 0 ||
                responsibilities.length > 0) {
                issues.push('config.testLane must remain empty and inactive because this foundation has no action command');
            }
        }
    }
    ...
    if (lane && identities) {
        ...
        if (identities.ebayEnvironment !== expectedEnvironment) {
            issues.push(`config.identities.ebayEnvironment must be ${expectedEnvironment} for lane ${lane}`);
        }
        if (lane === 'production-shadow' && identities.marketplaceConnectAccount === null) {
            issues.push('config.identities.marketplaceConnectAccount is required for lane production-shadow');
        }
        if (lane === 'production-shadow' && ownershipValue) {
            for (const responsibility of ['price', 'inventory', 'orderImport']) {
                if (ownership[responsibility]?.currentOwner !== 'marketplace-connect') {
                    issues.push(`config.ownership.${responsibility}.currentOwner must remain marketplace-connect in the production-shadow foundation`);
                }
            }
        }
    }
...
export function assertPathInsideRoot(root, candidate, label) {
...
export async function validateRepositoryRoot(repoRoot) {
...
export async function loadOperatorConfig(repoRoot, requestedConfigPath) {
    ...
    let parsed;
    try {
        parsed = JSON.parse(await fs.readFile(realConfigPath, 'utf8'));
    }
    catch {
