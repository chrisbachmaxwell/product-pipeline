...
import { assertPathInsideRoot, canonicalJson, OPERATOR_AUDIT_LOG_PATH, sha256Digest, } from './config.js';
...
function isAuditRecord(value) {
    ...
    const timestampIsCanonical = typeof record.timestampUtc === 'string' &&
...
export function verifyAuditText(text) {
    if (text === '')
        return { valid: true, recordCount: 0, headHash: null };
    if (!text.endsWith('\n')) {
        return {
            valid: false,
            recordCount: 0,
...
async function readAuditText(filePath, allowMissing) {
...
async function resolveAuditPath(repoRoot, requestedPath, createParent) {
    ...
    for (const directory of [localDirectory, auditDirectory]) {
        ...
        if (!stat)
            return path.join(realRoot, OPERATOR_AUDIT_LOG_PATH);
...
export async function verifyAuditLog(repoRoot, requestedPath, allowMissing = false) {
...
export async function appendAuditRecord(repoRoot, requestedPath, input, options = {}) {
